...
use common::*;
use extraction::*;
use git_internal::internal::object::{provenance::Provenance, run::Run, run_usage::RunUsage};
use managed_inputs::*;
use managed_run::*;
...
const EMBEDDED_PYTHON_HELPER_SOURCE: &str =
    include_str!("../../internal/ai/providers/claude_sdk/helper.py");
const EMBEDDED_MANAGED_SYSTEM_PROMPT_APPEND: &str =
    include_str!("../../internal/ai/providers/claude_sdk/system_prompt_append.txt");
...
enum ClaudeSdkSubcommand {
    ...
    #[command(about = "Start an interactive multi-turn Claude Agent SDK chat session")]
    Chat(ChatManagedArgs),
    #[command(about = "Run a single Claude Agent SDK turn through the Python helper")]
    Run(RunManagedArgs),
    #[command(
        name = "sync-sessions",
    ...
    #[command(
        name = "build-managed-evidence-input",
        about = "Build a managed-run patch/evidence input artifact from a persisted Claude audit bundle"
    )]
    BuildManagedEvidenceInput(BuildManagedEvidenceInputArgs),
    #[command(
        name = "build-decision-input",
        about = "Build a Claude approval/decision input artifact from managed runtime signals"
    )]
    BuildDecisionInput(BuildDecisionInputArgs),
    #[command(
    ...
    #[command(
        name = "persist-patchset",
        about = "Persist a formal PatchSet for a bridged Claude SDK ai_session"
    )]
    PersistPatchSet(PersistPatchSetArgs),
    #[command(
...
pub async fn execute_safe(args: ClaudeSdkArgs, output: &OutputConfig) -> Result<()> {
    match args.command {
        ClaudeSdkSubcommand::Import(args) => import_artifact(args).await,
        ClaudeSdkSubcommand::Chat(args) => chat_managed(args, output).await,
        ClaudeSdkSubcommand::Run(args) => run_managed(args, output).await,
        ...
        ClaudeSdkSubcommand::BuildManagedEvidenceInput(args) => {
            build_managed_evidence_input(args).await
        }
        ClaudeSdkSubcommand::BuildDecisionInput(args) => build_decision_input(args).await,
        ...
        ClaudeSdkSubcommand::PersistPatchSet(args) => persist_patchset(args).await,
...
pub(super) async fn bridge_run_internal(args: BridgeRunArgs) -> Result<BridgeRunResult> {
    ...
    .await?
    {
        if !formal_run_binding_objects_exist(&storage_path, &existing).await? {
            ...
            load_audit_bundle_for_run_binding(&storage_path, &existing, &args.ai_session_id)
                .await?;
...
async fn bridge_run_plan_id(
    ...
    let Some(plan_steps) = extract_bridge_plan_steps(&audit_bundle.raw_artifact.messages) else {
        return Ok(None);
    };
