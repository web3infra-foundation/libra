...
fn install_json_hooks(
    ...
    if changed {
        println!("Installed {} hook forwarding at {}", spec.slug, path.display());
    } else {
        println!(
...
fn uninstall_json_hooks(spec: &PromotedSpec) -> Result<()> {
    let path = config_path(spec)?;
    if !path.exists() {
        println!("{} hook settings not found at {}", spec.slug, path.display());
        return Ok(());
    }
    let changed = match spec.shape {
    ...
    if changed {
        println!("Removed {} hook forwarding at {}", spec.slug, path.display());
    } else {
        println!(
...
fn install_ts_plugin(spec: &PromotedSpec, binary_path: &str, prefix: &str) -> Result<()> {
    ...
    println!("Installed {} capture plugin at {}", spec.slug, path.display());
    Ok(())
}
...
fn uninstall_ts_plugin(spec: &PromotedSpec) -> Result<()> {
    let path = config_path(spec)?;
    if !path.exists() {
        println!("{} capture plugin not found at {}", spec.slug, path.display());
        return Ok(());
    }
...
pub fn parse_promoted_hook_event(
    ...
    let kind = match normalized.as_str() {
        ...
        "userpromptsubmit" | "userpromptsubmitted" | "beforesubmitprompt" | "prompt"
...
pub static COPILOT_PROVIDER: PromotedProvider = PromotedProvider { spec: &COPILOT_SPEC };
...
pub static FACTORY_PROVIDER: PromotedProvider = PromotedProvider { spec: &FACTORY_SPEC };
...
pub static OPENCODE_PROVIDER: PromotedProvider = PromotedProvider { spec: &OPENCODE_SPEC };
...
mod tests {
    ...
    fn matcher_install_is_idempotent() {
        let mut file = MatcherFile::default();
        assert!(upsert_matcher(&mut file, &CODEX_SPEC, BIN, "agent hooks codex", None, Some(30), true));
        assert!(
            !upsert_matcher(&mut file, &CODEX_SPEC, BIN, "agent hooks codex", None, Some(30), true),
    ...
    fn factory_matcher_uses_empty_string_matcher() {
        let mut file = MatcherFile::default();
        upsert_matcher(&mut file, &FACTORY_SPEC, BIN, "agent hooks factory-ai", Some(""), None, true);
    ...
    fn matcher_install_preserves_foreign_hooks_and_keys() {
        ...
        upsert_matcher(&mut file, &CODEX_SPEC, BIN, "agent hooks codex", None, Some(30), true);
        assert!(file.extra.contains_key("$schema"), "unknown top-level key preserved");
        let cmds: Vec<&str> = file.hooks["SessionStart"]
            .iter()
            .flat_map(|g| g.hooks.iter())
            .map(|h| h.command.as_str())
            .collect();
        assert!(cmds.contains(&"/other/tool run"), "foreign hook preserved: {cmds:?}");
        assert!(
            cmds.iter().any(|c| c.contains("agent hooks codex session-start")),
    ...
    fn cursor_flat_install_uses_command_field() {
        ...
        assert!(entry.get("type").is_none(), "cursor entries carry no type field");
    }
    ...
    fn copilot_flat_install_uses_bash_field_with_type_and_comment() {
        ...
        assert_eq!(entry.get("comment").and_then(Value::as_str), Some("Libra agent capture"));
    }
    ...
    fn flat_install_is_idempotent_and_preserves_foreign_entries() {
        ...
        file.hooks.insert(
            ...
            vec![Value::Object({
                let mut m = Map::new();
                m.insert("command".to_string(), Value::String("other thing".to_string()));
                m
            })],
        );
        assert!(upsert_flat(&mut file, &CURSOR_SPEC, BIN, "agent hooks cursor", "command", false, None, true));
        assert!(
            !upsert_flat(&mut file, &CURSOR_SPEC, BIN, "agent hooks cursor", "command", false, None, true),
        ...
        let stop_cmds: Vec<&str> = file.hooks["stop"]
            ...
            .collect();
        assert!(stop_cmds.contains(&"other thing"), "foreign entry preserved: {stop_cmds:?}");
    }
