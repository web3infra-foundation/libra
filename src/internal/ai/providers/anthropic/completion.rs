...
struct AnthropicUsage {
    ...
    output_tokens: u64,
    /// Anthropic cache read tokens, when present.
    #[serde(default)]
    cache_read_input_tokens: Option<u64>,
    /// Anthropic cache write tokens, when present.
    #[serde(default)]
...
impl CompletionUsage for AnthropicResponse {
    fn usage_summary(&self) -> Option<CompletionUsageSummary> {
        Some(CompletionUsageSummary {
            ...
            cached_tokens: merge_optional_usage_tokens(
...
fn merge_optional_usage_tokens(left: Option<u64>, right: Option<u64>) -> Option<u64> {
    match (left, right) {
