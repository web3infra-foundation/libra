...
mod tests {
    ...
    #[test]
    fn workspace_sync_error_display_pins_each_variant() {
        let perm = || io::Error::new(io::ErrorKind::PermissionDenied, "permission denied");

        assert_eq!(
            WorkspaceSyncError::ContractViolation(
                "task touched a path outside its allowed scope".to_string(),
            )
            .to_string(),
            "task touched a path outside its allowed scope",
        );
        assert_eq!(
            WorkspaceSyncError::RetryableConflict {
                path: PathBuf::from("src/main.rs"),
                reason: "main branch advanced concurrently".to_string(),
            }
            .to_string(),
            "retryable sync conflict at 'src/main.rs': main branch advanced concurrently",
        );
        assert_eq!(
            WorkspaceSyncError::HardConflict {
                path: Some(PathBuf::from("src/lib.rs")),
                reason: "main and task both edited the same hunk".to_string(),
            }
            .to_string(),
            "hard sync conflict: main and task both edited the same hunk",
        );
        assert_eq!(
            WorkspaceSyncError::FuseInfrastructure {
                stage: "mount overlay",
                path: PathBuf::from("/tmp/task-worktree/overlay"),
                message: "fuse: device not configured".to_string(),
            }
            .to_string(),
            "FUSE infrastructure failure while mount overlay at \
             '/tmp/task-worktree/overlay': fuse: device not configured",
        );

        let perm_display = perm().to_string();
        assert_eq!(
            WorkspaceSyncError::Io {
                stage: "copy workspace entry",
                path: PathBuf::from("/tmp/task-worktree/src/main.rs"),
                source: perm(),
            }
            .to_string(),
            format!(
                "workspace sync failed while copy workspace entry at \
                 '/tmp/task-worktree/src/main.rs': {perm_display}",
            ),
        );
    }
}
