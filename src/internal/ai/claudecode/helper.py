...
def collect_provider_env(request: dict[str, Any]) -> dict[str, str]:
    """Collect provider-facing environment after applying Libra bootstrap overrides."""
    env = dict(os.environ)
    overrides = request.get("providerEnvOverrides")
    if overrides is not None:
        if not isinstance(overrides, dict):
            raise RuntimeError("providerEnvOverrides must be an object when present")
        for key, value in overrides.items():
            if not isinstance(key, str) or not isinstance(value, str):
                raise RuntimeError("providerEnvOverrides must contain only string pairs")
            env[key] = value

    unset = request.get("providerEnvUnset")
    if unset is not None:
        if not isinstance(unset, list):
            raise RuntimeError("providerEnvUnset must be an array when present")
        for key in unset:
            if not isinstance(key, str):
                raise RuntimeError("providerEnvUnset must contain only strings")
            env.pop(key, None)
...
def get_interactive_response_dir(state: dict[str, Any]) -> str | None:
    path = state.get("interactive_response_dir")
...
def assert_interactive_input_available(state: dict[str, Any]) -> None:
    if state["scripted_responses"]:
        return
    if get_interactive_response_dir(state):
        return
...
def next_interactive_request_id() -> str:
    return f"claude-{uuid.uuid4()}"


async def wait_for_interactive_response(
    response_dir: str,
    request_id: str,
    touch_idle_timeout: Callable[[], None] | None = None,
) -> dict[str, Any]:
    response_path = Path(response_dir) / f"{request_id}.json"
    while True:
        if touch_idle_timeout is not None:
            touch_idle_timeout()
        ...
        try:
            raw = await asyncio.to_thread(response_path.read_text, encoding="utf-8")
        ...
        if not isinstance(parsed, dict):
            raise RuntimeError(
                f"interactive response '{response_path}' must contain a JSON object"
            )
        return parsed
...
PLAN_MODE_READ_ONLY_TOOLS = {"Read", "Glob", "Grep", "LS"}


def is_plan_mode_read_only_tool(tool_name: str) -> bool:
    return tool_name in PLAN_MODE_READ_ONLY_TOOLS


def requires_manual_tool_approval(tool_name: str) -> bool:
    return tool_name == "Bash"


def approval_required_but_unavailable_message(tool_name: str) -> str:
    return (
        f"{tool_name} requires explicit user approval, but interactive approvals are disabled "
        "for this Claude managed session."
    )
...
def build_plan_mode_gate_lines(tool_name: str, tool_input: dict[str, Any]) -> list[str]:
    lines = [
        "",
        "Plan approval required",
        "Claude is trying to start execution while this session is still in plan mode.",
        f"Tool: {tool_name}",
        f"Input: {json.dumps(tool_input, indent=2, sort_keys=True)}",
        "",
        "Choose one:",
...
def parse_plan_mode_gate_decision(answer: str) -> str | None:
    normalized = answer.strip().lower()
    if normalized in {
        "1",
        "a",
    ...
    if normalized in {
        ...
        return "deny"
    if normalized in {"3", "b", "abort", "cancel"}:
        return "abort"
    return None
...
def normalize_question_payload(tool_input: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, str]]:
    questions = tool_input.get("questions")
    if not isinstance(questions, list):
        questions = []

    normalized_questions: list[dict[str, Any]] = []
    answer_key_by_id: dict[str, str] = {}
    for index, question in enumerate(questions):
        ...
        while question_id in answer_key_by_id:
        ...
        answer_key_by_id[question_id] = response_key

    return normalized_questions, answer_key_by_id
...
async def next_tool_approval_decision(
    ...
    context: Any,
    *,
    plan_mode_gate: bool = False,
) -> dict[str, str]:
    session_upgrade_available = plan_mode_gate or has_accept_edits_suggestion(suggestions)
    if state["scripted_responses"]:
        scripted = state["scripted_responses"].pop(0)
        if not isinstance(scripted, dict) or scripted.get("kind") not in {
            "tool_approval",
            "plan_mode_gate",
        }:
            raise RuntimeError("expected scripted tool_approval response")
        decision = scripted.get("decision")
        if plan_mode_gate and decision in {"approve", "approve_for_session"}:
    ...
    response_dir = get_interactive_response_dir(state)
    if response_dir:
        request_id = next_interactive_request_id()
        touch_idle_timeout = state.get("touch_idle_timeout")
        if callable(touch_idle_timeout):
            touch_idle_timeout()
        if plan_mode_gate:
            emit_ndjson_event(
                state.get("stream_mode") is True,
                "ask_user_question",
                {
                    ...
                    "questions": [
                        {
                            ...
                            "header": "Plan",
                            "question": (
                                f"Claude is ready to start execution with {tool_name}. "
                                "Approve execution, or keep refining the plan first?"
                            ),
                            "options": [
                                {
                                    "label": "Approve and start execution",
                                    "description": "Exit plan mode and switch this session to acceptEdits.",
                                },
                                {
                                    "label": "Keep planning",
                                    "description": "Deny execution for now and let Claude keep refining the plan.",
                                },
                                {
                                    "label": "Abort",
                                    "description": "Stop this action immediately.",
                                },
                            ],
                        }
                    ],
            ...
            emit_ndjson_event(
                ...
                "approval_request",
                {
                    "requestId": request_id,
                    "toolName": tool_name,
                    "toolInput": convert_jsonable(tool_input),
                    "sessionUpgradeAvailable": session_upgrade_available,
                    "reason": getattr(context, "description", None),
                },
            )
        response = await wait_for_interactive_response(
            response_dir,
            request_id,
            touch_idle_timeout if callable(touch_idle_timeout) else None,
        )
        if plan_mode_gate:
            raw_answers = response.get("answers")
            answer = raw_answers.get("plan_gate") if isinstance(raw_answers, dict) else ""
            decision = parse_plan_mode_gate_decision(answer if isinstance(answer, str) else "")
            if decision is None:
                raise RuntimeError("interactive plan gate response is missing a supported decision")
        else:
            decision = response.get("decision")
            if not isinstance(decision, str):
                raise RuntimeError("interactive approval response is missing 'decision'")
        if decision == "switch_session" and not session_upgrade_available:
            decision = "approve_for_session"
        if decision not in {
            "approve",
            ...
            raise RuntimeError(f"unsupported interactive approval decision: {decision!r}")
        return {
            "decision": decision,
            "prompt_source": (
                "interactive_response_dir_plan_gate"
                if plan_mode_gate
                else "interactive_response_dir"
            ),
        }
    ...
    while True:
        if plan_mode_gate:
            answer = await prompt_via_tty_async(
                build_plan_mode_gate_lines(tool_name, tool_input),
                "Choice [1] approve/[2] keep planning/[3] abort: ",
            )
            decision = parse_plan_mode_gate_decision(answer)
        else:
            prompt = (
        ...
        if decision is not None:
            return {
                "decision": decision,
                "prompt_source": (
                    "interactive_tty_plan_gate" if plan_mode_gate else "interactive_tty"
                ),
            }
...
async def collect_ask_user_question_answers(
    ...
    if response_dir:
        request_id = next_interactive_request_id()
        questions, answer_key_by_id = normalize_question_payload(tool_input)
        touch_idle_timeout = state.get("touch_idle_timeout")
        if callable(touch_idle_timeout):
            touch_idle_timeout()
        emit_ndjson_event(
        ...
        for question_id, response_key in answer_key_by_id.items():
            answer = raw_answers.get(question_id)
            if isinstance(answer, str):
                answers[response_key] = answer
        return {"answers": answers, "prompt_source": "interactive_response_dir"}
...
async def execute_query(request: dict[str, Any]) -> None:
    ...
    idle_timeout_state: dict[str, Any] = {"seconds": None, "scope": None}
    ...
    if isinstance(idle_timeout_seconds, (int, float)) and idle_timeout_seconds > 0:
        idle_timeout_state["seconds"] = float(idle_timeout_seconds)

    def bump_idle_timeout() -> None:
        scope = idle_timeout_state.get("scope")
        idle_timeout = idle_timeout_state.get("seconds")
        if scope is None or idle_timeout is None:
            return
        try:
            scope.reschedule(asyncio.get_running_loop().time() + idle_timeout)
        except Exception:
            idle_timeout_state["scope"] = None

    def emit_stream_event(event_type: str, payload: dict[str, Any] | None = None) -> None:
        bump_idle_timeout()
        emit_ndjson_event(stream_mode, event_type, payload)
    ...
    state = {
        ...
        "libra_plan_mode": request.get("libraPlanMode") is True,
        "interactive_approvals_enabled": request.get("interactiveApprovals") is True,
        "session_id": None,
        "model": None,
        "stream_mode": stream_mode,
        "interactive_response_dir": request.get("interactiveResponseDir"),
        "touch_idle_timeout": bump_idle_timeout,
    }
    ...
    env = collect_provider_env(request)
    ...
    options = ClaudeAgentOptions(
        ...
        hooks=build_hooks(
            hook_events,
            lambda event_type, payload: emit_stream_event(event_type, payload),
            emit_snapshot,
        ),
    ...
    if tools and (
        request.get("interactiveApprovals") is True
        or request.get("libraPlanMode") is True
    ):
        if request.get("interactiveApprovals") is True:
        ...
        async def can_use_tool(tool_name: str, tool_input: dict[str, Any], context: Any):
            ...
            plan_mode_gate_active = (
                state.get("libra_plan_mode") is True
                and tool_name != "AskUserQuestion"
                and not is_plan_mode_read_only_tool(tool_name)
            )
            if tool_name == "AskUserQuestion":
                if state.get("interactive_approvals_enabled") is not True:
                    return PermissionResultDeny(
                        message=approval_required_but_unavailable_message(tool_name)
                    )
            ...
            if plan_mode_gate_active:
                hook_events.append(
                    {
                        ...
                        "input": build_hook_input(
                            ...
                            context,
                            {
                                "interaction_kind": "plan_mode_gate",
                                "approval_decision": "deny",
                                "approval_scope": "request",
                                "prompt_source": "libra_plan_mode_gate",
                                "cached": False,
                            },
                        ),
                    }
                ...
                return PermissionResultDeny(
                    message=(
                        "Stay in plan mode and continue refining the plan before executing tools"
                    )
                )

            if not requires_manual_tool_approval(tool_name):
                hook_events.append(
                    {
                        "hook": "CanUseTool",
                        "input": build_hook_input(
                            ...
                            context,
                            {
                                ...
                                "approval_scope": "request",
                                "prompt_source": "auto_allow_non_bash",
                                "cached": False,
                            },
                        ),
                    }
                ...
                hook_events.append(
                    {
                        ...
                        "input": build_hook_input(
                            ...
                            context,
                            {
                                ...
                                "approval_decision": "deny",
                                "approval_scope": "request",
                                "prompt_source": "approval_disabled_by_policy",
                                "cached": False,
                            },
                        ),
                    }
            ...
            response = await next_tool_approval_decision(
                state, tool_name, tool_input, suggestions, context
            )
    ...
    async def consume_response() -> None:
        ...
        async with ClaudeSDKClient(options) as client:
            ...
            async for raw_message in client.receive_response():
                ...
                emit_stream_event("sdk_message", {"message": message})

                if message.get("type") == "system" and message.get("subtype") == "init":
                    emit_stream_event("session_init", {"message": message})
    ...
    async def consume_response_with_idle_timeout(idle_timeout: float) -> None:
        ...
        async with asyncio.timeout(None) as timeout_scope:
            idle_timeout_state["scope"] = timeout_scope
    ...
    try:
        ...
        emit_stream_event("error", {"error": "helper_timed_out"})
    except Exception as exc:
        helper_error = str(exc)
        emit_stream_event("error", {"error": helper_error})

    artifact = build_artifact(request, hook_events, messages, helper_timed_out, helper_error)
    if stream_mode:
        emit_snapshot()
        emit_stream_event("final_artifact", {"artifact": artifact})
    else:
