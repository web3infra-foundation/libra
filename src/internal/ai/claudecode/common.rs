...
pub(super) fn build_helper_wire_request(
    request: &ManagedHelperRequest,
) -> ManagedHelperWireRequest {
    // Keep session controls and provider credentials out of the JSON payload entirely.
...
pub(super) async fn invoke_helper(
    ...
    let wire_request = build_helper_wire_request(request);
