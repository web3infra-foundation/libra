...
impl<M: CompletionModel + Clone + 'static> App<M> {
    ...
    async fn start_plan_workflow(&mut self, request: &str) {
        ...
        let handle = tokio::spawn(async move {
            ...
            // Phase 2: Generate implementation plan based on IntentSpec
            struct NoOpObserver;
            impl crate::internal::ai::agent::ToolLoopObserver for NoOpObserver {}
            let mut noop_observer = NoOpObserver;

            let implementation_prompt = format!(
                "Based on the following IntentSpec, generate a detailed step-by-step implementation plan. \
                For each step, specify:\n\
                - File path to modify\n\
                - Type of change (create/modify/delete)\n\
                - Brief description of the change\n\
                \nIntentSpec:\n```json\n{}\n```\n\
                \nProvide the implementation plan in a clear, numbered list format.",
                pretty_json
            );
            new_history.push(Message::user(implementation_prompt));

            let implementation_result = run_tool_loop_with_history_and_observer(
                &model,
                new_history.clone(),
                "",
                &registry,
            ...
            let implementation_plan = match implementation_result {
                Ok(turn) => {
                    if let Some(msg) = turn.history.last() {
                        match msg {
                            Message::Assistant { content, .. } => {
                                ...
                                content
                                    ...
                                    .filter_map(|c| match c {
                                        crate::internal::ai::completion::AssistantContent::Text(
                                            t,
                                        ) => Some(t.text.clone()),
                                        _ => None,
                                    })
                            ...
                            _ => "No implementation plan generated.".to_string(),
                        }
                    } else {
                        "No implementation plan generated.".to_string()
                    }
                }
                Err(e) => format!("Failed to generate implementation plan: {}", e),
            };

            new_history.push(Message::assistant(implementation_plan.clone()));
            summary.push_str(&format!(
                "\n\n## Implementation Plan\n\n{}",
                implementation_plan
            ));
