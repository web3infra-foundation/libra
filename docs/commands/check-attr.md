...
Report the `.libra_attributes` attributes that apply to one or more pathnames —
the Libra analogue of `git check-attr`.
...
> `.libra_attributes`, not a filter driver. Libra's attribute model currently
> expresses a single meaningful attribute: `filter` (value `lfs` for an
> LFS-tracked path).
...
Libra recognizes only the `filter` attribute (mapped to LFS), and does not run
smudge/clean filters. Git's `--cached`, `--source`, and arbitrary
`.gitattributes` macros are not applicable.
