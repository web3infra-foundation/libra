...
`pull` does not implement `--rebase`, `--ff-only`, `--squash`, custom merge strategies, or pull-specific strategy flags.
...
| Rebase on pull | Not supported | `git pull --rebase` | N/A |
...
Every `PullError` variant maps to an explicit `StableErrorCode`. Fetch and merge sub-errors are forwarded with a `phase` detail for diagnostics.
...
When a fetch or merge sub-operation fails, the error JSON includes a `phase` key in the details object (`"fetch"` or `"merge"`) so agents can distinguish which stage failed.
