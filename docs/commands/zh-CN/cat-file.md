...
| `--batch-command[=<fmt>]` | | 从 stdin 逐行读命令：`info <object>`（仅 header）或 `contents <object>`（header + 内容）。`flush` 需 `--buffer`（未公开）。 |
...
Git 的 batch 模式从 stdin 读取对象 ID（或命令）以批量检查。Libra 已公开 `--batch-check`、`--batch` 和 `--batch-command`（后者逐行分发 `info`/`contents`），三者共享同一个逐对象 formatter，并支持可选的 `=<format>` 原子展开。对代理而言，仍推荐 `--json`——它在一次调用中返回类型化字段。流式 `--buffer`/`flush` 与 `--batch-all-objects` 未公开；没有 `--buffer` 时，`flush` 命令会像 Git 一样被拒绝。
...
| 检查存在性 | `-e` | `-e` | N/A |
| Batch 模式 | `--batch`, `--batch-check`, `--batch-command`（带可选 `=<format>`） | `--batch`, `--batch-check`, `--batch-command` | N/A |
