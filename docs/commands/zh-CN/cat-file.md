...
Git 的 batch 模式从 stdin 读取对象 ID（或命令）以批量检查。Libra 已公开 `--batch-check`、`--batch` 和 `--batch-command`（后者逐行分发 `info`/`contents`），三者共享同一个逐对象 formatter，并支持可选的 `=<format>` 原子展开。`--batch-all-objects`（配合 `--batch`/`--batch-check`）遍历存储中的每个对象——loose 加 packed——按 id 排序，替代从 stdin 读取。对代理而言，仍推荐 `--json`——它在一次调用中返回类型化字段。流式 `--buffer`/`flush` 与 `--follow-symlinks` 未公开；没有 `--buffer` 时，`flush` 命令会像 Git 一样被拒绝。
