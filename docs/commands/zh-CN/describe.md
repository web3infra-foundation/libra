...
### 为什么没有 `--long`、`--match`、`--exclude`？

Git 的 `describe` 多年来积累了许多选项：`--long` 即使在精确匹配时也强制长格式，`--match` 和 `--exclude` 按 glob 过滤标签名，`--candidates` 控制考虑多少标签，`--first-parent` 限制遍历。Libra 有意提供最小子集，覆盖主要用例：识别构建版本并提供人类可读的提交引用。基于 BFS 的算法直接且可预测。如果真实用户或代理需要额外标志，可以逐步添加；但从小集合开始可以避免让 Git `describe` 行为难以推理的组合复杂性（例如 `--match`、`--exclude` 和 `--candidates` 之间的交互）。
