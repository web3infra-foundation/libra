...
Libra 已公开 `--tags`、`--abbrev`、`--always`、`--exact-match`、`--long`、`--dirty[=<mark>]`、`--first-parent`、`--match`/`--exclude`（wax glob，≤256 字符，exclude 优先）以及 `--candidates <N>`（`N=0` 等价 `--exact-match`；`N≥1` 维持确定性最近-tag BFS，不强制 Git 的正数候选上界）。基于 BFS 的算法直接且可预测。`--contains` 与 `--all` 尚未实现：它们依赖 Git 的反向遍历 / 全 ref 算法，Libra 可预测的 BFS 不提供；如果真实用户或代理需要，可以逐步添加。
