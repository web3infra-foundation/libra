...
`libra stash` 将本地修改保存为新的 stash 条目，并把工作目录还原到与 HEAD 一致。之后可以用 `libra stash pop` 或 `libra stash apply` 恢复这些修改。如果在干净工作树上运行 `stash push`，命令会作为无操作成功退出，并报告没有可保存的本地更改。
...
Git 的 `stash push --keep-index` 会贮藏更改，但保留索引（已暂存文件）不变。它主要用于提交前测试已暂存更改。实践中，这个标志容易制造令人困惑的状态：工作树被重置，但索引没有重置，产生让用户意外的错配。Libra 的做法更简单：stash 捕获全部内容，而 `libra restore --staged` 提供了更清晰的索引独立操作方式。

### 为什么没有 `--include-untracked` / `--all`？

Git 的 `--include-untracked`（`-u`）和 `--all`（`-a`）分别会贮藏未跟踪文件和被忽略文件。这些标志很少需要，并且会显著增加 stash 存储格式的复杂度（需要额外的树对象）。在 Libra 中，未跟踪文件不属于版本控制状态，应通过其他方式管理（例如用 `libra clean` 移除，或直接保留在原处）。

### 为什么采用精选子命令模型？

Git 的 stash 经过长期演化，支持把 `git stash` 作为 `git stash push` 的简写，还支持 `git stash save`（已弃用）以及 plumbing 组合 `git stash create` / `git stash store`。Libra 暴露用户实践中实际会用到的八个子命令：`push`、`pop`、`list`、`apply`、`drop`、`show`、`branch` 和 `clear`。Plumbing 组合（`create` / `store`）以及 `save` 简写被推迟；参见 [`docs/improvement/compatibility/declined.md`](../improvement/compatibility/declined.md) 的 D8 和 D9 小节。这让日常工作流保持与标准 Git 对齐，同时把很少使用的 plumbing 留在维护面之外。
...
| 保留索引 | 不支持 | `--keep-index` / `--no-keep-index` | N/A |
| 包含未跟踪 | 不支持 | `-u` / `--include-untracked` | N/A |
| 包含全部（也含忽略文件） | 不支持 | `-a` / `--all` | N/A |
| Pathspec（部分 stash） | 不支持 | `-- <pathspec>...` | N/A |
