...
显式 exclude-source 标志（`-x` / `--exclude-from`）、resolve-undo 和 sparse-checkout 集成仍未公开。
...
| `-i`、`--ignored` | 只显示被忽略的集合：`-i -o` 列出被忽略的未跟踪文件（`-o` 的反集），`-i -c` 列出匹配 exclude 模式的已跟踪文件。须配合 `-o`/`-c` 且需要 `--exclude-standard`（否则退出码 128），与 git 一致。 |
