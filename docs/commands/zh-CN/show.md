...
Git 的 `--format` / `--pretty=format:` 机制很强大但复杂，包含数十个 `%` 占位符和条件格式。Libra 改为提供结构化 JSON 输出（`--json`），为程序消费者提供类型良好的 schema 中的每个字段。人类用户获得合理的默认呈现。这避免了维护格式迷你语言的负担，同时给代理一个严格更好的接口（类型化 JSON 字段，而不是字符串解析）。
...
| `--format` / `--pretty=format:` | 否（使用 `--json`） | 是 | 否（使用模板） |
| `--quiet` | 是（仅验证） | 否 | N/A |
