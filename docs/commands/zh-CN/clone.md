...
首个 Cloudflare 恢复表面不接受 Git 传输整形标志：`--branch`、`--depth`、`--single-branch`、`--bare` 和 `--mirror` 会在查找 clone-domain 配置之前、创建目标目录之前返回 `LBR-CLI-002`。请在源 URL 上使用 `?ref=<branch|tag|full-ref>` 选择检出目标。
