...
| 标志 | 短选项 | 说明 | 默认值 |
|------|-------|-------------|---------|
| `<PACK_FILE>` | | 要索引的 `.pack` 文件路径（必需）。除非给出 `-o`，否则必须以 `.pack` 结尾。 | |
...
| `--stdin`（从 stdin 读取 pack） | 未实现 | 是 | N/A |
| `--fix-thin`（为 thin pack 添加 base） | 未实现 | 是 | N/A |
| `--keep`（创建 .keep 文件） | 未实现 | 是 | N/A |
| `--threads`（并行解压） | 内部使用（8 线程） | `--threads=<N>` | N/A |
| 进度输出 | 未实现 | `--progress` / `--no-progress` | N/A |
| JSON 输出 | `--json` | 无 | N/A |
