...
```
libra config [--get | --get-all | --unset | --unset-all | -l | --add | --import | --get-regexp | --show-origin] [--local | --global] [-z | --null] [--type <t> | --bool | --int | --path] [key] [value] [-d <default>]
...
| `--type=<bool\|int\|path>`、`--bool`、`--int`、`--path` | 读取时（配合 `--get`/`--get-all`/`--get-regexp`）规范化值：bool 变体 → `true`/`false`；int 支持可选 k/m/g（1024 倍率）；path 展开开头的 `~`/`~/`。非法值与非 get 模式会被拒绝（exit 129）。 |
...
| 类型转换 | `--type=bool\|int\|path` | 无（TOML 类型） | `--type=bool\|int\|path` + `--bool`/`--int`/`--path`（读取时规范化；SET 侧校验暂未实现） |
