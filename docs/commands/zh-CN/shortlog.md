...
```
libra shortlog [<revision>] [-n] [-s] [-e] [-c] [--no-merges | --merges] [--since <date>] [--until <date>] [-w[<W>[,<I1>[,<I2>]]]]
```
...
Libra 将修订作为位置参数，并始终直接从提交图读取。这更简单、更快（无需序列化/反序列化），并且与 `--json` 输出模式自然配合。对于超出内置过滤器（`--since`/`--until`、`--author`、`--merges`/`--no-merges`）能力的过滤，请使用 `libra log --json` 加外部工具。
