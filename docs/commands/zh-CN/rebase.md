...
Git 的 `--onto` 标志允许将提交子集 rebase 到任意 base 上，独立于 upstream 引用。这很强大但很少使用，并且会让三参数形式（`git rebase --onto <newbase> <upstream> [<branch>]`）产生困惑。

Libra 简化为始终将从共同祖先到 HEAD 的所有提交 rebase 到指定 upstream 上。这覆盖绝大多数 rebase 用例。如果对更精确提交范围选择有需求，将来可以添加 `--onto` 标志。
