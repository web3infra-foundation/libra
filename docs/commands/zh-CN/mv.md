...
该命令验证所有来源路径都存在、已在索引中跟踪、不处于冲突状态，并且位于仓库工作目录内。目录移动作为单次文件系统 rename 执行，同时为目录内每个已跟踪文件更新对应索引条目。被移动目录内的未跟踪文件会随文件系统 rename 一起移动，但不会被添加到索引。
...
Git 的 `mv` 支持 `--sparse`，以允许移动 sparse-checkout cone 外的文件。Libra 尚未实现 sparse checkout，因此该标志没有意义。如果并且当 sparse checkout 支持实现时，它会被加入。
...
Git 的 `mv` 命令设计类似：它在工作树中移动文件并更新索引。它支持少量额外标志（`-k` 用于跳过移动错误，`--sparse`），其他方面较直接。
...
| 跳过错误 | 不支持 | `-k` | N/A |
| Sparse checkout | 不支持 | `--sparse` | N/A |
