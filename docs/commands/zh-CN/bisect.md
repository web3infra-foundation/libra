...
显示当前 bisect 状态：good / bad 边界、当前 HEAD、剩余候选和任何 skipped commits。
...
Git 的 `git bisect --first-parent` 将搜索限制到 first-parent commits，这对包含大量 merge commits 的工作流有用。Libra 的 bisect 使用 BFS 遍历完整提交图，这更简单，并且对所有拓扑都正确。First-parent 限制主要适合严格 merge-commit 工作流的大型项目；Libra 的目标用例是 trunk-based 开发，历史通常更线性，不需要此优化。
...
| 显示当前状态 | `bisect view` | `bisect visualize`（GUI / log） | N/A |
...
| 可视化（GUI） | 不支持 | `bisect visualize` | N/A |
| 仅 first-parent | 不支持 | `--first-parent` | N/A |
