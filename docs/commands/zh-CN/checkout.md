...
`libra checkout` 是一个 Git 兼容表面，内部委托给 `switch` 和 `restore`。它支持最常见的 `git checkout` 模式：显示当前分支、切换到已有分支、用 `-b` 创建新分支、自动跟踪远程分支，以及在存在显式 `--` 分隔符时恢复路径。
...
| 找不到分支（无远程匹配） | `LBR-CLI-003` | "path specification '{name}' did not match any files known to libra" | 128 |
