...
| 不用 overlay | | `--no-overlay` | 不以 overlay 模式恢复（source 中缺失的路径仍会从目标移除）。为对齐 Git 而接受的 no-op：Libra 的 restore 从不处于 overlay 模式，已是 Git 默认。（Git 的 `--overlay` 未实现。） |
...
> **尚未支持：** `--merge` / `--conflict=<style>`（重渲染冲突标记）、`--overlay`（正向 overlay 模式）与 `-p` / `--patch` 仍被推迟。参见 [COMPATIBILITY.md](../../../COMPATIBILITY.md)。
...
| Overlay 模式 | `--no-overlay`（no-op；从不处于 overlay 模式）；`--overlay` 不支持（推迟） | `--overlay` / `--no-overlay` | N/A |
| 冲突解决 | `--ours` / `-2`、`--theirs` / `-3`（仅工作树）；`--merge` / `--conflict` 推迟 | `--ours` / `--theirs` / `--merge` | `--restore-descendants` |
