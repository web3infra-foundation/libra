...
`--contains` 和 `--no-contains` 过滤器（别名为 `--with` 和 `--without`）可将分支列表缩小到历史中包含或不包含某个提交的分支；省略提交参数时默认为 HEAD。`--merged [<commit>]` / `--no-merged [<commit>]` 列出已合并（或尚未合并）入某提交的分支——即 tip 是否可从该提交到达，缺省 HEAD，是 `--contains` 的反方向。`--sort <key>` 按 `refname` 或 `version:refname`（数值感知）排序，前导 `-` 反转。
...
| | `--sort` | `<key>` | 按 `refname` 或 `version:refname`（`v:refname`）排序；前导 `-` 反转（dash 形式用 `--sort=-refname`） |
| | `--column[=<mode>]` | `always` / `auto` / `never` | 将分支列表按列布局而非每行一个（bare `--column` 即 `always`；`auto` 仅在 stdout 为终端时）。列模式显示纯文本无颜色的名称。 |
| `-v` | `--verbose` | | 每个分支附带其 tip 的短 sha 与提交 subject。可重复（`-vv`）；Git 的 `-vv` 上游 tracking 细节尚未显示，故 `-v` 与 `-vv` 渲染相同。优先于 `--column`。 |
...
| 列表排序 | `git branch --sort <key>` | `libra branch --sort <key>`（refname / version:refname） | `jj branch list`（revset 顺序） |
| 列布局 | `git branch --column[=<mode>]` | `libra branch --column[=<mode>]` | N/A |
| 详细列表 | `git branch -v` / `-vv` | `libra branch -v`（sha + subject；`-vv` 同，暂无上游） | N/A |
