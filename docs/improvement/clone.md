...
- `InitError → CliError` 映射已完成显式 `StableErrorCode` 绑定（`init.rs:116` 起的 `impl From<InitError> for CliError`），每个 `InitError` 变体都有确定的 `StableErrorCode`
...
- `restore::execute_checked(args) -> io::Result<()>`（`restore.rs:438`）会把“引用无法解析”“索引读取失败”“对象读取失败”“工作树写入失败”等不同原因全部压平成 `io::Error`
- `restore::execute_safe(args, output) -> CliResult<()>`（`restore.rs:177`）又会进一步包成通用 `CliError::fatal(e.to_string())`
...
当前 `From<CloneError> for CliError`（`clone.rs:380` 起的 impl block）已为每个变体和关键 source 子类型显式映射 `StableErrorCode`：
...
- `restore::execute_checked(args) -> io::Result<()>`（`restore.rs:438`）：底层 checkout 实现，跳过 `require_repo()` 检查
- `restore::execute_safe(args, output) -> CliResult<()>`（`restore.rs:177`）：包裹层，带 `require_repo()` 前置检查
...
> **注**：Tip 格式与 init 的 `render_init_result()` 保持一致（`init.rs:373` 起），使用 `display_home_relative()` 缩短路径显示。
