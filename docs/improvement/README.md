...
| **31** | `mv` / `rm` / `worktree` | 部分落地：`mv` / `rm` / `worktree list` / `worktree remove` 成功路径已有 JSON/machine；worktree 其余子命令与 destructive typed error 仍缺结构化输出 | destructive 路径的结果模型、显式错误码、确认消息 |
