...
| **32** | `merge` / `rebase` | 部分落地：[`merge`](merge.md) 已有 `run_merge()` / `render_merge_output()`、fast-forward / already-up-to-date / remote ref JSON/machine、non-fast-forward JSON error envelope 和显式错误码；`rebase` 仍待收口 | rebase 状态结构化、冲突契约、typed error；merge 后续仅做兼容 additive 扩展 |
