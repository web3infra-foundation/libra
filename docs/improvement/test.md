...
本文目标：给出整个 `libra code` 功能的端到端测试范围地图，并把历史 Wave 规划与当前实现状态对齐。第 5 节按“功能面 × 测试分层 × 现状/缺口/优先级”组织 20 项纵深面；第 6 节内嵌 L2 远端矩阵方案；第 7–10 节给出统一覆盖矩阵、文件清单、验证命令和 12 PR Wave roadmap。当前剩余项集中在 Wave 9 binary 级 Codex、MCP-originated write-observe 与 Wave 12 长时 SSE soak；不要再把已落地的 harness/matrix 文件列为前置阻塞。
...
| 🟡 需新增中等复杂度组件 | 需要补齐 binary 级编排或跨入口观察，但已有明确 helper 可参考 | Codex binary boot smoke、MCP-originated `tools/call` → web SSE write-and-observe |
...
| MCP 双入口一致 | partial | control.json mcpUrl + --stdio mutex + dual-reachability smoke + web `/messages` → SSE/MCP observe；MCP-originated write → web SSE 仍 deferred |
...
- **现状 ✅⚠️**：[tests/code_codex_default_tui_test.rs](../../tests/code_codex_default_tui_test.rs) 提供静态守卫；[tests/code_codex_runtime_test.rs](../../tests/code_codex_runtime_test.rs) 已有 `--codex-port 0` validation smoke、`MockCodexWsServer` helper、handshake round-trip smoke，以及真实 `libra code --provider codex --codex-port <mock>` binary boot smoke。binary smoke 已验证运行时向 mock app-server 发出 `initialize` / `thread/start`，转发选定 model，并在 Codex 默认 plan mode 下带上 `developerInstructions` / `baseInstructions` payload。
- **缺口**：
  - 仍待：mock app-server 回放 Codex notification 后，验证落到 `.libra/objects/` 与 history index。
...
- **现状 ✅⚠️**：[tests/mcp_integration_test.rs](../../tests/mcp_integration_test.rs)、[tests/e2e_mcp_flow.rs](../../tests/e2e_mcp_flow.rs) 覆盖资源/工具列表与 initialize 握手；[tests/code_mcp_dual_entry_test.rs](../../tests/code_mcp_dual_entry_test.rs) 已覆盖 control.json mcpUrl、`--stdio` mutex、同进程 web/MCP dual-reachability smoke，以及 web `/messages` 写入后由 web SSE 与 MCP `list_tasks` 同时观察的实际一致性路径。
- **缺口**（Code 路径）：
  ...
  - 仍待：MCP-originated `tools/call` 写入广播到 web SSE transcript 的反向一致性；当前代码尚未把外部 MCP 工具写入映射为 Code UI transcript event。
- **优先级**：P1。
...
| 5.13 | Codex 旁路运行时 | – | – | partial | – | ⚠️ | notification persistence / plan approve gate / reconnect |
| 5.14 | MCP 双入口一致 | – | partial | smoke + web→MCP observe | – | ⚠️ | MCP-originated write → web SSE observe |
...
- 扩展 [tests/code_mcp_dual_entry_test.rs](../../tests/code_mcp_dual_entry_test.rs)：MCP-originated `tools/call` 写入与 web SSE 观察同线程编排；web `/messages` → SSE/MCP observe 方向已由 `web_message_turn_is_observable_through_sse_and_mcp_task_list` 覆盖。
...
| 9 | Codex runtime + MCP 双入口 + resume | ⚠️ partial | Codex notification persistence / plan approve gate / reconnect、MCP-originated write → web SSE observe | 无新增依赖 |
...
落地完成判定的全部门：剩余阻塞项均为明确的 roadmap-sized 多 PR 工作 —— Wave 9 §5.13 notification persistence / plan approve gate / reconnect（在现有 `MockCodexWsServer` 上继续扩展）和 §5.14 MCP-originated write-and-observe（MCP `tools/call` 写入映射到 web SSE transcript 观察）。已闭合：Wave 9 §5.13 partial（`--codex-port 0` validation + `MockCodexWsServer` 协议 helper + handshake round-trip smoke + binary boot smoke + 默认 plan-mode payload）、§5.14 item 1/2/3-smoke + web `/messages` → SSE/MCP observe、§5.16 happy-path + SIGTERM-mid-turn、Wave 10 §5.2 全量 7/7 provider + missing-key / `--api-base` quick-follow、Wave 10 §5.8 TUI render 复杂状态。Wave 11 已具备完整工作流（harness wiring + 每日 nightly + L0 regression）；剩余条件（5 天连续 ≥ 90% 通过率）依赖 maintainer 配置 `DEEPSEEK_API_KEY` 并等待 5 个 nightly run。Wave 12 perf smoke 现含 1k events SSE broadcast monotonic 验证；1-小时真网 SSE soak 需独立 nightly job。当前仓库状态对应"基础矩阵已落地 + Wave 10/11/12 已 wire + Wave 9 部分 deferred"。
