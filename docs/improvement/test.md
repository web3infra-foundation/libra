...
本文目标：给出整个 `libra code` 功能的端到端测试范围地图，并把历史 Wave 规划与当前实现状态对齐。第 5 节按“功能面 × 测试分层 × 现状/缺口/优先级”组织 20 项纵深面；第 6 节内嵌 L2 远端矩阵方案；第 7–10 节给出统一覆盖矩阵、文件清单、验证命令和 12 PR Wave roadmap。当前剩余项集中在 Wave 9 Codex notification / plan approve / reconnect 与 Wave 12 长时 SSE soak；不要再把已落地的 harness/matrix 文件列为前置阻塞。
...
| 🟡 需新增中等复杂度组件 | 需要补齐 Codex notification / plan approve / reconnect 编排，但已有明确 helper 可参考 | Codex mock WS notification persistence、plan approve gate、reconnect |
...
| 9 | Codex runtime + MCP 双入口 + resume | ⚠️ partial | Codex notification persistence / plan approve gate / reconnect | 无新增依赖 |
