...
| 9 | ⚠️ partial | `--resume` CLI 表面 3 条（unknown UUID、unknown 非 UUID 字符串、happy-path 跨进程恢复 transcript）；§5.16 closure criterion ✅；§5.13 partial — `--codex-port 0` validation smoke ✅；§5.14 item 1 (control.json carries mcpUrl with distinct MCP/web port) + item 2 (--stdio + --web-only mutex) ✅；§5.13 full Codex WS runtime mock (handshake + notifications + persistence + plan gate + reconnect) 仍未交付（roadmap-sized 多 PR 工作）；§5.14 item 3 (dual-entry MCP↔web 同线程一致性) 仍未交付（需并行 rmcp + matrix attach SSE 同会话编排）；§5.16 SIGTERM-mid-turn 仍未交付 |
...
落地完成判定的全部门：剩余阻塞项均为明确的 roadmap-sized 多 PR 工作 —— Wave 9 §5.13 Codex WebSocket runtime mock（需 tokio-tungstenite WS+JSON-RPC handshake + 通知回放 + persistence + plan-mode gate + reconnect）和 §5.14 item 3 dual-entry 同线程一致性（需并行 rmcp + matrix attach SSE 同会话编排）。已闭合：Wave 9 §5.13 `--codex-port 0` validation smoke、§5.14 item 1/2、§5.16 happy-path、Wave 10 §5.2 全量 7/7 provider、Wave 10 §5.8 TUI render smoke。Wave 10 missing-API-key + --api-base override 为 quick-follow 不阻塞。Wave 11 已具备完整工作流（harness wiring + 每日 nightly + L0 regression）；剩余条件（5 天连续 ≥ 90% 通过率）依赖 maintainer 配置 `DEEPSEEK_API_KEY` 并等待 5 个 nightly run。Wave 12 perf smoke 现含 1k events SSE broadcast monotonic 验证；1-小时真网 soak 需独立 nightly job。当前仓库状态对应"基础矩阵已落地 + Wave 11/12 已 wire + Wave 9 / 10 部分 deferred"。
