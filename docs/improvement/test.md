...
本文目标：给出整个 `libra code` 功能的端到端测试范围地图，并把历史 Wave 规划与当前实现状态对齐。第 5 节按“功能面 × 测试分层 × 现状/缺口/优先级”组织 20 项纵深面；第 6 节内嵌 L2 远端矩阵方案；第 7–10 节给出统一覆盖矩阵、文件清单、验证命令和 12 PR Wave roadmap。当前剩余项集中在 Wave 12 长时 SSE soak 与 Model generation 5-day pass-rate 这类外部运行条件；不要再把已落地的 harness/matrix 文件列为前置阻塞。
...
| 🟡 需新增中等复杂度组件 | 当前无未闭合项；后续新增测试面仍应复用既有 harness / mock server | 无 |
...
- **现状 ✅**：[tests/code_codex_default_tui_test.rs](../../tests/code_codex_default_tui_test.rs) 提供静态守卫；[tests/code_codex_runtime_test.rs](../../tests/code_codex_runtime_test.rs) 已有 `--codex-port 0` validation smoke、`MockCodexWsServer` helper、handshake round-trip smoke、真实 `libra code --provider codex --codex-port <mock>` binary boot smoke、`thread/started` notification 持久化 smoke、plan gate smoke，以及 managed startup reconnect smoke。binary smoke 已验证运行时向 mock app-server 发出 `initialize` / `thread/start`，转发选定 model，并在 Codex 默认 plan mode 下带上 `developerInstructions` / `baseInstructions` payload；notification smoke 已验证 mock `thread/started` 写入 `.libra/objects/` 并通过 MCP history index 可读；plan gate smoke 已验证 mock command-execution approval 先在 Code UI 中成为 pending interaction，且 browser controller 响应前不会向 Codex 发送 `requestApproval/resolve`；startup reconnect smoke 已验证 readiness probe 的短连接断开后，真实 Code UI runtime 会重新连接并完成握手。
- **缺口**：无（5.13 已关闭）。如果后续要支持 mid-turn Codex 进程重启恢复，应作为独立 resilience hardening 处理，不再阻塞本 12-wave 测试计划。
- **优先级**：已完成。
- **测试位置**：**L2 已覆盖** `tests/code_codex_runtime_test.rs` + 简易 mock WS server（用 `tokio-tungstenite` 接受连接 + 回放固定脚本）。
- **AI 落地提示**：`tokio-tungstenite` 已在 `Cargo.toml`。后续 Codex runtime 扩展继续复用现有 `MockCodexWsServer`，不要另起并行 helper。
...
- [tests/code_codex_runtime_test.rs](../../tests/code_codex_runtime_test.rs)：§5.13 当前闭合；后续 Codex resilience hardening 继续复用该文件与 `MockCodexWsServer`。
...
- 后续并行工作应只围绕 PR 12 的独立 nightly soak，以及 Model generation 的 secret/nightly 稳定性窗口。
