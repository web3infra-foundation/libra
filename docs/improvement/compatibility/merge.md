...
- 合并质量：criss-cross 历史下的真实 LCA merge-base 选择；rename detection（`--find-renames[=<n>]`/`--no-renames` + `merge.renames` / `merge.renameLimit`，默认 50% Dice 相似度阈值），让一侧的编辑跟随另一侧的重命名，重命名后若仍冲突则回退到 delete/modify 冲突。
...
`merge.commit` 已作为 Libra 兼容配置实现：`false` 等同 `--no-commit`，CLI `--commit` / `--no-commit` 优先。
