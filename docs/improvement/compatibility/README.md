...
- [`.github/CODEOWNERS`](../../../.github/CODEOWNERS) 已使用仓库维护团队 handle；仍需在 GitHub UI 验证团队存在性、Code Owners 自动请求评审和 branch protection 绑定。
...
- C1 的 CODEOWNERS 文件已写入维护团队 handle；不得把这等同于平台层完成，仍需在 GitHub UI 核对团队权限、Code Owners review 和 required-checks 配置。
