...
- **当前代码证据**：[`src/internal/ai/hooks/config.rs:92`](../../../src/internal/ai/hooks/config.rs#L92) 只读取 `.libra/hooks.json` 与用户级 `libra/hooks.json`；[`src/internal/ai/hooks/runtime.rs:132`](../../../src/internal/ai/hooks/runtime.rs#L132) 的 `libra hooks <command>` 入口处理 provider lifecycle envelope，不读取 `.git/hooks` 或 `core.hooksPath`。
...
- **当前代码证据**：[`src/command/clone.rs:68`](../../../src/command/clone.rs#L68) 的 `CloneArgs` 只公开 remote/path、`--branch`、`--single-branch`、`--bare`、`--depth`，没有 `--recurse-submodules`。
...
- **当前代码证据**：[`src/cli.rs:373`](../../../src/cli.rs#L373) 的 `Bisect` 子命令只包含 `start` / `bad` / `good` / `reset` / `skip` / `log` / `run` / `view`，没有 `replay`。
...
- **当前代码证据**：[`src/cli.rs:373`](../../../src/cli.rs#L373) 的 `Bisect` 子命令没有 `terms`。
...
- **当前代码证据**：[`src/cli.rs:323`](../../../src/cli.rs#L323) 的 `Stash` 子命令只包含 `push` / `pop` / `list` / `apply` / `drop` / `show` / `branch` / `clear`，没有 `create`。
...
- **审计原文要点**：审计列入 stash 完整子命令面。
- **当前代码证据**：[`src/cli.rs:323`](../../../src/cli.rs#L323) 的 `Stash` 子命令没有 `store`。
- **当前状态**：C4 落地后 [stash-surface.md](stash-surface.md) 已记录延后。
...
- **当前代码证据**：[`src/command/clone.rs:68`](../../../src/command/clone.rs#L68) 的 `CloneArgs` 没有 `--sparse` flag；[`src/cli.rs:170`](../../../src/cli.rs#L170) 的 `Commands` 没有 `SparseCheckout` 变体；`src/internal/` 中没有 sparse-checkout reader 实现。
