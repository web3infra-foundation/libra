...
- **当前代码证据**：[`src/command/push.rs:1329`](../../../src/command/push.rs#L1329) 已有 "submodule is not supported yet" 警告；[`README.md`](../../../README.md) 明确声明 Libra 不提供 submodule / subtree（按需在 README 内 grep "submodule"）。
...
- **当前代码证据**：[`src/cli.rs:391`](../../../src/cli.rs#L391) 的 `Bisect` 子命令只包含 `start` / `bad` / `good` / `reset` / `skip` / `log` / `run` / `view`，没有 `replay`。
...
- **当前代码证据**：[`src/cli.rs:391`](../../../src/cli.rs#L391) 的 `Bisect` 子命令没有 `terms`。
...
- **当前代码证据**：[`src/cli.rs:341`](../../../src/cli.rs#L341) 的 `Stash` 子命令只包含 `push` / `pop` / `list` / `apply` / `drop` / `show` / `branch` / `clear`，没有 `create`。
...
- **审计原文要点**：审计列入 stash 完整子命令面。
- **当前代码证据**：[`src/cli.rs:341`](../../../src/cli.rs#L341) 的 `Stash` 子命令没有 `store`。
- **当前状态**：C4 落地后 [stash-surface.md](stash-surface.md) 已记录延后。
...
- **当前代码证据**：[`src/command/clone.rs:98`](../../../src/command/clone.rs#L98) 的 `CloneArgs` 没有 `--sparse` flag；[`src/cli.rs:185`](../../../src/cli.rs#L185) 的 `Commands` 没有 `SparseCheckout` 变体；`src/internal/` 中没有 sparse-checkout reader 实现。
