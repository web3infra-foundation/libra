...
- **当前代码证据**：[`src/cli.rs:385`](../../../src/cli.rs#L385) 的 `Bisect` 子命令只包含 `start` / `bad` / `good` / `reset` / `skip` / `log` / `run` / `view`，没有 `replay`。
...
- **当前代码证据**：[`src/cli.rs:385`](../../../src/cli.rs#L385) 的 `Bisect` 子命令没有 `terms`。
...
- **当前代码证据**：[`src/cli.rs:335`](../../../src/cli.rs#L335) 的 `Stash` 子命令只包含 `push` / `pop` / `list` / `apply` / `drop` / `show` / `branch` / `clear`，没有 `create`。
...
- **审计原文要点**：审计列入 stash 完整子命令面。
- **当前代码证据**：[`src/cli.rs:335`](../../../src/cli.rs#L335) 的 `Stash` 子命令没有 `store`。
- **当前状态**：C4 落地后 [stash-surface.md](stash-surface.md) 已记录延后。
