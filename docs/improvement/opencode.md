...
>
> Libra 仓库 `/Volumes/Data/GitMono/libra`。截至 2026-05-21，本文已从落地方案推进为实现追踪文档：Provider adapter / agent profile / sub-agent dispatcher / context handoff / Goal supervisor / GA 配置面均有对应代码与测试落点，后续维护需保持本文件与实现同步。
...
- [PR 切片建议](#pr-切片建议)
- [Agent 自动开发与健壮性规范](#agent-自动开发与健壮性规范)
- [风险与反模式](#风险与反模式)
- [非目标](#非目标)
...
Libra 现有 `ApprovalCachePolicy`（[src/internal/ai/sandbox/mod.rs:174](../../src/internal/ai/sandbox/mod.rs)）覆盖了 TTL、Scope、Sensitivity Tier，但**没有 `Always` 与 `request.always[]` 的 pattern-level 持久化合同**。OC-Phase 2 必须在 `ApprovalCachePolicy` 之上新增 `ApprovedRuleset` 投影：
...
### 安全隔离与路径规范化（Path Canonicalization）

在评估基于文件路径的权限规则（如 `read_file`、`write_file`、`apply_patch` 等 `edit` 或 `read` 相关权限）时，匹配算法必须防御**路径穿越攻击（Path Traversal Attacks）**。

#### 漏洞风险
如果直接将 LLM/Sub-Agent 传入的原始路径字符串与 Ruleset 中的 Pattern 进行通配符匹配（例如使用 `wildcard_match("src/main.rs", "src/**")`），恶意 Agent 可通过传入包含相对路径或符号链接的路径（如 `./src/../../etc/passwd` 或指向系统敏感文件的软链接）绕过匹配前缀，但系统在实际执行文件 I/O 时会解析该路径，从而造成未授权的文件读写或信息泄露。这在生产环境中属于 P0/P1 级安全漏洞。

#### 防御规范与契约
所有涉及文件系统 I/O 的工具处理器（`ToolHandler`）以及权限评估服务（`PermissionService`），在进行权限匹配和实际执行 I/O 之前，必须强制执行以下规范：

1. **路径规范化（Canonicalization）**：
   - 必须先将待操作的路径解析为绝对路径。在 Rust 中，应使用 `std::fs::canonicalize` 来解析路径中的 `.`、`..` 以及所有的符号链接（Symbolic Links）。
   - 如果目标文件尚未创建（例如准备写新文件），由于 `std::fs::canonicalize` 对不存在的路径会返回错误，必须**先规范化其已存在的父目录的绝对路径，然后再拼接上目标文件名**。

2. **工作区边界检查（Workspace Containment Check）**：
   - 规范化后的绝对路径，**必须以规范化后的用户工作区根目录（Workspace Root）绝对路径为前缀**。
   - 若不满足此条件，必须立即拒绝执行，并返回 `TaskFailure::WorkspaceContainmentViolation`（或对应的结构化安全错误），不得触发规则评估或 `PermissionAction::Ask` 弹窗。

3. **规则匹配输入**：
   - 传递给 `evaluate(permission, pattern, rulesets)` 的 `pattern` 必须是**基于规范化后的工作区根目录的相对路径**，或者直接使用绝对规范化路径（取决于规则集的定义格式，统一为相对工作区根目录的路径，例如 `src/main.rs`）。
   - 禁止在规则匹配中直接传入未经过 canonicalize 的原始字符串。

4. **测试要求**：
   - 测试用例必须在 `tests/ai_subagent_permission_test.rs` 中包含路径穿越防御测试：尝试读写 `./src/../../secret.txt` 或构建软链接指向外部，确保被安全拦截。
...
Libra 必须采用同等语义。`tokio_util::sync::CancellationToken` 树的核心约束不仅是「父 cancel triggers 子 token」，还包括「父 abort 必须等待子 runner 写完 terminal cancel 投影」；当前落地形态是父 JSONL 写 `AgentRunEvent::Cancelled { reason: UserRequested }`，child JSONL 写 terminal `SessionSnapshot.metadata.status = "cancelled"`：
...
- **dispatcher 在 cleanup 阶段必须 `yield* cancel` 等价的「await 子 cancel 完成」**：
  ```rust
  // 伪代码：runner cleanup 必须实现
  let child_loop = run_tool_loop_with_history_and_observer(...);
  tokio::select! {
      result = child_loop => result,
      _ = parent_abort.cancelled() => {
          // CRITICAL: persist terminal child cancellation state before returning
          append_child_cancelled_snapshot();
          Err(TaskFailure::Cancelled { source: ParentAbort })
      }
  }
  ```
  不允许直接丢弃 child future 然后立即返回；child JSONL 必须看到 terminal cancellation snapshot 才算 cleanup 完成。
- 子 token 单独 cancel（如 budget hard cap）不影响父 session；父继续走，task tool 返回 `TaskFailure::Cancelled { source: BudgetHardCap }`。
- 父 abort 时，dispatcher 必须保证父 JSONL 写入同一 `agent_run_id` 的 `Spawned` + `Cancelled { reason: UserRequested }`，且子 JSONL 的 latest snapshot 必须从 `running` 转为 `cancelled`，便于 resume / replay。
- `AbortToken::child()` 是 single-direction：子 abort 不会冒泡回父。
- **`UserInitiated` 入口的 cancel 必须等价工作**：opencode 的 `tests/session/prompt.test.ts:861-895` 测试验证「slash command subtask 创建后调用 `prompt.cancel(parent_chat_id)`，parent 与 child SessionStatus 都必须从 `busy` 变为 `idle`」。Libra 的 `tests/ai_subagent_user_initiated_cancel_test.rs` 已复刻等价场景：
  - 创建父 session，调 `task.dispatch(parent_id, agent, prompt)` 派发 sub-agent
  - 让 fake provider hang
  - 断言 pre-flight cancel 短路且不写 JSONL
  - mid-flight 调 `AbortToken::cancel()`
  - await dispatcher future 退出
  - 断言 parent JSONL 为 `Spawned + Cancelled { reason: UserRequested }`
  - 断言 child JSONL 末尾 replay state 为 `metadata.status = "cancelled"`
...
opencode 的 `variants(model)` 函数对每个 provider 维护一个**模型 ID 子串列表**，用 `mistralId.includes(id)` 做匹配。这是新模型版本上线时的最低维护成本路径。Libra 落点 [src/internal/ai/providers/transform.rs](../../src/internal/ai/providers/transform.rs) 的 `reasoning_ids` 表已采用同样模式：
...
const OPENAI_REASONING_IDS: &[&str] = &[
    "o1",
    "o3",
    "o4",
    "gpt-5",
...
fn supports_reasoning(provider_id: &str, model_id: &str) -> bool {
    let id = model_id.to_lowercase();
    match provider_id {
        "anthropic" => ANTHROPIC_REASONING_IDS.iter().any(|s| id.contains(s)),
        "openai" => OPENAI_REASONING_IDS.iter().any(|s| id.contains(s)),
        ...
    }
}
```

每次新增 reasoning 能力的模型，**只需在对应常量列表追加 ID**；Provider Transform 通过 `variants(model_id)` 自动启用 variants。v0.17.1097 起这条规则写入 [docs/agent/provider-capability-update-guide.md](../agent/provider-capability-update-guide.md) 并由 `tests/ai_provider_transform_test.rs::provider_capability_update_guide_documents_reasoning_variant_workflow` pin 住。
...
测试文件：`tests/ai_goal_supervisor_test.rs`（含 supervisor 纯状态机与
driver/tool-loop `submit_goal_complete` 终端工具回归）+
`tests/ai_goal_completion_gate_test.rs`。
...
为了让 AI Agent 能够高质量、无回归地自动开发和落地该方案，必须在编码实现、异常处理和并发控制上制定严格的健壮性契约：

### 1. 严格的异常处理与 `unwrap`/`expect` 禁用策略
根据项目规范（`RULE[AGENTS.md]`），生产代码（包括 CLI 启动、调度器、适配器及提供商实现）中**严禁使用**任何可能导致程序 panic 的调用，包括 `unwrap()`、`expect()`、`panic!()` 等。

- **Invariant 约束**：在逻辑上绝对不可能失败的极少数场景下，如果要使用 `unwrap()` 或 `expect()`，必须在同一行或前一行附加 `// INVARIANT: <详细解释为什么这里永远不会出错>` 注释。
- **Result 传播**：所有其他容易出错的路径（如 JSON 解析、I/O 读写、网络请求等）必须返回 `Result`，并使用 `?` 向上层传播。
- **用户友好错误（User-friendly Errors）**：
  - 向上层抛出的错误（例如在 `SubAgentDispatcher` 中返回的 `TaskFailure` 和 `ProviderError`）必须经过包装。使用 `anyhow::Context` 或自定义的 `thiserror` 变体，注入清晰、具有行动指引的上下文。
  - 例如，如果由于解析模型绑定失败导致出错，错误信息中应明确包含：哪个 Agent 配置（或 `toml` 文件）中配置的模型绑定不符合 `provider/model` 的语法规范。

### 2. 子 Agent 并发控制与资源隔离
当多个子 Agent 并行或递归派发时，如果不对并发资源进行控制，可能会由于并发访问过载而导致 CPU 飙升、文件句柄耗尽、LLM 速率限制（Rate Limits）超出或内存溢出。

- **基于 Semaphore 的并发门限**：
  - `SubAgentDispatcher` 必须内置一个基于 `tokio::sync::Semaphore` 的并发控制器，其许可证数量初始化为 `max_concurrent_subagents`。
  - 在派发每个子 Agent 的 `run_tool_loop` 之前，必须调用 `.acquire_owned()` 获取并发许可证。
  - 若许可证耗尽，派发流程应根据配置的超时时间等待，若超时仍未获取则返回 `TaskFailure::ConcurrencyExceeded`，防止系统资源无限制增长。

### 3. 测试断言稳定性与错误输出固定（Display-pin Testing）
为了保证用户界面（TUI / CLI）展示的报错以及给模型反向修正的提示足够稳定，所有新增的错误枚举及 Display 实现必须保持稳定。

- **错误输出固定测试**：
  - 在 `tests/` 中编写针对错误类型的测试用例（如对 `TaskFailure` 及其子错误的序列化/展示测试）。
  - 使用显式字符串断言（或 Snapshot 机制）固定 `Display` 输出的报错文本，防止后续代码重构时无意中更改了用户或模型可见的错误文案。
...
- **2026-05-22（OC-Phase 6 landed-status pass）**：把方案中关于 Goal 模式的「计划」叙述与已落地代码对齐：
  - **P6.1 schema floor**（`goal/{spec,event,state}.rs`）已落地，含 `SessionEvent::Goal` round-trip 与 `parse_session_event_value` 分支。
  - **P6.2 verifier**（`goal/verifier.rs`）`DeterministicGoalVerifier` 已落地。
  - **P6.3 pure-decision step + driver wrapper**：`GoalSupervisor::step` 已落地为纯函数；continuation prompt builder 拆到 `goal/prompt.rs`（v0.17.711）；P6.3 driver `goal/driver.rs` 单轮 wrapper 与 `run_goal_supervised_tool_loop` 已落地（v0.17.712），并在 v0.17.719 升级为多轮 `loop` 驱动（`DEFAULT_MAX_CONTINUATION_LOOPS=8`，超限时合成 `LoopLimitReached` 并让 supervisor fold 到 `Blocked { LoopLimitNeedsUser }`）。
  - **P6.4 tools**：`tools/handlers/{update_goal_progress,submit_goal_complete}.rs` 拆分为两个独立 handler 已落地。
  - **P6.5/6.6 CLI/TUI/Control**：`libra code --goal "..."` flag、`/goal start|status|cancel|criteria add` 全部已落地（最后一个 v0.17.714）；NDJSON `goal.start/status/cancel` 与 `task.dispatch` 已落地；`TuiControlError::TaskInvalidRequest`+`GoalInvalidObjective` 等边界已 pin。
  - **底栏 Goal 指示器**：v0.17.716 落地 `render_goal_status_line` + `BottomPane::set_goal_status_line`，每次 Goal mutation 自动刷新；满足 L723 contract 的 id + status + criteria 字段。v0.17.1101 起 status line 会在 `GoalState.plan` 已投影时追加当前 step（优先 InProgress，其次 Pending / Skipped / latest Completed）；tokens/cost 由同一 `BottomPane` 的 `usage_line` 展示，仍不塞进 `GoalState`。
  - **ToolLoopConfig.goal_stop_policy wiring**：v0.17.713 落地字段（默认 `None`）；v0.17.1082 起 TUI Goal 路径在创建 `GoalSupervisor` 前把 `Some(GoalBound { goal_id })` 写入 `ToolLoopConfig.goal_stop_policy`，并用同一 policy 构造 supervisor，确保配置面不再只是 schema-only。
  - **P6.7 `--resume <thread>` replay**：v0.17.718 已把 active Goal 从 session JSONL 中重新构建（`GoalSession::from_replay`），`--goal` flag 仍优先；底栏指示器在 `run_in_alt_screen` setup 时被刷新。
  - **P6.7 driver/tool-loop Goal protocol E2E**：v0.17.1084 起 `goal_turn_outcome_from_tool_loop_turn` 会从 tool-loop history 识别成功的 `submit_goal_complete` / `update_goal_progress` 调用，并映射成 `CompletionClaim` / `ProgressUpdate`；`tests/ai_goal_supervisor_test.rs::s6_driver_maps_submit_goal_complete_tool_call_to_completion_claim` 覆盖真实 `run_goal_supervised_tool_loop` + `SubmitGoalCompleteHandler` 路径，防止终端工具成功文本被误当成普通 final text。
  - **P3.5 wire-up（Spawned / Completed）**：v0.17.739 在 `SubAgentDispatcher::dispatch` 通过 `session_store.append` 写入 `Spawned`（gate 全部通过后立刻 mint `AgentRunId`）与 `Completed`（placeholder result 之后）；`Failed` / `Cancelled` / `TimedOut` 等待 P3.4 的真实 child loop。`task_id` placeholder 改为携带 `agent_run_id` 便于 replay 关联。
  - **OC-Phase 4 `ContextHandoffBuilder`**：v0.17.740 在 `context_budget/handoff.rs` 落地 builder，构造时强制借用 parent `ContextFrameEvent`，`build()` 必经 `parse_handoff_template`，保证产出的 `ContextHandoff` 一定 schema-valid。dispatcher P3.4 在接入 ContextHandoff 时直接用本 builder，不再 raw 构造。
  - **OC-Phase 4 P4.5 `ContextFrameLoader` 真实形态**：v0.17.744 给 `ContextFrameLoader` 加上 `latest_frame_for_session(&SessionJsonlStore)`，walking parent JSONL 并返回最新 `ContextFrameEvent`；step 10 P3.4 接入时直接驱动 builder 即可。
  - **OC-Phase 3 P3.7 parent-abort cancel propagation**：v0.17.743 在 `SubAgentDispatcher::dispatch` 两处（pre-gate + post-ask）加 `ctx.abort_token.is_cancelled()` 检查；v0.17.767 起 `DefaultSubAgentChildRunner::run` 用 `tokio::select!` 竞速 child loop 与 parent abort，mid-flight cancel 会在 fake provider 5s delay 前返回 `TaskFailure::Cancelled { source: ParentAbort }`；v0.17.1083 追加回归，确认 parent JSONL 写入同一 `agent_run_id` 的 `Spawned + Cancelled { reason: UserRequested }` 事件；v0.17.1123 补 pin child JSONL terminal cancelled snapshot。
  - **OC-Phase 3 P3.8 byte-level flag-off regression fixture**：v0.17.742 的 `dispatch_rejects_when_feature_flag_disabled` 现在断言 events.jsonl 为空字节（flag-off 必不能写入），配合 v0.17.739 的 flag-on Spawned+Completed 测试构成"flag-off = 0 bytes, flag-on = exactly 2 events"两侧合同。
  - **OC-Phase 3 acceptance criterion 5（agent-traces 保护）**：v0.17.745 把 `checkout` 三处 INTENT_BRANCH-only 检查扩展到 INTENT_BRANCH ∨ AGENT_TRACES_BRANCH；v0.17.746 给 reset / switch 补齐 agent-traces 回归测试（restore 此前已覆盖）。整套 restore / reset / switch / checkout 现在都拒绝 agent-traces。
  - **OC-Phase 3 worktree-readonly registry contract**：v0.17.750 落地 `tests/ai_subagent_worktree_readonly_test.rs`，2 个测试 pin "[{edit:*:deny}] 下 EDIT_TOOLS 全员剥离" 合同。
  - **OC-Phase 5 budget enforcement（session + goal + per-agent）**：v0.17.747 把 `BudgetTracker::drain_warnings` 接到 `AgentEvent::UsageUpdated` 历史 cell；v0.17.751 进一步把 `check_session` / `check_goal` 接入：任一返回 `BudgetExceededError` 立即发 history cell 并 `cancel_current_turn(CancelSource::Budget)`。`SubAgentUsageUpdated` 已把子 agent usage 按 `agent_name` 归因后调用 `check_agent`，`tests/ai_multi_agent_e2e_test.rs::s7_per_agent_cost_cap_breach_surfaces_agent_budget_exceeded_code` 固化 `BudgetScope::Agent` + `LBR-AGENT-001` 合同。
  - **OC-Phase 3 P3.4 step 9 prerequisite**：v0.17.752 新增 `DispatchContext::resolve_provider_build_options(&ModelBinding)`，封装"resolver 优先，否则父级 options 克隆"规则，供 P3.4 child-loop 直接调用 `ProviderFactory::build`。
  - **OC-Phase 3 P3.4 child session snapshots + tool events**：v0.17.1095 起 `DefaultSubAgentChildRunner` 会在 `SessionJsonlStore::child(agent_run_id)` 下写入子 session snapshots，保留 child prompt、final assistant reply、`agent_run_id` / parent ids / provider-model metadata；v0.17.1098 起同一路径在 child `ToolLoopObserver` 的 `on_tool_call_begin` / `on_tool_call_end` 中追加 `tool_call` / `tool_result` snapshot message；v0.17.1099 起 `SessionEvent::ToolCall` / `SessionEvent::ToolResult` 专用 JSONL event schema 已落地；v0.17.1100 起 `tests/fixtures/sub_agent/child_tool_transcript_sequence.json` 固化 normalized parent/child transcript sequence。`flag_on_does_not_touch_main_worktree` 的 fake-provider E2E 断言父侧 `Spawned` 可定位到该 child session，且 child JSONL 顺序保留 snapshot → tool_call event → snapshot → tool_result event → snapshot → final snapshot。
  - 剩余：P3.4/S3 的 runtime transcript 合同已闭合；若后续引入 raw JSONL event compaction / migration，再追加未 normalized 的 raw-bytes fixture。
- **2026-05-05（upstream-sync pass，HEAD `25ecf0af6`）**：根据 opencode dev 分支过去 24 小时的 5 个相关提交同步本计划。新增 / 修改如下：
  - **OC-Phase 3 Cancel/Abort 合同升级**（依据 PR #25798 `fix(session): cancel subtask child sessions`）：opencode 把 `TaskPromptOps.cancel` 从 `() => void` 改为 `Effect.Effect<void>`，配合 `EffectBridge.make()` 与 `if (Exit.hasInterrupts(exit)) yield* cancel` 实现「父 abort **必须 await 子 cancel 完成** 才能 release」。Libra 用 `tokio::select!` + terminal child snapshot 复刻该 contract；父 JSONL 看到 `AgentRunEvent::Cancelled { reason: UserRequested }`、child JSONL replay 到 `metadata.status = "cancelled"` 后才算 cleanup 完成。`tests/ai_subagent_user_initiated_cancel_test.rs` 已覆盖 pre-flight + mid-flight cancel，复刻 opencode `tests/session/prompt.test.ts:861-895` 的 SubtaskPart 取消验证。
  ...
  - **新增 Variants 表维护模式**（依据 PR #25887 `fix: mistral medium 3.5/2604`）：opencode 用「per-provider 模型 ID 子串列表 + `String.contains()` 匹配」维护 reasoning capability。Libra `providers/transform.rs` 的 `reasoning_ids` 表已采用同样模式（`ANTHROPIC` / `OPENAI` / `DEEPSEEK` / `KIMI` / `GEMINI` / `ZHIPU` 常量），新增 reasoning 模型只需追加 ID。v0.17.1097 起约定写入 `docs/agent/provider-capability-update-guide.md`，并由 `provider_capability_update_guide_documents_reasoning_variant_workflow` 防止文档缺失或脱节。
...
- **2026-05-05（landability pass）**：把方案推到「可直接拆 PR」状态。基于 `/Volumes/Data/opencode/packages/opencode/src/permission/index.ts`、`tool/task.ts`、`session/prompt.ts:548 handleSubtask`、`session/compaction.ts:43-78 SUMMARY_TEMPLATE` 的逐行细节，新增以下 4 大类落地合同：
  ...
  - 增强 OC-Phase 3：`SubAgentDispatcher` trait + `DispatchContext` + `TaskFailure` 完整类型；14 步主流程；区分 `LlmInitiated` 与 `UserInitiated { bypass_permission_ask }` 两类入口；`tokio_util::CancellationToken` 树形 abort 传播合同；Sub-Agent Worktree 临时合同（OC-Phase 3 readonly，CEX-S2-11 workspace 抽象 + CEX-S2-12 sub-agent dispatcher 之后启用 isolated workspace）；`assert_no_escalation` 实现伪代码与测试矩阵。
