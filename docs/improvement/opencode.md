...
- **2026-05-22（OC-Phase 6 landed-status pass）**：把方案中关于 Goal 模式的「计划」叙述与已落地代码对齐：
  ...
  - **OC-Phase 3 P3.4 child session snapshots**：v0.17.1095 起 `DefaultSubAgentChildRunner` 会在 `SessionJsonlStore::child(agent_run_id)` 下写入子 session snapshots，保留 child prompt、final assistant reply、`agent_run_id` / parent ids / provider-model metadata；`flag_on_does_not_touch_main_worktree` 的 fake-provider E2E 断言父侧 `Spawned` 可定位到该 child session。
  - 剩余：P3.4/S3 的完整 parent/child transcript fixture 仍未完全达到上方 S3 字节级事件序列；当前 dispatcher 已具备 model build、handoff、child `run_tool_loop`、父侧 `Spawned`/terminal event、child session prompt/final snapshots 与 fake-provider Llm/UserInitiated E2E，尚缺 per-tool child JSONL events / byte-for-byte transcript fixture。
