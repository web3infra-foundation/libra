...
- **2026-05-22（OC-Phase 6 landed-status pass）**：把方案中关于 Goal 模式的「计划」叙述与已落地代码对齐：
  ...
  - **OC-Phase 3 P3.7 parent-abort cancel propagation（部分）**：v0.17.743 在 `SubAgentDispatcher::dispatch` 两处（pre-gate + post-ask）加 `ctx.abort_token.is_cancelled()` 检查；任一处中标即返回 `TaskFailure::Cancelled { source: ParentAbort }`。Spawned 后才 abort 的窗口（即 P3.4 真实 child loop 中的 cancel）仍待 child handle await 实装。
  ...
  - 剩余：P6.7 fake-provider E2E；P3.4 dispatcher steps 9-13（真实 child loop）；P3.7 child handle await（Cancelled event 写入子 JSONL）。
