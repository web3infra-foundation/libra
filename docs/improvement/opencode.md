...
- **2026-05-22（OC-Phase 6 landed-status pass）**：把方案中关于 Goal 模式的「计划」叙述与已落地代码对齐：
  ...
  - **P6.3 pure-decision step + driver wrapper**：`GoalSupervisor::step` 已落地为纯函数；continuation prompt builder 拆到 `goal/prompt.rs`（v0.17.711）；P6.3 driver `goal/driver.rs` 单轮 wrapper 与 `run_goal_supervised_tool_loop` 已落地（v0.17.712）。多轮循环仍 deferred 给一个更大的 PR。
  ...
  - 剩余：`--resume <thread>` 替换重放 active Goal（P6.7 partial）；driver 多轮循环；P6.7 fake-provider E2E。
