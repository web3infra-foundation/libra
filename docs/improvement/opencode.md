...
- **2026-05-22（OC-Phase 6 landed-status pass）**：把方案中关于 Goal 模式的「计划」叙述与已落地代码对齐：
  ...
  - **底栏 Goal 指示器**：v0.17.716 落地 `render_goal_status_line` + `BottomPane::set_goal_status_line`，每次 Goal mutation 自动刷新；满足 L723 contract 的 id + status + criteria 字段，`step`/`tokens` 字段待 plan/usage 子系统接入。
  ...
  - **OC-Phase 3 P3.4 child session snapshots + tool events**：v0.17.1095 起 `DefaultSubAgentChildRunner` 会在 `SessionJsonlStore::child(agent_run_id)` 下写入子 session snapshots，保留 child prompt、final assistant reply、`agent_run_id` / parent ids / provider-model metadata；v0.17.1098 起同一路径在 child `ToolLoopObserver` 的 `on_tool_call_begin` / `on_tool_call_end` 中追加 `tool_call` / `tool_result` snapshot message；v0.17.1099 起 `SessionEvent::ToolCall` / `SessionEvent::ToolResult` 专用 JSONL event schema 已落地，`flag_on_does_not_touch_main_worktree` 的 fake-provider E2E 断言父侧 `Spawned` 可定位到该 child session，且 child JSONL 顺序保留 snapshot → tool_call event → snapshot → tool_result event → snapshot → final snapshot。
  - 剩余：P3.4/S3 的完整 parent/child transcript fixture 仍未完全达到上方 S3 字节级事件序列；当前 dispatcher 已具备 model build、handoff、child `run_tool_loop`、父侧 `Spawned`/terminal event、child session prompt/tool-call/tool-result/final snapshots、dedicated tool_call/tool_result events 与 fake-provider Llm/UserInitiated E2E，尚缺 byte-for-byte transcript fixture。
