...
- **2026-05-22（OC-Phase 6 landed-status pass）**：把方案中关于 Goal 模式的「计划」叙述与已落地代码对齐：
  ...
  - **OC-Phase 5 budget enforcement（session + goal）**：v0.17.747 把 `BudgetTracker::drain_warnings` 接到 `AgentEvent::UsageUpdated` 历史 cell；v0.17.751 进一步把 `check_session` / `check_goal` 接入：任一返回 `BudgetExceededError` 立即发 history cell 并 `cancel_current_turn(CancelSource::Budget)`。Per-agent enforcement（`check_agent`）需要 sub-agent attribution（P3.4），仍待。
  ...
  - 剩余：P6.7 fake-provider E2E；P3.4 dispatcher steps 9-13（真实 child loop）；P3.7 child handle await（Cancelled event 写入子 JSONL）；per-agent budget enforcement（依赖 P3.4 sub-agent attribution）。
