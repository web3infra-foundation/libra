...
| PR 4 | Phase C.2：headless approvals/request-user-input/mutating tools 与 plan/patchset 最小投影已完成 | browser interaction scenarios |
...
- Headless v1 已完成 provider factory、runtime context、guarded mutating tools 与 direct plan/patchset projection 收敛。后续完整 IntentSpec plan approval workflow 必须先抽共享 session driver，不能复制 ratatui `App` 状态机或只给某个 provider 打开第二套 workflow。
