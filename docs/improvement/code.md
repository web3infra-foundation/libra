...
║  ─────────────────────────────────────────────────────────────────────────────────── ║
║  confirmed IntentSpec  ·  planning prompt assembly  ·  impl/test plan set            ║
║                                                                                      ║
║   ┌─────────────────────────────────┐    ┌─────────────────────────────────────┐     ║
...
║                    └════════╪══════════╪═══════════╪══════════┘                      ║
║                             │          │           └──→ Decision(Cancelled)[E]       ║
║                             │          └──→ back to Phase 0 (new Intent revision)    ║
║                             │                                                        ║
║                             │   new Plan[S] · Task[S]                                ║
║                             │   Scheduler.current_plan_heads ← reviewed Plan Set     ║
║                             │   Execute 后: Scheduler.selected_plan_ids ← approved   ║
║                             │   (Return Review loop)                                 ║
...
║                                                                                      ║
║  Libra Scheduler                                                                     ║
║  │  next_ready_task() across implementation/test plan set                            ║
║  │  build composite DAG from selected_plan_ids                                       ║
...
║  │          └─────────────────────────────────────────────────────────────┘          ║
║  │  mark_task_complete()                                                             ║
║  └─ advance to next ready task / next ready test task                                ║
║                                                                                      ║
...
║                                                                                      ║
║  fail (test-plan gap)  ─→ Phase 2  (revise impl/test plan set) → rerun               ║
║  fail (broader replan) ─→ Phase 1  (new plan set revision) → Phase 2                 ║
...
| Phase 1 | 基于已确认 `IntentSpec` 发起 provider-facing planning 调用，允许 readonly tools，至少生成并审查两个 `Plan`（`implementation` + `validation/test`）及其 `Task`，处理 `Execute / Modify Plan / Revise Intent / Cancel`，并把当前 plan set heads 写回 Scheduler projection | 不允许开始 task execution，不允许写 `Run` / `PatchSet` / `Provenance`，不允许调用任何 mutating tool，不允许跳过 plan review 直接执行 |
| Phase 2 | 由 Scheduler 驱动 `selected_plan_ids` 对应的 composite DAG 执行 implementation/test plan set，记录 run/patch/evidence/context 等执行事实 | 不允许 provider 自行推进整条计划，不允许 UI 旁路 formal writes |
| Phase 3 | 运行系统级验证与审计，消费 implementation/test plan artifacts，形成 release candidate 视图和结构化验证结果；测试计划不足时把流程送回 Phase 2 rework loop | 不允许把 system validation 混回 provider tool loop，不允许跳过审计直接发布 |
...
| Phase 1 | Plan set 构建与审查 | `Plan`、`Task` | `ToolInvocation`、`ContextFrame`、可选 terminal `Decision` / `IntentEvent` | `selected_plan_ids`、`current_plan_heads`、plan-set review UI、ready queue preview |
| Phase 2 | implementation/test plan set 执行与过程事实 | `Run`、`PatchSet`、`Provenance` | `TaskEvent`、`RunEvent`、`PlanStepEvent`、`ToolInvocation`、`Evidence`、`ContextFrame`、`RunUsage` | `active_run_id`、live_context_window、composite DAG / staging 状态 |
...
4. 把 provider 返回的 `IntentSpec` 转换成 Markdown review，并支持 `Confirm / Modify / Regenerate / Cancel` 循环。
...
sequenceDiagram
    ...
    participant O as git-internal / Projection DB

    U->>C: User input / optional --resume <thread_id>
    C->>R: start_phase0(input, resume?)

    alt resume existing thread
    ...
    alt new thread
        R->>O: write root draft Intent[S]
        R-->>R: canonical thread_id = root_intent_id
    else resume with semantic refinement
        R->>O: write draft Intent revision[S]
    ...
    loop provider-assisted IntentSpec refinement
        ...
        P-->>R: IntentSpec candidate
        R->>O: append ToolInvocation[E] / ContextFrame[E]
        R->>O: write Intent revision[S] if candidate differs
        R-->>C: render Markdown IntentSpec review
        C-->>U: Confirm / Modify / Regenerate / Cancel
    end

    alt Cancel
        R->>O: append Decision(Cancelled)[E]
        R->>O: append optional IntentEvent[E]
        ...
        X->>O: upsert ThreadProjection / SchedulerState / query-index seed
...
| provider 细化 | draft intent、反馈、readonly context | candidate `IntentSpec` | provider 只允许做意图细化和只读分析 |
| Phase 0 tool/event 收口 | provider tool/use 流 | `ToolInvocation[E]`、`ContextFrame[E]` | 只记录只读分析事实，不写 `Run` |
| Markdown review loop | candidate `IntentSpec`、用户反馈 | `Confirm` / `Modify` / `Regenerate` / `Cancel` | `Modify` / `Regenerate` 留在 Phase 0 |
...
| 展示 `IntentSpec` Markdown review，并等待 `Confirm / Modify / Regenerate / Cancel` | Phase 0 |
...
Phase 1 的职责是把 Phase 0 已确认的 `IntentSpec` 转成可审查的 plan set，并在任何执行发生之前完成计划层的强制 review gate。这个 plan set 至少包含一份 `implementation` plan 和一份 `validation/test` plan。它与 Phase 0 的区别不是“有没有 LLM”，而是“LLM 当前是在细化目标，还是在生成计划”。
...
3. 把 planning 结果规范化为不可变 `Plan[S]` / `Task[S]` revision，且至少产出 `implementation` 与 `validation/test` 两类 plan。
...
1. `Modify Plan` 的“只改一类”不等于 single-plan 写入：`Runtime` 仍只接收完整 plan set；未修改的一类必须沿用当前 head 透传。
...
|---|---|---|---|---|
| 成功，已生成可审查的 implementation/test plan set | `Execute` | 选定当前 `current_plan_heads` 对应的 implementation/test `Plan[S]` / `Task[S]` | `selected_plan_ids = current_plan_heads` | 进入 Phase 2 |
| 成功，已生成可审查的 implementation/test plan set | `Modify Plan` | 写新的 implementation/test `Plan[S]` / `Task[S]` revision（可只改一类；若只改一类，另一类必须透传当前 head 组成完整 plan set） | 刷新 `current_plan_heads`（不提前写 `selected_plan_ids`） | 留在 Phase 1 |
| 成功，已生成可审查的 implementation/test plan set | `Revise Intent` | 不继续推进当前 plan head；回写新的 `Intent` revision 请求 | 清理当前 plan 选择或保留为历史 head | 回到 Phase 0 |
| 成功，已生成可审查的 implementation/test plan set | `Cancel` | 不进入执行；写 terminal `Decision[E]`，必要时写 `IntentEvent[E]` | Scheduler 清空 active 选择 | 终止 |
| 失败，或候选计划质量不足以进入 review | 不适用 | 不推进不完整的 implementation/test `Plan` / `Task` 为选中 head；保留只读 analysis events | 保持上一可用 projection | 留在 Phase 1 重试或报错 |
...
sequenceDiagram
    ...
    P->>T: readonly tool calls
    T-->>P: repo / docs / history context
    P-->>S: plan set candidate / plan text
    S->>R: append ToolInvocation[E] / ContextFrame[E]
    S->>R: normalize_and_write_plan_set()
    R->>O: write implementation/test Plan[S] / Task[S]
    R->>O: update Scheduler.current_plan_heads
    ...
    alt Execute
        ...
        R->>O: update Scheduler.selected_plan_ids
        S-->>S: enter Phase 2 with selected_plan_ids
    else Modify Plan
        S->>P: revise_plan(confirmed_intent, plan_feedback)
        P-->>S: revised plan set candidate
        S->>R: append ToolInvocation[E] / ContextFrame[E]
        S->>R: write revised implementation/test Plan / Task
        R->>O: update current_plan_heads
        ...
        R->>O: append optional IntentEvent
...
| plan set 规范化 | provider 结果 | canonical `PlanSpec(role=implementation|validation)` / `TaskSpec[]` | 收敛不同 provider 的 planning 结果结构 |
...
3. Phase 1 至少要产出一份 `Plan(role=implementation)` 和一份 `Plan(role=validation)`；后者负责定义测试 / 验证任务，不允许省略。
4. `Plan[S]` / `Task[S]` 是不可变结构定义；mutable 状态只能进入 projection 或 event。
5. `Modify Plan` 不允许原地改写既有 `Plan` / `Task`，必须走 revision chain。
6. `Modify Plan` 只改一类时，写入仍必须提交完整 plan set；未修改的一类沿用当前 head 透传，不因此放开 single-plan 写入接口。
7. `Execute` 时必须同时选定 implementation/test 两类 plan head，形成 `selected_plan_ids`。
...
Phase 2 的职责是由 Libra Scheduler 把当前 plan set（至少包含 `implementation` plan 和 `validation/test` plan）物化为一个 composite DAG runtime，并驱动该 DAG 执行。这里的推荐实现明确收敛到 `dagrs 0.8.1`：Libra 负责从 `Plan` / `Task` snapshot 构建 composite DAG、订阅 DAG 事件、把 DAG 结果翻译成 formal writes，并把 provider 限定为单个 task node 的执行器。它的核心是“执行控制权归 Libra + DAG runtime”，不是“让 provider 自己跑完整条计划”。

#### Phase 2 目标

1. 从当前 `selected_plan_ids` 派生 canonical composite DAG，并用 `dagrs 0.8.1` 构建 Phase 2 runtime graph。
...
7. 允许 implementation plan 和 validation/test plan 在依赖允许时并行执行。
8. 当 Phase 3 指出 test-plan 缺口或可自动修复的验证不足时，能够在 Phase 2 内分析并追加新的 implementation/test plan revision，再次执行。
...
|---|---|---|---|
| DAG node 成功 | `Run[S]`、可选 `PatchSet[S]`、`Provenance[S]`、执行 events | 标记 node 对应 task 完成，推进 composite DAG progress | 继续 Phase 2 或 DAG 全部完成后进 Phase 3 |
| retryable failure | `Run[S]`、失败 events / evidence | 递增 retry 计数，按 policy 重建 node attempt | 留在 Phase 2 |
| validation feedback rework required | 当前执行证据、Phase 3 validator 反馈、追加 `Plan[S]` / `Task[S]` revision | 刷新 `selected_plan_ids`，重建 composite DAG | 留在 Phase 2 |
| broader replan required | 当前 attempt 的 events / evidence | `active_run_id = None`，终止当前 DAG runtime，准备新 plan set head | 回到 Phase 1 |
...
sequenceDiagram
    ...
    S->>D: build_graph_from_selected_plan_set(implementation_plan, validation_plan, tasks)
    D-->>S: Graph ready
    S->>D: async_start()

    loop each ready DAG node (implementation or validation/test)
        ...
        R->>O: write PatchSet[S]
        R->>O: append TaskEvent / RunEvent / PlanStepEvent
        R->>O: append ToolInvocation / Evidence / ContextFrame / RunUsage
    end

    D-->>S: GraphEvent / ExecutionReport
    S->>R: translate dagrs events into projection updates

    alt success
        S-->>S: mark composite DAG complete and enter Phase 3
    else retryable
        S-->>S: rebuild retry path inside current DAG policy
    else validation feedback rework required
        S-->>S: append plan set revision and rebuild composite DAG
...
| 子步骤 | 输入 | 输出 | 说明 |
|---|---|---|---|
| DAG 物化 | `selected_plan_ids`、`Plan.steps`、`Task.dependencies`、显式跨 plan 依赖边 | `dagrs::Graph` | Scheduler 按声明关系把 implementation/test plan set 物化为 composite DAG runtime |
...
| control-state 更新 | node 结果、retry policy、replan policy | next ready node / retry / replan / plan-set rework | Phase 2 的 mutable 控制状态归 Scheduler |

#### Phase 2 产物与约束

1. Phase 2 的 canonical runtime 明确采用 `dagrs 0.8.1`，由 Libra 从 implementation/test `Plan` / `Task` snapshot 构建 composite DAG，并通过 `Graph::async_start().await` 驱动执行。
...
8. implementation plan 和 validation/test plan 的 task 可以并行执行，但 projection 更新必须维持单写者序列化提交；dagrs 只负责 DAG execution，不拥有系统真相。
9. 跨 plan 依赖必须显式声明在 `Plan.steps` / `Task.dependencies` 中；`DagRuntimeBuilder` 只负责按声明建边，禁止根据文件名、tool 类型或启发式语义自动推断依赖。
10. validation/test task 只要其显式依赖满足即可提前启动；不要求等待所有 implementation task 全部结束。
11. 若 required validation/test node 或其上游 required node 发生 permanent failure，Scheduler 可以 early-stop：取消尚未启动的 downstream node、按取消策略收口已运行 node、写 routing evidence，然后进入 retry / Phase 2 rework / Phase 1 replan；early-stop 后不得直接进入 Phase 3。
...
13. 当 Phase 3 指出 test-plan 缺口时，Phase 2 必须能够在当前 confirmed intent 下追加新的 implementation/test `Plan` / `Task` revision，并重新执行，而不是直接跳过到决策阶段。
14. 只有当 composite DAG 中所有 required implementation/test task 都收口后，Phase 2 才能进入 Phase 3。
...
1. **属于 Phase 2 的内容**：
   - provider 在 composite DAG node 中执行任务
   - `Sandbox` / `Worktree` 中产生代码修改、命令输出、工具调用、局部验证结果
   - implementation plan 和 validation/test plan 中的测试任务、lint、局部回归检查
   - per-task `PatchSet`、per-task `Evidence`、`RunEvent`、`ToolInvocation`、`ContextFrame`
   - 任何仍然服务于“让某个 task 完成并产出候选结果”的行为
2. **属于 Phase 3 的内容**：
   - 以全部 implementation/test task artifacts 为输入构造 release candidate
...
4. **特别约束**：
   ...
   - 只有在 composite DAG 全部 required task 收口、release candidate 已可构造后，system-level validator 才能启动，这才进入 Phase 3。
   - 如果 Phase 3 认定“测试计划本身不足”，必须返回 Phase 2 追加或修改 validation/test plan，而不是把这类补救工作塞进 Phase 3。

### Phase 3 Detailed Analysis

Phase 3 的职责是把 Phase 2 产出的候选结果提升为“系统级可发布候选”，执行独立于 provider tool loop 的验证与审计流水线，并给出结构化验证结论。它的核心不是再跑一轮代码生成，而是验证、归档和路由。

#### Phase 3 目标

1. 从 Phase 2 的 implementation/test per-task artifacts 构造 release candidate 视图。
2. 运行 `integration -> security -> release` 验证流水线。
...
| test-plan gap / auto-fixable validation deficiency | fail `Evidence[E]`、Phase 2 rework routing 信息、必要时新的 plan revision 请求 | 清理 active run，刷新 `selected_plan_ids`，准备新 composite DAG | 回到 Phase 2 |
...
sequenceDiagram
    ...
    R->>O: append Evidence[E]

    alt pass
        R->>O: write optional final ContextSnapshot[S]
        S-->>S: enter Phase 4
    else test-plan gap / auto-fixable deficiency
        R->>O: write Phase 2 rework routing evidence / plan-revision request
        S-->>S: clear active run and return Phase 2 rework loop
    else broader replan
        R->>O: write routing evidence / decision-prep facts
...
| 子步骤 | 输入 | 输出 | 说明 |
|---|---|---|---|
| candidate 聚合 | implementation/test task artifacts、patchsets、usage、evidence | release candidate 视图 | 从执行产物构造系统级候选 |
| validator 执行 | release candidate、validation policy | stage results | `integration`、`security`、`release` 必须独立于 provider tool loop |
...
2. `Evidence[E]` 和 `ValidationReport` 必须结构化，不允许只保留文本日志。
3. `ValidationReport` 持久化时必须携带生成它的 validator / policy version，保证后续 replay / rebuild 有确定输入。
4. validator failure 不得静默吞掉；至少要写 evidence 或 terminal failure 记录。
5. final `ContextSnapshot[S]` 只用于冻结稳定候选，不作为运行时增量上下文容器。
6. Phase 3 可以把流程路由回 Phase 2（test-plan / implementation rework）或 Phase 1（broader replan），但不能直接绕过 Scheduler 重新启动 provider execution。
...
| request changes（plan-level） | `Decision(RequestChanges)`，随后写 replan 请求 / 新的 implementation/test `Plan` revision | 指向新的 plan set head | 回到 Phase 1 |
| request changes（intent-level） | `Decision(RequestChanges)`，随后写新的 `Intent` revision | 指向新的 intent head，并清理当前 plan 选择 | 回到 Phase 0 |
| cancel / abandon | `Decision(Cancelled / Abandon)` | Scheduler 置 idle | 终止 |
...
sequenceDiagram
    ...
    D-->>R: DecisionProposal

    alt auto path
        R->>O: write Decision(E)
        R->>O: advance ThreadProjection / SchedulerState
    else human review required
        R->>UI: show summary / evidence / impact / patchset
        UI-->>R: approve / reject / request-changes
        R->>O: write Decision(E)
        alt request-changes (plan-level)
            R->>O: create replan / new plan-set revision path
        else request-changes (intent-level)
            R->>O: create new Intent revision path
        else approve / reject
            R->>O: advance or clear projections
        end
    end
```
...
4. 若冻结失败：不得阻塞线程读取；需写入失败 evidence（如 `Evidence(kind=ContextSnapshotFreezeFail)`）并把 projection 标记为可恢复的 stale 状态，等待重试或人工介入。
...
8. `claudecode` 不是过渡期保留组件；删除必须在 `Implementation Phase 1` 的同一合入波次中完成：切完最后一个调用点后立即一次性删除其全部运行时代码、CLI 标志、文档说明和测试，不允许跨 phase 保留可发布的双栈、feature flag、deprecated path、兼容别名或中间运行态。唯一允许的兼容只限本地 `thread_id` 数据迁移时一次性读取旧字段，但不能反向保留旧的用户可见契约。
...
7. `Delivery Order`：理解这批改造为什么必须按 `Implementation Phase 1` → `Implementation Phase 5` 执行。
8. `Implementation Phase 1 / 2 / 3 / 4 / 5`：按执行顺序阅读实施内容、影响模块、完成定义和用户影响。
...
| 术语 | 在本文中的含义 | 明确不是 |
|---|---|---|
| `Workflow Phase 0-4` | Libra runtime 中真实发生的 workflow 阶段：Intent、Planning、Execution、Validation、Decision | 不是实施顺序，不等于 `Implementation Phase 1-5` |
| `Implementation Phase 1-5` | 这份重构计划的交付顺序，用来描述先做哪一批改造 | 不是运行时状态机，不参与 formal object 写入 |
...
| plan set | Phase 1 产出的成组计划，至少包含 `implementation` 与 `validation/test` 两类 `Plan` | 不是单个 `Plan`，也不是 provider 随手返回的一段 plan text |
| `current_plan_heads` | 当前 revision 链上最新、可继续修改或审查的 plan heads | 不表示“已批准执行” |
| `selected_plan_ids` | 已经在 `Execute` 分支被批准、将进入 Phase 2 执行的 plan heads | 不等于所有历史 plan heads，也不等于 draft review 中的候选集合 |
...
|---|---|---|---|
| Phase 0 review 待完成 | 存在当前 draft / revision `Intent`，但无 confirmed current intent；或存在未决 `Confirm / Modify / Regenerate / Cancel` review 状态 | 1) 重建当前 intent draft / review markdown；2) 重新打开 IntentSpec review；3) 仅在用户显式选择时重新触发 provider 细化 | 不允许把上次 provider 暂存输出当作已确认 `Intent`；不允许跳过 Intent review 直接进入 Phase 1 |
...
pub struct PlanSetSpec {
    pub implementation: ExecutionPlanSpec,
    pub validation: ExecutionPlanSpec,
}

pub struct SelectedPlanSet {
    pub implementation_plan_id: Uuid,
    pub validation_plan_id: Uuid,
}
...
1. `PlanSetSpec` / `SelectedPlanSet` 是 Phase 1 / Phase 2 的唯一主 contract：必须同时包含 `implementation` 与 `validation` 两类 plan；缺一即为非法输入，不允许退化成 single-plan shortcut。
2. `PlanSetSpec` 必须保持 named-field 结构（`implementation` / `validation`），而不是 `Vec<Plan>` / roleless collection；这样 role completeness 能在类型层被表达。
3. 若 `Modify Plan` 只改一类，未修改的一类必须继续引用当前 head 对应的既有 revision；不得为了凑齐 `PlanSetSpec` 再伪造一份内容等价的新 revision。
4. `DagRuntimeBuilder` 只接受 `SelectedPlanSet`，负责把 implementation/test 两类 `Plan.steps` + `Task.dependencies` 一起编译成 `dagrs 0.8.1` composite graph，不允许 provider 自己组 DAG。
5. Scheduler 进入 Phase 2 时只能携带 `SelectedPlanSet`；任何缺少任一 role 的候选 plan 都必须在 review gate 或 scheduler mutation 校验阶段被拒绝。
6. `dagrs` 只是 Phase 2 的 runtime engine，不替代 `Runtime`、`ProjectionResolver` 或 formal object store。
7. `ExecutionReport`、`GraphEvent::Progress`、`GraphEvent::ExecutionTerminated` 必须被 Scheduler 映射成统一 `WorkflowEvent`。
...
impl Runtime {
    ...
    pub async fn write_plan_set(&self, intent_id: Uuid, plans: &PlanSetSpec) -> Result<PlanSetBundle>;
...
4. `write_plan_set()` 必须一次性接收 implementation/test 双 plan 的 `PlanSetSpec`；不得暴露 single-plan 写入入口。
5. `Modify Plan` 若只改一类，调用者仍必须构造完整 `PlanSetSpec`：变更的一类使用新 revision，未变更的一类透传当前 head；这里的“透传”指引用既有 revision，而不是复制出新的等价 revision；不因此放开 single-plan 写入入口。
...
2. Phase 0 把 draft + feedback 发给 provider 生成 / 修改 `IntentSpec`，provider 可调用 readonly tools，并把对应 `ToolInvocation` / `ContextFrame` 写入 formal events；开发者在 Markdown review 中执行 `Confirm / Modify / Regenerate / Cancel`。
...
4. Phase 1 把 confirmed `IntentSpec` 发给 provider 生成 plan set（至少包含 implementation/test 两类 `Plan` / `Task`），provider 仍只允许 readonly tools；开发者在 Markdown review 中执行 `Execute / Modify Plan / Revise Intent / Cancel`。
5. Phase 1 `Modify Plan` 写新的 implementation/test `Plan` / `Task` revision 并回到 review；若只改一类，另一类必须沿用当前 head 组成完整 plan set。`Revise Intent` 回 Phase 0 写新的 `Intent` revision；`Cancel` 直接写 terminal `Decision` / `IntentEvent`。
6. Phase 2 由 Scheduler 把 `selected_plan_ids` 物化为 `dagrs 0.8.1` composite graph，并以 DAG runtime 驱动 implementation/test node 执行；每个 node 的 `Sandbox` / `Worktree` 执行环境都由 Libra provision，并通过 `finalize_for_task()` 统一完成 sync / cleanup。
7. Phase 2 失败分流：`Retryable` 进入新 attempt；`ValidationReworkRequired` 追加 implementation/test `Plan` revision 并重建 composite DAG；`BroaderReplanRequired` 终止当前 DAG 并回 Phase 1；`Cancelled` / `TimedOut` / `ProviderDisconnected` / `PermanentFailure` 写 terminal event 并交给 Phase 4。
8. Phase 3 先汇总 implementation/test per-task patchset 形成 `release_candidate patchset`，再执行 `ValidatorEngine(integration -> security -> release)`。
...
本批次按"先定共享契约，再收口 identity / projection，再替换 UI 真相源，再补系统验证 / 决策，最后收紧安全和工程保障"的顺序推进，不采用长期双栈并存。
...
先定义共享 `Runtime`、`TaskExecutor`、`DagRuntimeBuilder`、`PromptPackage` 和 `WorkflowEvent`，把 Scheduler 固化为唯一控制面，并把 Phase 2 的 DAG runtime 正式收敛到 `dagrs 0.8.1`。这个步骤同时删除 `claudecode` managed runtime 及其 CLI surface，避免旧 contract 干扰新边界。

### Scope

1. 新建 `src/internal/ai/runtime/`，把 `ExecutionAuditSession` 收敛成统一 facade。
2. 定义 `TaskExecutionContext` / `TaskExecutionResult` / `TaskExecutionError` / `CancellationReason`。
3. 抽出 `IntentPromptBuilder`、`PlanningPromptBuilder` 和 `TaskPromptBuilder`，结束 Codex / generic 双入口 prompt assembly。
4. 把 Phase 2 DAG runtime 正式收敛到 `dagrs 0.8.1`，升级 `Cargo.toml` 与 `execute_dag()` 的 graph build / event / report / error 接口。
5. 把 `sandbox/*` 与 `orchestrator/workspace.rs` 收敛成 Libra 官方 `ExecutionEnvironmentProvider`，统一 provision / finalize（含 sync / cleanup）。
6. 把 Codex 主循环改造成 `CodexTaskExecutor`，把通用 provider 改造成 `CompletionTaskExecutor<M>`。
7. 删除 `CodeProvider::Claudecode`、相关 flags、帮助文本和 managed runtime 目录。
...
6. Phase 1 形成的 selected plan set 至少包含 implementation/test 两类 plan head，Phase 2 可将它们编译为 composite DAG。
...
5. 补齐 authz、redaction、durability、fault injection、perf、mock 设施。

### Performance Budgets

这些预算是 release gate，不是“尽量优化”的软目标：

1. composite DAG build：对 100 个 task、implementation/test 双 plan 的 `SelectedPlanSet`，P95 `<= 250ms`。
...
| `tests/helpers/mock_codex.rs` | `MockCodexServer` |
| `tests/helpers/mock_completion_model.rs` | `MockCompletionModel` |
...
| `src/internal/ai/runtime/phase1.rs` | 承载 Phase 1 的 plan-set 生命周期：implementation/test 两类 `Plan` / `Task` revision 写入、`current_plan_heads` / `selected_plan_ids` 更新、plan review gate 相关 mutation，以及从 `Modify Plan` / `Revise Intent` / `Execute` 分支回写 Scheduler 所需的 helper。 |
...
| `tests/helpers/mock_codex.rs` | 提供可脚本化的 `MockCodexServer`，用于覆盖 Codex transport、plan/review loop、tool stream、approval、disconnect、timeout、malformed event 等测试场景，而不依赖真实 Codex app-server。 |
| `tests/helpers/mock_completion_model.rs` | 提供可脚本化的 `MockCompletionModel`，用于通用 provider 路径的单元/集成测试，覆盖 planning、task attempt、tool loop 输出、retry、validation deficiency、broader replan 等分支。 |
...
验证按实现 Phase 1 → Phase 5 的顺序组织。测试策略从"验收条目"升级为分层测试矩阵。
...
| Phase 1 | `Plan(role=implementation|validation)` / `Task` / `Scheduler.selected_plan_ids` | 需验证 | 需验证 | 需验证 |
...
|---|---|---|---|---|
| Phase A（共享 contract） | Implementation Phase 1 | 固定 `Runtime`、`TaskExecutor`、prompt/event contract、execution environment 边界，并删除 `claudecode` 主路径 | Phase 0-4 contract、模块命名、hard constraints 已稳定 | Codex / 通用 provider 统一走 `Runtime + TaskExecutor`；formal writes 不再分散；`claudecode` surface 删除 |
...
前置条件：
`Workflow Contract`、`Implementation-Level Spec`、目标态模块边界和 `claudecode` 删除约束已经稳定，否则不应开始大面积切换调用点。

建议拆分为 6 个连续子任务：
1. 先做一个 `dagrs 0.8.1` spike（最小 PR 或临时分支），执行 `cargo check` / 编译验证本节列出的 API 形状；若 upstream 实际 API 与本文假设不符，先修正文档与 contract，再切主路径。
2. 先引入 `runtime/contracts.rs`、`runtime/mod.rs` 和最小 `Runtime` facade，只定义共享类型、phase 枚举、prompt package、task execution result、workflow event，不立即切主路径。
3. 把 `IntentPromptBuilder`、`PlanningPromptBuilder`、`TaskPromptBuilder` 抽出来，结束 Codex / generic 双入口 prompt assembly，但暂时保留旧调用点作为薄适配层。
4. 引入 `TaskExecutor`、`ExecutionEnvironmentProvider`、`CodexTaskExecutor`、`CompletionTaskExecutor<M>`，把单次 attempt 的输入输出、sandbox、task `Worktree`、sync back、cleanup contract 固化。
5. 把 Phase 2 DAG runtime 收敛到 `dagrs 0.8.1`，统一 graph build、execution report、termination、retry / cancel 错误模型，再把 Scheduler 主循环切到新 runtime。
6. 在所有调用点完成 cutover 的同一合入波次中，立即删除 `CodeProvider::Claudecode`、CLI flags、帮助文本、文档、测试和 `src/internal/ai/claudecode/` 目录。
