...
| 步骤 | 目标 | 并发策略 | 状态 |
|------|------|----------|------|
| **Step 1** | 按此前方案补齐单 Agent 的代码理解、弱模型兼容、安全底线、意图识别、可恢复编辑和生态基础 | 保持 orchestrator 串行；不启用 sub-agent 并行 | v1 核心基线已落地；旧 MCP config migration 与 VCS event-source producer wiring 仍为后续扩展 |
...
| 顺序 | 任务 | 为什么现在做 | 完成后解锁 |
|------|------|--------------|------------|
| 1 | Step 1.10 follow-up：VCS event-source wiring | CEX-14 / CEX-15 core 已有，OpenAPI fixture tool generation、`/source reload`、automation runtime dispatch core 和 hook lifecycle history bridge 已补齐；旧 MCP config migration、VCS event-source runtime 接线仍未完成 | Step 2 capability package / source-triggered workflows |
