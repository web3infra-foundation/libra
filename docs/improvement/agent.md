...
| CP-S2-3 Single sub-agent gate（含 flag-off 字节级等价） | CEX-S2-12 + CEX-S2-13 完成 | `cargo test ai_subagent_single`、`cargo test ai_subagent_flag_off_regression`、`cargo test ai_subagent_hook_dispatch`、`cargo test ai_subagent_merge`、`step1-regression` CI check；**`code.sub_agents.enabled = false`（默认）下 `cargo test --all` 字节级等价于 CEX-00 commit `48ea0ae`**——这里才是真正的 flag-off 等价检查；TUI 手动验证 explorer / worker / reviewer 三种 agent type 各跑一次 | 通过后才能开 controlled parallel（CEX-S2-14）；human-gated merge 必须默认开启 |
...
- **Hook 物理拦截层**（与 sandbox / approval 并联，不替代）：
  ...
  **Hook exit-code 权威映射表（Step 2 / Step 3 共享，不可单方面修改）：**
  ...
  | Exit code | 决策语义 | 后续路径 | AgentRunEvent |
  |-----------|----------|----------|----------------|
  | `0` | `allow` | dispatch 继续；后续 sandbox / approval 仍按各自规则执行 | `hook_passed` |
  | `2` | `deny` | dispatch 被阻止；agent 收到结构化拒绝反馈 | `blocked_by_hook`（含 hook 路径与 stdout/stderr 截断） |
  | `3` | `needs-human` | dispatch 暂停；走 Layer 1 approval（与 Step 1.6 Approval TTL 集成） | `hook_requested_human` |
  | 其他（含 1、负数、>3） | `deny`（**fail-closed**） | 等同 exit 2，不视为 “warn 但通过”；安全边界永远偏向拒绝 | `blocked_by_hook_failure`（`reason=unknown_exit_code`） |
  | panic | `deny`（**fail-closed**） | 同上 | `blocked_by_hook_failure`（`reason=panic`） |
  | timeout（默认 30s，可由 `[hooks].timeout_ms` 覆盖） | `deny`（**fail-closed**） | 同上 | `blocked_by_hook_failure`（`reason=timeout`） |

  - 所有 sub-agent dispatch path 的 hook 处理都按此表执行；Step 3.E Hook Harness 扩展新 hook 类型时必须复用此表，**不允许**对“其他 exit code”重新定义为 `warn but pass`。
  - 测试覆盖：`tests/ai_subagent_hook_dispatch_test.rs` 至少 6 个 fixture（exit 0 / 2 / 3 / 1 / panic / timeout），每条断言对应 `AgentRunEvent` 类型与 dispatch 是否进行。
