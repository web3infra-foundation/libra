...
任一 `diff` 非空、任一 `cargo build` 失败、normalizer 不存在或 fixture 不存在都视为 CP-S2-3 失败。CEX-S2-12 必须把 `scripts/normalize_session.sh`、`tests/data/flag_off_smoke.json` 与上面的比较脚本一并入库（参见 CEX-S2-12 Write set），不能依赖手写脚本。**字节级等价的强制点是步骤 5 的 8 个 `diff -u`——`cargo_all.status` / `cargo_all.failed` / `cargo_all.stdout.normalized` / `cargo_all.stderr.normalized` / `baseline7.stdout.normalized` / `code4.stdout.normalized` / `smoke.libra.normalized` / `test.libra.normalized` 都是完整内容比较，不是 wc -l 摘要**；任何主路径 / `.libra` 副作用都会被 normalizer 抹掉非确定性字段后呈现在 diff 中。
...
| `AgentRunEvent[E]` | Event | started / tool-call / blocked / completed / failed / cancelled / timed-out / **budget_exceeded**（CEX-S2-12 完成判定 (3) 输出，含 `dimension` 字段：`token` / `tool_call` / `wall_clock` / `source_call` / `cost`） / **hook_passed** / **blocked_by_hook** / **hook_requested_human** / **blocked_by_hook_failure**（含 `reason` 字段：`unknown_exit_code` / `panic` / `timeout` / `killed_by_signal:<signo>` / `spawn_enoent` / `spawn_eacces` / `needs_human_timeout`） / **post_tool_review_required**（PostToolUse 阶段所有异常都通过此 variant 表达，由 `reason` 区分） / **workspace_materialized**（CEX-S2-11 输出）。**所有变体都按 unknown-event-safe 序列化** —— 旧 reader 跳过未知 variant，新增 variant 不破坏 replay |
...
- **Hook 物理拦截层**（与 sandbox / approval 并联，不替代）：
  ...
  **needs-human (exit 3) Approval 集成约束**（exit 3 唯一进入 Approval 路径，其他路径不命中 cache）：
  ...
  - **No-cache 名单**（与 [Step 1.6 No-cache 名单](#step-16approval-ttl-与细粒度记忆m54)一致，所有四个类别都覆盖）：(1) Step 1.1 黑名单命令；(2) protected branch mutating 操作（含 hook approval）；(3) `network_domain` 非空且不在 `[approval].cacheable_domains` 白名单内的请求；(4) `hook_reason` 含 `secret_leak` / `cred_exposure` / `pii` 关键字时，**永不写入 ApprovalTTL cache**——每次都重新询问。任何一类 No-cache 触发都不进入 `ApprovalMemo`，即使 `ApprovalKey` 完全匹配旧 cache。
...
- Layer 1 负责（**注意：apply-to-main 路径属于 CEX-S2-13，不是本 Step**）：
  ...
  - 决定 accept/reject 的 schema 字段（本 Step 完成；Decision 写 `Decision[E]`）
