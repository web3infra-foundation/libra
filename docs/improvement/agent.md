...
| `impl<M> TaskExecutor for CompletionTaskExecutor<M>` 块 | Wave 1B Definition of Done #1 | **Schema + 最小 body 已落地（v0.17.579 schema, v0.17.593 单次完成调用 body）**：同文件含 `pub struct CompletionTaskExecutor<M: CompletionModel> { model: Arc<M> }` + `pub fn new(model)` / `pub fn from_arc(model)` 构造器 + `impl<M: CompletionModel + Send + Sync + 'static> TaskExecutor for CompletionTaskExecutor<M>`；execute_task_attempt body 已 wire：把 `TaskExecutionContext::prompt` (`PromptPackage::preamble` + `.messages`) 翻译成 `CompletionRequest`，调用 `M::completion`，把 assistant text 缝合到 `TaskExecutionResult.summary`，返回 `Completed` 终态；context.run_id 缺失时分配新 Uuid；模型错误映射成 `TaskExecutionError::Provider`；7 单测覆盖 happy path + run_id 自动分配 + run_id 透传 + prompt → CompletionRequest 映射（preamble + chat_history 验证）+ 空 prompt 边界 + 错误映射 + `Box<dyn TaskExecutor>` dyn 兼容；full tool-loop / sandbox / approval 集成仍待 follow-up | 中（剩余 full tool-loop 集成是 cross-cutting follow-up） |
