...
| `ProjectionResolver` rebuild / resume / degraded read / query-index materialization | `projection/resolver.rs` / `projection/rebuild.rs` tests（`rebuild_materializes_run_state_and_indexes` 覆盖 7 个 `ai_index_*` 表的落库关系与 latest 标记） |
