...
| Part B Implementation Phase 5 | Security / Permission / Diagnostics / Testing Hardening | 进行中：`AuditSink` trait 已落地（`hardening.rs:333`），含 `record_decision` / `record_event` 语义化入口；**from_env → resolve_env 改造已完成（v0.17.549..v0.17.556）**：API 表面 7 个 provider 全部提供 vault-aware `Client::from_resolved_env(LocalIdentityTarget<'_>)` 异步构造器（deepseek / kimi 在更早 patch 已就位；gemini / openai / anthropic / zhipu / ollama 由 v0.17.549..v0.17.553 补齐）；call-site cutover 在 v0.17.556 完成 — `src/internal/config.rs` 暴露 `pub fn resolve_env_sync(name)`，`src/command/code.rs::build_any_completion_model_for_args` 把 lookup closure 从 `std::env::var(key).ok()` 切换到 `resolve_env_sync`，让 `libra config --global add vault.env.<PROVIDER>_API_KEY <…>` 配置真正生效。剩余 Phase 5 收尾项：security-runtime test harness 扩展、durable audit retention 策略、`from_env()` 弃用通告（保留兼容 surface 但 deprecation warning 出现在 v0.18 release notes） |
...
|--------|-------------------------------|---------|
| S2-INV-01 三层架构 | Layer 1 入口：`src/command/code.rs` 与 `src/internal/ai/orchestrator/mod.rs`；Layer 3 现有：`src/internal/ai/runtime/contracts.rs:97` (`MaterializedProjection`) + `src/internal/ai/runtime/hardening.rs:414` (`ToolBoundaryRuntime`)；Layer 2 `src/internal/ai/agent_run/` 已存在但 **schema-only**（10 个文件：`mod.rs` / `task.rs` / `run.rs` / `event.rs` / `patchset.rs` / `evidence.rs` / `decision.rs` / `permission.rs` / `budget.rs` / `context_pack.rs`，共 1244 行）——无 dispatcher / merge pipeline / Scheduler 调用入口，runtime wiring 仍 pending CP-4 | 可冻结。CEX-S2-10 schema 已落地；runtime wiring 在 CEX-S2-12 启动 sub-agent loop 时接入，与现有 Layer 1/3 不冲突 |
| S2-INV-02 Runtime 唯一 formal write | `runtime/contracts.rs:120` (`SchedulerMutation`) + `runtime/contracts.rs:312` (`TaskExecutor` trait) 已是写入合同源；但缺顶级 `Event` trait 让 `LifecycleEvent` 等 event 类型形成统一 append-only 语义 | 可冻结。CEX-00.5 引入 `Event` / `Snapshot` 后通过 trait bound 把 “write through Runtime” 形式化 |
...
| S2-INV-12 evidence raw fact 链 | **持久化 `Evidence[E]` schema** 实际由 git-internal 提供（`git_internal::internal::object::evidence::{Evidence, EvidenceKind}`，本 crate 通过 `src/internal/ai/mcp/resource.rs:34` 重导入使用）；与之对照的 `src/internal/ai/runtime/contracts.rs:183` `EvidenceKind` 是**运行时分类枚举**（`Test` / `Lint` / `Build` / `ToolPolicyViolation` 等），不是持久化 Snapshot；两者都尚未含 `agent_run_id` / `source_event_id` / `tool_call_id` / `source_call_id` 字段 | 可冻结。CEX-S2-10 在 `agent_run/evidence.rs` 必须**扩展 + 引用 git-internal 的持久化 `Evidence[E]`**（增加 raw-fact-chain 字段），同时引用 `runtime/contracts.rs:183` 运行时 `EvidenceKind` 作为分类；不能复制 `evidence` 业务字段或新建 parallel snapshot 对象（与“与现有 IntentSpec 的映射”表 `AgentEvidence` 行约束一致） |
...
|------|---------|----------------|----------|
| `AgentPermissionProfile` 字段 | `sandbox/mod.rs:371` `ApprovalStore` 已为 session 级 cache；`runtime/hardening.rs:164` `ToolBoundaryPolicy` 已有 `decide()` | Step 1.1 OS 黑名单 + 参数白名单；Step 1.6 Approval TTL canonical key | profile 字段需引用最终 `ApprovalKey` 形态（含 `sensitivity_tier` / `scope` / `blast_radius`），未冻结前任何字段都可能改名 |
...
- **R-A4（CEX-S2-10 收口，已修订）**：持久化 `Evidence[E]` schema **由 git-internal 提供**（`git_internal::internal::object::evidence::{Evidence, EvidenceKind}`，本 crate 通过 `src/internal/ai/mcp/resource.rs:34` 导入），**不是** `src/internal/ai/runtime/contracts.rs:183` 的运行时 `EvidenceKind` 枚举。CEX-S2-10 新增 `agent_run/evidence.rs` 必须**扩展 + 引用 git-internal 的 `Evidence[E]` Snapshot**，并把 `runtime/contracts.rs:183` 运行时 `EvidenceKind` 当成 *分类标签* 引用（不是 schema 来源）；不能新建与 `Evidence[E]` 平行的 snapshot（避免 “与现有 IntentSpec 的映射” 表 `AgentEvidence` 行约束违例）。R-A4 修订前曾错误指向 `runtime/contracts.rs:183`，现已澄清。**命名碰撞警告**：两个 `EvidenceKind` 同名不同 path，CEX-S2-10 在 `agent_run/evidence.rs` 同时使用时必须显式 `as` 别名（推荐 `use git_internal::internal::object::evidence::EvidenceKind as PersistentEvidenceKind;` 与 `use crate::internal::ai::runtime::contracts::EvidenceKind as RuntimeEvidenceKind;`），避免编译期歧义和 review 时的视觉混淆。
...
- **R-A6（已修）**：原 CEX-S2-02 Scheduler 行写 “`SchedulerMutation` 已含 disable/enable 节点”，与 `runtime/contracts.rs:120-155` 实际 variants 不符；现实际为 **8 个 expected-version 写入变体**（`SeedThread` / `SetCurrentPlanHeads` / `SelectPlanSet` / `StartStage` / `MarkTaskActive` / `ClearActiveRun` / `MarkProjectionStale` / `ApplyRebuild`，由 `:160-167` `expected_versions()` 匹配确认），无 enable/disable；Codex round 2 发现初版 R-A6 还漏列 `MarkProjectionStale` / `ApplyRebuild` 两个变体，已补全；并把 enable/disable 标为 “CEX-S2-14 新增” 缺口。
...
| 日期 | 作者 | 变更摘要 |
|------|------|----------|
| 2026-05-20 | Claude Code | 关闭 2026-05-04 follow-up (f)：`MemoryAnchorReplay::apply_event` 增补 `last_recorded_at` 单调性 guard——同一 `anchor_id` 的事件若 `recorded_at` 严格早于已应用的最新记录则忽略，避免 JSONL 重放或部分加载把已 `Confirmed` 的 anchor 静默回退为 `Drafted`；等于已记录时间戳的事件仍允许重复应用以保证幂等。`tests/ai_memory_anchor_test.rs` 新增 `memory_anchor_replay_rejects_stale_event_for_same_anchor` / `memory_anchor_replay_idempotent_on_equal_recorded_at` 两个回归用例。 |
| 2026-05-20 | Claude Code | 关闭 2026-05-04 follow-up (g)：`src/internal/ai/agent_run/` 模块下 13 个公开枚举（`BudgetDimension` / `ReviewState` / `RiskLevel` / `AgentType` / `AnchorScope` / `AgentRunStatus` / `ApprovalRouting` / `HookPhase` / `HookKind` / `HookFailureReason` / `PostToolReason` / `WorkspaceStrategy` / `CancellationReason`）补齐 `#[non_exhaustive]` 属性，使下游 embedder 在 match 时必须保留 wildcard arm，未来 Step 2/Step 3 引入新变体不再破坏外部消费者；`tests/compat/agent_run_non_exhaustive_guard.rs` 守护回归。`AgentRunEvent` 与 `AgentRunEventEnvelope` 已通过 `Unknown(serde_json::Value)` 显式 catch-all 保证向前兼容，本次不强制叠加 `#[non_exhaustive]`。 |
...
| `SecretRedactor` / `AuditEvent` / `AuditSink` | [src/internal/ai/runtime/hardening.rs:260, 311, 333](../../src/internal/ai/runtime/hardening.rs) | 含可直接复用的 `TracingAuditSink`（:476） |
