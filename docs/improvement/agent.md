...
| `src/internal/ai/runtime/revision.rs` | 跨 phase 的 revision chain helper | **Schema 已落地（v0.17.577）**：模块存在，提供 `RevisionKind { Intent, ExecutionPlan, TestPlan }` enum + `.label()` 跨 phase 审计标签 + `RevisionChainEntry { kind, previous_id, revision, logical_id }` 类型 + `is_first()` / `is_continuation()` 链头/续接判断；`handle_modify_request()` 与 next-version skeleton derivation 函数体仍待 Wave 1B 把 modify-plan / modify-intent 路径接入；3 个单测覆盖 label 稳定性、链位置标志、Clone 合同 | 中 |
