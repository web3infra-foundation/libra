...
| 步骤 | 目标 | 并发策略 | 状态 |
|------|------|----------|------|
| **Step 1** | 按此前方案补齐单 Agent 的代码理解、弱模型兼容、安全底线、意图识别、可恢复编辑和生态基础 | 保持 orchestrator 串行；不启用 sub-agent 并行 | v1 核心基线已落地；VCS event-source producer wiring 仍为后续扩展 |
...
| CEX-15 | Automation MVP（已完成 core，2026-05-03；hook lifecycle history bridge v0.17.86） | CEX-11、CEX-13c、CEX-14、CEX-12.5 | `src/internal/ai/hooks/`、`src/internal/db/migration.rs`、`src/command/mod.rs` | `src/internal/ai/automation/{mod.rs,config.rs,events.rs,scheduler.rs,executor.rs,history.rs,runtime.rs}`、`src/internal/ai/mod.rs`、`src/command/automation.rs`、`src/command/mod.rs`、`tests/ai_automation_test.rs` | `cargo test --test ai_automation_test` | TOML rules、cron 模拟、webhook / shell / prompt action、history log 可测；Shell action 复用 Step 1.1 safety；cron 触发的 shell action对 deny / needs-human fail-closed，不绕过 scope / blast radius 检查；`automation_log` 表通过 CEX-12.5 migration 注册；hook lifecycle `session_start` / `session_end` 可匹配 automation rules 并写入 history；VCS event-source dispatch 留后续接线 |
...
| 顺序 | 任务 | 为什么现在做 | 完成后解锁 |
|------|------|--------------|------------|
| 1 | Step 1.10 follow-up：VCS event-source wiring | CEX-14 / CEX-15 core 已有，OpenAPI fixture tool generation、`/source reload`、旧 MCP config migration、automation runtime dispatch core 和 hook lifecycle history bridge 已补齐；VCS event-source runtime 接线仍未完成 | Step 2 capability package / source-triggered workflows |
| 2 | CEX-S2-11 起的 Step 2 runtime | 只有 CP-4 通过后才允许 sub-agent runtime；当前 `agent_run/` 只是 schema-only，CEX-16 usage TUI 依赖已补齐但 CP-6 仍需按 release gate 跑完整验证 | sub-agent workspace / single-agent flag / controlled parallel |
...
- [ ] 统一事件流：hooks lifecycle + VCS events（v0.17.82 已新增 `AutomationRuntimeEvent` 与 scheduler runtime dispatch core；v0.17.86 已把 hook lifecycle `session_start` / `session_end` 接入 automation history；VCS producer 到 automation history 的进程内接线仍未完成）。
