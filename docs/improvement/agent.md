...
| Storage / migration | `src/internal/db/migration.rs`、`schema_versions` entity、`sql/migrations/README.md`、`tests/db_migration_test.rs` 已形成 CEX-12.5 基线；fresh / establish 路径都会运行 migration runner；`automation_log` 与 CEX-16 core 的 `agent_usage_stats` 已通过 built-in migration 注册；v0.17.373 已落地 `AutomationHistory::prune_before()` + `libra automation prune --retention-days <days>`（默认 90 天，支持 `[automation].retention_days` 项目配置覆盖，幂等删除按 `finished_at` cutoff） | usage failure-row backfill、source telemetry 持久化仍未收口 |
