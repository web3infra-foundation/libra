...
| Storage / migration | `src/internal/db/migration.rs`、`schema_versions` entity、`sql/migrations/README.md`、`tests/db_migration_test.rs` 已形成 CEX-12.5 基线；fresh / establish 路径都会运行 migration runner；`automation_log` 与 CEX-16 core 的 `agent_usage_stats` 已通过 built-in migration 注册；v0.17.373 已落地 `AutomationHistory::prune_before()` + `libra automation prune --retention-days <days>`（默认 90 天，支持 `[automation].retention_days` 项目配置覆盖，幂等删除按 `finished_at` cutoff）；v0.17.383 已落地 usage failure-row backfill 的 observer 通路：`ToolLoopObserver::on_model_missing_usage_recorded` 与 `on_model_usage_failed` default impls，让 tool loop 在 missing-usage / provider-failure 两条分支也通知 TUI / web observer 推进 wall-clock 与失败计数，与持久化的 `agent_usage_stats` failure row 对齐 | source telemetry 持久化仍未收口 |
...
| Provider / JSON / usage | Completion providers 和 hidden fake provider 存在；provider response 里已有 token usage 字段的历史对象路径（如 Codex `RunUsage`）；`completion/json_repair.rs` 已提供 provider-neutral JSON repair core、结构化错误和 fixture gate；OpenAI-compatible 家族（OpenAI / DeepSeek / Kimi / Zhipu）与 Ollama 的 tool-call `arguments` 字符串解析失败时已统一先 repair 再 raw-string fallback，Anthropic / Gemini 的原生结构化 tool input 不需要字符串 repair；CEX-16 core 已新增 `UsageRecorder`、SQLite `agent_usage_stats` 聚合、missing/failure rows、OpenAI/Anthropic/Gemini usage metadata 映射、`libra usage report --by=model` 和 TUI compact bottom-pane usage line；pricing table/config override、cancel/timeout failure kind、`[usage].retention_days` prune config 已接入；`libra code` TUI direct provider bootstrap 与 Ollama headless web-only bootstrap 已通过 `ProviderFactory` 统一解析 API key / base URL / `OLLAMA_COMPACT_TOOLS`，并覆盖 missing-key 与 `--api-base` 回归；v0.17.376 在 Anthropic provider 上添加 `Client::from_resolved_env(local_target)` 异步构造器，走 `resolve_env_for_target` 三级链路（process env → repo `vault.env.*` → global `vault.env.*`），保留 `from_env()` 作为 process-env-only 低层 API；v0.17.377 OpenAI provider 同步落地 `from_resolved_env` (`OPENAI_API_KEY` / `OPENAI_BASE_URL`)；v0.17.378 Gemini provider 同步落地 `from_resolved_env` (`GEMINI_API_KEY`，无 base URL override 维持 from_env 单字段契约)；v0.17.379 DeepSeek provider 同步落地 `from_resolved_env` (`DEEPSEEK_API_KEY`，无 base URL override)；v0.17.380 Kimi provider 同步落地 `from_resolved_env` (`MOONSHOT_API_KEY` / `MOONSHOT_BASE_URL`)；v0.17.381 Zhipu provider 同步落地 `from_resolved_env` (`ZHIPU_API_KEY` / `ZHIPU_BASE_URL`)；v0.17.382 Ollama provider 同步落地 `from_resolved_env` (`OLLAMA_BASE_URL` 必查；`OLLAMA_API_KEY` 仅在 cloud base URL 下查询；local target 保持不读取 API key)，**七个 provider 全部接入 vault-aware 解析链路** | 完整 TUI L1/L3 实时展示、streaming incremental token 仍未完成 |
...
| `AuditSink` / Snapshot / Event 抽象 | ✅ 已落地 | `runtime/hardening.rs` 已有 `AuditSink::append` / `flush` / `record_decision` / `record_event`，`TracingAuditSink` / `InMemoryAuditSink` 可复用；`runtime/event.rs` / `runtime/snapshot.rs` 已冻结顶级 `Event` / `Snapshot` trait，`LifecycleEvent` 实现 `Event`，`MaterializedProjection` 实现 `Snapshot`；v0.17.374 扩展 `Snapshot` 实现到 `ThreadProjection` 与 `projection::rebuild::MaterializedProjection`，让 projection 层在 audit / observability 路径上具备 `&dyn Snapshot` 抽象统一性 |
...
| 持久化功能表 | ⚠️ 部分落地 | migration runner 已有，JSONL session 文件真相源已落地；`ContextFrame` / `CompactionEvent` 已接入 session JSONL 和 attachments；`automation_log` 与 CEX-16 core `agent_usage_stats` 已通过 built-in migration 接入；v0.17.373 已落地 `automation_log` retention（`AutomationHistory::prune_before()` + `libra automation prune --retention-days <days>` + `[automation].retention_days` config）；v0.17.383 已落地 usage failure-row observer 通路（tool loop 在 missing-usage / provider-failure 分支调用新的 `ToolLoopObserver::on_model_missing_usage_recorded` / `on_model_usage_failed`）。source telemetry 持久化仍未完成 |
...
| 2026-05-04 | Claude Code | 本轮 Step 1 安全审计修订：修复 `branch --delete=<protected>`/`-D=<protected>` 内联值绕过 protected-branch deny 的回归（`libra_vcs::branch_delete_targets_protected_branch` 现额外解析 `--delete=` / `-d=` / `-D=` inline value），加入对应 `tests/ai_libra_vcs_safety_test.rs` 回归 fixture；为 `ApprovalStore::ApprovedForAllCommands` 增补 `revoke_allow_all_for_scope` / `active_allow_all_scopes` 与 `/approvals revoke allow-all <scope>` TUI 入口，避免一次 allow-all 投票导致整个 session 不可撤销；将 `ApprovalMemo::ttl` 的 `Duration::MAX`/overflow 路径硬上限到 7 天，避免静默退化为永久 memo；`network_domain_from_token` 在 scheme-qualified URL 上返回 `__non_ascii__:<host>` sentinel，使 IDN/非 ASCII 主机不再绕过 `no_cache_unknown_network` / `allowed_network_domains` 策略。已知未处理的 follow-up（按优先级登记，非本轮 ship 阻塞）：(a) ✅ v0.17.391 已落地：`ApprovalStore` 新增 `revoke_by_prefix(&mut self, prefix: &str) -> usize` 方法，删除所有以指定前缀开头的 cache key（如 `shell::*`），返回删除条数；空前缀视为 no-op 防止误清空整个 store；幂等回调亦覆盖；保留原 `revoke(&mut self, key)` 精确匹配语义不变；(b) ✅ v0.17.384 已落地：shell 突变 guard 对 `ln -s` 跨工作区符号链接、`tee` 写入系统/绝对路径、`dd of=<absolute non-/dev path>` 三类攻击形态新增分类（System / Workspace blast radius），由 `destructive_shell_blast_radius` 调用 `ln_args_target_outside_workspace` / `tee_args_blast_radius` / `dd_args_blast_radius`；`>/path` 重定向已由 `contains_redirection_or_pipeline` 路径走 needs-human review，无需在 destructive 表里重复登记；(c) ✅ v0.17.385 已落地：JSON repair `normalize_smart_quotes` 改用三态状态机（Outside / AsciiString / SmartString）追踪字符串字面量边界；ASCII-delimited 字面量内的 `“` / `”` 作为内容保留不改写，smart-delimited 字面量按对解析并规范化为 `"` 配对；单独出现的孤立 `”` 仍规范化为 `"` 以保留旧的兜底语义。新增三条 fixture 防御回归（内含 smart double / single quote 的字符串字面量、smart-delimited key + ASCII value with smart content）；(d) ✅ v0.17.386 已落地：deepseek 与 kimi streaming 的 `KimiStreamToolCallBuilder::is_complete` / `DeepSeekStreamToolCallBuilder::is_complete` 在 strict JSON 失败后改用 `parse_json_repaired`，与 `parse_tool_call_arguments_with_repair` 的 salvage 边界对齐；每个 provider 各加一条 trailing-comma fixture 防御回归；(e) ✅ v0.17.387 已落地：`ContextBudgetAllocator::allocate` 的 compressible 排序改为 `(priority_rank, candidate.id, index)` 三键稳定 tie-break，HashMap 来源候选不再出现非确定性 selection；新增跨输入顺序回归测试 `allocator_tie_break_is_stable_across_input_orderings` 防御回归；(f) ✅ v0.17.388 已落地：`MemoryAnchorReplay` 新增 `last_applied: BTreeMap<Uuid, DateTime<Utc>>` 跟踪每个 anchor 最近一次应用的 `recorded_at`，`apply_event` 在 `event.recorded_at <= previous_recorded_at` 时跳过该事件，确保重复/乱序/篡改 replay 都不能让状态机回退（覆盖回退到 Draft、equal-recorded_at duplicate、tampered content overwrite 三个回归场景）；(g) ✅ v0.17.389 已落地：agent_run 模块 12 个 schema enum 全部添加 `#[non_exhaustive]` 标记（`BudgetDimension` / `ApprovalRouting` / `ReviewState` / `RiskLevel` / `AgentType` / `AgentRunStatus` / `HookPhase` / `HookKind` / `HookFailureReason` / `PostToolReason` / `WorkspaceStrategy` / `CancellationReason` / `AgentRunEvent` / `AnchorScope`），后续添加变体不会破坏 semver；当前仓库内无穷举 match 受影响；`ai_subagent_contract_test` 20 项契约测试在 `subagent-scaffold` feature 下全部通过；(h) ✅ v0.17.390 已落地：`uuid_newtype!` 宏移除了 `impl Default` block（之前会让 `AgentTaskId::default()` 返回新 v4 UUID 而非 stable sentinel，是 footgun），保留 `new()` 作为唯一显式分配入口并加 `#[allow(clippy::new_without_default)]` 抑制误导性 lint；`AgentBudget::default()` / `MergeDecisionPayloadV0::default()` 为 "all-None / empty stub" 的有意默认（被 contract test 覆盖为"unlimited budget"/"empty payload"），不属于陷阱保留。已知未处理的 follow-up（按优先级登记，非本轮 ship 阻塞）已全部清空。 |
