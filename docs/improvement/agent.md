...
| Step 2.2 | CEX-S2-11 | Isolated workspace abstraction | 方案阶段（**未落地**）——sub-agent 仍能写主 worktree；CEX-S2-13 human-gated merge candidate 也依赖此抽象先落地 |
| Step 2.3 | CEX-S2-12 | Single sub-agent behind flag | **已落地（v0.17.737..v0.17.788）**：`code.sub_agents.enabled = true` 时 `DefaultSubAgentDispatcher` + `DefaultSubAgentChildRunner` 走完整 OC-Phase 3 路径，所有 9 个 gates + 5 个终止 event 类型 + 3 个 doc-named E2E fixture + hook dispatch（v0.17.806/.807 S2-INV-13）+ 异步 cancel（v0.17.767）+ per-agent budget（v0.17.769）+ context handoff（v0.17.773/.785）均已落地；workspace isolation 由 Step 2.2 单独承担，本 step 的 sub-agent 仍能写主 worktree |
