...
| `src/internal/ai/runtime/phase1.rs` | Phase 1 Plan 的 formal write helper（`write_plan_set` / `advance_scheduler`） | **`write_plan_set` + `apply_scheduler_mutation`（全部 8/8 mutation variants 已 wire）函数体已落地（v0.17.580 write_plan_set, v0.17.588 MarkTaskActive/ClearActiveRun, v0.17.589 batch SetCurrentPlanHeads/SelectPlanSet/StartStage/MarkProjectionStale/ApplyRebuild, v0.17.590 SeedThread）**：`pub async fn write_plan_set(...)` 通过 `orchestrator::persistence::write_plan_set_with_outcome` 桥接到真实 plan-set 持久化。`pub fn apply_scheduler_mutation(current, mutation) -> Result<SchedulerState, ApplySchedulerMutationError>` 是 `advance_scheduler` 的 pure-function 核心：所有 8 个 SchedulerMutation variant 已 wire（SeedThread / SetCurrentPlanHeads / SelectPlanSet / StartStage / MarkTaskActive / ClearActiveRun / MarkProjectionStale / ApplyRebuild）；`SeedThreadMismatch` fail-closed 防止跨线程 seed；`VariantNotWired` 保留作为 forward-compat marker for 未来新增 variants；12 个单测覆盖每个 wired variant 的 state-transition + version mismatch + cross-thread seed 拒绝 | ✅ 已落地（apply_scheduler_mutation 全 8 variant 完成；剩余 advance_scheduler async 包装 + write_plan_set 调用方接入 是 mechanical follow-up） |
...
| `impl<M> TaskExecutor for CompletionTaskExecutor<M>` 块 | Wave 1B Definition of Done #1 | **Schema 已落地（v0.17.579）**：同文件含 `pub struct CompletionTaskExecutor<M: CompletionModel>` + `impl<M: CompletionModel + Send + Sync + 'static> TaskExecutor for CompletionTaskExecutor<M>`；body 同样为自识别 stub error，per-attempt completion-model + tool-loop wiring 函数体仍待 follow-up；3 单测覆盖 stub error 标识、`Box<dyn TaskExecutor>` dyn 兼容 | 中（Wave 1B impl-body follow-up） |
