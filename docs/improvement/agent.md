...
- **R-A1（已修）**：基线表第 162 行原写 “`AuditSink` 作为可注入 trait 尚未定义” 与代码不符。`runtime/hardening.rs:394` 已有 `pub trait AuditSink: Send + Sync` 含 `append`（hardening.rs:398）+ `flush`（hardening.rs:401）；`ToolBoundaryRuntime::append_audit` 在 :514 调用它。已更新为 “trait 已存在但形态需 CEX-00.5 演化”。
