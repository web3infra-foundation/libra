...
run_artifacts() {
  ...
  # 4b. capture the full normalized stdout (not only line counts) so byte-level
  #     equivalence is actually enforced; line-count diff alone is insufficient
  "$TREE/scripts/normalize_session.sh" "$OUT/cargo_all.stdout" > "$OUT/cargo_all.stdout.normalized"

  # 4c. baseline 7 + code_test 4 with isolated $HOME
  (
    cd "$TREE" && \
    HOME="$OUT/home_test" XDG_CONFIG_HOME="$OUT/home_test/.config" \
    LIBRA_CONFIG_OVERRIDE="$CFG" \
...
# 5. compare every captured artifact (content, not just line counts)
...
- **Hook 物理拦截层**（与 sandbox / approval 并联，不替代）：
  ...
  **Hook exit-code 权威映射表（Step 2 / Step 3 共享，不可单方面修改）：**
  ...
  > **AgentRunEvent 命名约定**（消除 PostToolUse 歧义）：dispatch 之前的拦截结果一律写 `blocked_by_hook` 系列（`blocked_by_hook` / `hook_requested_human` / `blocked_by_hook_failure`）；dispatch 之后的 PostToolUse 异常或拒绝**不能阻止已发生的工具结果**，统一写 `post_tool_review_required` 并把 `reason` 字段标注异常类型。每行最后一列分两段：`PreToolUse 阶段事件 / PostToolUse 阶段事件`。
