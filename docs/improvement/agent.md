...
- **mock smoke session 产物**（`smoke.libra.normalized` / `test.libra.normalized`）字节级等价于 baseline 等价命中：smoke session 由 CEX-S2-12 引入的 `flag_off_smoke.json` 驱动，因此 baseline commit 必须包含该 fixture——具体取 CP-S2-3 触发那一刻的 CEX-S2-12 完成 commit（脚本里 `BASELINE_COMMIT` 变量从配置读取，详见下方）。
