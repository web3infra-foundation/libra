...
| `src/internal/ai/runtime/revision.rs` | 跨 phase 的 revision chain helper | **Schema + 派生 helper 已落地（v0.17.577 schema、v0.17.584 derive_next_revision_skeleton）**：模块存在，提供 `RevisionKind { Intent, ExecutionPlan, TestPlan }` enum + `.label()` 跨 phase 审计标签 + `RevisionChainEntry { kind, previous_id, revision, logical_id }` 类型 + `is_first()` / `is_continuation()` 链头/续接判断；`pub fn derive_next_revision_skeleton(previous, previous_persisted_id) -> RevisionChainEntry` 纯函数已落地——保留 `kind` / `logical_id`、递增 `revision`、把 `previous_id` 链到 just-persisted 的 id；`handle_modify_request()` 仍需 modify-request payload 类型设计才能 wire；5 个单测覆盖 label 稳定性、链位置标志、Clone 合同、`derive_next_revision_skeleton` 递增链接 + 首链头升级路径 | 中（剩余 `handle_modify_request` 需 modify-request 类型先冻结） |
| `src/internal/ai/mcp/authz.rs` | Phase 5 安全层 `McpAuthorizer` | **Schema + 全面 server wiring 已落地（v0.17.573 schema、v0.17.582 list_resources、v0.17.583 read_resource + list_resource_templates、v0.17.585 batch tool wiring）**：trait + `McpOperation` / `AuthzDecision` / `AuthzError` 类型在 `authz.rs` 声明（4 个单测）；`LibraMcpServer` 已新增 `authz: Arc<Mutex<Option<Arc<dyn McpAuthorizer>>>>` 字段 + `set_authz(...)` + `pub(crate) async fn authorize_or_error(McpOperation<'_>)` helper。**13 个 MCP 入口已 wire**：3 个 resource 入口（`list_resources_impl` / `read_resource_impl` / `list_resource_templates`）+ 10 个 tool 入口（`create_intent` / `list_intents` / `create_task` / `list_tasks` / `create_run` / `list_runs` / `create_context_snapshot` / `list_context_snapshots` / `create_plan` / `list_plans` 全部通过 `McpOperation::CallTool { tool_name }` gate）。6 个单测覆盖 Allow / Deny 路径与跨 resource/tool 类别的语义统一。剩余：per-request principal threading (caller token → PrincipalContext) | 低（per-request principal threading 是单一 follow-up） |
...
| Phase 5 Security / Permission / Diagnostics / Hardening | 100% | ~85% | `mcp/authz.rs` schema 已落地（v0.17.573：`McpAuthorizer` trait + `McpOperation` / `AuthzDecision` / `AuthzError` + 4 单测）；server.rs wiring 全面落地（v0.17.582：`LibraMcpServer.authz` + `set_authz` + `authorize_or_error` + `list_resources_impl` + 2 单测；v0.17.583：`read_resource_impl` + `list_resource_templates` + 2 单测；v0.17.585：10 个 tool entry (`create_intent` / `list_intents` / `create_task` / `list_tasks` / `create_run` / `list_runs` / `create_context_snapshot` / `list_context_snapshots` / `create_plan` / `list_plans`) batch wired + `create_task_impl_is_blocked_by_deny_authz` tool-side 单测）；TUI direct provider bootstrap 与 Ollama headless web-only bootstrap 已收敛到 `ProviderFactory`；vault/env 统一接入已在 v0.17.549..v0.17.556 完成（`Client::from_resolved_env(LocalIdentityTarget<'_>)` × 7 providers + `resolve_env_sync` 接入 `build_any_completion_model_for_args`）；provider crate 公开 `from_env()` 保留至 v0.18 弃用通告 |
