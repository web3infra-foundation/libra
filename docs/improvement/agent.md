...
| `src/internal/ai/mcp/authz.rs` | Phase 5 安全层 `McpAuthorizer` | **Schema + server.rs wiring 已落地（v0.17.573 schema、v0.17.582 list_resources wiring、v0.17.583 read_resource + list_resource_templates wiring）**：trait + `McpOperation` / `AuthzDecision` / `AuthzError` 类型在 `authz.rs` 声明（4 个单测）；`LibraMcpServer` 已新增 `authz: Arc<Mutex<Option<Arc<dyn McpAuthorizer>>>>` 字段 + `set_authz(...)` 安装器 + `authorize_or_error(McpOperation<'_>)` async helper（Allow / Deny / NeedsHuman 三态映射到 `ErrorData`，未安装 authz 时 fast-path return Ok 保留 pre-Phase-5 unconditional-allow 语义）；3 个 server-side resource 入口（`list_resources_impl` / `read_resource_impl` / `list_resource_templates`）已 wire；4 个新单测覆盖 "no authz set → OK / 原有错误路径" 与 "Deny authz → invalid_request error" 两种语义。剩余：把 wiring 扩展到 `resource.rs` 的各 `create_*_impl` / `list_*_impl` create-tool 入口；per-request principal threading (caller token → PrincipalContext) | 低（Phase 5 wiring pattern 已确立，剩余只是 mechanical 扩展） |
...
| Phase 5 Security / Permission / Diagnostics / Hardening | 100% | ~70% | `mcp/authz.rs` schema 已落地（v0.17.573：`McpAuthorizer` trait + `McpOperation` / `AuthzDecision` / `AuthzError` + 4 单测）；server.rs wiring 已落地（v0.17.582：`LibraMcpServer.authz` 字段 + `set_authz` + `authorize_or_error` helper + `list_resources_impl` 接入 + 2 单测；v0.17.583：`read_resource_impl` + `list_resource_templates` 接入 + 2 单测）；TUI direct provider bootstrap 与 Ollama headless web-only bootstrap 已收敛到 `ProviderFactory`；vault/env 统一接入已在 v0.17.549..v0.17.556 完成（`Client::from_resolved_env(LocalIdentityTarget<'_>)` × 7 providers + `resolve_env_sync` 接入 `build_any_completion_model_for_args`）；provider crate 公开 `from_env()` 保留至 v0.18 弃用通告 |
