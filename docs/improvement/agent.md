...
| **Code UI 尚未收敛到共享 read model / interaction state / controller lease** | 高 | TUI / Web 仍各自维护 session snapshot 与交互状态；`pending_plan_revision` 已进入共享 snapshot，但 `pending_post_plan` 仍依赖本地 TUI 状态，controller lease 与更完整的 shared interaction lifecycle 仍需继续收口 |
