...
|---------------|----------------------|------|
| Step 1.0 | Implementation Phase 0: Contract Stabilization | 基线测试、mock / schema 迁移前置保护。 |
...
| Performance tests | semantic tools vs grep、JSONL resume、workspace materialization、Source Pool concurrency | 通过独立 perf 脚本或 release/nightly job 记录；不使用 `benches/` / `cargo bench` |
...
| `Implementation Phase 0: Contract Stabilization` | 真正切主路径前的工程底座补丁：dagrs spike、mock、schema migration plan、核心 enum / mutation / freshness contract | 不是运行时 Phase 0 Intent workflow，也不是可跳过的准备工作 |
...
- `Implementation Phase 0` 必须先于主路径 cutover，否则 Phase 1 的 gate 会缺少 dagrs API 事实、mock provider、schema migration 和核心 contract。
...
6. 记录：每个 Phase commit message 或 PR note 必须记录实际运行的 gate，包括 `cargo check`、相关 `cargo test` filter、migration test 和本地 provider 验收结果。
...
在任何主路径切换前，先把会反向卡住 Phase 1 的工程底座补齐：`dagrs 0.8.1` API 事实、mock provider、schema migration 节奏，以及核心 enum / mutation / freshness contract。
...
3. 定义 schema migration 节奏：新增幂等 `sql/sqlite_YYYYMMDD_ai_runtime_contract.sql`，并同步更新 fresh bootstrap 和 SeaORM entity。不能只改 `sql/sqlite_20260309_init.sql`。
4. 补齐 `PlanSetWriteInput`、`SchedulerMutation`、`EvidenceKind`、`ProjectionFreshness`、`FinalDecisionVerdict` 语义和 Task dependency 规则。
...
3. schema migration 文件、bootstrap、entity 三者必须同批更新；缺任一项都视为 Phase 0 未完成。
...
6. 非 Codex `--web-only` placeholder 已消灭（HeadlessCodeRuntime v1）；继续把通用 provider 的 read model / interaction state 收敛到 Phase 3 shared projection。
...
5. 补齐 authz、redaction、durability、fault injection、perf targets；mock 设施必须已在 Phase 0 前置完成。

### Performance Budgets

这些预算是性能目标，不再依赖 Phase 0 性能采集脚本或 baseline commit。若未来恢复 release gate，必须通过独立 perf 脚本或 release/nightly job 另行落地，且不使用 `benches/` / `cargo bench`。
...
6. perf 预算只作为目标和诊断记录；除非另行落地独立 perf 脚本或 release/nightly job，否则不得宣称其为 CI / release gate。
...
| `tests/ai_code_ui_projection_test.rs`（✅ 已落地） | Implementation Phase 3: TUI/Web 共享 interaction、typed delta、controller lease 与 Web takeover |
...
4. `PlanSetWriteInput`、`SchedulerMutation`、`EvidenceKind`、`ProjectionFreshness`、`FinalDecisionVerdict` 有 contract tests。
...
|---|---|---|---|---|
| Phase 0（contract stabilization） | Implementation Phase 0 | 固定 dagrs spike、mock、schema migration、核心 contract | Phase 0-4 workflow contract 已稳定 | Phase 1 cutover 的测试、schema 和 contract gate 可运行 |
...
前置条件：
Implementation Phase 0 已完成：`dagrs 0.8.1` spike 通过、mock 基础设施可用、schema migration 节奏明确、核心 contract 已冻结。否则不应开始大面积切换调用点。
...
4. 落地 controller lease takeover：Web 可申请接管当前 thread 的输入控制权，TUI 收到事件后降级为 observer；非 Codex `--web-only` 通用 provider 路径已具备 headless runtime，后续只收敛共享 interaction / IntentSpec workflow。
