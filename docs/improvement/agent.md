...
| 模块 | 关键交付 | 状态 |
|------|---------|------|
| Part B Implementation Phase 0 | Contract Stabilization | 已完成：`RuntimeConfig` (`src/internal/ai/runtime/mod.rs:39`) + `Runtime` (`src/internal/ai/runtime/mod.rs:52`)、`TaskExecutor` trait (`contracts.rs:414`)、`PromptPackage` / `WorkflowPhase` 已导出 |
| Part B Implementation Phase 1 / Wave 1A | 共享 Runtime + TaskExecutor + dagrs 0.8.1 接入 | **已完成**：`dagrs = "0.8.1"`（`Cargo.toml:77`）、`runtime/` 模块完整 |
| Part B Implementation Phase 1 / Wave 1B | Codex `CodexTaskExecutor` + `CompletionTaskExecutor<M>` 收敛 | **trait + impl schema 已落地（v0.17.579，2026-05-19）**：`pub trait TaskExecutor` 在 `runtime/contracts.rs:414` 定义；`impl TaskExecutor for CodexTaskExecutor` 和 `impl<M: CompletionModel + Send + Sync + 'static> TaskExecutor for CompletionTaskExecutor<M>` 在 `runtime/task_executors.rs` 落地（schema-only，函数体返回自识别 stub error 指向后续 wiring 工作）。Wave 1B "Definition of Done" 第 1 条 schema 部分已达成；Phase 2 主循环仍待 follow-up patch 接入真实 attempt loop（Codex WebSocket / completion-model + tool-loop）才能完整通过 `TaskExecutor` 驱动 |
...
| Part B Implementation Phase 5 | Security / Permission / Diagnostics / Testing Hardening | 进行中：`AuditSink` trait 已落地（`hardening.rs:394`），含 `record_decision` / `record_event` 语义化入口；**from_env → resolve_env 改造已完成（v0.17.549..v0.17.556）**：API 表面 7 个 provider 全部提供 vault-aware `Client::from_resolved_env(LocalIdentityTarget<'_>)` 异步构造器（deepseek / kimi 在更早 patch 已就位；gemini / openai / anthropic / zhipu / ollama 由 v0.17.549..v0.17.553 补齐）；call-site cutover 在 v0.17.556 完成 — `src/internal/config.rs` 暴露 `pub fn resolve_env_sync(name)`，`src/command/code.rs::build_any_completion_model_for_args` 把 lookup closure 从 `std::env::var(key).ok()` 切换到 `resolve_env_sync`，让 `libra config --global add vault.env.<PROVIDER>_API_KEY <…>` 配置真正生效。剩余 Phase 5 收尾项：security-runtime test harness 扩展、durable audit retention 策略、`from_env()` 弃用通告（保留兼容 surface 但 deprecation warning 出现在 v0.18 release notes） |
...
>
> **代码基线（2026-05-02）**：claudecode 已删除（`src/internal/ai/claudecode/` 不存在；CLI 仅保留 `removed_code_claudecode_hints` 给老用户的迁移提示，详见 `src/cli.rs:599`），dagrs 已升级到 0.8.1（`Cargo.toml:77`），`Runtime` / `RuntimeConfig` / `TaskExecutor` / `Event` / `Snapshot` / `AuditSink` traits 全部已落地（`src/internal/ai/runtime/{mod,contracts,event,snapshot,hardening}.rs`）。Phase 3/4 已有专用模块 `phase3.rs` / `phase4.rs`。
