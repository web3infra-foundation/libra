...
| `src/internal/ai/mcp/authz.rs` | Phase 5 安全层 `McpAuthorizer` | **Schema + server.rs wiring 已落地（v0.17.573 schema、v0.17.582 wiring）**：trait + `McpOperation` / `AuthzDecision` / `AuthzError` 类型在 `authz.rs` 声明（4 个单测）；`LibraMcpServer` 已新增 `authz: Arc<Mutex<Option<Arc<dyn McpAuthorizer>>>>` 字段 + `set_authz(...)` 安装器 + `authorize_or_error(McpOperation<'_>)` async helper（Allow / Deny / NeedsHuman 三态映射到 `ErrorData`，未安装 authz 时 fast-path return Ok 保留 pre-Phase-5 unconditional-allow 语义）；`list_resources_impl` 已 wire 调用 `authorize_or_error(McpOperation::ListResources)`；2 个新单测覆盖 "no authz set → OK" 与 "Deny authz → invalid_request error" 路径。剩余：把 wiring 扩展到 `read_resource_impl` / `list_resource_templates` / 各 `*_impl` create-tool 入口；per-request principal threading (caller token → PrincipalContext) | 低（Phase 5 wiring 已示范，剩余只是扩展） |
