...
| `src/internal/ai/runtime/revision.rs` | 跨 phase 的 revision chain helper | **Schema + 派生 helper + modify-request 协调器已落地（v0.17.577 schema、v0.17.584 derive_next_revision_skeleton、v0.17.587 handle_modify_request + ModifyRequest 类型）**：模块存在，提供 `RevisionKind { Intent, ExecutionPlan, TestPlan }` enum + `.label()` 跨 phase 审计标签 + `RevisionChainEntry { kind, previous_id, revision, logical_id }` 类型 + `is_first()` / `is_continuation()` 链头/续接判断；`pub fn derive_next_revision_skeleton(previous, previous_persisted_id) -> RevisionChainEntry` 纯函数；`pub struct ModifyRequest { kind, logical_id, reason }` 修改请求 envelope（audit 字段，per-kind payload 留给 follow-up）；`pub enum ModifyRequestError { KindMismatch, LogicalIdMismatch }`；`pub fn handle_modify_request(previous, previous_persisted_id, request) -> Result<RevisionChainEntry, ModifyRequestError>` 协调器——校验 request 的 kind/logical_id 与 chain 一致后委托给 `derive_next_revision_skeleton`，fail-closed 防止跨链 id 泄露；8 个单测覆盖 label 稳定性、链位置标志、Clone 合同、`derive_next_revision_skeleton` × 2 路径、`handle_modify_request` happy path + KindMismatch + LogicalIdMismatch 两种失败模式 | ✅ 已落地（revision-chain 协调层完整） |
