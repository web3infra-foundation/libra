...
| ID | 目标 | 依赖 | Read first | Write set | Verification | 完成判定 |
|----|------|------|------------|-----------|--------------|----------|
| CEX-S2-10 | Agent contracts / event schema（不启动 sub-agent） | CP-4、CEX-S2-00、CEX-S2-01、CEX-00.5 | `src/internal/ai/runtime/contracts.rs`、`src/internal/ai/runtime/event.rs`、`src/internal/ai/runtime/snapshot.rs`、`src/internal/ai/intentspec/`、`src/internal/ai/orchestrator/types.rs` | `src/internal/ai/runtime/contracts.rs`、`src/internal/ai/agent_run/{mod.rs,task.rs,run.rs,event.rs,patchset.rs,evidence.rs,decision.rs,permission.rs,budget.rs,context_pack.rs}`、`src/internal/ai/runtime/mod.rs`、`tests/ai_subagent_contract_test.rs` | `cargo test ai_subagent_contract`；`cargo test ai_runtime_contract`；`cargo test --all`（确保 flag-off 不变） | (1) `AgentTask` / `AgentRun` / `AgentPatchSet` / `AgentEvidence` 引用现有 `IntentSpec` / `Plan` / `Task` / `Evidence`，不复制业务字段。(2) 所有新 event 实现 CEX-00.5 的 `Event` trait，含 `agent_run_id` 字段。(3) JSONL `agents/{run_id}.jsonl` schema 通过 unknown-event-safe 测试（旧 reader 跳过 future event）。(4) `AgentEvidence` 含 `source_event_id` / `tool_call_id` / `source_call_id` / `confidence` / `applies_to_scope` / `distillable` 字段以满足 S2-INV-12 与 Step 2.8 接入点。(5) `AgentRunEvent::PreToolUse` / `PostToolUse` schema 字段就位以满足 S2-INV-13 (尚未挂 dispatch)。(6) `cargo test --all` 在 sub-agent flag-off 下与 `48ea0ae`（CEX-00 commit）输出一致 |
| CEX-S2-11 | Isolated workspace abstraction（worktree / sparse / blocked） | CEX-S2-10、CEX-S2-02 | `src/internal/ai/orchestrator/workspace.rs`、`src/command/worktree.rs`、`src/command/worktree-fuse.rs`、`tests/command/worktree_test.rs`、git-internal worktree API | `src/internal/ai/orchestrator/workspace.rs`、`src/internal/ai/agent_run/workspace_strategy.rs`、`src/command/worktree.rs`、`tests/ai_subagent_workspace_test.rs` | `cargo test ai_subagent_workspace`；`cargo test command::worktree_test`；100K-file fixture 物化 < 30s | (1) 三种策略可测：`Worktree`（≤1GB / <100K files）、`Sparse`（>100K files 或 ≥1GB）、`Blocked`（写入越出 sparse scope）。(2) `Full copy fallback` 仅在显式 `agent.allow_full_copy = true` 时启用，并在 audit log 写 warning。(3) 每次 workspace 创建写 `AgentRunEvent::WorkspaceMaterialized` 含 `strategy` / `elapsed_ms` / `materialized_file_count` / `source_repo_size` / `fallback_reason`。(4) 越界路径访问返回 user-friendly error（mentions extending `AgentContextPack` scope）。(5) cleanup 在 `AgentRun` drop / kill -9 后均能恢复（leak-free） |
| CEX-S2-12 | Single sub-agent behind flag（flag-gated runtime） | CEX-S2-11 | `src/command/code.rs`、`src/internal/ai/agent/runtime/tool_loop.rs`、`src/internal/ai/projection/scheduler.rs`、`src/internal/ai/orchestrator/executor.rs`、`src/internal/ai/sandbox/mod.rs`、`tests/ai_agent_baseline_test.rs`（flag-off 回归参照） | `src/command/code.rs`、`src/internal/ai/agent/runtime/sub_agent_loop.rs`、`src/internal/ai/projection/scheduler.rs`、`src/internal/ai/orchestrator/executor.rs`、`src/internal/ai/agent_run/dispatcher.rs`、`tests/ai_subagent_single_test.rs`、`tests/ai_subagent_flag_off_regression_test.rs` | `cargo test ai_subagent_single`；`cargo test ai_subagent_flag_off_regression`（必须等价于 CEX-00 baseline 7 测试）；`step1-regression` CI check | (1) `code.sub_agents.enabled = false`（默认）下 `cargo test --all` 与 CEX-00 commit 一致，单 Agent path 字节级不变。(2) `enabled = true` + `max_sub_agents = 1` 下，主 Agent 可委派 explorer / worker / reviewer 三种 agent type，每种 type 必须显式声明 `AgentPermissionProfile`（默认 deny）。(3) sub-agent 失败、超时、取消、`AgentBudget` 超限分别有 1 条 fixture，`AgentRunEvent` 写入对应 lifecycle event。(4) sub-agent 不能批准自己的 approval（S2-INV-06）；尝试时返回结构化错误。(5) flag-on 时所有 Step 1 hardening（黑名单 / Approval TTL / allowed_tools）仍生效，不被 sub-agent path 绕过 |
| CEX-S2-13 | Human-gated merge candidate（内部 PR） | CEX-S2-12 | `src/internal/ai/runtime/phase3.rs`、`src/internal/ai/runtime/phase4.rs`、`src/internal/ai/orchestrator/verifier.rs`、`src/internal/tui/app.rs`、`src/internal/tui/bottom_pane.rs`、`src/internal/ai/sandbox/mod.rs` | `src/internal/ai/runtime/phase3.rs`、`src/internal/ai/runtime/phase4.rs`、`src/internal/ai/agent_run/merge_candidate.rs`、`src/internal/ai/agent_run/decision.rs`、`src/internal/tui/app.rs`、`src/internal/tui/merge_review.rs`、`tests/ai_subagent_merge_test.rs` | `cargo test ai_subagent_merge`；`cargo test ai_code_ui_projection`；TUI 手动验证 `/agent merge <run_id>` 流 | (1) 所有 `MergeCandidate` 默认 `review_state = needs-human-review`，主 worktree 在用户确认前 0 字节变化。(2) Patch 冲突（重叠 hunk / 同 symbol / 测试 + lockfile 交叉修改）显式标 `conflict`，不自动合并。(3) `MergeDecision` 写入 `Decision[E]`，含 risk score、conflict 列表、test evidence、`distillable_evidence_ids`。(4) Reviewer sub-agent 仅产 `AgentEvidence`（read-only），不能直接 `accept`。(5) auto-merge 必须由独立 feature flag 控制，不在本任务卡范围 |
| CEX-S2-14 | Controlled parallel execution + observability | CEX-S2-13 | `src/internal/ai/projection/scheduler.rs`、`src/internal/ai/orchestrator/decider.rs`、`src/internal/tui/app.rs`、`src/internal/tui/bottom_pane.rs`、`src/internal/ai/tools/handlers/mcp_bridge.rs`、`src/internal/ai/usage/`（CEX-16 输出） | `src/internal/ai/projection/scheduler.rs`、`src/internal/ai/agent_run/parallel.rs`、`src/internal/tui/agent_pane.rs`、`src/internal/ai/tools/handlers/mcp_bridge.rs`、`tests/ai_subagent_parallel_test.rs` | `cargo test ai_subagent_parallel`；`cargo test ai_code_ui_projection`；100 并发 Source-Pool 调用压测脚本 | (1) Disjoint file scope（不同 path tree）的两个 Worker 可并行，wall-clock < 1.5×（取最长者）。(2) 同 file scope 进 conflict 队列，串行化或拒绝并提示用户分裂任务。(3) `code.sub_agents.max_parallel = 2`（默认）；超限时新任务进入 `queued` 不立即 spawn。(4) Source Pool 按 source slug 限流（默认 4 并发，可配），超限时排队。(5) trace id 链：`thread_id → agent_run_id → tool_call_id → source_call_id` 在 JSONL 与 `agent_usage_stats` 中均可关联 |
| CEX-S2-15 | Merge / Validation pipeline + risk score（Step 2.5 全量落地） | CEX-S2-13、CEX-S2-14、CEX-15（automation event source） | `src/internal/ai/runtime/phase3.rs`、`src/internal/ai/runtime/phase4.rs`、`src/internal/ai/orchestrator/verifier.rs`、`src/internal/ai/orchestrator/policy.rs`、`src/internal/ai/agent_run/merge_candidate.rs`（CEX-S2-13 输出） | `src/internal/ai/runtime/phase4.rs`、`src/internal/ai/agent_run/risk_score.rs`、`src/internal/ai/agent_run/validator.rs`、`src/internal/ai/orchestrator/verifier.rs`、`tests/ai_subagent_validation_test.rs` | `cargo test ai_subagent_validation`；`cargo test ai_runtime_phase4`；fixture 含 5 条 risk-score corpus | (1) Risk score 输入：sub-agent 数量、conflict 次数、failed run 次数、unverified patch scope、`AgentBudget` 超限次数；输出 `RiskScore { level, factors }`。(2) Phase 3 ValidatorEngine 触发 test DAG / verification commands；失败时路由回具体 `AgentTask`（不向上 escalate 到 Phase 4）。(3) Reviewer sub-agent 产生的 evidence 必须显示在 `MergeCandidate.review_evidence` 但不替代 Layer 1 decision。(4) auto-merge feature flag `code.sub_agents.auto_merge.enabled = false` 默认；启用前必须满足 30 天 conflict_rate < 5% / rollback_rate < 1% 指标 fixture（在测试中以 mock 数据满足） |
| CEX-S2-16 | UI / MCP observability for sub-agent runs（Step 2.6 全量） | CEX-S2-12、CEX-S2-15、CEX-16（usage TUI L1/L2/L3） | `src/internal/tui/app.rs`、`src/internal/tui/bottom_pane.rs`、`src/internal/tui/chatwidget.rs`、`src/internal/tui/slash_command.rs`、`src/internal/ai/mcp/`、`src/internal/ai/projection/scheduler.rs` | `src/internal/tui/agent_pane.rs`（CEX-S2-14 已建）、`src/internal/tui/slash_command.rs`、`src/internal/ai/mcp/resources/agents.rs`、`src/internal/ai/mcp/server.rs`、`tests/ai_subagent_ui_test.rs`、`tests/ai_subagent_mcp_test.rs` | `cargo test ai_subagent_ui`；`cargo test ai_subagent_mcp`；`cargo test mcp_integration`；TUI 手动跑 `libra code`，触发 `/agents` / `/agent cancel <id>` | (1) TUI agent pane 列出 queued / running / blocked / completed / failed 五种状态；显示 current tool / current file / elapsed / token usage / budget remaining / cost estimate / source calls / transcript path / context pack hash / permission profile。(2) `/agents` slash 命令打开 agent pane，`/agent cancel <id>` 取消单个 run（不杀主 session）。(3) MCP resources：`libra://agents/runs`（list）、`libra://agents/runs/{id}`（detail）、`libra://agents/runs/{id}/permissions`、`libra://agents/runs/{id}/budget`、`libra://agents/runs/{id}/context`、`libra://agents/merge-candidates/{id}`。(4) Approval prompt 显示 `agent_id` / `task_id` / `command` / `scope`（与 CEX-11 Approval TTL UI 对齐）。(5) replay：删除 `.libra/cache/` 后从 JSONL 重建 agent pane 状态完全一致（snapshot 测试） |
| CEX-S2-17 | Capability Package / Plugin Trust（Step 2.7 全量） | CEX-S2-12、CEX-14（Source Pool）、CEX-15（automation） | `src/internal/ai/sources/`（CEX-14 输出）、`src/internal/ai/automation/`（CEX-15 输出）、`src/internal/ai/agent/profile/`、`src/internal/ai/commands/`、`src/internal/ai/sandbox/mod.rs` | `src/internal/ai/capability_package/{mod.rs,manifest.rs,checksum.rs,installer.rs,permission_diff.rs}`、`src/command/package.rs`、`src/cli.rs`、`tests/ai_capability_package_test.rs` | `cargo test ai_capability_package`；`libra package install <local-fixture>` / `libra package list` / `libra package diff <id>` smoke | (1) Manifest schema 含 `package_id` / `version` / `publisher` / `checksum` / `bundled.{skills,commands,sources,sub_agents}` / `requested_permissions` / `install_warnings`。(2) `libra package install` 显示 capability diff（新增 tool / source / agent type / hook event / network domain / credential ref）；用户必须显式确认才注册。(3) 默认 deny: package 内 mutating source / Worker sub-agent 必须由项目配置或用户二次确认才能启用。(4) 更新（不同 checksum）必须重新计算 permission diff 并要求确认。(5) 卸载时 tool definitions / source connections / agent definitions 均被清理（`/sources reload` 后空）。(6) Package 内 sub-agent definition 强制走 Step 2.2 `AgentPermissionProfile`，不能 frontmatter 继承 mutating tools |
| CEX-S2-18 | Evidence → Memory Distillation 接入点（Step 2.8 read-only） | CEX-S2-10、CEX-S2-15、CEX-13c（MemoryAnchor lifecycle） | `src/internal/ai/agent_run/evidence.rs`（CEX-S2-10 输出）、`src/internal/ai/context_budget/memory_anchor.rs`（CEX-13c 输出）、`src/internal/ai/runtime/contracts.rs` | `src/internal/ai/agent_run/evidence_query.rs`、`src/internal/ai/runtime/contracts.rs`、`src/internal/ai/agent_run/decision.rs`、`tests/ai_subagent_evidence_query_test.rs` | `cargo test ai_subagent_evidence_query`；`cargo test ai_memory_anchor`（CEX-13c 联动） | (1) `evidence_query_by_scope(scope)` / `evidence_stream(filter)` / `merge_decision_distillable_evidence(decision_id)` 三个 read-only API 实现，单元测试覆盖。(2) `MergeDecision` 写入时附带 `distillable_evidence_ids: Vec<EvidenceId>`，可由 projection 读出。(3) sub-agent flag 关闭时 `agent_run/evidence_query.rs` 不出现在主 session 持久化中（CEX-S2-12 flag-off 回归覆盖）。(4) 字段 stable 性测试：在 evidence schema 上加新字段 → 旧 reader 跳过；删除字段 → 测试 fail（防止破坏 Step 3.D 兼容性）。(5) **不实现** distillation pipeline / AI 摘要 / 跨 session 自动加载，仅 read-only API + 字段冻结 |
...
| CP-S2-2 Contracts gate | CP-4 + CEX-S2-10 + CEX-S2-11 完成 | `cargo +nightly fmt --all`、`cargo clippy --all-targets --all-features -- -D warnings`、`cargo test ai_subagent_contract`、`cargo test ai_subagent_workspace`、`cargo test --all`（flag-off 必须等价 CEX-00 baseline 7 测试） | 通过后才能开 sub-agent runtime（CEX-S2-12）；flag-off 字节级回归是硬要求 |
| CP-S2-3 Single sub-agent gate | CEX-S2-12 + CEX-S2-13 完成 | `cargo test ai_subagent_single`、`cargo test ai_subagent_flag_off_regression`、`cargo test ai_subagent_merge`、`step1-regression` CI check；TUI 手动验证 explorer / worker / reviewer 三种 agent type 各跑一次 | 通过后才能开 controlled parallel（CEX-S2-14）；human-gated merge 必须默认开启 |
| CP-S2-4 Parallel gate | CEX-S2-14 + CEX-S2-15 完成 | `cargo test ai_subagent_parallel`、`cargo test ai_subagent_validation`、`cargo test ai_runtime_phase4`、`cargo test ai_code_ui_projection`；并发压测脚本 100 source-pool calls 不超时 | 通过后才能进入 ecosystem CEX-S2-16 / 17 / 18；auto-merge feature flag 仍默认关 |
| CP-S2-5 Step 2 exit gate | CEX-S2-16 / 17 / 18 全部完成 | 全量 `cargo test --all`、`cargo test mcp_integration`、`cargo test ai_capability_package`、`cargo test ai_subagent_evidence_query`；TUI 手动跑 `/agents` / `/agent cancel` / `/usage`；MCP `libra://agents/*` 五个 resource 在 stdio mode 可读 | 通过后 Step 2 出口标准达成；为 Step 3 候选解锁 |
...
1. **Acceptance criterion 必须直接被测试，不能通过“相关测试”近似覆盖。**
   ...
   - “MergeCandidate 默认 needs-human-review” → 必须有测试模拟用户拒绝 → 主 worktree 不变，**不能只测 review_state 字段值**
...
3. **CONTRACT / INVARIANT 注释挂在易变断言上**，标明跨 CEX 的耦合点。CEX-00 在 `ReadFileHandler` 的 `L<n>:` 前缀、`ApplyPatchHandler` 的文件名 echo、`allowed_tools` 的错误字符串、Mutex unwrap 处都加了注释。Step 2 等价位置：
   ...
   - `MergeCandidate.review_state` 默认值 → `// INVARIANT: must default to needs-human-review per S2-INV-07; auto-merge is a separate flag`
   - hook fail-closed 行为 → `// INVARIANT: hook panic / timeout / unknown exit code => deny per S2-INV-13`
...
6. **`code.sub_agents.enabled = false` 默认下不允许有任何字节级 diff。**
   CEX-S2-12 的核心验收是“flag-off 与 CEX-00 commit 字节级一致”。这意味着 sub-agent 相关的 schema 写入、JSONL 文件创建、TUI 渲染全部必须 lazy / flag-gated。任何在 flag-off 时仍然 emit 的代码路径都视为破坏 S2-INV-08。CEX-S2-18 的 evidence_query API 之所以是 read-only，也是为了不破坏 flag-off 不变量。
...
|------|------|---------------|----------------|
| 兼容性 | sub-agent flag 关闭时，Step 1 单 Agent 行为和测试保持不变（与 CEX-00 commit `48ea0ae` 字节级一致） | CEX-S2-12（flag-off regression） | CP-S2-2 / CP-S2-3 |
...
| 安全 | sub-agent 不能绕过 Step 1.1 黑名单、approval TTL 和 allowed_tools；hook fail-closed | CEX-S2-10（PreToolUse / PostToolUse schema）、CEX-S2-12（flag-on hardening） | CP-S2-3 |
