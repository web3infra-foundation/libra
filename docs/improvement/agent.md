...
| Part B Implementation Phase 5 | Security / Permission / Diagnostics / Testing Hardening | 进行中：`AuditSink` trait 已落地（`hardening.rs:333`），含 `record_decision` / `record_event` 语义化入口；**from_env → resolve_env 改造已完成（v0.17.549..v0.17.553）**：7 个 provider 全部提供 vault-aware `Client::from_resolved_env(LocalIdentityTarget<'_>)` 异步构造器（deepseek / kimi 在更早 patch 已就位；gemini / openai / anthropic / zhipu / ollama 由本批次补齐）。剩余 Phase 5 收尾项：把 `libra code` bootstrap / TUI provider 启动路径从 `from_env()` 切到 `from_resolved_env()`，让 `libra config --global add vault.env.<PROVIDER>_API_KEY <…>` 配置真正生效 |
