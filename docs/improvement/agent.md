...
| `AgentRunEvent` 持久化 | `session/store.rs:29` 当前为 JSON blob；`hooks/lifecycle.rs:74` 是 in-memory 结构（`LifecycleEvent`） | Step 1.8 JSONL append + lock；Step 1.9 CompactionEvent envelope | JSONL header / attachment / replay 语义未定，sub-agent run 直接生成 JSONL 会与未来 schema 冲突 |
