...
|------|------|---------------------|
| Workspace（worktree） | `src/command/worktree.rs:43` `WorktreeSubcommand` 含 9 个 variant：`Add` / `List` / `Lock` / `Unlock` / `Move` / `Prune` / `Remove` / `Umount`(`#[cfg(unix)]`) / `Repair`；`worktree-fuse.rs` 提供 FUSE-based provision；`orchestrator/workspace.rs:120` `FuseProvisionState` + `:160` `FuseAttemptOutcome` 描述 fallback | 缺三档物化策略（Worktree / Sparse / Blocked）的 dispatch 表与 100K-file fixture；缺 `AgentRunEvent::workspace_materialized` 写入 hook |
