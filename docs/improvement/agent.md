...
| `src/internal/ai/runtime/phase1.rs` | Phase 1 Plan 的 formal write helper（`write_plan_set` / `advance_scheduler`） | **`write_plan_set` + `apply_scheduler_mutation` 函数体已落地，7/8 mutation variants 已 wire（v0.17.580 write_plan_set, v0.17.588 apply_scheduler_mutation MarkTaskActive/ClearActiveRun, v0.17.589 batch SetCurrentPlanHeads/SelectPlanSet/StartStage/MarkProjectionStale/ApplyRebuild）**：`pub async fn write_plan_set(...)` 通过 `orchestrator::persistence::write_plan_set_with_outcome` 桥接到真实 plan-set 持久化。`pub fn apply_scheduler_mutation(current, mutation) -> Result<SchedulerState, ApplySchedulerMutationError>` 是 `advance_scheduler` 的 pure-function 核心：7 个 variant 已 wire（MarkTaskActive / ClearActiveRun 处理 active task/run，SetCurrentPlanHeads / SelectPlanSet 处理 plan-head 列表，StartStage / MarkProjectionStale / ApplyRebuild 通过 metadata.stage / metadata.stale_reason / metadata.rebuild_versions 管理 projection freshness）；只剩 SeedThread 一个 variant 返回 `VariantNotWired { variant: "SeedThread" }`（需 Phase0Bundle 解构 + thread 初始化路径，独立 follow-up）；11 个单测覆盖 7 个 wired variant happy path + version mismatch fail-closed + 1 个 SeedThread 自识别错误 | 低（剩余 SeedThread 是单一 follow-up，需要 thread 初始化路径协调） |
| `src/internal/ai/runtime/phase2.rs` | Phase 2 Execution 的 formal write helper（attempt 生命周期 + formal writes） | **Schema 已落地（v0.17.576）**：模块存在，提供 `AttemptWriteOutcome { task_id, run_id, status, summary }` 类型 + `is_failure()` helper（`status != Completed`）配合 Phase 3 routing；`write_attempt_start` / `write_attempt_finish` 函数体仍待 Wave 1B 把 `orchestrator::persistence::ExecutionAuditSession` 的 channel-based runtime audit lift 到本模块；2 个单测覆盖 outcome clone + Completed-only 失败判断 | 高（Wave 1B 阻塞项） |
