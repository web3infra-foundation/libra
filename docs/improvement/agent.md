...
- [ ] 统一事件流：hooks lifecycle + VCS events（v0.17.82 已新增 `AutomationRuntimeEvent` 与 scheduler runtime dispatch core；hook/VCS producer 到 automation history 的进程内接线仍未完成）。
...
- [ ] full TUI 仍待补齐：v0.17.83 已新增 `/usage` SQLite-backed detail popup、UsageUpdated 刷新和 render snapshot；v0.17.84 已新增 `[usage].tui_detail_popup` 开关；streaming token incremental refresh 仍未完成。
