...
| `src/internal/ai/runtime/phase1.rs` | Phase 1 Plan 的 formal write helper（`write_plan_set` / `advance_scheduler`） | **`write_plan_set` + `apply_scheduler_mutation` 函数体已落地（v0.17.580 write_plan_set, v0.17.588 apply_scheduler_mutation MarkTaskActive/ClearActiveRun + VariantNotWired error）**：`pub async fn write_plan_set(mcp_server, intent_id, parent_exec, parent_test, plan) -> Result<PlanWriteOutcome, OrchestratorError>` 通过 `orchestrator::persistence::write_plan_set_with_outcome(...)` 桥接到真实 plan-set 持久化（`create_plan_revision_for_role` × 2 + `build_plan_set`）。`pub fn apply_scheduler_mutation(current: &SchedulerState, mutation: SchedulerMutation) -> Result<SchedulerState, ApplySchedulerMutationError>` 是 `advance_scheduler` 的 pure-function 核心：MarkTaskActive + ClearActiveRun 两个 variant 已 wire（version bump、`active_task_id` / `active_run_id` 更新、`updated_at` 刷新）；其他 6 个 variant 返回 `VariantNotWired { variant }` 自识别错误；6 个单测覆盖 happy path × 2 + version mismatch fail-closed + unwired variant 自识别 | 低（剩余 6 个 mutation variants 的 wiring 是 mechanical follow-up） |
