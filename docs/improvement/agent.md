...
| **Query Index / Rebuild 合同部分落地** | 高 | v0.17.1113 起 `ProjectionResolver::{load_query_indexes, load_or_rebuild_query_indexes}` 返回 `ThreadQueryIndexes { freshness, ... }`，并复用 targeted rebuild / `StaleReadOnly` / `Unavailable` 降级合同；v0.17.1127 新增 `ThreadQueryIndexes::diagnostics`，对 scheduler 引用的 plan/task/run 缺失 denormalized index row 以及 targeted rebuild failure 给出结构化诊断。剩余项收窄为 formal object 存在性 / Snapshot 全量一致性深查 |
