...
| Step 1.10 | M5.1 / M5.2 | Source Pool 与 Automation | Source Pool Phase A + Automation MVP core 已完成（2026-05-03：`Source` / `CapabilityManifest` / `SourcePool`、`McpSource`、legacy MCP shim、source-prefixed naming、trust-tier enablement、session namespace/call-record core；Automation TOML rules、cron simulation、prompt/webhook/shell actions、shell safety preflight、failure isolation、`automation_log` migration + CLI `automation list|run|history` 均已落地）；OpenAPI fixture tool generation、`/source` hot reload、旧 MCP config migration 和 runtime VCS event-source wiring 已补齐 |
| Step 1.11 | — | 模型用量与耗时统计（SQLite 聚合，按 provider / model 区分） | 核心+schema follow-up 已完成（2026-05-03：`agent_usage_stats` built-in migration、`UsageRecorder`、provider/model aggregate query、`libra usage report --by=model` with `--since` / `--until` / `--session` / `--thread` / `--include-failed` / `--format`、`libra usage prune --retention-days`、tool-loop 调用耗时写入、missing/failure rows、cached/reasoning/total/tool-call 聚合字段、OpenAI/Anthropic/Gemini usage metadata 映射、TUI usage badge formatter、chat-area header usage badge、compact bottom-pane usage line和 built-in `/usage` transcript view 已落地）；pricing table / retention config 已收口，v0.17.83 补齐 `/usage` SQLite-backed detail popup core，v0.17.84 补齐 `[usage].tui_detail_popup` 开关，v0.17.85 补齐 streaming text delta 的实时 output-token 估算刷新并在最终 usage 到达时替换为 provider 真值 |
| Step 2 架构基线 | CEX-S2-00 / 01 / 02 | 不变量、readiness matrix、依赖图（不启动 runtime） | 已完成（audit closure 落于 "Step 2 audit closure" 节，2026-05-02） |
| Step 2.1 | CEX-S2-10 | Agent contracts / event schema | **Schema 落地完整 + runtime wiring 大部分已落地**：`src/internal/ai/agent_run/` 下 `budget.rs` / `context_pack.rs` / `decision.rs` / `event.rs` / `evidence.rs` / `patchset.rs` / `permission.rs` / `run.rs` / `task.rs` 全部存在；**v0.17.737..v0.17.788 把 dispatcher + flag-on 执行路径 + Scheduler 调用入口同步落地**（详见 [Runtime / formal objects 行](#当前单-agent-基线情况)），仅 merge pipeline（CEX-S2-13）仍未启动 |
| Step 2.2 | CEX-S2-11 | Isolated workspace abstraction | **已落地（v0.17.737..v0.17.788 后续收口）**：`src/internal/ai/orchestrator/workspace.rs` 提供 `TaskWorktree` / `MaterializedSubAgentWorkspace` / `materialize_sub_agent_workspace`，并在 `AgentRunEvent::WorkspaceMaterialized` 中记录 sizing、preferred/final strategy 与 fallback reason；`cleanup_task_worktree` / `WorkspaceCleanupGuard` 覆盖 copy/FUSE teardown |
| Step 2.3 | CEX-S2-12 | Single sub-agent behind flag | **已落地（v0.17.737..v0.17.788，workspace wiring 后续补齐）**：`code.sub_agents.enabled = true` 时 `DefaultSubAgentDispatcher` + `DefaultSubAgentChildRunner` 走完整 OC-Phase 3 路径，所有 9 个 gates + 5 个终止 event 类型 + 3 个 doc-named E2E fixture + hook dispatch（v0.17.806/.807 S2-INV-13）+ 异步 cancel（v0.17.767）+ per-agent budget（v0.17.769）+ context handoff（v0.17.773/.785）均已落地；`src/command/code.rs::build_subagent_runtime_for_session` 现在通过 `.with_workspace_isolation(WorkspaceIsolationConfig { ... })` 把 dispatcher 接到 CEX-S2-11 workspace materialization，mutating child tools 被重定向到 isolated workspace 而非主 worktree |
...
| Part B Implementation Phase 1 / Wave 1B | Codex `CodexTaskExecutor` + `CompletionTaskExecutor<M>` 收敛 | **trait + impl body 已推进（v0.17.579 schema，v0.17.1158 Codex adapter body）**：`pub trait TaskExecutor` 在 `runtime/contracts.rs:547` 定义；`impl TaskExecutor for CodexTaskExecutor` 和 `impl<M: CompletionModel + Send + Sync + 'static> TaskExecutor for CompletionTaskExecutor<M>` 在 `runtime/task_executors.rs` 落地。Codex path 不再是 schema-only stub：配置 `CodeUiProviderAdapter` 后会提交 Runtime prompt、观察 Code UI terminal snapshot、返回 `TaskExecutionResult`，并对 pending interaction fail-closed；未配置 adapter 时返回 `TaskExecutionError::Environment`。Completion path 已有 no-tool single-shot body（prompt → `CompletionRequest` → provider → assistant text summary），且当 provider 返回 tool call 时以 `TaskExecutionError::ToolPolicy` fail-closed，避免把未执行工具的 attempt 误报为 `Completed`。剩余 follow-up 是 Phase 2 主循环 / provider callsite cutover 与 full tool-loop / interaction mediation |
...
| Part B Implementation Phase 4 | ArtifactLedger + ValidatorEngine + DecisionProposal | 部分完成：`runtime/phase4.rs` 已存在并提供 `DecisionProposal`（`:115`）+ `DecisionProposalSummary`（`:105`）+ `DecisionProposalStore`（`:230`）；`ArtifactLedger`（`runtime/phase3.rs:20`）与 `ValidatorEngine`（`runtime/phase3.rs:138`）实际定义在 phase3，仍归 planner gate 路径 |
| Part B Implementation Phase 5 | Security / Permission / Diagnostics / Testing Hardening | 进行中：`AuditSink` trait 已落地（`hardening.rs:451`），含 `record_decision` / `record_event` 语义化入口；**from_env → resolve_env 改造已完成（v0.17.549..v0.17.556）**：API 表面 7 个 provider 全部提供 vault-aware `Client::from_resolved_env(LocalIdentityTarget<'_>)` 异步构造器（deepseek / kimi 在更早 patch 已就位；gemini / openai / anthropic / zhipu / ollama 由 v0.17.549..v0.17.553 补齐）；call-site cutover 在 v0.17.556 完成 — `src/internal/config.rs` 暴露 `pub fn resolve_env_sync(name)`，`src/command/code.rs::build_any_completion_model_for_args` 把 lookup closure 从 `std::env::var(key).ok()` 切换到 `resolve_env_sync`，让 `libra config --global add vault.env.<PROVIDER>_API_KEY <…>` 配置真正生效；`from_env()` 公开策略已在 v0.17.1048 收口（0.17 保留兼容 helper，v0.18 release notes 标注 deprecated/migration path）。剩余 Phase 5 收尾项：security-runtime test harness 扩展、durable audit retention 策略 |
| Part C Phase 0 | Contract（v1 HTTP / lease / redaction 表） | 已交付（本文 Part C 即 Phase 0 输出） |
| Part C Phase 1 | Security Envelope（CLI 参数 + token / info / lock 文件 + 校验 helper） | **已落地**：`ControlMode` enum (`code.rs:232`)、`--control` / `--control-token-file` / `--control-info-file` (`code.rs:428` 起的 CodeArgs 字段)、`code_control_files.rs` 模块、`ControlInfo` / `ControlPaths` / `ControlLockGuard` 全部已存在 |
...
| 领域 | 已确认代码事实 | 未完成边界 |
|------|----------------|------------|
| Safety / VCS | `SafetyDecision` / `SafetyDisposition` / `BlastRadius` / `CommandSafetySurface` 已在 `runtime/hardening.rs` 落地；`classify_ai_command_safety()` 已在 `tools/utils.rs` 提供 fixture classifier；`classify_run_libra_vcs_safety()` 已在 `libra_vcs.rs` 做参数级 allow / needs-human / deny，MCP `run_libra_vcs_impl()` 会在 spawn 前返回结构化 safety error；`ShellHandler` 已在 spawn 前消费 shell `SafetyDecision`，拒绝 destructive / wrapped bypass / workspace 外 mutating target，并把 needs-human shell 命令接入 sandbox approval reason；**protected branch 默认列表 + 配置已落地**：`ApprovalCachePolicy.protected_branches`（默认 `main` / `master` / `trunk` / `develop` / `release/*`）+ `branch_delete_targets_protected_branch` + `is_protected_branch` 在 `libra_vcs.rs` 已对 `branch -d`、`branch -D`、`push -f` 等场景做参数级拦截；`protected_branch_in_command()` 同时在 sandbox shell command 拒绝路径执行；**Approval TTL / canonical scope 已在 Step 1.6 收口** | denial tracking 的结构化持久化（写入 `audit_log` 表 / `Evidence[E]` 投影）仍是后续 hardening 项，protected-branch 命中点的内存级别已完成 |
| Runtime / formal objects | `Runtime` / `RuntimeConfig`、`TaskExecutor`、`PromptPackage`、`WorkflowPhase`、顶级 `Snapshot` / `Event`、`AuditSink::record_decision` / `record_event` 均存在；`src/internal/ai/agent_run/` 已有 Step 2 schema-only 类型和 `AgentRunEvent`；**sub-agent dispatcher 已落地（v0.17.737..v0.17.788）**：完整 OC-Phase 3 路径（gates + ask + Spawned + child runner + tool loop + Completed/Failed/Cancelled/TimedOut/BudgetExceeded + 3 doc-named E2E）；**flag-on execution 已落地（v0.17.776）**：`build_subagent_runtime_for_session` 在 `code.sub_agents.enabled = true` 时构造 `SubAgentToolLoopRuntime` 并附加到 `config.subagent_runtime`；**workspace isolation 已接线**：`build_subagent_runtime_for_session` 通过 `.with_workspace_isolation(...)` 配置 `sessions_root` / `allow_full_copy`，dispatcher materializes per-run workspace、re-roots child registry、rebases sandbox `writable_roots`，`flag_on_does_not_touch_main_worktree` 覆盖 child `apply_patch` 不触碰主 worktree；**hook dispatch runtime 已落地（v0.17.806/v0.17.807）**：`hook_runner` 字段从 App `ToolLoopConfig` 沿 `SubAgentToolLoopRuntime -> DispatchContext -> DefaultSubAgentChildRunner` 透传给子 `ToolLoopConfig`，子 sub-agent 工具调用同样触发 PreToolUse/PostToolUse hooks（S2-INV-13） | merge pipeline（CEX-S2-13）仍属 Step 2 后续多 PR 范围 |
| Storage / migration | `src/internal/db/migration.rs`、`schema_versions` entity、`sql/migrations/README.md`、`tests/db_migration_test.rs` 已形成 CEX-12.5 基线；fresh / establish 路径都会运行 migration runner；`automation_log` 与 CEX-16 core 的 `agent_usage_stats` 已通过 built-in migration 注册；**长期 retention / cleanup 已落地（v0.17.791）**：`command/usage::auto_prune_at_session_start(storage_root)` 在 session bootstrap 时读取 `[usage] retention_days` 并通过 `UsageRecorder::prune_before` 删除过期行，软失败仅 `tracing::warn!` 不阻塞启动；**usage failure-row backfill 已落地（v0.17.797）**：`cancel_current_turn(CancelSource::*)` 在 abort JoinHandle 前先 `record_failure(context, "cancelled_<esc|quit|automation|budget>", None)`，确保 Esc / SlashQuit / Automation / Budget 中断的 turn 都在 `agent_usage_stats` 表里留下显式失败行；**source telemetry 持久化已端到端落地（v0.17.800..v0.17.804）**：v0.17.800 落迁移 `2026052301` + SeaORM entity（13 字段镜像 `SourceCallRecord` + admin `id`/`created_at`，4 个索引：session/source_slug/tool_call_id/created_at）；v0.17.801 让 `libra db upgrade` 同时升级 `~/.libra/config.db`（消除新增迁移导致所有 libra 命令拒绝执行的 UX 陷阱）；v0.17.803 在 producer 侧加 `SourceCallLog::with_persistence(db)` + `record(...)` 内 `tokio::spawn` SeaORM Insert；v0.17.804 在 session bootstrap 调用 `SourcePool::with_persistence(conn)`，每个 libra-code 会话现在都会把所有 Source/MCP/OpenAPI 工具调用持久化到 `source_call_log` 表 | （本行已收口） |
...
| Provider / JSON / usage | Completion providers 和 hidden fake provider 存在；provider response 里已有 token usage 字段的历史对象路径（如 Codex `RunUsage`）；`completion/json_repair.rs` 已提供 provider-neutral JSON repair core、结构化错误和 fixture gate；OpenAI-compatible 家族（OpenAI / DeepSeek / Kimi / Zhipu）与 Ollama 的 tool-call `arguments` 字符串解析失败时已统一先 repair 再 raw-string fallback，Anthropic / Gemini 的原生结构化 tool input 不需要字符串 repair；CEX-16 core 已新增 `UsageRecorder`、SQLite `agent_usage_stats` 聚合、missing/failure rows、OpenAI/Anthropic/Gemini usage metadata 映射、`libra usage report --by=model` 和 TUI compact bottom-pane usage line；pricing table/config override、cancel/timeout failure kind、`[usage].retention_days` prune config 已接入；`libra code` TUI direct provider bootstrap 与 Ollama headless web-only bootstrap 已通过 `ProviderFactory` 统一解析 API key / base URL / `OLLAMA_COMPACT_TOOLS`，并覆盖 missing-key 与 `--api-base` 回归；**TUI 用量展示已收口（v0.17.1104 复核）**：bottom-pane/header/detail popup、streaming output-token 增量估算、最终 provider usage 替换估算避免双计均有单测覆盖；**vault/env 统一接入已完成（v0.17.549..v0.17.556）**：7 个 provider 全部提供 `Client::from_resolved_env(LocalIdentityTarget<'_>)` 异步构造器，`src/internal/config.rs::resolve_env_sync` 同步入口与 `build_any_completion_model_for_args` 的 closure 已切到 vault-first 路径；v0.17.1048 已公开 `from_env()` 保留/弃用策略 | 本行已收口；后续仅保留真实 provider / nightly 运行门 |

### Part B 模块缺口与 Phase 进度估算（2026-05-02 baseline）

本节补齐"代码基线核对"表未显式列出的具体模块缺口与各 Phase 落地进度，作为 Codex 任务排期与 review checklist 使用。

**已落地的 runtime 子模块**：`mod.rs` / `contracts.rs` / `event.rs` / `snapshot.rs` / `hardening.rs` / `phase0.rs`（v0.17.574 起，`write_intent` formal-write 入口）/ `phase1.rs`（v0.17.575 起，`PlanWriteOutcome` schema + `ordered_plan_ids` helper；v0.17.580 起 `write_plan_set` 桥接 `orchestrator::persistence::write_plan_set_with_outcome`；v0.17.1143 已补真实 MCP/history bridge test）/ `phase2.rs`（v0.17.576 起，`AttemptWriteOutcome` schema + `is_failure` helper；v0.17.1144 起 `write_attempt_start_with_session` / `write_attempt_finish_with_session` 桥接 `ExecutionAuditSession`，写入 per-task Run + TaskEvent / RunEvent / PlanStepEvent lifecycle）/ `revision.rs`（v0.17.577 起，`RevisionKind` / `RevisionChainEntry` schema + `is_first` / `is_continuation` helper；`handle_modify_request` 函数体仍待 Wave 1B 接入）/ `phase3.rs` / `phase4.rs` / `prompt_builders.rs` / `environment.rs` / `derived_records.rs`

**Part B Affected Modules 表中声明应新建但当前不存在**（按合并优先级）：

| 缺失文件 | 在文档中的角色 | 当前状态 | 优先级 |
|---------|---------------|----------|--------|
| `src/internal/ai/runtime/phase0.rs` | Phase 0 Intent 的 formal write helper（`write_intent` / `write_context_snapshot_if_needed`） | **`write_intent` + `write_context_snapshot_if_needed` 全部已落地（v0.17.574 write_intent, v0.17.594 write_context_snapshot_if_needed）**：模块提供 `pub async fn write_intent(spec, mcp_server) -> Result<IntentWriteOutcome>`（委托给 `intentspec::persistence::persist_intentspec`）+ `pub async fn write_context_snapshot_if_needed(request, mcp_server) -> Result<Option<ContextSnapshotWriteOutcome>>`（在 items 与 summary 同时为空时短路返回 `Ok(None)`，否则把 `ContextSnapshotRequest` 翻译成 `CreateContextSnapshotParams` 委托给 `LibraMcpServer::create_context_snapshot_impl`，解析返回 ID 包装成 `ContextSnapshotWriteOutcome { snapshot_id, summary, item_count }`）+ `pub fn snapshot_needed(&request) -> bool` 纯 helper + `ContextSnapshotItem` / `ContextSnapshotRequest` / `ContextSnapshotWriteOutcome` 类型。**10 个单测**覆盖：IntentWriteOutcome × 2（旧）+ snapshot_needed 三种 truthy/falsy 组合（空 → false、仅 items → true、仅 summary → true）+ 空请求短路（async 入口验证 gate）+ ContextSnapshotWriteOutcome Clone/PartialEq + parse_snapshot_id 三种路径（成功提取、缺失 ID 标记、空 ID） | ✅ 已落地（Phase 0 formal-write surface 完整） |
| `src/internal/ai/runtime/phase1.rs` | Phase 1 Plan 的 formal write helper（`write_plan_set` / `advance_scheduler`） | **`write_plan_set` + `apply_scheduler_mutation`（8/8 variants）+ `advance_scheduler` async 包装全部已落地（v0.17.580 write_plan_set, v0.17.588..v0.17.590 apply_scheduler_mutation 8 variant, v0.17.592 advance_scheduler）**：`pub async fn write_plan_set(...)` 通过 `orchestrator::persistence::write_plan_set_with_outcome` 桥接持久化；v0.17.1143 起 `write_plan_set_persists_execution_and_test_plan_pair` 用真实 MCP/history server 覆盖桥接路径，断言 execution/test plan 均落库、task_id → plan_id 路由按 Implementation/Gate 分流。`pub fn apply_scheduler_mutation(current, mutation)` 是 pure-function 核心，所有 8 个 SchedulerMutation variant 已 wire + `SeedThreadMismatch` fail-closed。`pub async fn advance_scheduler(repo, thread_id, mutation) -> Result<SchedulerState, AdvanceSchedulerError>` 是 async 包装：load → apply → CAS-save 完整路径，`AdvanceSchedulerError::{StateMissing, Apply, Cas, Load}` 透明 re-export apply-side + CAS-side 错误，让调用方按需路由；13 个 unit 单测覆盖每个 wired variant + write_plan_set bridge + 3 个 integration test (tests/ai_projection_resolver_test.rs) 覆盖 end-to-end advance + StateMissing + version-mismatch-before-CAS | ✅ 已落地（完整 Phase 1 formal-write surface；剩余 write_plan_set 调用方 cutover 是 orchestrator 内部 refactor） |
| `src/internal/ai/runtime/phase2.rs` | Phase 2 Execution 的 formal write helper（attempt 生命周期 + formal writes） | **Schema + 纯函数体 + stateful bridge 已落地（v0.17.576 schema, v0.17.591 write_attempt_start + write_attempt_finish, v0.17.596 is_terminal, v0.17.1144 ExecutionAuditSession bridge）**：模块存在，提供 `AttemptWriteOutcome { task_id, run_id, status, summary }` 类型 + `is_failure()` (any !Completed including Interrupted) + `is_terminal()` (Completed/Failed/Cancelled/TimedOut，**Interrupted 排除作为 in-flight marker**)、`AttemptStartParams` 输入类型、`pub fn write_attempt_start(params) -> AttemptWriteOutcome` 把 attempt 标记为 `TaskExecutionStatus::Interrupted` (in-flight marker) 同时透传 task_id/run_id/summary、`pub fn write_attempt_finish(task_id, run_id, status, summary) -> AttemptWriteOutcome` 写入终态。v0.17.1144 起新增 `write_attempt_start_with_session(session, task, model_name, summary)` / `write_attempt_finish_with_session(session, task, status, summary)`，委托 `ExecutionAuditSession::{record_attempt_start, record_attempt_finish}` 创建 per-task Run，并将 start/terminal lifecycle 写入 TaskEvent / RunEvent / PlanStepEvent；`phase2_session_bridge_persists_attempt_lifecycle_events` 用真实 MCP/history server 覆盖 run_id identity、Patching→Completed RunEvent、Running→Done TaskEvent、Progressing→Completed PlanStepEvent。剩余 follow-up 是 executor/provider 调用方 cutover 与 PatchSet / Evidence 细分写入，不再是 attempt lifecycle stateful bridge 缺口；9 个 phase2 单测继续覆盖 pure outcome clone + is_failure Completed-only + start 标记为 Interrupted + None summary 透传 + finish Completed 翻转 is_failure + Failed terminal flag + start→finish identity preservation + is_terminal 排除 Interrupted + is_failure/is_terminal 组合分区验证 | 低（剩余是调用方 cutover / artifact 扩展） |
| `src/internal/ai/runtime/revision.rs` | 跨 phase 的 revision chain helper | **Schema + 派生 helper + modify-request 协调器 + plan/test-plan 配对校验 + audit label 已落地（v0.17.577 schema、v0.17.584 derive_next_revision_skeleton、v0.17.587 handle_modify_request、v0.17.595 validate_paired_plan_revisions、v0.17.597 audit_label）**：模块存在，提供 `RevisionKind { Intent, ExecutionPlan, TestPlan }` enum + `.label()` 跨 phase 审计标签 + `RevisionChainEntry { kind, previous_id, revision, logical_id }` 类型 + `is_first()` / `is_continuation()` 链头/续接判断 + `audit_label() -> String` 稳定单行 audit 标签（三种格式：head / continuation from / forked from）；`pub fn derive_next_revision_skeleton(previous, previous_persisted_id) -> RevisionChainEntry` 纯函数；`pub struct ModifyRequest { kind, logical_id, reason }` 修改请求 envelope；`pub enum ModifyRequestError { KindMismatch, LogicalIdMismatch }`；`pub fn handle_modify_request(previous, previous_persisted_id, request)` 协调器；`pub fn validate_paired_plan_revisions(execution, test) -> Result<(), PairingError>` 跨链 lockstep 校验（rule 3：(n)-th plan 必须与 (n)-th test-plan 配对，从不 (n-1) / (n+1)）+ `pub enum PairingError { NotExecutionPlan, NotTestPlan, RevisionMismatch }` 三种失败形态（kind 错误优先级：execution-side 在前，确定性 audit log）；17 个单测覆盖 label 稳定性、链位置标志、Clone 合同、`derive_next_revision_skeleton` × 2 路径、`handle_modify_request` happy path + KindMismatch + LogicalIdMismatch、`validate_paired_plan_revisions` happy path (任意 rev N) + first-link pair (rev 1) + drift (n-1, n+1) × 2 + NotExecutionPlan + NotTestPlan + execution-kind error priority + PairingError Clone/Eq 合同 + `audit_label()` 三种链位置（head / continuation / forked）格式 pin + audit_label 以 kind label 起头 | ✅ 已落地（revision-chain 协调层完整 + lockstep 校验 + audit 标签） |
| `src/internal/ai/mcp/authz.rs` | Phase 5 安全层 `McpAuthorizer` | **Schema + 全面 server wiring + per-request principal threading 已落地（v0.17.573 schema、v0.17.582/583/585 server wiring、v0.17.586 per-request principal）**：trait + `McpOperation` / `AuthzDecision` / `AuthzError` 类型在 `authz.rs` 声明（4 个单测）；`LibraMcpServer` 已新增 `authz` 字段 + `set_authz` + `pub(crate) async fn authorize_or_error(op)` (系统 principal) + `pub(crate) async fn authorize_or_error_with_actor(op, &ActorRef)` (per-actor principal) + `authorize_with_principal_or_error(op, PrincipalContext)` 共享 helper；**13 个 MCP 入口已 wire**：3 个 resource 入口（`list_resources_impl` / `read_resource_impl` / `list_resource_templates`，系统 principal）+ 10 个 tool 入口（5 个 `create_*_impl` 使用 per-actor `authorize_or_error_with_actor` 把 caller 的 `ActorRef` 通过 `PrincipalContext::from_actor` 映射成 principal；5 个 `list_*_impl` 使用系统 principal）。`PrincipalContext::from_actor` 实现完整 5-variant `ActorKind` → `PrincipalRole` 映射 (Human/Agent/McpClient→Contributor, System→System, Other→Observer fail-closed)。8 个单测：4 个 mcp::tests + 2 个 Allow/Deny 验证 + `create_task_impl_threads_actor_into_authz_principal` per-actor 端到端 + `principal_context_from_actor_maps_actor_kinds_to_roles` 全 5-variant 映射 | ✅ 已落地（Phase 5 authz 完成） |
| `impl TaskExecutor for CodexTaskExecutor` 块 | Wave 1B Definition of Done #1 | **Schema + adapter-driven body 已落地（v0.17.579 schema，v0.17.1158 body）**：`src/internal/ai/runtime/task_executors.rs` 含 `pub struct CodexTaskExecutor { adapter, terminal_timeout }` + `impl TaskExecutor for CodexTaskExecutor`；`execute_task_attempt` 在配置 `CodeUiProviderAdapter` 时会提交 prompt、等待 terminal snapshot、保留 task/run id、汇总 assistant transcript，并在 pending interaction / submit error / timeout 时返回结构化错误。3 个单测覆盖未配置 adapter fail、terminal snapshot happy path、pending interaction fail-closed。剩余是生产 Phase 2 调用方把 managed Codex Code UI adapter 注入该 executor，而不是继续旁路 Runtime | 低（调用方 cutover follow-up） |
| `impl<M> TaskExecutor for CompletionTaskExecutor<M>` 块 | Wave 1B Definition of Done #1 | **Schema + 最小 body 已落地（v0.17.579 schema, v0.17.593 单次完成调用 body；v0.17.1106 补 tool-call fail-closed gate）**：同文件含 `pub struct CompletionTaskExecutor<M: CompletionModel> { model: Arc<M> }` + `pub fn new(model)` / `pub fn from_arc(model)` 构造器 + `impl<M: CompletionModel + Send + Sync + 'static> TaskExecutor for CompletionTaskExecutor<M>`；execute_task_attempt body 已 wire：把 `TaskExecutionContext::prompt` (`PromptPackage::preamble` + `.messages`) 翻译成 `CompletionRequest`，调用 `M::completion`，把 assistant text 缝合到 `TaskExecutionResult.summary`，返回 `Completed` 终态；context.run_id 缺失时分配新 Uuid；模型错误映射成 `TaskExecutionError::Provider`；provider 返回 tool call 时映射为 `TaskExecutionError::ToolPolicy`，要求调用方走 full tool-loop executor；8 单测覆盖 happy path + run_id 自动分配 + run_id 透传 + prompt → CompletionRequest 映射（preamble + chat_history 验证）+ 空 prompt 边界 + tool-call fail-closed + 错误映射 + `Box<dyn TaskExecutor>` dyn 兼容；full tool-loop / sandbox / approval 集成仍待 follow-up | 中（剩余 full tool-loop 集成是 cross-cutting follow-up） |

**各 Phase 实施进度估算**（基于 baseline 验证 + 模块覆盖率，**仅供排期参考**，不替代正式审查）：

| Phase | 设计完整度 | 实装进度 | 主要缺口 |
|-------|-----------|---------|----------|
| Phase 0 Contract Stabilization | 100% | ~85% | `write_intent` + `write_context_snapshot_if_needed` Phase 0 formal-write surface 全部落地（v0.17.594）；**Phase0Input struct 已确认不需要**：`write_intent` 直接接受 `(&IntentSpec, &Arc<LibraMcpServer>) -> Result<IntentWriteOutcome>`（`runtime/phase0.rs:67`），等价的 input bundling 由调用方在 IntentSpec 构造时完成，无须中间封装类型 |
| Phase 1 Wave 1A（Runtime contract + dagrs 0.8.1） | 100% | **100%** | — |
| Phase 1 Wave 1B（TaskExecutor impl 收敛） | 100% | **~95%** | `CodexTaskExecutor` 的 schema + adapter-driven `impl TaskExecutor` 已在 `runtime/task_executors.rs` 落地（v0.17.579 schema，v0.17.1158 body：提交 Code UI prompt、等待 terminal snapshot、pending interaction fail-closed）；`CompletionTaskExecutor<M>` schema + 单次完成调用 body 已落地（v0.17.593：把 prompt 翻译成 `CompletionRequest`、stitch assistant text 到 `summary`、错误映射到 `TaskExecutionError::Provider`；v0.17.1106：provider tool-call response fail-closed 为 `TaskExecutionError::ToolPolicy`，避免未执行工具被误报 Completed）；`runtime/phase0.rs::write_intent` 已落地（v0.17.574）；`write_context_snapshot_if_needed` 已落地（v0.17.594，委托给 `LibraMcpServer::create_context_snapshot_impl`）；`runtime/phase1.rs::write_plan_set` 函数体已落地（v0.17.580，桥接到 `create_plan_set_revision`）；`apply_scheduler_mutation` 8/8 variants 已 wire（v0.17.588..v0.17.590）；`advance_scheduler` async 包装已落地（v0.17.592，load→apply→CAS-save 完整路径 + `AdvanceSchedulerError`）；`write_attempt_start` / `write_attempt_finish` pure helper 已落地（v0.17.591），v0.17.1144 起 `write_attempt_start_with_session` / `write_attempt_finish_with_session` 已桥接真实 MCP/history attempt lifecycle；`handle_modify_request` 协调器已落地（v0.17.587）；`validate_paired_plan_revisions` 已落地（v0.17.595，跨链 lockstep 校验 + `PairingError`）；`audit_label()` 已落地（v0.17.597）。剩余 ~5%：生产 Phase 2 主循环 / provider callsite cutover、completion full tool-loop / sandbox / approval 集成，以及 PatchSet/Evidence 细分写入扩展 |
| Phase 1 Wave 1C（claudecode 硬删除） | 100% | **100%** | — |
| Phase 2 Thread ID + Projection Resolver | 100% | ~76% | execution → barrier → test 两阶段语义仍在收口；v0.17.1109 起 `execute_dag()` 会把 dagrs `ExecutionReport` 的 succeeded/failed/skipped/total 计数写回 `RunStateStore`，不再只依赖事件 monitor 的中间 progress；v0.17.1110 起 `ProjectionResolver::load_for_resume()` 会先走 targeted rebuild / freshness 处理，再返回 `phase_at_resume`、`resume_reason`、`resume_actions`；v0.17.1113 起 query index read contract 由 `ThreadQueryIndexes` 暴露并复用 freshness / targeted rebuild 降级语义；`ExecutionTerminated` 仍由 monitor 作为停止信号消费 |
| Phase 3 Code UI Source Of Truth | 100% | ~52% | `CodeUiCommandAdapter` 已存在；非 Codex `--web-only` 已有 headless runtime，不再是 read-only placeholder（v0.17.1111 以 `ai_code_ui_headless_test::initial_snapshot_is_writable_non_placeholder_runtime` pin 住）；完整 unification（shared interaction state / typed delta / gap recovery / IntentSpec workflow）仍是后续 |
| Phase 4 ArtifactLedger + ValidatorEngine + DecisionProposal | 100% | ~80% | **`ValidationReport` 字段在 `runtime/phase3.rs:115` 已完整定义**（`report_id` / `thread_id` / `run_id` / `policy_version` / `stale` / `is_latest` / `summary` / `created_at` / `updated_at`）；**`RiskScoreBreakdown` 字段在 `runtime/phase4.rs:50` 已完整定义**（`breakdown_id` / `thread_id` / `validation_report_id` / `policy_version` / `stale` / `is_latest` / `summary` / `created_at` / `updated_at`）；**持久化层已落地**：`ai_validation_report` 表的 update_many+find via `is_latest` flag + `risk_to_active_model` 写入路径已在 `runtime/phase3.rs:196..225` 和 `runtime/phase4.rs:259..332` 落地；`ai_decision_proposal` / `ai_risk_score_breakdown` 通过同样模式持久化；v0.17.1142 起 `write_latest_with_session_mirror` 会把 `ValidationReport` / `RiskScoreBreakdown` / `DecisionProposal` 同步投影为 session JSONL `ai_artifact`，`SessionJsonlStore::load_ai_artifacts` 提供直接消费路径；`ValidatorEngine` 仍归 planner gate，剩余复核项是 formal final `Decision` write helper 与 release-candidate patchset 消费闭环 |
...
| 步骤 | 目标 | 并发策略 | 状态 |
|------|------|----------|------|
| **Step 1** | 按此前方案补齐单 Agent 的代码理解、弱模型兼容、安全底线、意图识别、可恢复编辑和生态基础 | 保持 orchestrator 串行；不启用 sub-agent 并行 | v1 核心基线已落地；Step 2 runtime 仍需后续 gate |
...
| Source Pool | ✅ 已落地 | 新增 `src/internal/ai/sources/`：`Source` trait、`SourceKind { Mcp, RestApi, LocalDocs }`、`CapabilityManifest`、trust tier、`SourcePool`、source-prefixed tool naming、per-session state namespace 和 call record core；`McpSource` 已复用旧 MCP tool schema / dispatch，legacy `McpBridgeHandler` 保持旧工具名兼容；v0.17.88 已将旧 `[mcp]` project config 映射为内建 MCP source view 并在加载时输出 deprecation warning。REST OpenAPI fixture tool generation 已由 `openapi_tool_capabilities_from_fixture()` + `tests/ai_source_pool_test.rs::openapi_fixture_generates_rest_tool_specs` 覆盖；`/source list|enable|disable|reload` 已作为 TUI built-in 控制面接入；source telemetry 持久化已由 `SourceCallLog::with_persistence` + `source_call_log` migration 接入并有单测覆盖 |
| Automation | ✅ MVP core 已落地 | 新增 `src/internal/ai/automation/`：`.libra/automations.toml` parser、cron simulation、prompt/webhook/shell action executor、shell safety preflight、failure isolation、`automation_log` migration/history 和 CLI `automation list|run|history`；v0.17.86 已把 hook lifecycle `session_start` / `session_end` 接入 automation history；v0.17.89 已把 `post_add` / `post_commit` / `post_push` / `post_branch` / `post_switch` VCS events 接入 automation history |
...
| 模型用量统计 | ✅ 已落地 | `UsageRecorder` 在 tool-loop provider 调用后记录 prompt/completion/cached/reasoning/total token、tool-call count、wall-clock、provider/model 和可选 cost；provider 缺 usage 写 estimated row，provider error 写 failure row；`UsageQuery` / `libra usage report --by=model` 可按 `(provider, model)` 聚合；TUI compact bottom-pane usage line、chat header、`/usage` SQLite detail popup、render snapshot、`[usage].tui_detail_popup` 和 streaming output-token 增量估算均已接入；最终 provider usage 到达时替换估算避免双计 |
| 持久化功能表 | ✅ 已落地 | migration runner 已有，JSONL session 文件真相源已落地；`ContextFrame` / `CompactionEvent` 已接入 session JSONL 和 attachments；`automation_log`、CEX-16 core `agent_usage_stats` 与 `source_call_log` 均已通过 built-in migration 接入；usage retention（`libra usage prune --retention-days` + `[usage].retention_days` 配置默认值）和 failure backfill（`UsageRecorder::record_failure` 写 provider-error 行 + `record_missing_usage` 写 estimated 行）也已落地（详见 CEX-16 行）。本行剩余项已收口；后续新增持久化对象继续走 migration runner + entity + bootstrap 三件套 |
...
| `src/internal/ai/completion/`、`src/internal/ai/providers/{...}/completion.rs`、`src/internal/db.rs`、`src/internal/model/`、`src/internal/ai/usage/` | provider request / response、token usage 字段、Libra SQLite migration runner；CEX-16 core 已通过 `UsageRecorder` 在 tool-loop 调用后持久化 prompt/completion/cached/reasoning/total token、tool-call count、wall-clock、provider/model 和可选 cost；missing usage / provider error 会写入 estimated/failure row；`UsageQuery` / `libra usage report --by=model` 聚合；pricing table/config override、retention config 和 CLI filters 已接入 | Step 1.11 follow-up 补齐 TUI L1/L3 与 streaming 增量展示，并与 Step 2 `RunUsage[E]` / `agent_run_id` 归因严格对齐 |
...
| CEX-15 | Automation MVP（已完成 core，2026-05-03；hook lifecycle history bridge v0.17.86；VCS event-source history bridge v0.17.89） | CEX-11、CEX-13c、CEX-14、CEX-12.5 | `src/internal/ai/hooks/`、`src/internal/db/migration.rs`、`src/command/mod.rs` | `src/internal/ai/automation/{mod.rs,config.rs,events.rs,scheduler.rs,executor.rs,history.rs,runtime.rs}`、`src/internal/ai/mod.rs`、`src/command/automation.rs`、`src/command/mod.rs`、`tests/ai_automation_test.rs` | `cargo test --test ai_automation_test` | TOML rules、cron 模拟、webhook / shell / prompt action、history log 可测；Shell action 复用 Step 1.1 safety；cron 触发的 shell action对 deny / needs-human fail-closed，不绕过 scope / blast radius 检查；`automation_log` 表通过 CEX-12.5 migration 注册；hook lifecycle `session_start` / `session_end` 可匹配 automation rules 并写入 history；VCS `post_add` / `post_commit` / `post_push` / `post_branch` / `post_switch` 可在对应命令成功写入后匹配 automation rules 并写入 history |
| CEX-16 | 模型用量与耗时统计入库（core + full TUI follow-up 已完成，v0.17.85） | CEX-05、CEX-09b、CEX-12、CEX-12.5 | `src/internal/ai/completion/mod.rs`、`src/internal/ai/providers/{openai_compat,anthropic,gemini}/`、`src/internal/ai/agent/runtime/tool_loop.rs`、`src/command/{code.rs,usage.rs}`、`src/internal/tui/{app.rs,app_event.rs,bottom_pane.rs,chatwidget.rs,slash_command.rs}`、`src/internal/db/migration.rs`、`src/cli.rs` | `src/internal/ai/usage/{mod.rs,recorder.rs,query.rs,format.rs,pricing.rs}`、`src/internal/ai/mod.rs`、`src/command/usage.rs`、`src/command/mod.rs`、`src/cli.rs`、`src/internal/ai/agent/runtime/tool_loop.rs`、`src/command/code.rs`、`tests/ai_usage_stats_test.rs`、`tests/ai_usage_tui_test.rs` | `cargo test --test ai_usage_stats_test`；`cargo test --test ai_usage_tui_test`；`cargo test --lib usage_`；`cargo clippy --all-targets --all-features -- -D warnings` | Done: `agent_usage_stats` table registered via built-in CEX-12.5 migration; schema includes session/thread/agent_run/request_kind/intent, cached/reasoning/total/tool-call, failure/error and micro-dollar cost fields; usage recorder records success, missing-usage estimated rows and provider-error failure rows; OpenAI-compatible / Anthropic / Gemini usage metadata maps into provider-neutral summary; `UsageQuery` aggregates by `(provider, model)` and supports time/session/thread/success filters; `libra usage report --by=model` emits human/JSON/CSV output with `--since` / `--until` / `--session` / `--thread` / `--include-failed`; pricing table/config override populates `cost_estimate_micro_dollars` when provider cost is absent; `libra usage prune --retention-days` deletes rows before the cutoff and `[usage].retention_days` supplies the default when the flag is omitted; TUI chat-area header badge, compact bottom-pane usage line, `/usage` SQLite detail popup, `[usage].tui_detail_popup` gate and streaming output-token estimate are driven by usage/stream observer events; final provider usage replaces the estimate to avoid double counting |

**当前基线后的 Codex 优先队列（2026-05-03）：**

| 顺序 | 任务 | 为什么现在做 | 完成后解锁 |
|------|------|--------------|------------|
| 1 | CEX-S2-11 起的 Step 2 runtime | CEX-14 / CEX-15 / CEX-16 follow-up 已收口；只有 CP-4 通过后才允许 sub-agent runtime | sub-agent workspace / single-agent flag / controlled parallel |
| 2 | CP-6 release gate | Step 1 后续项已补齐后，需按 release gate 跑完整验证 | 进入 Step 2 实现前的质量闸 |
...
| CEX-S2-10 | Agent contracts / event schema（不启动 sub-agent） | CP-4、CEX-S2-00、CEX-S2-01、CEX-00.5 | `src/internal/ai/runtime/contracts.rs`、`src/internal/ai/runtime/event.rs`（CEX-00.5 输出）、`src/internal/ai/runtime/snapshot.rs`（CEX-00.5 输出）、`src/internal/ai/intentspec/`、`src/internal/ai/orchestrator/types.rs` | `src/internal/ai/runtime/contracts.rs`、`src/internal/ai/agent_run/{mod.rs,task.rs,run.rs,event.rs,patchset.rs,evidence.rs,decision.rs,permission.rs,budget.rs,context_pack.rs}`、`src/internal/ai/runtime/mod.rs`、`tests/ai_subagent_contract_test.rs` | `cargo test ai_subagent_contract`；`cargo test ai_runtime_contract`；`cargo test --all`（schema-only 主路径无副作用，**`code.sub_agents.enabled` config 字段 schema 已在 v0.17.688 落地（`SubAgentsConfig { enabled, max_parallel }`，默认 `enabled = false`、`max_parallel = 2`），v0.17.692 追加嵌套 `AutoMergeConfig { enabled }` 子表（默认 `enabled = false`，并新增 `AutoMergeRequiresSubAgentsEnabled` 交叉校验：`auto_merge.enabled = true` 必须配合父表 `sub_agents.enabled = true`，否则 config 装载失败），runtime gate 仍待 CEX-S2-12 dispatcher 接入；当前没有代码路径 consumes 这些 flag，所以主路径行为不变**——真实 flag-off 等价检查在 CP-S2-3） | (1) `AgentTask` / `AgentRun` / `AgentPatchSet` / `AgentEvidence` 引用现有 `IntentSpec` / `Plan` / `Task` / `Evidence`，不复制业务字段。(2) 所有新 event 实现 CEX-00.5 的 `Event` trait；**单 run 范围 events** 含 `agent_run_id: AgentRunId` 字段（如 `AgentRunEvent::*`、`AgentEvidence`）；**聚合 events**（覆盖一个 `MergeCandidate` 包含的多个 patchset / agent runs）改用 `merge_candidate_id: MergeCandidateId + agent_run_ids: Vec<AgentRunId>` 字段对（适用于 `MergeDecision[E]` event；`MergeCandidate[S]` 是 Snapshot 而非 Event，但其字段也包含同样的聚合 ID 对以便 projection 关联）；其余 events 至少有 `thread_id` 即可。schema 注释要显式说明每个 event 的 ID 字段集合，避免 dispatcher 把 aggregate 事件错误地用单 run id 写入。(3) JSONL `agents/{run_id}.jsonl` schema 通过 unknown-event-safe 测试（旧 reader 跳过 future event）。(4) `AgentEvidence` 含 `source_event_id` / `tool_call_id` / `source_call_id` / `confidence` / `applies_to_scope` / `distillable` / **`source_agent_type: enum { Explorer, Worker, Reviewer }`**（与 “与现有 IntentSpec 的映射” 表的 `AgentEvidence` 行一致——sub-agent type 必须显式记录，便于 Phase 3 / Phase 4 按 agent type 过滤 evidence） / `agent_run_id` 字段以满足 S2-INV-12 与 Step 2.8 接入点。(5) Hook dispatch 相关 schema 字段在本卡完整冻结以满足 S2-INV-13（dispatch loop 在 CEX-S2-12，但所有 schema 字段都必须在本卡定型，CEX-S2-12 不允许新增字段）：
   ...
   - **CEX-S2-12 的 hook dispatch 实现只允许使用本卡声明的字段**；如需新增字段，必须开新 CEX-S2-* 卡或开 R-* finding 走文档变更流程。(6) `cargo test --all` 在本卡完成时（**`code.sub_agents.enabled` config 字段 schema 已在 v0.17.688 落地（默认 `enabled = false`），v0.17.692 追加嵌套 `auto_merge.enabled` 子表（默认 `false`），runtime gate（dispatcher 实际 consumes 该 flag）仍由 CEX-S2-12 创建**）按 [CP-S2-3 三类比较](#cp-s2-3-flag-off-等价测试规范)的子集执行：(a) `cargo test --all` 整体 pass/fail 等价（不要求 stdout/stderr 字节级等价；引入新 schema 类型不影响测试结果）；(b) CEX-00 baseline 测试套件（baseline 7 + code_test 4）字节级等价于 `CEX00_BASELINE_COMMIT`（默认 `48ea0ae`）——这是本卡 “主路径无副作用” 的硬约束（如某 Step 1.x CEX 修改了这两个文件，按 R9-1 规则更新 baseline commit hash）；(c) 不跑 mock smoke session（fixture 未引入，`code.sub_agents.enabled` config schema 已在 v0.17.688 落地但 runtime gate 仍待 CEX-S2-12，留给 CEX-S2-12 + CP-S2-3）。真正的全量 “flag-off 等价” 检查在 CP-S2-3 执行 |
...
**Normalization 规则（diff 前必须执行，由 `tests/helpers/normalize_session.sh` 实现）：**
...
NORMALIZER="$REPO/tests/helpers/normalize_session.sh"        # CEX-S2-12 output
...
for TREE in "$CEX00_TREE" "$CP4_TREE" "$CANDIDATE"; do
  ...
  cp "$NORMALIZER"  "$TREE/tests/helpers/normalize_session.sh"
done
...
run_artifacts() {
  ...
  "$TREE/tests/helpers/normalize_session.sh" "$OUT/baseline7.stdout" > "$OUT/baseline7.stdout.normalized"
  (
    cd "$TREE" && \
    HOME="$OUT/home_test" XDG_CONFIG_HOME="$OUT/home_test/.config" \
    LIBRA_CONFIG_OVERRIDE="$CFG" \
  ...
  "$TREE/tests/helpers/normalize_session.sh" "$OUT/code4.stdout" > "$OUT/code4.stdout.normalized"
  ...
  "$TREE/tests/helpers/normalize_session.sh" "$OUT/home_test/.libra"  > "$OUT/test.libra.normalized"  2>/dev/null || echo "<empty>" > "$OUT/test.libra.normalized"
  "$TREE/tests/helpers/normalize_session.sh" "$OUT/home_smoke/.libra" > "$OUT/smoke.libra.normalized" 2>/dev/null || echo "<empty>" > "$OUT/smoke.libra.normalized"
}
...
- [x] (v0.17.64) REST OpenAPI fixture 自动生成 tool spec：`openapi_tool_capabilities_from_fixture()` 覆盖 OpenAPI operationId、path/query/header/cookie 参数和 JSON request body schema。
...
- [x] (v0.17.79) `/source list|enable|disable|reload` 已作为 TUI built-in Source Pool 控制面接入；`list` 展示 source kind / trust / enablement / capability counts，`enable` / `disable` 更新 session enablement，`reload` 可刷新内建 MCP source 并由 SourcePool 下一次 handler build 读取最新 manifest。
...
- Phase A（Step 1.10 初期）：
  ...
  - [x] (v0.17.88) 旧 `mcp` 配置段自动映射到新的 `sources` 配置视图，加载时打印 deprecation warning，但不改变用户现有行为。
...
- Web 与迁移期 TUI 回归参照多客户端连接同一 thread 时，approval 决策需要绑定 `thread_id + client_id + tool_call_id`，避免另一个客户端误批准。
- automation 触发的 agent run 默认独立 session scope，不继承交互式 session 的 approval memo。

**Automation 应该完成：**
- [x] 新增 `.libra/automations.toml`。
- [x] (v0.17.89) 统一事件流：hooks lifecycle + VCS events（v0.17.82 已新增 `AutomationRuntimeEvent` 与 scheduler runtime dispatch core；v0.17.86 已把 hook lifecycle `session_start` / `session_end` 接入 automation history；v0.17.89 已把 VCS `post_add` / `post_commit` / `post_push` / `post_branch` / `post_switch` 成功路径接入 automation history）。
...
- [x] (v0.17.80) source hot reload 在下一轮生效：`ToolLoopConfig` 在每次 tool-loop run 开始时从当前 `SourcePool` 合并启用的 source-prefixed handlers；`/source reload builtin_mcp` 更新 SourcePool 后，下一轮模型请求会重新构建工具定义。
...
- [x] `libra usage report --by=model|agent|agent-provider-model` 可读取 SQLite aggregate，支持 human / JSON / CSV 输出和 `--since` / `--until` / `--session` / `--thread` / `--include-failed` 过滤。
...
- [x] (v0.17.65) cancel/timeout 专用 `error_kind` 已接入 provider failure usage row 分类。
- [x] (v0.17.66) pricing table / config override 已接入：`UsageRecorder` 在 provider 未返回 `cost_usd` 时按 built-in model capability price table 估算 `cost_estimate_micro_dollars`，并允许 `.libra/config.toml [usage.pricing.<provider>.<model>]` 覆盖 input / output / cached / reasoning 单价；`libra usage` human / CSV / JSON 输出均能暴露估算成本。
- [x] (v0.17.67) retention config 已接入：`libra usage prune` 在未传 `--retention-days` 时读取 `.libra/config.toml [usage].retention_days`，CLI flag 优先于项目配置，非法 `0` / 类型错误会 fail closed，避免误删 usage rows。
- [x] (v0.17.85) full TUI 已补齐：`/usage` SQLite-backed detail popup、UsageUpdated 刷新、render snapshot、`[usage].tui_detail_popup` 开关，以及 streaming text delta 的实时 output-token 估算刷新；最终 provider usage 到达时会替换估算值，避免双计。
...
|--------|-------------------------------|---------|
| S2-INV-01 三层架构 | Layer 1 入口：`src/command/code.rs` 与 `src/internal/ai/orchestrator/mod.rs`；Layer 3 现有：`src/internal/ai/runtime/contracts.rs:232` (`MaterializedProjection`) + `src/internal/ai/runtime/hardening.rs:532` (`ToolBoundaryRuntime`)；Layer 2 `src/internal/ai/agent_run/` 已存在但 **schema-only**（10 个文件：`mod.rs` / `task.rs` / `run.rs` / `event.rs` / `patchset.rs` / `evidence.rs` / `decision.rs` / `permission.rs` / `budget.rs` / `context_pack.rs`，共 1244 行）——无 dispatcher / merge pipeline / Scheduler 调用入口，runtime wiring 仍 pending CP-4 | 可冻结。CEX-S2-10 schema 已落地；runtime wiring 在 CEX-S2-12 启动 sub-agent loop 时接入，与现有 Layer 1/3 不冲突 |
| S2-INV-02 Runtime 唯一 formal write | `runtime/contracts.rs:255` (`SchedulerMutation`) + `runtime/contracts.rs:547` (`TaskExecutor` trait) 已是写入合同源；但缺顶级 `Event` trait 让 `LifecycleEvent` 等 event 类型形成统一 append-only 语义 | 可冻结。CEX-00.5 引入 `Event` / `Snapshot` 后通过 trait bound 把 “write through Runtime” 形式化 |
| S2-INV-03 sub-agent 默认无主 worktree 写权 | 主 worktree 当前可变路径有三类：(a) `tools/handlers/apply_patch.rs`（直接落盘）、(b) `tools/handlers/shell.rs`（shell 命令，已拒 `git`）、(c) `tools/handlers/mcp_bridge.rs::McpBridgeHandler::is_mutating()`（当前 mcp_bridge.rs:83，已改为透明 delegate 到内层 handler；旧 `run_libra_vcs` 直判逻辑已下沉到 `run_libra_vcs` 工具的 handler 本身）通过 `libra_vcs.rs:11` 9 项 ALLOWED_COMMANDS 中的 `add` / `commit` / `switch` 改 worktree / 仓库状态；三类都过 `runtime/hardening.rs` boundary（具体：`src/internal/ai/tools/registry.rs:82` 的 `hardening: Option<ToolBoundaryRuntime>` 字段在 `:287-293` 每次 dispatch 前 `decide(&operation)`+ `append_audit`，由 `src/command/code.rs` `.hardening(ToolBoundaryRuntime::system(...))` 在 `libra code` 启动期注入；如未注入则 fall-through 即不强制 boundary）；workspace 隔离（`orchestrator/workspace.rs:120` `FuseProvisionState`）目前只在 worktree-fuse 路径上 | 可冻结。CEX-S2-11 在 `agent_run/workspace_strategy.rs` 强制隔离；CEX-S2-12 dispatcher 必须把 (a) / (b) / (c) 三类入口的 sub-agent 调用都重定向到隔离 workspace（`run_libra_vcs add/commit/switch` 不能直接打到主 worktree），并保证 sub-agent 维度的 registry 永不在 `hardening = None` 状态下运行 |
| S2-INV-04 sub-agent 不拥决策权 | 当前 `runtime/phase4.rs` 是 final decision 入口；`orchestrator/types.rs:601` `OrchestratorObserver` 仅观察，不写决策 | 可冻结。Reviewer sub-agent 的 evidence 路径在 CEX-S2-13 / S2-15 落实 |
...
| S2-INV-06 approval 归 Layer 1 | `sandbox/mod.rs:402` `ApprovalStore` 当前 session-scoped，只有一个 owner；缺 `agent_run_id ≠ approver_id` 的结构化拒绝 | 可冻结。CEX-S2-12 “sub-agent 不能批准自己的 approval” 测试需要在 dispatcher 层实现 |
...
| S2-INV-08 flag-off 行为不变 | `code.sub_agents.enabled` 配置键 schema **已在 v0.17.688 落地**（`SubAgentsConfig { enabled, max_parallel }`），v0.17.692 追加嵌套 `auto_merge.enabled` 子表；当前任何代码路径都**不**读取这些 flag（runtime gate dispatcher 仍由 CEX-S2-12 创建），所以全 cargo test 套件 flag-on / flag-off 路径行为字节级一致 — 现状满足 “感知不到 sub-agent” 这一条 invariant 的所有可测条件 | 可冻结。CP-S2-3 三类比较的 baseline commit 选择规则在 [CP-S2-3 等价测试规范](#cp-s2-3-flag-off-等价测试规范) 已写明 |
...
| S2-INV-10 unknown-event-safe | 当前 `hooks/lifecycle.rs:35` `LifecycleEventKind` 是 closed enum，新增 variant 会导致旧 reader 反序列化失败；session JSON blob 无 forward-compat 测试 | **存在缺口**。R-A3：CEX-00.5 的 `Event` trait 必须强制 `#[serde(other)]` 或 envelope-with-typed-payload pattern；CEX-S2-10 unknown-event-safe 测试沿用此 pattern，否则同名约束在 LifecycleEventKind 上同样要求适用 |
| S2-INV-11 channel-agnostic lifecycle | `hooks/lifecycle.rs:74` `LifecycleEvent` 字段不绑 TUI / web；MCP server (`ai/mcp/server.rs`) 通过相同 lifecycle 路径触发 | 可冻结。CEX-S2-10 `AgentRunEvent` 沿用 envelope 形态即可 |
| S2-INV-12 evidence raw fact 链 | **持久化 `Evidence[E]` schema** 实际由 git-internal 提供（`git_internal::internal::object::evidence::{Evidence, EvidenceKind}`，本 crate 通过 `src/internal/ai/mcp/resource.rs:34` 重导入使用）；与之对照的 `src/internal/ai/runtime/contracts.rs:393` `EvidenceKind` 是**运行时分类枚举**（`Test` / `Lint` / `Build` / `ToolPolicyViolation` 等），不是持久化 Snapshot；两者都尚未含 `agent_run_id` / `source_event_id` / `tool_call_id` / `source_call_id` 字段 | 可冻结。CEX-S2-10 在 `agent_run/evidence.rs` 必须**扩展 + 引用 git-internal 的持久化 `Evidence[E]`**（增加 raw-fact-chain 字段），同时引用 `runtime/contracts.rs:393` 运行时 `EvidenceKind` 作为分类；不能复制 `evidence` 业务字段或新建 parallel snapshot 对象（与“与现有 IntentSpec 的映射”表 `AgentEvidence` 行约束一致） |
| S2-INV-13 hook 拦截点 | `hooks/lifecycle.rs:38` 已含 `LifecycleEventKind::ToolUse` 变体（enum 定义在 `:35`），可作 PreToolUse / PostToolUse 物理基础；hook **runner 仍为 lifecycle 事件源**而非 dispatch-blocking 屏障；exit-code → 决策映射尚未实现 | 可冻结但有依赖。CEX-S2-12 在 `agent_run/hook_dispatch.rs` 实现表中 10 个终止状态映射；CEX-S2-10 **必须**先冻结 `HookInvocationPayload` 与 5 个 outcome variants schema（见 CEX-S2-10 完成判定 (5)，本节上方 “Step 2 Runtime 任务卡” 段落） |

##### CEX-S2-01：readiness matrix 锚点

| 合同 | 现状锚点 | Step 1 阻塞输出 | 阻塞原因 |
|------|---------|----------------|----------|
| `AgentPermissionProfile` 字段 | `sandbox/mod.rs:402` `ApprovalStore` 已为 session 级 cache；`runtime/hardening.rs:282` `ToolBoundaryPolicy` 已有 `decide()` | Step 1.1 OS 黑名单 + 参数白名单；Step 1.6 Approval TTL canonical key | profile 字段需引用最终 `ApprovalKey` 形态（含 `sensitivity_tier` / `scope` / `blast_radius`），未冻结前任何字段都可能改名 |
...
| `AgentRunEvent` 持久化 | `session/store.rs:29` `SessionStore` 已切到 JSONL（CEX-12 完成 2026-05-03，事件落 `.libra/sessions/{id}/events.jsonl`，`SessionJsonlStore` 提供 `append` / `load_state`，[`session/jsonl.rs:22`](../../src/internal/ai/session/jsonl.rs) `SESSION_EVENTS_FILE` + [`session/migration.rs:53`](../../src/internal/ai/session/migration.rs) 旧 `.json` 透明迁移）；`hooks/lifecycle.rs:74` 是 in-memory 结构（`LifecycleEvent`） | Step 1.8 JSONL append + lock；Step 1.9 CompactionEvent envelope | JSONL header / attachment / replay 语义已部分定型（CEX-12 + CEX-13b ContextFrame / Attachment / CompactionEvent envelope），sub-agent run 在主 session JSONL 之外新建 `agents/{run_id}.jsonl` 隔离面，避免与 CEX-S2-10 schema 冲突 |
...
| Workspace materialization API | `orchestrator/workspace.rs:120` `FuseProvisionState`（worktree-fuse）+ `src/command/worktree.rs:62` `WorktreeSubcommand`；git-internal 暴露 reservation 但无 sparse | Step 1.8 session storage（lock 复用）；git-internal worktree 增强（CEX-S2-11 RFC） | sparse / blocked 策略需读 `AgentContextPack` scope；后者依赖 Step 1.3/1.9 |
...
|------|------|---------------------|
| Workspace（worktree） | `src/command/worktree.rs:62` `WorktreeSubcommand` 含 9 个 variant：`Add` / `List` / `Lock` / `Unlock` / `Move` / `Prune` / `Remove` / `Umount`(`#[cfg(unix)]`) / `Repair`；`worktree-fuse.rs` 提供 FUSE-based provision；`orchestrator/workspace.rs:120` `FuseProvisionState` + `:162` `FuseAttemptOutcome` 描述 fallback | 缺三档物化策略（Worktree / Sparse / Blocked）的 dispatch 表与 100K-file fixture；缺 `AgentRunEvent::workspace_materialized` 写入 hook |
...
| TUI observability | `src/internal/tui/app.rs:448` `App<M>` 主循环；`src/internal/tui/bottom_pane.rs` 用于 approval prompt；现有 `slash_command.rs` 已含 builtin 命令但无 `/agents`、`/agent cancel <id>` | CEX-S2-16 新建 `agent_pane.rs`（5 状态 + budget/cost 显示），扩展 `slash_command.rs`；approval prompt 增加 `agent_id` / `task_id` 字段 |
| MCP observability | `src/internal/ai/mcp/server.rs:185` `list_resources_impl` 注册 **2 个 resource**：`libra://history/latest`、`libra://context/active`；`:474` `list_resource_templates` 注册 **2 个 template**：`libra://object/{object_id}`、`libra://objects/{object_type}`；read 路径在同文件内处理 4 种 URI 形态 | CEX-S2-16 在同一文件扩展 **6 个新 URI 模板**（与 CEX-S2-16 任务卡 (3) 一致——见本节上方 “Step 2 Runtime 任务卡” 段落）：`libra://agents/runs`（list）、`libra://agents/runs/{id}`（detail）、`libra://agents/runs/{id}/permissions`、`libra://agents/runs/{id}/budget`、`libra://agents/runs/{id}/context`、`libra://agents/merge-candidates/{id}` |
| Scheduler / projection | `src/internal/ai/projection/scheduler.rs:29` `SchedulerState` + `:198` `SchedulerStateRepository`（CAS 写入）；现有 `SchedulerMutation`（`runtime/contracts.rs:255` 起，并由 `:293` 的 `expected_versions()` 匹配）共 **8 个 expected-version 写入变体**：`SeedThread` / `SetCurrentPlanHeads` / `SelectPlanSet` / `StartStage` / `MarkTaskActive` / `ClearActiveRun` / `MarkProjectionStale` / `ApplyRebuild`——**没有 enable / disable 节点变体**，sub-agent 的“暂停/恢复”能力需 CEX-S2-14 新增 | CEX-S2-14 需扩 scheduler 状态机：新增节点级 enable/disable、disjoint scope detection、`max_parallel = 2`、queued/running/blocked/completed/failed 五态；新增节点级写入必须保持 `expected: ProjectionVersions` 不变量（与既有 8 个 variant 一致）；trace id 链 `thread_id → agent_run_id → tool_call_id → source_call_id` 中前两段已由 CEX-16（`sql/migrations/2026050302_agent_usage_stats.sql`，`agent_usage_stats` 表的 `thread_id` / `agent_run_id` 列）落地，后两段（`tool_call_id` / `source_call_id`）在 `agent_usage_stats` 中仍**无字段**，需 CEX-S2-14 通过 sea-orm migration 追加并写入 recorder |

##### Audit revision items（必须随 PR 一并落地）

- **R-A1（已修）**：基线表第 162 行原写 “`AuditSink` 作为可注入 trait 尚未定义” 与代码不符。`runtime/hardening.rs:451` 已有 `pub trait AuditSink: Send + Sync` 含 `append`（hardening.rs:455）+ `flush`（hardening.rs:458）；`ToolBoundaryRuntime::append_audit` 在 :571 调用它。已更新为 “trait 已存在但形态需 CEX-00.5 演化”。
- **R-A2（已修）**：基线表第 161 行原写 8 个 `run_libra_vcs` 命令（漏 `show-ref`），已补齐到 9 个，并加 `src/internal/ai/libra_vcs.rs:11` 锚点。
- **R-A3（待 CEX-00.5 / CEX-S2-10 落地）**：`hooks/lifecycle.rs:35` `LifecycleEventKind` 为 closed enum，未来 sub-agent event 加新 variant 时旧 reader 会失败。CEX-00.5 的 `Event` trait 必须强制 envelope-with-typed-payload（如 `{ "kind": "<known-or-unknown>", "payload": <json> }` + `#[serde(other)]` 兜底分支），CEX-S2-10 的 `AgentRunEvent` 与 `LifecycleEventKind` 在迁移中按相同模式重构。否则 S2-INV-10 会被 LifecycleEventKind 自身打破。
- **R-A4（CEX-S2-10 收口，已修订）**：持久化 `Evidence[E]` schema **由 git-internal 提供**（`git_internal::internal::object::evidence::{Evidence, EvidenceKind}`，本 crate 通过 `src/internal/ai/mcp/resource.rs:34` 导入），**不是** `src/internal/ai/runtime/contracts.rs:393` 的运行时 `EvidenceKind` 枚举。CEX-S2-10 新增 `agent_run/evidence.rs` 必须**扩展 + 引用 git-internal 的 `Evidence[E]` Snapshot**，并把 `runtime/contracts.rs:393` 运行时 `EvidenceKind` 当成 *分类标签* 引用（不是 schema 来源）；不能新建与 `Evidence[E]` 平行的 snapshot（避免 “与现有 IntentSpec 的映射” 表 `AgentEvidence` 行约束违例）。R-A4 修订前曾错误指向 `runtime/contracts.rs:393`，现已澄清。**命名碰撞警告**：两个 `EvidenceKind` 同名不同 path，CEX-S2-10 在 `agent_run/evidence.rs` 同时使用时必须显式 `as` 别名（推荐 `use git_internal::internal::object::evidence::EvidenceKind as PersistentEvidenceKind;` 与 `use crate::internal::ai::runtime::contracts::EvidenceKind as RuntimeEvidenceKind;`），避免编译期歧义和 review 时的视觉混淆。
...
- **R-A6（已修）**：原 CEX-S2-02 Scheduler 行写 “`SchedulerMutation` 已含 disable/enable 节点”，与 `runtime/contracts.rs:175-210` 实际 variants 不符；现实际为 **8 个 expected-version 写入变体**（`SeedThread` / `SetCurrentPlanHeads` / `SelectPlanSet` / `StartStage` / `MarkTaskActive` / `ClearActiveRun` / `MarkProjectionStale` / `ApplyRebuild`，由 `:215-222` `expected_versions()` 匹配确认），无 enable/disable；Codex round 2 发现初版 R-A6 还漏列 `MarkProjectionStale` / `ApplyRebuild` 两个变体，已补全；并把 enable/disable 标为 “CEX-S2-14 新增” 缺口。
...
- **R-A8（已修）**：原 CEX-S2-02 Workspace 行的 `WorktreeSubcommand` 列表 (`Add` / `List` / `Remove` / `Mount` / `Umount`) 不准确——实际 `src/command/worktree.rs:58-111` 含 9 个 variant：`Add` / `List` / `Lock` / `Unlock` / `Move` / `Prune` / `Remove` / `Umount`(`#[cfg(unix)]`) / `Repair`，**没有 `Mount` variant**（FUSE provision 在 `worktree-fuse.rs` 内部完成，不通过子命令暴露）。已更正。
...
- flag 开启但 `max_parallel = 1` 时，结果可审计、可 replay、可 rollback。
...
- 默认并行度保守，例如 `max_parallel = 2`（与 v0.17.688 落地的 `SubAgentsConfig::max_parallel` 默认一致）。
- 写 scope 冲突时串行化。
...
### Step 3.B：WebSocket Control Routing（外部控制）

**为什么需要**：Libra 的 Agent 外部控制面需要统一入口。新的方向是 Web / WebSocket / Web API 负责发起 agent run、订阅状态、给 approval；MCP stdio 拆到独立 `libra mcp` 命令，只承载 tools/resources/protocol，不作为 Agent 调度面。

**核心想法**：
- 抽象 **Control Channel** 概念：Web UI、WebSocket/Web API、未来的 IM bot adapter 都通过同一 AgentRuntime queue 进入；TUI 只作为迁移期行为基线，MCP 只作为工具/资源协议或 bounded event source。
- 每个 control channel 有独立的 approval handler、消息队列、用户身份验证。
...
| 日期 | 作者 | 变更摘要 |
|------|------|----------|
| 2026-05-09 | Codex | 计划可信度复核：CP-S2-3 baseline 脚本继续要求 `CP4_BASELINE_COMMIT` 显式提供，并用 `git rev-parse --verify` 校验真实 commit；历史 R17-7 中的 placeholder 默认值方案已废弃，不能作为实施依据。 |
...
| Query Index | 指定 thread 相关 Snapshot + Event；读侧按 thread 的 intent / plan / task / run id 集合裁剪 `ai_index_*` 表 | `ProjectionResolver::load_or_rebuild_query_indexes(thread_id, rebuilder)` 命中缺失或 `StaleReadOnly` projection 时 |
...
读 Query Index（intent->plans 等）：
  1. 优先 DB `ai_index_*` 行，并通过当前 `ThreadProjection` / `SchedulerState` 推导 thread 作用域
  2. 若 thread projection 缺失或降级：走 targeted rebuild 后再返回 `ThreadQueryIndexes`
  3. 返回值必须携带 `ProjectionFreshness`；`StaleReadOnly` 只允许 UI / MCP / diagnostics 读取，不允许推进 Scheduler
  4. Index 扫描不影响主路径正确性，只影响读性能
...
- Query Index 由 `ProjectionResolver::load_query_indexes()` 以 read-only bundle 形式暴露；`load_or_rebuild_query_indexes()` 与 thread bundle 使用同一 targeted rebuild / degraded read 策略，rebuild 失败时返回 `Unavailable`（若已有降级视图）或带上下文错误（若完全无可读 projection）。
- Phase 3/4 structured outputs 允许独立 replay / materialize；它们是可丢弃并可重算的 derived records，不是独立真相源。
- rebuild 期间若已有可读 projection，读取允许返回"stale"标记，不允许因派生索引缺失直接失败；若完全无可读 projection 且 rebuild 本身失败，必须返回带 thread_id / rebuild 阶段上下文的错误。
...
| Headless Web UI 仍缺完整 workflow | 中 | 非 Codex `--web-only` 已通过 `HeadlessCodeRuntime` 提供可写 browser turn、streaming、tool / approval / plan / patchset projection，不再是 placeholder；剩余缺口是完整 IntentSpec plan approval workflow 与 shared interaction state |
...
4. Web 必须成为 `libra code` 的唯一生产交互面，展示 shared projection 派生的 snapshot + typed delta；外部 submit/respond/cancel/observe 参考 Codex 走 WebSocket/Web API，而不是通过 TUI 或 MCP 调度 Agent。
5. controller lease 的申请、转移、释放都必须走共享 schema；TUI 只作为迁移期行为基线并最终删除，MCP 只保留 tools/resources/protocol 能力，不拥有 Agent turn 控制权。
...
5. 建立 Web-only controller 协议：WebSocket/Web API 是外部控制 Agent 的入口；TUI 不再作为可接管对象，MCP 不作为 Agent 调度面。
6. 非 Codex `--web-only` placeholder 已消灭（HeadlessCodeRuntime v1）；继续把通用 provider 的 read model / interaction state 收敛到 Phase 3 shared projection，并为默认 Web 入口做准备。

### Affected Modules

| 模块 / 文件 | 调整内容 |
|---|---|
| `src/internal/ai/web/code_ui.rs` | 拆分命令面与读模型面；事件改为 typed delta；WebSocket/Web API controller lease 由共享协议驱动 |
| `src/internal/ai/web/mod.rs` | `/session`、`/events`、`/controller/*`、WebSocket/Web API 输入入口基于 shared read model |
| `src/internal/tui/app.rs` | 迁出本地 plan revision 真相；迁移期只作为行为基线，默认 Web 后删除生产路径 |
| `src/internal/tui/app_event.rs` | interaction / review 事件收敛；迁移完成后不作为 Agent 操作入口 |
...
2. Web runtime 与迁移期 TUI 回归参照读取同一 interaction / plan-set / task / patchset 事实；生产控制面只保留 Web。
3. `pending_plan_revision` 等 TUI 私有状态从代码库中消失。
4. `--web-only` 通用 provider 不再是 placeholder；完整 IntentSpec plan approval workflow 继续依赖共享 session driver。
5. review / modify / cancel / approval 都使用共享 interaction schema。
6. Web 可以显示 thread / interaction / plan-set / patchset 视图，并通过 WebSocket/Web API controller lease 承载外部 submit/respond/cancel/observe；TUI/MCP 不承担 Agent 操作入口。
...
| `src/internal/ai/projection/resolver.rs` | 承载 `ProjectionResolver`、`ThreadBundle`、`ResumeBundle`、`ThreadQueryIndexes`、targeted rebuild / degraded read / query-index read / phase-aware resume 的统一入口；对上层隐藏 projection freshness、CAS/version 校验和 rebuild apply 细节。 |
...
`用户原始输入 -> CodexCodeUiAdapter::submit_message(text) -> turn/start -> Codex (plan-first system prompt 已由 codex 端注入) 返回 plan text / response text -> Libra 本地 resolve_intentspec(...) -> Markdown review`
...
通用 completion provider 的 TUI 路径当前先进入 Phase 0 `IntentSpec` review，再进入 Phase 1 planning review；Phase 0 review 的 `Intent` 持久化已通过 `runtime::phase0::write_intent` formal-write helper 收口，且 dirty-worktree `ContextSnapshot` 冻结判定已通过 `runtime::phase0::write_context_snapshot_if_needed` 收口（`phase0_review_persistence_uses_runtime_write_helper` pin 住调用方不再直接调用底层 `persist_intentspec` / MCP snapshot writer；`phase0_context_snapshot_request_*` 覆盖 clean skip 与 changed-path request）。Phase 1 review 持久化已从 single execution-plan 过渡到 execution/test 双 `Plan` 预览 bundle（`persist_plan_review_bundle` 复用 plan-set 持久化，execution 再复用该 preview plan set），但 UI 展示和 provider draft 仍是单 ordered draft，完整 Scheduler cutover 仍由 runtime `SelectPlanSet` 路径负责。

该过渡路径的对象边界如下：

1. provider 只能通过 `submit_plan_draft` 返回 planning draft；draft 只包含 `explanation` 和有序 `steps[].title`，不包含 runtime status。
2. Libra 本地 planner 把 draft 规范化为执行计划，再按 task kind 持久化为 `Plan(role=execution)` / `Plan(role=test)` 双 plan；`Task` snapshots 仍由 review bundle 一次性创建。
3. 每个 `Task.origin_step_id` 必须指向其所属 role plan 的 `Plan.steps[*].step_id`，`plan_id_by_task_id` 记录 execution/test 归属。
...
| `ProjectionResolver` rebuild / resume / degraded read / query-index materialization | `projection/resolver.rs` / `projection/rebuild.rs` tests（`rebuild_materializes_run_state_and_indexes` 覆盖 7 个 `ai_index_*` 表的落库关系与 latest 标记；`projection_resolver_rebuilds_and_loads_thread_query_indexes` / `projection_resolver_returns_stale_read_only_query_indexes_when_scheduler_row_is_missing` 覆盖 resolver read contract） |
...
| `tests/ai_schema_migration_test.rs`（✅ 已落地） | Implementation Phase 0: fresh bootstrap、已部署 DB migration、重复执行幂等、legacy backfill dry-run/verify contract |
| `tests/ai_runtime_contract_test.rs`（✅ 已落地） | Implementation Phase 1: Codex 与通用 provider 都通过 `Runtime + TaskExecutor` 完成 attempt |
| `tests/command/code_thread_id_test.rs`（✅ 已落地） | Implementation Phase 2: thread_id 统一、legacy backfill、provider_thread_id 仅内部（5 个测试覆盖：canonical lookup × working_dir 匹配、缺失 thread_id 返回 None、preview/apply 幂等、provider_thread_id 仅作为 metadata 不作为 resume key、backfill 不覆盖已有 canonical/provider id） |
| `tests/ai_code_ui_projection_test.rs`（✅ 已落地） | Implementation Phase 3: shared interaction、typed delta、WebSocket/Web API controller lease 与 Web-only 控制面 |
| `tests/ai_validation_decision_flow_test.rs`（✅ 已落地） | Implementation Phase 4: `ArtifactLedger`、`ValidatorEngine`、`DecisionProposal` 闭环 |
| `tests/ai_storage_flow_test.rs`（✅ 已扩展） | `Runtime` 集成，formal write 顺序和最小对象集：`runtime_formal_writes_preserve_order_and_minimal_object_set` 覆盖 Phase 0 Intent formal write、空 ContextSnapshot skip、Phase 1 execution/test plan-set 写入、`ExecutionAuditSession` 复用已持久化 Intent/Plan set、Phase 2 attempt start/finish，并用精确对象计数 pin 住最小 Intent/Plan/Task/Run 集合 |
| `tests/intent_flow_test.rs`（✅ 已扩展） | Phase 0 Intent formal write + ContextSnapshot 条件写入：`phase0_runtime_helpers_persist_intent_and_context_snapshot_conditionally` 覆盖 Intent 写入、空 snapshot request skip、非空 snapshot request 落 `snapshot` history |
| `tests/ai_security_runtime_test.rs`（✅ 已落地） | Implementation Phase 5: authz、redaction、shell boundary、durable audit（6 个测试覆盖：default markers 全部 strip + idempotence、`PrincipalContext::system()` 稳定 id/role、`InMemoryAuditSink` append-only 顺序、`record_decision` 默认实现 redact reason、`ToolBoundaryRuntime::system` 写入 redacted audit、`ToolBoundaryPolicy::default_runtime()` 对 Contributor/Observer 的 readonly/mutating 边界） |
| `tests/ai_concurrency_lock_test.rs`（✅ 已落地，文件层 advisory lock 部分） | Implementation Phase 0/2: repo+thread advisory lock（5 个测试覆盖：lock + Drop release roundtrip、同 id 并发互斥（前持有者 drop 后后者快速取得）、跨 id 独立无争用、lock 文件含 `pid=` + `created_at_ns=` 用于 stale 检测、50 次 acquire/release 循环稳定）。SQLite busy 与 worktree reservation 由 `db_migration_test.rs` 与 worktree-fuse 测试分别覆盖 |

**Layer 3：E2E 测试**

| 测试文件 | 覆盖内容 |
|---|---|
| `tests/ai_isomorphism_test.rs`（⚠️ L3 deferred） | DAG 同构：同一需求分别用 Codex 和通用 provider 执行，断言对象图在深度、引用关系和 Phase 推进上同构。**前置条件**：需要同时拉起 Codex app-server mock + generic provider mock，并消费匹配的 fixture script；属于 `test-provider` + 真实 provider boot-smoke 的交集场景，非 L1 hermetic 可表达。**计划**：等 CEX-S2-12 落地后并入 `tests/ai_subagent_isomorphism_test.rs`，作为 sub-agent 二阶段验收的 phase-recovery 验证之一；现阶段不单独建立独立 L3 entry-test 文件 |
| `tests/ai_rebuild_recovery_test.rs`（✅ 已落地，由 `tests/ai_projection_resolver_test.rs` 等价覆盖） | Rebuild 恢复：中断后仅凭 thread_id 重建 SchedulerState 并继续执行（`projection_resolver_rebuilds_missing_thread_projection_from_history` / `projection_resolver_returns_stale_read_only_when_scheduler_row_is_missing` / `scheduler_repository_loads_selected_plan_set_and_enforces_cas` 三个测试已对 `ProjectionResolver::load_or_rebuild_thread_bundle` + `SchedulerStateRepository` CAS + StaleReadOnly 路径全部断言；新增独立测试文件冗余，故复用） |
| `tests/ai_web_multi_observer_test.rs`（✅ 已落地，由 `tests/code_ui_scenarios.rs` + `tests/code_ui_remote_sse_matrix.rs` 等价覆盖） | 同一 thread 上 TUI + Web 并发观察时事件一致性与 gap recovery（`local_tui_reclaim_invalidates_browser_lease` / `second_browser_attach_with_different_client_returns_conflict` / `browser_*` 写通道 + SSE matrix 7 个 case 覆盖 attach/detach/cancel/conflict/oversize 与 SSE lagged-recovery，新增独立测试文件冗余，故复用） |
| `benches/ai_runtime_baseline.rs`（✅ 已落地） | Phase 0 benchmark baseline：DAG build、targeted rebuild、compaction、audit/query |
...
```rust
// tests/helpers/mock_codex.rs（✅ 已落地）
...
// tests/helpers/mock_completion_model.rs（✅ 已落地）
...
|---|---|---|---|---|
| Phase 0 | `Intent` / `ContextSnapshot` | Runtime Phase 0 helper + shared MCP/history persistence 已验证；generic TUI Phase 0 review `Intent` 调用方已切到 `write_intent`，dirty-worktree `ContextSnapshot` 调用方已切到 `write_context_snapshot_if_needed`；Codex/Code UI 调用方与完整 Phase0Bundle / projection seed 仍待完成 | 已验证（`runtime::phase0::{write_intent, write_context_snapshot_if_needed}`；generic TUI 调用方由 `phase0_review_persistence_uses_runtime_write_helper` 固定，dirty-worktree request 由 `phase0_context_snapshot_request_*` 覆盖） | `phase0_write_helpers_persist_intent_and_context_snapshot` pin 住 `Intent` id 与 `ContextSnapshot` 的 history `snapshot` id 可解析；`phase0_review_persistence_uses_runtime_write_helper` 防止 TUI review 回退到底层 `persist_intentspec` / MCP snapshot writer；`review_renders_context_snapshot_id_when_present` pin 住 review surface |
...
| Phase 3 | `ArtifactLedger` / `release_candidate patchset` / `ValidationReport` | 共享 persistence derived-record 路径已验证；Codex provider path 仍待共享 Runtime cutover | 已验证（`ArtifactLedger` release-candidate check + `ValidationReportStore`） | `artifact_ledger_tracks_release_candidate_patchset` pin 住 release-candidate 选择；`commit_validation_requires_release_candidate_patchset_in_artifact_ledger` / `commit_validation_accepts_release_candidate_patchset_from_artifact_ledger` 覆盖 release gate；`test_persist_execution_creates_object_chain` 覆盖真实 persisted execution 的 release `ValidationReport` |
| Phase 4 | `DecisionProposal` / final `Decision` / terminal `IntentEvent` | 共享 execution-audit finalize 路径已验证；Codex provider path 仍待共享 Runtime cutover | 已验证（`ExecutionAuditSession::finalize` + `DecisionProposalStore` + MCP/history） | `execution_audit_session_persists_runtime_side_objects` 覆盖 final `Decision` id 可解析、latest `DecisionProposal` id/route/verdict 与 derived record 对齐、terminal `IntentEvent::Completed` |
| Projection | `ThreadProjection` / `SchedulerState` / `live_context_window` | 共享 projection resolver 路径已验证；Codex provider path 仍待共享 Runtime cutover | 已验证（`ProjectionResolver::load_thread_bundle` + `SchedulerStateRepository`） | `projection_resolver_loads_fresh_thread_scheduler_and_live_context_window` pin 住 Fresh bundle 中 `ThreadProjection` identity、`SchedulerState.selected_plan_ids` ordering 与 `live_context_window` |
| UI | `CodeUiSessionSnapshot` / typed event payload / interaction state | 已验证（Codex `publish_code_ui_snapshot` 通过 `CodeUiEventType::SessionUpdated` 发布 typed `CodeUiSessionSnapshot` payload） | 已验证（通用 `CodeUiSession` / headless runtime 共用 closed event type + typed snapshot payload，interaction state 随 `session_updated` 广播） | `event_envelope_round_trips_typed_event_and_snapshot_payload` pin 住 closed event type 与 typed snapshot payload；`interaction_update_broadcasts_typed_session_snapshot_event` 覆盖 interaction state 广播 |
...
1. Web runtime 与迁移期 TUI 回归参照共享同一 interaction / plan-set / task / patchset 真相；生产入口只保留 Web。
2. `pending_plan_revision`、`pending_post_plan` 等本地真相从代码库中消失。
3. Web 可以通过 WebSocket/Web API controller lease 控制当前 thread 的输入权；TUI 不作为接管对象，MCP 不作为 Agent 调度面。
4. `--web-only` 通用 provider 不再是 placeholder；完整 IntentSpec plan approval workflow 继续依赖共享 session driver。
...
| Phase C（UI 收敛） | Implementation Phase 3 | 把 Web 的真相源切到 shared projection 和 shared interaction schema，并关闭 TUI/MCP 作为 Agent 操作入口 | Phase B 的 `thread_id` / projection read contract 已可复用 | Web 共享 interaction / plan-set / patchset 事实；typed delta、WebSocket/Web API controller lease 可用；TUI 仅作迁移基线并最终删除 |
...
4. 落地 Web-only controller lease：WebSocket/Web API 可提交 message、响应 interaction、取消 turn、观察 snapshot；TUI 不再作为接管对象，MCP 不作为外部 Agent 调度面；非 Codex `--web-only` 通用 provider 路径已具备 headless runtime，后续只收敛共享 interaction / IntentSpec workflow。

这一阶段的合入门槛：
1. Web runtime 与迁移期 TUI 回归参照看到同一 interaction / plan-set / task / patchset 事实，生产入口只保留 Web。
2. WebSocket/Web API 控制面不再依赖 provider-specific snapshot 或 TUI bridge。
3. `--web-only` 通用 provider 不再是 placeholder；完整 IntentSpec plan approval workflow 继续依赖共享 session driver。
...
| UI 真相源切换导致交互丢失、事件乱序或 controller lease 异常 | TUI 本地状态移除后，typed delta、gap recovery、WebSocket/Web API 控制面没有一次性补齐 | 中 | 中 | `pending_plan_revision` / approval / `request_user_input` 消失或重复，Web runtime 与迁移期 TUI 回归参照观察到不同事实 | shared interaction schema 先行；`snapshot seed + typed delta + seq gap recovery` 一起落地；补 multi-observer、reconnect、controller lease tests |
...
`libra code` 默认启动 TUI，并同时启动本地 Web API 与 MCP HTTP server（参见 [src/command/code.rs](../../src/command/code.rs)；symbol-name anchors stable, line numbers shift with TUI bootstrap refactors）。Web 侧已有 `/api/code/*` snapshot / SSE / browser controller lease 雏形（[src/internal/ai/web/mod.rs::code_router](../../src/internal/ai/web/mod.rs) 当前位于 :137；route 注册块紧邻在下方），但普通 provider 的 TUI 仍由终端事件循环独占输入；`libra code --stdio` 是 MCP stdio tool server，不驱动正在运行的 TUI 会话。
...
| 文档 | 本计划关系 |
|------|------------|
| 本文 Part B（原 `docs/improvement/code.md`） | 主归属。controller lease、Code UI read model、future typed delta 和 WebSocket/Web API 控制面最终应收敛到 Part B 的 Implementation Phase 3 "Code UI Source Of Truth Unification"；TUI 不再作为可接管对象，MCP 不作为 Agent 调度面。 |
...
| `docs/development/mcp.md` | 独立命令拆分计划。`libra mcp --stdio` 的 CLI grammar、stdio runner、compat 文档和 MCP e2e 验收由该文档跟踪；本文只保留 “MCP 不作为 Agent 调度面” 的边界。 |
...
| 能力 | 位置 | 说明 |
|------|------|------|
| `/api/code/session` snapshot | [src/internal/ai/web/mod.rs:158（route）/ :375（code_session_handler）](../../src/internal/ai/web/mod.rs) | loopback-only 读取已就绪 |
| `/api/code/events` SSE | [src/internal/ai/web/mod.rs:159（route）/ :384（code_events_handler）](../../src/internal/ai/web/mod.rs) | broadcast channel + KeepAlive |
| `/api/code/messages` POST | [src/internal/ai/web/mod.rs:177, 611](../../src/internal/ai/web/mod.rs) | 已校验 controller token |
| `/api/code/interactions/{id}` POST | [src/internal/ai/web/mod.rs:178, 652](../../src/internal/ai/web/mod.rs) | 同上 |
| `/api/code/controller/attach`/`detach` | [src/internal/ai/web/mod.rs:163-164, 540-571](../../src/internal/ai/web/mod.rs) | 仅 browser kind |
| Loopback gate | [src/internal/ai/web/mod.rs:968](../../src/internal/ai/web/mod.rs) `ensure_loopback_api_request` | 已应用全部 7 个 handler |
| `ControllerLease` 结构 | [src/internal/ai/web/code_ui.rs:734](../../src/internal/ai/web/code_ui.rs) | UUID token, 120s TTL（曾名 `BrowserControllerLease`，已统一为 kind-aware `ControllerLease`） |
| `ensure_controller_write_access` | [src/internal/ai/web/code_ui.rs:1193](../../src/internal/ai/web/code_ui.rs) | controller token 校验逻辑 |
| `CodeUiCommandAdapter` trait | [src/internal/ai/web/code_ui.rs:647](../../src/internal/ai/web/code_ui.rs) | 写通道抽象（已被 codex / read-only 实现） |
| `CodexCodeUiAdapter` 实现 | [src/internal/ai/codex/mod.rs:1534](../../src/internal/ai/codex/mod.rs) | 仅 codex 后端走此路径 |
| `ReadOnlyCodeUiAdapter` | [src/internal/ai/web/code_ui.rs:1433](../../src/internal/ai/web/code_ui.rs) | 非 codex provider 占位 |
| Snapshot mutate + broadcast | [src/internal/ai/web/code_ui.rs:472, 609](../../src/internal/ai/web/code_ui.rs) | `mutate`, `broadcast_snapshot` |
| Pending interaction model | [src/internal/ai/web/code_ui.rs:129-199](../../src/internal/ai/web/code_ui.rs) | `CodeUiInteractionKind` + `Status` + `Option` + `Request`（id/kind/status/resolved_at）+ `Response` |
| `CodeUiControllerKind` 枚举 | [src/internal/ai/web/code_ui.rs:69](../../src/internal/ai/web/code_ui.rs) | 当前 None/Browser/Tui/Cli |
| AppEvent bus | [src/internal/tui/app_event.rs:148-298](../../src/internal/tui/app_event.rs) | 只承载 turn-scoped event；不要加入无 turn 的 control command |
| Pending interaction helpers | [src/internal/tui/app.rs](../../src/internal/tui/app.rs)（user-input / approval / managed-interaction helpers 现在散落，例如 `approvals_command_message` 在 :4837、`user_input_answers_from_code_ui` 在 :9833） | user input / approval / managed interaction 可抽 helper 复用；用 grep 符号名定位比固定行号更稳 |
| `SecretRedactor` / `AuditEvent` / `AuditSink` | [src/internal/ai/runtime/hardening.rs:378, 429, 451](../../src/internal/ai/runtime/hardening.rs) | 含可直接复用的 `TracingAuditSink`（:594） |
| `resolve_storage_root` | [src/command/code.rs:3268](../../src/command/code.rs) | 解析 `.libra/` 路径 |
...
- lease TTL 120 秒（沿用 [code_ui.rs:28](../../src/internal/ai/web/code_ui.rs) `DEFAULT_BROWSER_CONTROLLER_LEASE_SECS`）；写操作刷新 lease。
...
沿用 [hardening.rs:429](../../src/internal/ai/runtime/hardening.rs) `AuditEvent` 与 `TracingAuditSink`（[hardening.rs:594](../../src/internal/ai/runtime/hardening.rs)），但不要假设 `AuditEvent` 有 control 专属字段。新增一个内部 `ControlAuditRecord`，序列化成 redacted JSON 后写入 `AuditEvent.redacted_summary`；`AuditEvent` 的其它字段按下列规则填充：
...
- File: [src/internal/ai/web/code_ui.rs:69](../../src/internal/ai/web/code_ui.rs)
...
- File: [src/command/code.rs:428](../../src/command/code.rs) `CodeArgs`
- 新增字段：
  ```rust
...
- `impl CodeUiCommandAdapter`（trait 在 [code_ui.rs:647](../../src/internal/ai/web/code_ui.rs)）
- 内部持有：
...
- File: [src/internal/ai/web/code_ui.rs:734](../../src/internal/ai/web/code_ui.rs)
- 重命名 `BrowserControllerLease` → `ControllerLease { kind, client_id, token, expires_at }`（已落地，当前结构定义在 :734）。
...
- File: [src/internal/ai/web/mod.rs](../../src/internal/ai/web/mod.rs)
- `WebAppState` 加 `audit_sink: Arc<dyn AuditSink>`（启动时默认 `Arc::new(TracingAuditSink)`，复用 [hardening.rs:594](../../src/internal/ai/runtime/hardening.rs)）。
...
- 扩充 `SecretRedactor` markers（[hardening.rs:328-355](../../src/internal/ai/runtime/hardening.rs)）补 control token 模式（`control_token:` / `control-token:` / `x-code-controller-token:` / `x-libra-control-token:` 等已在 `default_runtime()` markers 列表中落地）。
...
| `docs/commands/code-control.md` | CLI 参考（✅ 已落地） | Phase 3 上线时合并 | 新 subcommand 用法、flag 表、JSON-RPC 2.0 method ↔ HTTP 映射、错误映射、最小 NDJSON 端到端示例。 |
| `docs/automation/local-tui-control.md` | 主指南（✅ 已落地） | 必交付 | 单一入口主文档，章节见下。 |
...
新增自包含一致性检查（**不新建 `scripts/`**——仿 `tests/compat/matrix_alignment.rs` 的去脚本化做法，写成 Rust 测试或内联 CI 步骤）或 PR checklist 项，至少覆盖：
...
# Code UI 路由↔docs 一致性已由 tests/compat/matrix_alignment.rs 覆盖（无 scripts/）：
cargo test --test compat_matrix_alignment docs_consistency_covers_code_ui_router_matrix
# 或人工跑：
...
## Decisions And Follow-Up Triggers

下列事项在 v1 中不再是开放问题；实施时必须按本节执行。若代码事实或用户需求推翻任一决策，先更新本文与 PR 描述，再调整实现，不能在代码里静默改变契约。

1. **Lease slot 共享 vs 分离**：v1 采用单 slot（同一时刻一个 writer，简化 takeover）。如需 browser-观察 + automation-写并存，必须开 v2 follow-up。
2. **`/control reclaim` 二次确认**：v1 不加二次确认（同机用户已物理控制键盘）。
3. **control token rotate**：v1 不提供在线 rotate；每次进程启动生成新 token 并覆盖旧文件，退出清理 best-effort。
4. **Windows 权限**：v1 unix-only enforce 0600；windows 上仅 best-effort（NTFS ACL 复杂，必须登记 follow-up issue）。
5. **observe SSE 节流**：v1 不加自定义节流；依赖 broadcast channel 默认 backpressure。若出现 lag/CPU 问题，先补可观测测试再调节流策略。
...
| 日期 | 作者 | 变更摘要 |
|------|------|----------|
| 2026-05-09 | Codex | 计划可信度复核：将 “Open Questions” 改为 v1 决策与后续触发条件，避免实施者把已定范围当作可自由选择；同时修正 Part C 中相对源码链接，所有 `src/` 链接从本文位置指向仓库根。 |
