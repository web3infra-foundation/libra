...
| CP-S2-2 Contracts gate | CP-4 + CEX-S2-10 + CEX-S2-11 完成 | `cargo +nightly fmt --all`、`cargo clippy --all-targets --all-features -- -D warnings`、`cargo test ai_subagent_contract`、`cargo test ai_subagent_workspace`、`cargo test --all` **字节级等价于 CEX-00 commit `48ea0ae`**（**注意**：此时 `code.sub_agents.enabled` 标志尚未引入——CEX-S2-12 才创建该标志，所以这里检查的是 “新类型 / workspace 抽象引入后行为不变”，**不是** flag-off 等价）；CEX-00 baseline 7 个测试逐一通过 | 通过后才能开 sub-agent runtime（CEX-S2-12）；新 schema / workspace 抽象不能产生任何主路径副作用 |
...
#    cannot leak any .libra/* state from the user shell or between baseline and
#    candidate.  The per-run home is then itself an artifact: any flag-off side
#    effect lands in $HOME_RUN/.libra and must be byte-equal across runs.
run_artifacts() {
  local TREE="$1" BIN="$2" OUT="$3" CFG="$4"
  mkdir -p "$OUT" "$OUT/home_test/.config" "$OUT/home_smoke/.config"

  # 4a. cargo test --all (isolated $HOME) — captures exit code, full content, failed set
  (
    cd "$TREE" && \
  ...
  # 4b. normalize EVERY captured text artifact (stdout AND stderr, all suites)
  #     so byte-level diff sees stable content, not raw output with timestamps,
  #     durations, UUIDs, etc. line-count diff alone is insufficient.
  "$TREE/scripts/normalize_session.sh" "$OUT/cargo_all.stdout" > "$OUT/cargo_all.stdout.normalized"
  "$TREE/scripts/normalize_session.sh" "$OUT/cargo_all.stderr" > "$OUT/cargo_all.stderr.normalized"

  # 4c. baseline 7 + code_test 4 with isolated $HOME — ALSO normalized
  (
    cd "$TREE" && \
    HOME="$OUT/home_test" XDG_CONFIG_HOME="$OUT/home_test/.config" \
    LIBRA_CONFIG_OVERRIDE="$CFG" \
  ...
  # 4d. mock-driven smoke session (separate $HOME so it does not contaminate
  #     the cargo-test home)
  HOME="$OUT/home_smoke" XDG_CONFIG_HOME="$OUT/home_smoke/.config" \
    ...
    > "$OUT/smoke.stdout" 2>&1 || true

  # 4e. normalize EVERY artifact dir (test home + smoke home) so any flag-off
  #     side effect anywhere in .libra/* shows up in the diff
...
# 5. compare every captured artifact (full normalized content, both streams)
...
diff -u "$WORK/baseline/cargo_all.stdout.normalized"    "$WORK/candidate/cargo_all.stdout.normalized"
diff -u "$WORK/baseline/cargo_all.stderr.normalized"    "$WORK/candidate/cargo_all.stderr.normalized"
...
任一 `diff` 非空、任一 `cargo build` 失败、normalizer 不存在或 fixture 不存在都视为 CP-S2-3 失败。CEX-S2-12 必须把 `scripts/normalize_session.sh`、`tests/data/flag_off_smoke.json` 与上面的比较脚本一并入库（参见 CEX-S2-12 Write set），不能依赖手写脚本。**字节级等价的强制点是步骤 5 的 7 个 `diff -u`——其中 `cargo_all.stdout.normalized` / `baseline7.stdout` / `code4.stdout` / `*.libra.normalized` 都是完整内容比较，不是 wc -l 摘要**；任何主路径 / `.libra` 副作用都会被 normalizer 抹掉非确定性字段后呈现在 diff 中。
...
| S2-INV-13 | sub-agent tool dispatch 经过 hook 拦截点 | 在 sub-agent dispatch tool 调用前必触发 `PreToolUse` hook，dispatch 后必触发 `PostToolUse` hook；exit code → 决策映射严格按 [Step 2.2 Hook exit-code 权威映射表](#step-22isolated-workspace--tool-boundary)（覆盖 9 种终止状态，fail-closed 默认）；该接入点不可被任何 sub-agent / capability package 绕过 | 可以 |
...
| `AgentRunEvent[E]` | Event | started / tool-call / blocked / completed / failed / cancelled / timed-out / **hook_passed** / **blocked_by_hook** / **hook_requested_human** / **blocked_by_hook_failure**（含 `reason` 字段：`unknown_exit_code` / `panic` / `timeout` / `killed_by_signal:<signo>` / `spawn_enoent` / `spawn_eacces` / `needs_human_timeout`） / **post_tool_review_required** / **workspace_materialized**（CEX-S2-11 输出）。**所有变体都按 unknown-event-safe 序列化** —— 旧 reader 跳过未知 variant，新增 variant 不破坏 replay |
...
- **Hook 物理拦截层**（与 sandbox / approval 并联，不替代）：
  ...
  **Hook exit-code 权威映射表（Step 2 / Step 3 共享，不可单方面修改）：**
  ...
  | exit `2` / `3` 且 stdout 为空 | 仍按 exit code | `deny` / `needs-human`；缺少 reason 时记 `reason=unspecified`，UI 提示 “hook 未给出原因” | 同 exit 2/3 行 | 同对应 exit code (`reason=unspecified`) |
  ...
  **needs-human (exit 3) Approval 集成约束**（exit 3 唯一进入 Approval 路径，其他路径不命中 cache）：
  ...
  - **No-cache 名单**：黑名单命令（Step 1.1）、protected branch 操作、`needs_human` 但 `hook_reason` 含 `secret_leak` / `cred_exposure` 关键字时，**永不写入 ApprovalTTL cache**——每次都重新询问。
...
- PreToolUse hook 的 exit code 严格按 [Step 2.2 权威映射表](#step-22isolated-workspace--tool-boundary)处理（dispatch-blocking 语义只适用于 PreToolUse；PostToolUse 见下方分相说明）；测试覆盖 **10 个终止状态**（与表中行数一致）：exit 0 / exit 0 + 空 stdout / exit 2 / exit 3 / exit 1（unknown）/ panic / timeout / SIGKILL / spawn_enoent / spawn_eacces，每条断言 `AgentRunEvent` 类型与 `reason` 字段。
...
|------|------|---------------|----------------|
| 兼容性 | sub-agent flag 关闭时，Step 1 单 Agent 行为和测试保持不变（与 CEX-00 commit `48ea0ae` 字节级一致） | CEX-S2-12（flag-off regression） | **CP-S2-3（真实 flag-off 等价）**；CP-S2-2 仅保证 schema-only 字节级等价（标志尚未引入），不视为本行验收依据 |
