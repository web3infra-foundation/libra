...
- Layer 1 负责（**注意：apply-to-main 路径属于 CEX-S2-13，不是本 Step**）：
  ...
  - 决定 accept/reject 调用 Runtime API 的入口（本 Step 完成）；**`Decision[E]` / `MergeDecision[E]` schema 字段形状（`risk_score` / `conflict_list` / `test_evidence` / `distillable_evidence_ids` / `MergeCandidate.review_evidence`）由 CEX-S2-13 owns**——CEX-S2-12 / CEX-S2-15 都不允许 mutate 字段名/类型，只填值；CEX-S2-12 完成时 schema 字段在 stub 形态（`Option<MergeDecisionV0>`），CEX-S2-13 后用真实 schema 替换
