...
14. **Hook fail-closed**：sub-agent dispatch 时 PreToolUse hook panic / 30s timeout / 异常 exit code 三种异常各跑一次，全部按 deny 处理；任何 capability package 或 sub-agent profile 试图关闭 hook 拦截时加载失败。
