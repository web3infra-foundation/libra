...
| `src/internal/ai/runtime/phase1.rs` | Phase 1 Plan 的 formal write helper（`write_plan_set` / `advance_scheduler`） | **`write_plan_set` 函数体已落地（v0.17.580）**：模块存在，提供 `PlanWriteOutcome { execution_plan_id, test_plan_id, plan_id_by_task_id }` 类型 + `ordered_plan_ids()` helper；`pub async fn write_plan_set(mcp_server, intent_id, parent_exec, parent_test, plan) -> Result<PlanWriteOutcome, OrchestratorError>` 已通过 `orchestrator::persistence::write_plan_set_with_outcome(...)` 桥接接入真实 plan-set 持久化路径（`create_plan_revision_for_role` × 2 + `build_plan_set`）。`advance_scheduler` 函数体仍待 Wave 1B follow-up；2 个单测覆盖 outcome 字段 + execution/test 顺序合同 | 中（Wave 1B impl-body follow-up） |
