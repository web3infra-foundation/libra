...
| Phase 5 Security / Permission / Diagnostics / Hardening | 100% | ~35% | `mcp/authz.rs` 缺失；TUI direct provider bootstrap 与 Ollama headless web-only bootstrap 已收敛到 `ProviderFactory`，但 provider crate `from_env()` 保留策略和 vault/env 统一接入仍未收口 |
