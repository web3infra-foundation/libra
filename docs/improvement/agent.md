...
| `src/internal/ai/runtime/revision.rs` | 跨 phase 的 revision chain helper | **Schema + 派生 helper + modify-request 协调器 + plan/test-plan 配对校验已落地（v0.17.577 schema、v0.17.584 derive_next_revision_skeleton、v0.17.587 handle_modify_request、v0.17.595 validate_paired_plan_revisions）**：模块存在，提供 `RevisionKind { Intent, ExecutionPlan, TestPlan }` enum + `.label()` 跨 phase 审计标签 + `RevisionChainEntry { kind, previous_id, revision, logical_id }` 类型 + `is_first()` / `is_continuation()` 链头/续接判断；`pub fn derive_next_revision_skeleton(previous, previous_persisted_id) -> RevisionChainEntry` 纯函数；`pub struct ModifyRequest { kind, logical_id, reason }` 修改请求 envelope；`pub enum ModifyRequestError { KindMismatch, LogicalIdMismatch }`；`pub fn handle_modify_request(previous, previous_persisted_id, request)` 协调器；`pub fn validate_paired_plan_revisions(execution, test) -> Result<(), PairingError>` 跨链 lockstep 校验（rule 3：(n)-th plan 必须与 (n)-th test-plan 配对，从不 (n-1) / (n+1)）+ `pub enum PairingError { NotExecutionPlan, NotTestPlan, RevisionMismatch }` 三种失败形态（kind 错误优先级：execution-side 在前，确定性 audit log）；15 个单测覆盖 label 稳定性、链位置标志、Clone 合同、`derive_next_revision_skeleton` × 2 路径、`handle_modify_request` happy path + KindMismatch + LogicalIdMismatch、`validate_paired_plan_revisions` happy path (任意 rev N) + first-link pair (rev 1) + drift (n-1, n+1) × 2 + NotExecutionPlan (Intent 占第一位) + NotTestPlan (Intent 占第二位) + execution-kind error priority (两侧均 Intent) + PairingError Clone/Eq 合同 | ✅ 已落地（revision-chain 协调层完整 + lockstep 校验） |
