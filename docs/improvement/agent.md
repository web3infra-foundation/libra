...
| Step 2.4 | CEX-S2-13 / S2-14 | Human-gated merge + controlled parallel | 部分落地：CEX-S2-13 schema / CEX-S2-18 read API 已存在；CEX-S2-14 scheduler-side controlled parallel admission 已新增 `agent_run::parallel`（max_parallel admission、普通队列、file-scope conflict queue、completion promotion、snapshot），runtime dispatch / Source Pool / usage trace wiring 仍待后续 |
...
| `src/internal/ai/runtime/revision.rs` | 跨 phase 的 revision chain helper | **Schema + 派生 helper + modify-request 协调器 + plan/test-plan 配对校验 + audit label 已落地（v0.17.577 schema、v0.17.584 derive_next_revision_skeleton、v0.17.587 handle_modify_request、v0.17.595 validate_paired_plan_revisions、v0.17.597 audit_label、v0.17.1179 revision overflow fail-closed）**：模块存在，提供 `RevisionKind { Intent, ExecutionPlan, TestPlan }` enum + `.label()` 跨 phase 审计标签 + `RevisionChainEntry { kind, previous_id, revision, logical_id }` 类型 + `is_first()` / `is_continuation()` 链头/续接判断 + `audit_label() -> String` 稳定单行 audit 标签（三种格式：head / continuation from / forked from）；crate-internal `derive_next_revision_skeleton(previous, previous_persisted_id) -> Result<RevisionChainEntry, RevisionDeriveError>` fallible 纯函数（checked-add；revision counter overflow 时不返回可持久化 skeleton）；`pub struct ModifyRequest { kind, logical_id, reason }` 修改请求 envelope；`pub enum ModifyRequestError { KindMismatch, LogicalIdMismatch }` 保持 validation-only exhaustive contract；crate-internal `RevisionDeriveError { RevisionOverflow }` + `HandleModifyRequestError { Validation, Derive }` 分离 overflow 报告；crate-internal `handle_modify_request(previous, previous_persisted_id, request)` 协调器（匹配 kind/logical_id 后调用 fallible derive，revision counter overflow 时拒绝；签名不作为 embedding API 承诺）；crate-internal `validate_paired_plan_revisions(execution, test) -> Result<(), PairingError>` 跨链 lockstep 校验（rule 3：(n)-th plan 必须与 (n)-th test-plan 配对，从不 (n-1) / (n+1)，签名不作为 embedding API 承诺）+ crate-internal `PairingError { NotExecutionPlan, NotTestPlan, RevisionMismatch }` 三种失败形态（kind 错误优先级：execution-side 在前，确定性 audit log）；21 个单测覆盖 label 稳定性、链位置标志、Clone 合同、`derive_next_revision_skeleton` × 3 路径（两条 happy path + u32::MAX overflow reject）、RevisionDeriveError Display pin、`handle_modify_request` happy path + KindMismatch + LogicalIdMismatch + RevisionOverflow + wrapper Display transparency、`validate_paired_plan_revisions` happy path (任意 rev N) + first-link pair (rev 1) + drift (n-1, n+1) × 2 + NotExecutionPlan + NotTestPlan + execution-kind error priority + PairingError Clone/Eq 合同 + `audit_label()` 三种链位置（head / continuation / forked）格式 pin + audit_label 以 kind label 起头 | ✅ 已落地（revision-chain 协调层完整 + lockstep 校验 + audit 标签） |
...
| CEX-S2-14 | Controlled parallel execution + scheduler observability state | CEX-S2-13、**CEX-14**（Source Pool 限流）、**CEX-16**（`agent_usage_stats` schema） | `src/internal/ai/projection/scheduler.rs`、`src/internal/ai/orchestrator/decider.rs`、`src/internal/ai/tools/handlers/mcp_bridge.rs`、`src/internal/ai/sources/`（CEX-14 输出）、`src/internal/ai/usage/`（CEX-16 Step 1 输出） | `src/internal/ai/projection/scheduler.rs`、`src/internal/ai/agent_run/parallel.rs`、`src/internal/ai/tools/handlers/mcp_bridge.rs`、`tests/ai_subagent_parallel_test.rs` | `cargo test --test ai_subagent_parallel_test`；`cargo test ai_code_ui_projection`；100 并发 Source-Pool 调用压测脚本 | **部分落地（scheduler-side core）**：`src/internal/ai/agent_run/parallel.rs` 已提供纯 admission 状态机与 `ParallelSchedulerSnapshot`，覆盖 disjoint write scope 并行、`max_parallel` 超限普通队列、overlap file-scope conflict queue、完成后 promotion；`tests/ai_subagent_parallel_test.rs` pin 住这些行为。(1)/(2)/(3) 的 scheduler 判定已落地；runtime dispatch 消费该状态机、Source Pool 限流复用、JSONL / `agent_usage_stats` trace id wiring、100 并发压测仍待后续切片。(6) **TUI agent pane 渲染不在本卡范围**（归 CEX-S2-16） |
