...
| 对象读取与树遍历 | `rev-parse`、`show-ref` / `show-ref --verify` / `show-ref --exists`、`cat-file`、`hash-object`、`show`、`rev-list`、`fsck`、sha256 object format；`ls-tree` 当前为缺口负向场景 | 2 | 已实现 | `cli.object-readback`, `cli.ls-tree-smoke`, `cli.sha256-object-readback` |
