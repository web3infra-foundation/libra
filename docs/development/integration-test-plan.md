...
| 对象读取与树遍历 | `rev-parse`、`show-ref` / `show-ref --branches` / `show-ref --hash[=<n>]` / `show-ref --abbrev[=<n>]` / `show-ref --dereference` / `show-ref --verify` / `show-ref --exists` / `show-ref --exclude-existing[=<pattern>]`、`for-each-ref --points-at`、`cat-file`、`hash-object --stdin` / `--path` / `--no-filters`、`show`、`rev-list` / `rev-list --count` / `rev-list -n` / `rev-list --skip` / `rev-list --merges` / `rev-list --no-merges` / `rev-list --min-parents` / `rev-list --max-parents` / `rev-list --parents` / `rev-list --timestamp`、`fsck`、sha256 object format；`ls-tree` 默认/递归/子目录/`--full-name`/`--full-tree` 路径场景 | 2 | 已实现 | `cli.object-readback`, `cli.show-ref-exclude-existing`, `cli.ls-tree-smoke`, `cli.sha256-object-readback` |
