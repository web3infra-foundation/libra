...
| 对象读取与树遍历 | `rev-parse`、`show-ref` / `show-ref --hash[=<n>]` / `show-ref --abbrev[=<n>]` / `show-ref --dereference` / `show-ref --verify` / `show-ref --exists`、`cat-file`、`hash-object`、`show`、`rev-list` / `rev-list --count` / `rev-list -n` / `rev-list --skip`、`fsck`、sha256 object format；`ls-tree` 当前为缺口负向场景 | 2 | 已实现 | `cli.object-readback`, `cli.ls-tree-smoke`, `cli.sha256-object-readback` |
