...
- 源码分层：主要实现文件为 `src/command/clean.rs`。参数/子命令类型包括：`CleanArgs`；输出、错误或状态类型包括：源码未暴露独立输出/错误类型，错误通过 `CliResult` 或上层命令错误统一传播；主要执行函数包括：`execute`、`execute_safe`。
...
- 2026-06-04 `afa5c40e`（`feat(clean): implement unit-testable interactive command loop state machine`）：功能演进：implement unit-testable interactive command loop state machine；该节点扩展了当前命令可用的参数或行为。
...
- Synopsis：`libra clean -n [-d] [-x | -X] [--exclude <pattern>]... [--json] [--quiet]`。
