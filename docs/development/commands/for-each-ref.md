...
- 兼容级别：`partial`。`--heads` / `--tags` / `--remotes` / `--all` / `--format` / `--sort`（`refname`/`objectname`/`version:refname`/`committerdate`/`authordate`/`creatordate`/`objectsize`/`*objectname`，均可加 `-` 反转）/ `--count` / `--points-at` / `--contains` / `--no-contains` / `--merged` / `--no-merged` / `--exclude` / `--shell` / `--perl` / `--python` / `--tcl`（互斥的输出引用模式）/ `<pattern>` supported; full Git atom language 的其余部分（如 `%(align)`/`%(if)` 与其它 `*`-deref atoms `%(*objecttype)` 等）are not exposed（`%(objectsize)` atom+`--sort=objectsize`、`%(*objectname)`/`%(*objectname:short)` atom+`--sort=*objectname` 已实现）
