...
`libra checkout` 的目标是保留 Git 兼容入口，同时把分支切换和文件恢复分别引导到 `switch` 与 `restore` 的清晰模型。实现需要覆盖可见的分支兼容面、`checkout -- <path>` 恢复别名、冲突阶段文件恢复，以及对 detached HEAD、orphan 和 patch 模式的部分支持说明。
...
- 源码分层：主要实现文件为 `src/command/checkout.rs`。参数/子命令类型包括：`CheckoutArgs`；输出、错误或状态类型包括：源码未暴露独立输出/错误类型，错误通过 `CliResult` 或上层命令错误统一传播；主要执行函数包括：`execute`、`execute_safe`。
...
- Synopsis：`libra checkout [<branch>]`。
