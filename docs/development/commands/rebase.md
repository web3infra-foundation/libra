...
| 类别 | 未完成项 | 当前处理 |
|---|---|---|
| 兼容矩阵说明 | `--onto`/`--autosquash`/`--reapply-cherry-picks`/`--no-autostash`(no-op)/`--no-rerere-autoupdate`(no-op) 已支持；interactive/`--rebase-merges`/`--autostash`/`--rerere-autoupdate` 未支持 | 按当前兼容矩阵保留；实现状态变化时同步 `_compatibility.md` 和测试证据。 |
