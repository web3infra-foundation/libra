...
| 功能缺口 | 标准 Git clone 参数 `--filter <spec>`、`--reference <repo>`、`--dissociate`、`--shared`、`--mirror`、`--shallow-since <date>`、`--shallow-exclude <rev>` 均不在 `CloneArgs`（`src/command/clone.rs`）中，当前未实现 | 后续实现时需要同步源码、测试和兼容矩阵。 |
