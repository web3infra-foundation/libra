...
- 兼容级别：`partial`。创建、列出、删除、重命名、复制（`-c`/`-C`/`--copy`，连同上游配置，保留源分支）、上游设置/清除、contains/no-contains、points-at、merged/no-merged、`--sort`（`refname`/`version:refname`，可加 `-` 反转）、ignore-case、`--column[=<always|auto|never>]`（列式列表布局）和 `-v`/`--verbose`（每个分支附带 tip sha 与提交 subject；`-vv` 接受但 Git 的上游 tracking 细节尚未显示）已支持；描述编辑、自定义 `--format`、其余 sort key（如 creatordate）尚未公开。
...
| ✅ 已实现（部分） | 详细列表 `-v` / `--verbose` | `branch_verbose_suffix` 在 List 输出追加 ` <短sha> <subject>`；`-vv` 接受但渲染同 `-v`，Git 的 `-vv` 上游 tracking（`[<upstream>: ahead N, behind M]`）尚未显示（可后续复用 `status::compute_ahead_behind`）。带集成测试（`branch_verbose_shows_sha_and_subject`）。 |
