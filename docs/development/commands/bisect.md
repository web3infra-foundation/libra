...
`libra bisect` 的目标是用二分法定位引入问题的提交，并提供 `start`、`bad`、`good`、`skip`、`run`、`reset`、`log`、`view` 等子命令。实现需要把状态持久化到仓库数据库，支持多 good 起点和路径规格预检，同时在不支持的 Git 扩展上给出明确拒绝。
...
- 2026-06-03 `9c0249d5`（`feat(bisect): accept multiple good commits as positional args to start (v0.17.1284)`）：功能演进：accept multiple good commits as positional args to start (v0.17.1284)；该节点扩展了当前命令可用的参数或行为。
...
- 公开参数/子命令包括：`Subcommand: `start`、`Subcommand: `bad`、`Subcommand: `good`、`Subcommand: `reset`、`Subcommand: `skip`、`Subcommand: `log`、`Subcommand: `run`、`Subcommand: `view`。
