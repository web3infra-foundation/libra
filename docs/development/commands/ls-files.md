...
`libra ls-files` 提供公开的 Git 兼容索引/工作树路径列举入口。当前目标是覆盖常用脚本和 AI 安全只读场景：缓存索引列表、已修改/已删除筛选、stage 样式输出、未跟踪文件列举、`.libraignore` 感知过滤、pathspec、`--error-unmatch`、`-z` 文本输出，以及标准 JSON / machine envelope。

## 对比 Git 与兼容性

- 兼容级别：`partial`。
- 已支持：默认 cached listing、`--cached`、`--deleted`、`--modified`、`--stage` / `-s`、`--others`、`--others --exclude-standard`、`<pathspec>...`、`--error-unmatch`、`-z`、`--json` 和 `--machine`。
