...
`libra describe` 的目标是根据可达 tag 为提交生成可读名称。当前实现刻意保持小而可验证的 Git 子集：支持 `[COMMIT]`、`--tags`、`--abbrev <N>`、`--always`、`--exact-match`、`--dirty[=<mark>]` 与 JSON 输出，并在无法描述时给出稳定错误；first-parent、候选数量、match/exclude pattern 等 Git 扩展尚未实现。

## 对比 Git 与兼容性

- 兼容级别：`partial`。基础 describe、`--tags`、`--always`、`--abbrev`、`--exact-match` 和 `--dirty[=<mark>]` 已支持；long/match/exclude/first-parent/contains/candidates/all 尚未公开。
...
- 当前 `src/command/describe.rs` 实现 `[COMMIT]`、`--tags`、`--abbrev <N>`、`--always`、`--exact-match`、`--dirty[=<mark>]` 与 `--json` 输出，基于一次有界 BFS 查找可达 tag；`--first-parent`、`--long`、`--contains`、`--candidates`、`--all`、`--match`、`--exclude` 尚未在当前代码中实现（参见 `docs/commands/describe.md` 中“Why no `--long`/`--match`/`--exclude`/`--first-parent`”的设计说明）。
...
- 公开参数/子命令包括：`[COMMIT]`、`--tags`、`--abbrev <N>`、`--always`、`--exact-match`、`--dirty[=<mark>]`。
