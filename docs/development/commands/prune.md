...
- 源码分层：主要实现文件为 `src/command/prune.rs`。参数/子命令类型包括：`PruneArgs`；输出、错误或状态类型包括：源码未暴露独立输出/错误类型，错误通过 `CliResult` 或上层命令错误统一传播；主要执行函数包括：`execute`、`execute_safe`。
