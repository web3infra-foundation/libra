...
`libra check-ignore` 的目标是：对一组路径，按当前 `.libraignore` 规则判定哪些被忽略，并在 `-v` 下给出做出判定的来源文件、行号与模式。它是只读查询，不修改 index 或工作树，对齐 `git check-ignore` 的退出码契约（0=有路径被忽略、1=无、128=用法/仓库错误）。

## 对比 Git 与兼容性

- 兼容级别：`partial`。
- 有意差异：规则来自 Libra 的 `.libraignore`（不是 `.gitignore`），匹配引擎/优先级/模式语法与 Git 相同（同一 `ignore` crate 引擎）。
...
- 行号说明：底层 `ignore` crate 的 `Glob` 暴露 `from()`（来源文件）与 `original()`（模式原文）但**不暴露行号**；`-v` 的行号由 `util::find_pattern_line` 重新扫描来源 `.libraignore`、取**末个**去空白后等于该模式的非注释非空行（单文件内最后命中的模式才是裁决规则；best-effort，找不到则省略）。
...
- 源码分层：
  ...
  - `src/utils/util.rs`：`check_gitignore_match(work_dir, target) -> Option<IgnoreMatchInfo>` —— 与 `check_gitignore`（返回 bool）共享同一遍历顺序（最近的 `.libraignore` 优先、单文件内最后命中的模式胜出），但返回做出判定的 glob 的 `source/line/pattern/ignored`，保证判定结论与 `check_gitignore` 永不分叉。`IgnoreMatchInfo`、`glob_match_info`、`find_pattern_line` 为配套类型/函数。
- 执行路径：
  ...
  5. 对每个路径：解析为绝对路径后用 `normalize_lexical` 折叠 `.`/`..`（不触碰文件系统），再校验仍位于 worktree 内——逃出 worktree 的路径是致命错误（exit 128，对齐 Git；并规避 `check_gitignore_match` 的 `starts_with` 断言、防止读取 worktree 外的 `.libraignore`）。index 跟踪查询用归一化后的 repo 相对 key（绝对/相对输入一致）。`--stdin` 读取上限 64 MiB（超限 128）。
...
- 底层操作对象：`.libra/index`（`Index::load`，跟踪状态查询）、worktree 路径、`.libraignore` 匹配引擎（经 `util::check_gitignore_match`）。无对象库/refs/网络写入。
...
| 类别 | 未完成项 | 当前处理 |
|---|---|---|
| 兼容差异项 | `--exclude` / `--exclude-from` / `--exclude-per-directory` | 不公开；命令仅查询 `.libraignore`，命令行附加排除模式后续按需再评估并同步矩阵与测试。 |
