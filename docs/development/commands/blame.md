...
`libra blame` 的目标是按行展示文件内容的最近修改提交、作者和时间信息。实现需要支持常用显示字段（`-l`/`-s`/`-t`/`--abbrev`/`-e`）、porcelain 输出和行号范围等兼容面，同时把空白忽略（`-w`）、移动/复制检测、反向 blame、incremental 等高阶能力列为后续工作（当前 HEAD 未实现）。
...
| ✅ 已实现 | 显示标志 `-l`/`-s`/`-t`/`--abbrev`/`-p` | `-l` 完整 hash；`--abbrev=<n>` n 位缩写（`-l` 优先）；`-s` 隐藏作者/日期列；`-t` 原始 epoch 时间戳；`-p` 为 `--porcelain` 短别名。复用现有 `BlameLine` 字段（新增 `timestamp` JSON 项）；仅人类格式。带集成测试（`test_blame_display_flags_long_suppress_timestamp_abbrev`）。 |
