...
`libra index-pack` 的目标是为已有 pack 归档构建或校验 pack index，是对象传输与 pack 处理的隐藏 plumbing 能力。实现需要继承 SHA-256、pack index handling 和错误码治理，同时把 stdin、thin pack 和进度输出列为未完成差异。

## 对比 Git 与兼容性

- 兼容级别：`partial`。hidden plumbing command；pack file 到 idx 的基础路径已支持，`--keep[=<MSG>]` 已支持；stdin/fix-thin/progress flags 尚未公开。
...
- 执行路径：`execute_safe` 负责 CLI 安全包装、错误映射和输出配置；核心实现读取 pack 文件、解析对象 metadata、计算 fanout/offset/checksum，并按 pack index v1/v2 规则写出 `.idx`；启用 `--keep[=<MSG>]` 时，在 `.idx` 构建成功后写出同名 `.keep` 文件。
...
- 公开参数/子命令包括：`<PACK_FILE>`、`-o <INDEX_FILE>`、`--keep[=<MSG>]`、`--index-version <INDEX_VERSION>`。
