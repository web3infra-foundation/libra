...
`libra rev-list` 的目标是列出从 revision 可达的提交对象。实现需要支持多 revision、排除语法、A..B/A...B 范围、数量/跳过/计数、父提交过滤、parents 和 timestamp 输出，为上层历史命令提供稳定 plumbing。
...
- 源码分层：主要实现文件为 `src/command/rev_list.rs`。参数/子命令类型包括：`RevListArgs`；输出、错误或状态类型包括：源码未暴露独立输出/错误类型，错误通过 `CliResult` 或上层命令错误统一传播；主要执行函数包括：`execute`、`execute_safe`。
...
- 2026-06-06 `d288b5f7`（`feat(rev-list): multi-spec, ^/A..B/A...B ranges, -n/--skip/--count, parent filters, --parents/--timestamp`）：功能演进：multi-spec, ^/A..B/A...B ranges, -n/--skip/--count, parent filters, --parents/--timestamp；该节点扩展了当前命令可用的参数或行为。
