...
> 本文件是 `libra agent`（外部 Agent 捕获子系统）的**公共 CLI/行为事实源**，已从 [`docs/development/code-agent-runtime.md`](../code-agent-runtime.md) 拆入 external-agent 捕获需要的 entireio/cli 对齐契约、AG-16~AG-24 任务卡、E1–E10 wire contract、checkpoint/export、review/investigate 与验收命令。`docs/development/code-agent-runtime.md` 继续负责内部 AgentRuntime、Web-only 迁移、MCP/control-plane 边界和 Gate 8 的总览追踪；两份文档任一处修改 public surface、E 契约、AG-16~AG-24 或有意差异时，必须同一 PR 同步。
...
- **transcript**：外部 agent 自己产生的原始会话文件（可能很大、含敏感信息），落盘前必经 redaction，默认 metadata-first 读取，完整正文走显式 detail/raw 路径。
...
- **Session / checkpoint 捕获与诊断**：`agent_session`、`agent_checkpoint`、`agent_usage_stats`、`.libra/sessions/agent/`、`refs/libra/traces` 和 `object_index` 共同构成外部捕获事实源。`session list/show` 与 `checkpoint list/show` 默认 metadata-first；读取 transcript、prompt、context、stderr、raw payload 必须走显式 detail/raw 路径，并受 size cap、streaming/chunk、redaction、audit 约束。`clean`、`doctor`、`push`、`rewind` 只处理这些外部捕获对象，不扩大为通用 Git/工作区维护命令。
...
4. **停用与移除**
   ...
   `libra agent disable --agent claude-code` / `remove claude-code` 只能删除 `libra hooks claude ...` managed entries；不得删除用户自定义 hook、`.claude/settings.json` 中其它配置或已捕获的 `agent_session` / `agent_checkpoint` / `refs/libra/traces` 数据。停用后 capability matrix 必须显示 `supported=true`、`hook_installable=true`、`installed=false`。
...
**总体结论**：文档方向、边界和落地门禁均已达到“合理”评级；当前实现仍有安全边界与净新建能力待落地，但这些不再是方案缺口，而是 AG-18/AG-20/AG-22/AG-23/AG-24 的明确交付项。后续改进重点是按本文门禁补齐实现证据（schema 版本、env fail-closed、provenance、crash recovery、分页、合规），并通过测试与文档守卫防止「有意差异」退化为「过时」。
...
| 可行性 | 满足。AG-16~AG-24 已拆分依赖、降级形态和 hard prerequisite，read-only 与 mutating fix 分层清楚。 | PR DoD 标明 read-only 或 fix-ready；`docs/development/code-agent-runtime.md` 给出 serialized fix bridge 源码锚点前，`--fix` 测试必须走 `LBR-AGENT-009` 或等价 stable unsupported。 |
| 完整性 | 满足。主路径、settings、migration、错误码、release/rollback、测试 target 已被列为 Gate 8 输入。 | AG PR checklist 包含 docs/tests/compat/release notes/rollback；AG-24 逐行核对“落地执行补充规格”和测试矩阵，没有证据的项必须写延后原因与重启条件。 |
...
4. provider-native envelope 可在测试 fixture 中保留，但生产持久化默认禁止。需要 raw/detail 时必须走 `--allow-raw` + audit log，且 raw export 不写回 `traces`。
...
- **trust / quarantine / provenance（AG-18 必落地）**：`agent.external_agents.enabled` 默认 `false`；未知 `libra-agent-*` 只 quarantine（`quarantined:true` + reason），不注册、不 invoke（`LBR-AGENT-001`）；`libra agent rpc trust` 记录 path/sha256/device/inode/mtime；hash 或 inode 变化重新 quarantine（`LBR-AGENT-004`）；built-in slug 冲突 skip-and-log（`LBR-AGENT-005`）。
- protocol version mismatch fail closed（`LBR-AGENT-002`）；discovery 整体 fail-open=skip-and-log；`rpc invoke` 不得调用未声明 method（`LBR-AGENT-003`）；env/stderr/redaction 安全失败（`LBR-AGENT-006`）。
...
- invalid envelope / path / UTF-8 校验失败 fail-closed（`LBR-AGENT-007`），不回显 raw stdin。
...
- **manifest-relative chunk paths**：分片 OID/路径必须在 `manifest.json` 中列出 logical role（如 `transcript/claude-code.jsonl.001`），`doctor`/export/`--transcript` 只通过 manifest 解析，不得硬编码 entire 式 `N/full.jsonl.%03d` 路径。
...
- ⚠️ **fix-bridge 前置**：上述「内部 AgentRuntime serialized fix 入口」当前**无源码锚点**。AG-22 启动前须由 `docs/development/code-agent-runtime.md` 给出源码锚点；未就绪时 AG-22/AG-23 只能交付 read-only（`LBR-AGENT-009`）。
...
- untrusted seed 进入 mutating workflow 须显式 flag（`LBR-AGENT-010`）；read-only 路径默认拒绝。
...
### E10：稳定错误码（fail-closed 目录）

所有 E2/E3/E4-libra/E8-libra fail-closed 路径须映射到 `StableErrorCode`（见强制补强项 #8 与「落地执行补充规格 §4」）。种子目录：

| 错误码 | 触发条件 | 主要 AG |
|---|---|---|
| `LBR-AGENT-001` | external agents 未启用但请求 invoke/trust | AG-18 |
| `LBR-AGENT-002` | protocol version mismatch | AG-18 |
| `LBR-AGENT-003` | undeclared capability / method | AG-18 |
| `LBR-AGENT-004` | provenance 拒绝或 hash 变化 | AG-18 |
| `LBR-AGENT-005` | built-in slug impersonation | AG-18 |
| `LBR-AGENT-006` | env/stderr/redaction 安全失败 | AG-18/AG-19 |
| `LBR-AGENT-007` | hook envelope UTF-8/JSON/schema/path 校验失败 | AG-19 |
| `LBR-AGENT-008` | checkpoint ref/DB/object/object_index 写入或恢复失败 | AG-20 |
| `LBR-AGENT-009` | review/investigate `--fix` bridge 未就绪 | AG-22/AG-23 |
| `LBR-AGENT-010` | untrusted seed 请求进入 mutating workflow | AG-22/AG-23 |

实际编号以 `docs/error-codes.md` 为准；新增/重命名须同步 `compat_error_codes_doc_sync`。
...
> **核对基线（2026-06-16）**：本章基于 `/Volumes/Data/entireio/cli`（Go 实现的 entire CLI，功能参考）与 `/Volumes/Data/entireio/cli-checkpoints`（entire 落盘的 checkpoint 归档）的**当前**源码逐项核对，并对照 Libra 现仓库实现。每条差距都映射到本文 AG-16~AG-24 与上文 E1–E10 wire 契约。旧 d0a714 分析版的 10 节结论已并入此处并升级为 source-grounded 形态；旧分析里引用的 `/run/media/eli/...` 路径与"9 agents"等口径以本章为准。
...
- 所有 fail-closed RPC 失败须映射 **E10** 稳定错误码（`LBR-AGENT-001`~`006`），错误消息含 binary path/slug/method 与可操作下一步。
...
- 建立 central validation：session id/provider session id 非空；tool/subagent event 必须有对应 id；`cwd` 可信；path 不得 traversal；unknown agent kind quarantine（**E3-JSONL** / `LBR-AGENT-007`）。
...
- 实现 `AgentReviewWorkflow`（**E8-entire** 语义 + **E8-libra** run wire）：多 reviewer 并发，事件 fan-in 到单一 dispatcher；sink serial dispatch 且不得阻塞；findings 写入 `.libra/sessions/agent-runs/<run_id>/`（`state.json`、`findings.md`、`manifest.json`、redacted reviewer logs）。read-only review 是 AG-22 的最低可交付形态；`--fix` 只有在内部 AgentRuntime serialized fix bridge 已有源码锚点且 approval/sandbox/tool gate 测试通过后才可启用（否则 `LBR-AGENT-009`）。
- 实现 `AgentInvestigateWorkflow`：**E8-entire** strict round-robin + **E8-libra** `state.json`/`manifest.json` 持久化 run id/topic/agents/quorum/max_turns/next_agent_idx/pending_turn/stances/findings_doc/starting_sha；支持 continue/show/clean。read-only investigate 是 AG-23 的最低可交付形态；fix 子路径与 AG-22 同一前置（`LBR-AGENT-009`/`LBR-AGENT-010`）。
...
- 补合规 closeout：transcript/prompt/context/stderr/review findings 的 retention、GC、本地删除一致性、raw export 显式授权、**redaction_report** 可审计字段必须进入用户文档或运维文档；只有在 AG-19/AG-20 引入并启用 D1/R2 mirror 后，才把 cloud restore/delete tombstone 作为发布门禁。
- 交付前复核上文「独立分析结论（11 维度）」唯一表与 **「评级门禁闭环核对」** 证据清单；所有「必须落到实施的门禁 / 证据」要么有测试和文档证据，要么在 release notes 中明确延后原因和重启条件；AG-24 须逐行核对「落地执行补充规格」§1~§10。
...
> 本节把上文评审建议转成执行附件。AG-16~AG-24 任一 PR 若触达对应能力，必须把本节相关行作为 PR checklist；不能只引用任务卡标题后直接实现。
...
| AG-24 audit | 新增 `agent_audit_log` append-only 表 | cloud mirror 启用时可同步不可变 audit 摘要 | down 不得删除审计数据；只能停止新写入 | raw export 写 audit；clean/GC 不删 audit |
...
| 错误码（暂定） | 触发条件 | 默认退出码 | 输出要求 | 文档/测试 |
|---|---|---:|---|---|
| `LBR-AGENT-001` | external agents 未启用但请求 invoke/trust 外部 binary | 129 | 提示启用 setting 或 trust 命令 | `docs/error-codes.md` + AG-18 |
| `LBR-AGENT-002` | protocol version mismatch | 128 | 包含 slug、binary path、expected/actual version | AG-18 protocol fixture |
| `LBR-AGENT-003` | undeclared capability / method | 128 | 包含 method、capability、next step | AG-18 capability fixture |
| `LBR-AGENT-004` | provenance 拒绝或 hash 变化 | 128 | 包含 redacted path、sha256 摘要、trust 指令 | AG-18 provenance fixture |
| `LBR-AGENT-005` | built-in slug impersonation | 128 | 包含 slug 与 external path；说明 built-in 不可覆盖 | AG-18 PATH conflict fixture |
| `LBR-AGENT-006` | env/stderr/redaction 安全失败 | 128 | 不输出 raw stderr/prompt；只给 redacted summary | AG-18/AG-19 redaction |
| `LBR-AGENT-007` | hook envelope UTF-8/JSON/schema/path 校验失败 | 128 | 不回显 raw stdin；说明 provider/verb | AG-19 hook tests |
| `LBR-AGENT-008` | checkpoint ref/DB/object/object_index 写入或恢复失败 | 128 | 包含 checkpoint_id、缺失对象类别、doctor 建议 | AG-20 crash tests |
| `LBR-AGENT-009` | review/investigate `--fix` bridge 未就绪 | 129 | 明确 read-only 可用、fix 延后条件 | AG-22/AG-23 unsupported tests |
| `LBR-AGENT-010` | untrusted seed 请求进入 mutating workflow | 129 | 提示显式 approval/flag；标 provenance | AG-22/AG-23 seed tests |

实际落地时错误码编号以 `docs/error-codes.md` 为准；本表是分配前的占位清单。新增、重命名或删除任何错误码必须同步 `StableErrorCode`、`docs/error-codes.md`、JSON snapshot 和 `compat_error_codes_doc_sync`。
...
libra review --fix ...   # fix bridge 未就绪时返回 LBR-AGENT-009
```
...
libra investigate fix <run_id>   # fix bridge 未就绪时返回 LBR-AGENT-009
```
...
| internal AgentRuntime fix bridge | `docs/development/code-agent-runtime.md`、本文 | AG-22/AG-23 fix bridge or unsupported tests |
| test target add/rename | `Cargo.toml`、`tests/INDEX.md`、本文验收命令 | target itself must be runnable |

AG-24 closeout 必须逐行核对本矩阵。若某项未同步，PR 不能声明 Gate 8 完成；只能声明对应 AG 的局部实现。
...
| roster | `STABLE_AGENT_SLUGS`=`claude-code`/`gemini`（`mod.rs:216`） | 与第一批 `claude-code`/`codex`/`opencode` 不一致 | 加 Codex/OpenCode `HookProvider`，gemini 降级 unsupported（保留 read-only adapter，不 installable） | AG-16/AG-19 |
...
- 改 `mod.rs:216` `STABLE_AGENT_SLUGS`：`claude-code`/`gemini` → `claude-code`/`codex`/`opencode`；新增按 `AgentKind` 索引的静态 capability matrix（`slug/agent_kind/db_value/supported/support_wave/registered/transcript_readable/hook_installable/installed/launchable_review/launchable_investigate/external_binary/config_paths/capabilities`，见「第一批支持项目」）。gemini/cursor/copilot/factory-ai → quarantine/unsupported。
...
- list JSON 字段冻结：`slug/agent_kind/stability/hook_installable/hooks_installed/transcript_readable/external_binary`（+ roster 段的 supported/support_wave/launchable_*）。
- 测试：`command_test::{agent_list_add_remove_aliases_parse, agent_list_json_contains_capability_fields, agent_add_non_hook_installable_returns_actionable_unsupported}`。
...
- 新增 Codex / OpenCode `HookProvider`（新 builtin adapter + `as_hooks()`）：Codex 写 `.codex/hooks.json`+`.codex/config.toml [features] hooks=true`（见「Codex 捕获目标契约」），OpenCode 写 `.opencode/hooks.json`（见「OpenCode 安装流程契约」）。gemini `HookProvider` 不再列 supported/installable。
...
> 每张卡落地必须同步：`Cargo.toml [[test]]`、`tests/INDEX.md`、本节锚点与下文测试矩阵；任一缺失视为未闭环（见 AG-24）。
...
rg -n "LifecycleEvent|LifecycleEventKind|refs/libra/traces|content_hash|full.jsonl|full.jsonl.zst" src/internal/ai src/command/agent docs/development/commands/agent.md
...
|---|---|---|---|---|
| stopped-session checkpoint transcript/prompt/context | 90 天 | settings 可覆盖 | 每日 GC 或 `libra agent clean --gc` | 删除对象 + `object_index` + DB 行 + 重写 `refs/libra/traces` |
...
- entireio/cli 对齐的具体执行步骤、wire 契约（E1–E10）、任务卡（AG-16~AG-24）与验收命令以本文作为 `libra agent` public surface 的事实源；[`docs/development/code-agent-runtime.md`](../code-agent-runtime.md) 只保留内部 AgentRuntime/Web-only 迁移、MCP/control-plane 边界与 Gate 8 总览追踪。本文与该计划任一处变更，另一处的边界说明与差距映射都要同步刷新。
...
| transcript | 外部 Agent 自己生成的会话记录文件或事件流，例如 JSONL 或 provider 原生 transcript。它可能很大且包含敏感信息，因此默认只读取 metadata/content hash；完整正文必须走显式 detail/raw 路径，并受 chunking、size cap、redaction 和 audit 约束。 |
...
| raw / detail 路径 | 用户显式要求读取原始 transcript、prompt、context、stderr 或 full export 的路径。该路径必须有 size cap、streaming/chunk、redaction、授权或 audit 记录，不能作为默认输出。 |
...
| `agent-traces`（旧名） | `refs/libra/traces` 的历史名称。早期仓库把捕获目录存放在 `agent-traces` 分支；`2026062301_rename_agent_traces_branch` 迁移会在 `libra db upgrade` 时把该 `reference` 行（及 reflog 历史）重命名为 `traces`。该旧名仍被 `is_locked_branch` / `is_ai_managed_branch`（常量 `LEGACY_TRACES_BRANCH`）当作锁定/AI 托管分支，确保尚未迁移的 ref 仍受破坏性操作保护。 |
...
| AG-16 ~ AG-24 | Gate 8 任务卡编号，覆盖 capability、CLI alias、RPC、安全、lifecycle、checkpoint/export、transcript intelligence、review/investigate、docs/compat closeout。 |
