...
| object/plumbing surface | `cat-file --batch/--batch-check/--batch-command`、`index-pack --fix-thin`、`ls-tree --format` 等。 | `cat-file` 只暴露 `-t/-s/-p/-e` 和 AI modes；`verify-pack` 接受一个或多个 idx file、`--pack`（仅单 idx）、`-v` 和 `-s/--stat-only`；`index-pack` 是隐藏 plumbing，接受 pack file、`--stdin`、`-o`、`--keep[=<MSG>]`、Git-style `--progress` / `--no-progress` 兼容入口和 test-only index version；`ls-tree` 已公开基础 tree inspection surface、子目录路径语义、`--full-name` 和 `--full-tree`，但仍缺少高级格式/REV:path 路径语法。 | 保留为 plumbing 兼容缺口；扩展参数时同步用户文档、命令文档、兼容矩阵和测试证据。 |
| inspection/reporting surface | `blame` 高级输出与 copy/move detection、`describe --match/--exclude/--first-parent/--contains/--candidates/--all`、`diff --summary/--word-diff/--binary/-w/ext-diff`、`grep --perl-regexp`、`shortlog` 分组/format/stdin/no-merges/author。 | `describe --long` / `--dirty` 已有 CLI、JSON 和集成场景证据；其余列出的 Git flags 仍未公开。`grep` regex dialect 仍需以代码和测试重新评估。 | 保留为低风险兼容增强池；新增时必须补命令级回归和测试证据。 |
