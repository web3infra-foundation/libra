...
|---|---|---|---|
| 命令接入治理 | `gc`、`ls-files`、`package`、`prune`、`stats` 的开发文档或源码文件存在，但用户可见 CLI 与 `COMPATIBILITY.md` 未公开。 | `for-each-ref` 已在 `src/cli.rs::Commands`、`COMPATIBILITY.md` 和命令开发文档中公开，不能再列为未公开命令；`ls-tree`、`archive` 和 `notes` 也已公开并从本缺口移除。其余命令仍需按当前 CLI surface 核对是否返回 `LBR-CLI-001` 或应降级为内部资料。 | 作为全局未收口项保留；后续必须二选一：接入 CLI 并同步 `COMPATIBILITY.md`、命令文档和集成场景，或把对应命令文档降级为内部/历史资料。 |
...
| staging/worktree Git surface | `add --intent-to-add`、`clean -i`、`clean <pathspec>`、`restore --overlay/--ours/--theirs/--merge/--progress`、`reset --merge/--keep`、`rm/mv --sparse`、`checkout -p` 以及跨命令 patch mode。 | `add`、`clean`、`restore`、`reset`、`rm`、`mv` 的参数结构未暴露这些 flag；patch mode 由 D15 拒绝；`switch --detach` 已实现，不能再把 detached HEAD 作为全局缺口。 | 作为命令级 Git 兼容缺口保留；实现时同步命令文档、`COMPATIBILITY.md` 和 integration scenarios。 |
| commit/rewrite/sequencer | `commit --allow-empty-message`、`commit -e/--edit`、`commit -v/--verbose`、`commit --porcelain`、`commit --status/--no-status`、`rebase -i/--edit-todo/--onto/--exec/--autosquash/--rebase-merges/--keep-empty`、`cherry-pick`/`revert` 的 `--edit`、`--mainline`、sequencer `--continue/--abort/--skip` 与 strategy 扩展。 | `CommitArgs` 已公开并实现 `--fixup`、`--squash` 和 `--cleanup`，这些不能再列为当前缺口；`--allow-empty-message` 仍由 D-empty-message 拒绝；`RebaseArgs` 只有 upstream/continue/abort/skip；`cherry-pick`/`revert` 不维护完整 Git sequencer surface。注意 `pull --rebase` 已实现，不列入缺口。 | 保留为重写/序列器能力缺口；不能把旧资料中曾标记为完成的 rebase `--exec`、`--autosquash` 或旧资料中曾标记为缺失的 commit `--fixup`、`--squash`、`--cleanup` 当作当前事实。 |
| merge/pull strategy surface | `merge --ff-only/--no-ff/--squash`、octopus merge、自定义 strategy/-X、merge message/signature validation、pull `--squash`/`--no-ff`。 | `MergeArgs` 只有 branch/continue/abort；`PullArgs` 已有 `--rebase` 与 `--ff-only`，但没有 squash/no-ff。 | 这些 merge strategy flags 不能按旧资料中的 `done` 视为完成；后续补实现，或修正命令文档和测试状态。 |
| object/plumbing surface | `cat-file --batch/--batch-check/--batch-command`、`verify-pack -s/--stat-only` 与 multi-index、`index-pack --stdin/--fix-thin/--keep/--progress`、`ls-tree --full-name/--full-tree/--format` 等。 | `cat-file` 只暴露 `-t/-s/-p/-e` 和 AI modes；`verify-pack` 只接受 idx file、`--pack`、`-v`；`index-pack` 是隐藏 plumbing 且只接受 pack file、`-o`、test-only index version；`ls-tree` 已公开基础 tree inspection surface，但仍缺少高级格式/路径语法。 | 保留为 plumbing 兼容缺口；扩展参数时同步用户文档、命令文档、兼容矩阵和测试证据。 |
| inspection/reporting surface | `blame` 高级输出与 copy/move detection、`describe --long/--match/--exclude/--first-parent/--dirty`、`diff --summary/--word-diff/--binary/-w/ext-diff`、`grep --perl-regexp`、`shortlog` 分组/format/stdin/no-merges/author。 | 对应命令参数结构未覆盖这些 Git flags；`grep` regex dialect 仍需以代码和测试重新评估。 | 保留为低风险兼容增强池；新增时必须补命令级回归和测试证据。 |
| refs/worktree/tag surface | `switch -C/--orphan`、`worktree add <path> <branch>`、`worktree --detach`、per-worktree branch isolation、tag signing/verification。 | `branch -m` 与 `tag -m` 已实现，不再列为缺口；`worktree` 以共享 `.libra` 状态注册物理工作树；`tag` 无 `-s/-v`。 | 保留剩余 Git surface 缺口；文档中已实现的旧缺口后续要在对应命令文档里清掉。 |
...
| Code Web-only / Agent runtime | `libra code` 默认 Web、拒绝 stdio、TUI startup 删除、Web harness 替代 PTY、Web graph parity、WorkflowPattern 和 per-workflow budget 未完成。 | `CodeArgs` 仍有 `--web/--web-only` 与 `--stdio`；默认分支仍调用 `execute_tui()`；`BudgetScope` 只有 Session/Agent/Goal；未发现 `WorkflowPattern` 层。 | 按 `agent.md` AG-01 到 AG-15 与 `web-only.md` W1 到 W12 继续；未完成前不能删除 TUI、graph、PTY harness 或切默认 Web。 |
