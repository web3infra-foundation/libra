...
| merge/pull strategy surface | octopus merge、自定义 strategy/`-X`、merge signature validation（`--verify-signatures`）。 | `MergeArgs` 已有 `-m`/`--ff-only`/`--no-ff`/`--squash`/`--no-commit`/`--no-edit`（octopus/自定义 strategy/`-X`/`--verify-signatures` 仍缺）；`PullArgs` 已有 `--rebase`、`--ff-only`、`--ff`、`--no-ff`、`--squash`、`--commit`、`--no-commit`、`--autostash` 与 fetch `--depth`。 | 仅 octopus/自定义 strategy/`-X`/signature validation 仍为缺口；不要再把已实现的 merge/pull strategy flags（`--ff-only`/`--no-ff`/`--squash`/`--no-commit`/`-m`/`--no-edit`、pull `--squash`/`--commit`/`--no-commit`/`--autostash`）当作缺失。 |
