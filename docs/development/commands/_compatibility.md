...
|---|---|---|---|
| 命令接入治理 | `for-each-ref`、`gc`、`ls-files`、`notes`、`package`、`prune`、`stats` 的开发文档或源码文件存在，但用户可见 CLI 与 `COMPATIBILITY.md` 未公开。 | `src/cli.rs::Commands` 无这些公开 variant；`target/debug/libra <cmd> --help` 返回 `LBR-CLI-001`；对应命令文档仍记录接入待决。`ls-tree` 和 `archive` 已公开并从本缺口移除。 | 作为全局未收口项保留；后续必须二选一：接入 CLI 并同步 `COMPATIBILITY.md`、命令文档和集成场景，或把对应命令文档降级为内部/历史资料。 |
...
| commit/rewrite/sequencer | `commit --fixup/--squash/--cleanup/--allow-empty-message`、`rebase -i/--edit-todo/--onto/--exec/--autosquash/--rebase-merges/--keep-empty`、`cherry-pick`/`revert` 的 `--edit`、`--mainline`、sequencer `--continue/--abort/--skip` 与 strategy 扩展。 | `CommitArgs` 无 fixup/squash/cleanup/empty-message；`RebaseArgs` 只有 upstream/continue/abort/skip；`cherry-pick`/`revert` 不维护 Git sequencer surface。注意 `pull --rebase` 已实现，不列入缺口。 | 保留为重写/序列器能力缺口；不能把旧资料中曾标记为完成的 rebase `--exec`、`--autosquash` 当作当前事实。 |
