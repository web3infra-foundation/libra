...
| object/plumbing surface | `cat-file --batch/--batch-check/--batch-command`、`index-pack --stdin/--fix-thin`、`ls-tree --format` 等。 | `cat-file` 只暴露 `-t/-s/-p/-e` 和 AI modes；`verify-pack` 接受一个或多个 idx file、`--pack`（仅单 idx）、`-v` 和 `-s/--stat-only`；`index-pack` 是隐藏 plumbing，接受 pack file、`-o`、`--keep[=<MSG>]`、Git-style `--progress` / `--no-progress` 兼容入口和 test-only index version；`ls-tree` 已公开基础 tree inspection surface、子目录路径语义、`--full-name` 和 `--full-tree`，但仍缺少高级格式/REV:path 路径语法。 | 保留为 plumbing 兼容缺口；扩展参数时同步用户文档、命令文档、兼容矩阵和测试证据。 |
