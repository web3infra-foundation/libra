...
| object/plumbing surface | `cat-file --batch/--batch-check/--batch-command`、`verify-pack` multi-index、`index-pack --stdin/--fix-thin/--keep/--progress`、`ls-tree --full-name/--full-tree/--format` 等。 | `cat-file` 只暴露 `-t/-s/-p/-e` 和 AI modes；`verify-pack` 接受单个 idx file、`--pack`、`-v` 和 `-s/--stat-only`；`index-pack` 是隐藏 plumbing 且只接受 pack file、`-o`、test-only index version；`ls-tree` 已公开基础 tree inspection surface，但仍缺少高级格式/路径语法。 | 保留为 plumbing 兼容缺口；扩展参数时同步用户文档、命令文档、兼容矩阵和测试证据。 |
