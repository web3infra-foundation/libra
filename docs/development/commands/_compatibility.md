...
| refs/worktree/tag surface | `worktree add <path> <branch>`、`worktree --detach`、per-worktree branch isolation、branch copy/description/sort/format、tag column/Git-GPG 互通。 | `switch -C/--orphan`、`branch -m`、`branch --unset-upstream`、`branch --points-at`、`branch --merged`/`--no-merged`、`branch --ignore-case`、`tag -m`、`tag -F`、`tag --contains/--no-contains`、`tag --merged/--no-merged`、`tag --sort` 与 vault-PGP `tag -s/-v` 已实现，不再列为缺口；`worktree` 以共享 `.libra` 状态注册物理工作树。 | 保留剩余 Git surface 缺口；文档中已实现的旧缺口后续要在对应命令文档里清掉。 |
