...
- 源码分层：主要实现文件为 `src/command/shortlog.rs`、`src/command/shortlog/format.rs`、`src/command/shortlog/mailmap.rs`、`src/command/shortlog/mailmap_tests.rs`、`src/command/shortlog/range.rs`、`src/command/shortlog/render.rs`、`src/command/shortlog/tests.rs`、`src/command/shortlog/wrap.rs`。参数/子命令类型包括：`ShortlogArgs`；输出、错误或状态类型包括：源码未暴露独立输出/错误类型，错误通过 `CliResult` 或上层命令错误统一传播；主要执行函数包括：`execute_to`、`execute`、`execute_safe`。
...
```mermaid
flowchart TD
    A["入口与分发<br/>src/cli.rs::Commands"] --> B["源码分层<br/>src/command/shortlog.rs / src/command/shortlog/*.rs"]
...
- 2026-06-06 `cec72108`（`feat(shortlog): add -c/--committer grouping and --no-merges filter`）：基础实现节点：add -c/--committer grouping and --no-merges filter；当前实现的主要轮廓可追溯到该提交。
- 2026-06-10 `3b170290`（`feat(shortlog): add --top option (#382)`）：功能演进：add --top option (#382)；该节点扩展了当前命令可用的参数或行为。
...
- 公开参数/子命令包括：`Option Details`、`YYYY-MM-DD`。
