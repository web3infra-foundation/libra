...
| 兼容差异项 | Batch mode（其余） | 原始对照：未实现；相关参数/替代：`--buffer`、`--follow-symlinks`；当前说明：延后。 后续实现时需要补对应回归测试并同步兼容矩阵。 |
