...
| [`show`](show.md) | `partial` | object/commit display, common name/stat flags, `--pretty` / `--format`, `--abbrev-commit`, and `--no-expand-tabs`/`--no-notes`/`--no-mailmap`/`--no-show-signature` (no-ops) supported; named pretty presets (short/full/fuller/raw), raw/summary formats, and `--expand-tabs`/`--notes`/`--mailmap`/`--show-signature` not exposed |
