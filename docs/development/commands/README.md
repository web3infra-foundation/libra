...
| [`log`](log.md) | `partial` | common log surface plus `--range`/`--all`/`--reverse`/`--author-date-order`/`--follow`/`-L`/`--parents`/`--children`/`-i`/`--invert-grep`; positional ranges and exact line history remain partial |
