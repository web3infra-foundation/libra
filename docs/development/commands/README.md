...
| [`log`](log.md) | `partial` | common log surface plus `--range`/`--all`/`--reverse`/`--author-date-order`/`--date-order`/`--no-expand-tabs`(no-op)/`--follow`/`-L`/`--parents`/`--children`/`-i`/`--invert-grep`; `--expand-tabs`, positional ranges, and exact line history remain partial |
