...
| [`restore`](restore.md) | `partial` | source/staged/worktree path restore + conflict-stage `--ours`/`-2` & `--theirs`/`-3` (worktree-only, index left unmerged) + `--ignore-unmerged` (unmerged guard: plain restore of an unmerged path → `LBR-CONFLICT-001`/128) + `--no-progress`(no-op) + `--no-overlay`(no-op — never in overlay mode) supported; `--overlay`/`--merge`/`--conflict` and the `--progress` meter not exposed |
