...
| [`rev-list`](rev-list.md) | `partial` | multi-revision reachability, exclusions/ranges, count/limit controls, author/committer/message/time filters, parent filters/reset aliases, first-parent traversal, parents and timestamp output supported; advanced path/cherry-pick traversal filters not exposed |
