...
| [`rev-list`](rev-list.md) | `partial` | multi-revision reachability, exclusions/ranges, count/limit controls, author/committer/message/path/time filters, parent filters/reset aliases, first-parent traversal, symmetric side/cherry filters, parents and timestamp output supported; `--cherry` shorthand and object/boundary traversal output remain incomplete |
