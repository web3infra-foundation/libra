...
| [`diff`](diff.md) | `partial` | staged/old-new/pathspec/name/stat/shortstat/summary output + `--exit-code`/`-s`/`--no-patch`/`-z`/`-U<n>`(`--unified`, context lines)/`-w`(`--ignore-all-space`, re-diff)/`--check`/`-R`/`-a`(`--text`, no-op)/`--no-ext-diff`(no-op)/`--no-color-moved`(no-op)/`--no-renames`/`--no-relative`/`--no-indent-heuristic`/`--no-textconv`(no-ops) supported; positional revspec, word diff, `--color-moved`, `--renames`, `--relative`, `--indent-heuristic`, `--textconv`, `--binary` patch, other whitespace flags(`-b`/`--ignore-space-at-eol`/`--ignore-blank-lines`)/ext-diff drivers incomplete |
