...
| [`for-each-ref`](for-each-ref.md) | `partial` | `--heads` / `--tags` / `--remotes` / `--all` / `--format` / `--sort` (`refname`/`objectname`/`version:refname`/`committerdate`/`authordate`/`creatordate`/`objectsize`/`*objectname`, each reversible) / `--count` / `--points-at` / `--contains` / `--no-contains` / `--merged` / `--no-merged` / `--exclude` / `<pattern>` and `--shell`/`--perl`/`--python`/`--tcl` output quoting modes, and the `%(objectsize)` atom + `--sort=objectsize`, and `%(*objectname)`/`%(*objectname:short)` + `--sort=*objectname`, and the `%(*objecttype)`/`%(*objectsize)` deref atoms supported; the rest of the full Git atom language is not exposed |
