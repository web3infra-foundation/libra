...
| [`diff`](diff.md) | `partial` | staged/old-new/pathspec/name/stat/shortstat/summary output + `--exit-code`/`-s`/`--no-patch`/`-z`/`--check` supported; positional revspec, word/binary/whitespace-ignoring(`-w`)/ext-diff incomplete |
