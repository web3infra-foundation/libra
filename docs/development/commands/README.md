...
| [`rev-list`](rev-list.md) | `partial` | multi-revision reachability, exclusions/ranges, count/limit controls, parent filters/reset aliases, parents and timestamp output supported; advanced traversal filters not exposed |
...
| [`show-ref`](show-ref.md) | `partial` | branch/tag/HEAD listing, `--branches` alias, `--hash[=<n>]`, `--abbrev[=<n>]`, `--dereference`, `--verify`, `--exists`, and `--exclude-existing[=<pattern>]` supported; `--no-*` aliases not exposed |
