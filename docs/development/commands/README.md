...
| [`config`](config.md) | `partial` | vault-backed local/global config plus section operations (`--remove-section`/`--rename-section`) and `-z`/`--null` output; system scope, editor round-trip and typed conversion incomplete |
...
| [`merge`](merge.md) | `partial` | fast-forward and single-head three-way merge supported; `-m`/`--ff-only`/`--no-ff`/`--squash`/`--no-commit`/`--no-edit`/`-n`(`--no-stat`, no-op)/`--no-verify-signatures`(no-op)/`--no-rerere-autoupdate`(no-op) supported; octopus/custom strategies/`--verify-signatures`/`--rerere-autoupdate`/`--stat` deferred |
...
| [`restore`](restore.md) | `partial` | source/staged/worktree path restore + `--no-progress`(no-op) + `--no-overlay`(no-op — never in overlay mode) supported; `--overlay`/conflict variants and the `--progress` meter not exposed |
