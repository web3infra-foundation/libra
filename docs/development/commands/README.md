...
| [`restore`](restore.md) | `partial` | source/staged/worktree path restore + `--no-progress`(no-op) supported; overlay/conflict variants and the `--progress` meter not exposed |
