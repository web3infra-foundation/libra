...
| [`ls-files`](ls-files.md) | `partial` | default cached listing plus modified/deleted/stage/untracked filters, `.libraignore`-aware `--others --exclude-standard`, pathspecs, `--error-unmatch`, `-z`, status tags `-t` (H/R/C/?/M), unmerged-only `-u`/`--unmerged`, and JSON/machine output supported |
...
| [`tag`](tag.md) | `partial` | lightweight/message/annotated tags, force/delete/list/`-n`, points-at, contains/no-contains, merged/no-merged, sort, and vault-PGP sign/verify supported; column and Git GPG interop not exposed |
