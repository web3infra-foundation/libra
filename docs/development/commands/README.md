...
| [`blame`](blame.md) | `partial` | numeric `-L` ranges, porcelain/line-porcelain (`-p`), `-e`/`--show-email`, display flags `-l`/`-s`/`-t`/`--abbrev`, and `--root` (no-op) supported; reverse/whitespace/incremental/copy-move detection remain incomplete |
...
| [`diff`](diff.md) | `partial` | staged/old-new/pathspec/name/stat/shortstat/summary output + `--exit-code`/`-s`/`--no-patch`/`-z`/`--check`/`-R`/`-a`(`--text`, no-op) supported; positional revspec, word diff, `--binary` patch, whitespace-ignoring(`-w`)/ext-diff incomplete |
