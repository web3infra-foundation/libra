...
| [`diff`](diff.md) | `partial` | staged/old-new/pathspec/name/stat/shortstat/summary output + `--exit-code`/`-s`/`--no-patch`/`-z` supported; positional revspec, word/binary/whitespace/ext-diff incomplete |
