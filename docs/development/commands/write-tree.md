...
- 兼容级别：`partial`。
- 已支持：无参数把 index 写成嵌套 tree（保留 mode 与 hash kind），`--json`/`--machine`。空 index → 规范空 tree。
...
- 源码分层：
  - `src/command/write_tree.rs`：`WriteTreeArgs`（无字段）、`execute`/`execute_safe`、`WriteTreeOutput`（`--json`）。
  - `src/internal/tree_plumbing.rs`：**单一**的 index↔tree 领域实现。`write_tree_from_index(index)` 收集 `(path, TreeItemMode, hash)` 叶子，调用 `write_tree_from_leaves`，后者按父目录分组、`ensure_ancestor_dirs` 注册所有祖先目录、递归构建并保存每个子 tree、返回根 tree id。**正确处理中间空目录**（如 `a/b/c.txt`，`a`/`a/b` 下无直接文件）——这是早期各命令各自的树构造器漏掉的 case。
- 执行路径：`require_repo` → `Index::load(path::index())` → `tree_plumbing::write_tree_from_index` → 打印/JSON 根 tree id。
...
- 输出与错误契约：human/`--json` 经 `OutputConfig`；仓库缺失 `repo_not_found()`，tree 构造失败 `RepoStateInvalid` → 128。
...
- 测试：`tests/command/write_tree_test.rs`（空 index→空 tree、嵌套目录、`--json`、非仓库 128、与 read-tree round-trip）；`src/internal/tree_plumbing.rs` 单元测试（签名冻结、mode 映射、中间目录注册）。
