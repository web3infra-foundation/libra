...
整体结论：**阶段切分与依赖顺序合理且安全**——先补 MCP stdio 替代入口（W1），再切默认 Web（W2/W3），抽 session bootstrap 与 plan/goal 行为（W5/W6/W7），最后才删 TUI bridge 与 startup（W8/W9）。"禁止提前执行的依赖" 一节正确地拦住了"未迁移即删除"导致的能力丢失。这套顺序可以照搬执行。
...
2. **舍弃 `local-tui-control.md`（已决策并执行）**：因为放弃 TUI，`docs/automation/local-tui-control.md` 已删除，其内容不再维护；control 文档以 `docs/commands/code-control.md` 为 canonical。`scripts/check_docs_consistency.sh` 原有 6 处 `require_doc … docs/automation/local-tui-control.md`（行 31/35/59-62）已随之清理：endpoint 循环（行 31）与 header 循环（行 35）repoint 到 `docs/commands/code-control.md`；`test-provider`/`code_ui_scenarios` repoint 到本文件；TUI-scenario 专属的 `target/code-ui-scenarios`/`Scenario::new` 两条断言删除。脚本仍钉 `tests/code_codex_default_tui_test.rs`（行 69，W9 改名时同步）与 base.yml `Run TUI automation scenarios`（行 64，W10 改名时同步）。完整记录见 **W13**。
3. **CLI flag 结构**：`--web` 是 `web_only` 字段（`code.rs:462`）的 clap alias（`code.rs:461`，`conflicts_with = "stdio"`）；`--mcp-stdio` 是 `stdio` 字段（`code.rs:600`）的 alias（`code.rs:599`，`conflicts_with = "web_only"`）。它们不是独立字段，改写要落到这两个字段上。`execute()`（`code.rs:696-705`）按 `stdio → web_only → else(tui)` 分发；`execute_stdio` 走 `resolve_code_working_dir(args)` + `init_mcp_server(&working_dir)`，`CodeArgs` 已有 `cwd`(:474)/`repo`(:478) 供 W1 复用。
...
- 删除 "TUI mode" 概念。`CodeArgs.web_only` 字段（`code.rs:462`，已带 alias `web`）保留为 no-op 即可；要重命名为 `web` 则需同步更新 `execute()` 分支（`code.rs:696-705`）和所有 `args.web_only` 引用。
...
`--stdio` 需要单独决策，因为它不是 Web 模式。注意 `--stdio` 与 `--mcp-stdio` 在 `CodeArgs` 中是**同一个字段** `stdio`（`code.rs:600`，alias `mcp-stdio` 在 `code.rs:599`），处理这两者只需改 `stdio` 字段：
...
- 标记 `docs/commands/code.md`、README、`docs/commands/code-control.md`、`docs/improvement/web.md` 中所有 TUI-default 文案（`docs/automation/local-tui-control.md` 已随放弃 TUI 删除，见 W13）。

Exit criteria:

- 有一张迁移清单，每个 TUI-owned 行为都有目标模块和测试目标。
- `rg "TUI|tui|--web-only|--stdio" docs/commands/code.md docs/commands/code-control.md README.md docs/improvement/web.md scripts/check_docs_consistency.sh` 的待改位置已确认。
...
- 将 `docs/commands/code-control.md` 从 "drive a local TUI" 改写为 local Code UI automation control（`docs/automation/local-tui-control.md` 已删除；`scripts/check_docs_consistency.sh` 已把相关 `require_doc` repoint 到 code-control.md / web-only.md，见 W13）。
...
9. W11 文档/help 收口。
10. W12 最终审计。
11. W13：舍弃 `docs/automation/local-tui-control.md` 并清理 `check_docs_consistency.sh`（删除与 repoint 已在制定计划时执行，随时收尾验收）。
...
若改 CI step 名（"Run TUI automation scenarios"），同步 scripts/check_docs_consistency.sh:64 与 .github/workflows/base.yml。
```
...
5. CI step 名称从 "Run TUI automation scenarios" 改为 "Run Code UI automation scenarios"；同步 `scripts/check_docs_consistency.sh:64` 的 `require_doc "Run TUI automation scenarios" ".github/workflows/base.yml"`。
...
- `bash scripts/check_docs_consistency.sh` 通过（CI step 名若改动需先同步脚本）。
...
- `scripts/check_docs_consistency.sh`（已在 W13 repoint 到 code-control.md / web-only.md，本卡只需复核通过）
...
4. `docs/automation/local-tui-control.md` 已随放弃 TUI 删除，不再补建；跑 `bash scripts/check_docs_consistency.sh` 确认绿，缺项补进 `code-control.md`。
...
- `bash scripts/check_docs_consistency.sh` 通过（控制文档/CI step 改名后脚本仍命中所有 `require_doc`）。
...
目标：放弃 TUI 后，`docs/automation/local-tui-control.md` 不再需要，连同 `scripts/check_docs_consistency.sh` 对它的依赖一起清掉。control 文档统一以 `docs/commands/code-control.md` 为 canonical。本卡的删除与脚本 repoint 已在制定本计划时执行，留此卡记录决策与收尾验收。

已执行的改动：

- 删除 `docs/automation/local-tui-control.md`（内容放弃）。
- `scripts/check_docs_consistency.sh`：
  - endpoint 循环（行 31）与 header 循环（行 35）的 `require_doc` 由 `docs/automation/local-tui-control.md` repoint 到 `docs/commands/code-control.md`。
  - `test-provider`、`code_ui_scenarios` 两条 repoint 到 `docs/development/web-only.md`。
  - 删除 TUI-scenario 专属的 `target/code-ui-scenarios`、`Scenario::new` 两条 `require_doc`。

收尾/验收：

- `rg -n "local-tui-control" scripts docs` 无结果（除历史迁移说明）。
- `bash scripts/check_docs_consistency.sh` 退出码为 0；若 `docs/commands/code-control.md` 缺某个 `/api/code/*` endpoint 或 control header，补进 code-control.md（本就是它的归属），不要复活 `local-tui-control.md`。
- 脚本仍钉 `tests/code_codex_default_tui_test.rs`（行 69）与 base.yml `Run TUI automation scenarios`（行 64），分别在 W9 / W10 改名时同步。
...
| 放弃 `local-tui-control.md` 后 `check_docs_consistency.sh` 断言悬空 | 控制面 endpoint/header 文档检查失效或脚本变红 | W13 已 repoint 到 `code-control.md`/`web-only.md`；W11 确保 code-control.md 覆盖全 endpoint/header |
...
| `CodeArgs.web_only` | 462 | 字段；`--web` 是其 alias（:461，`conflicts_with="stdio"`） |
| `CodeArgs.stdio` | 600 | 字段；`--mcp-stdio` 是其 alias（:599，`conflicts_with="web_only"`）。`mcp_stdio`/`web` ❌ 不是独立字段 |
...
| `execute` | 696 | dispatch：`stdio→execute_stdio` / `web_only→execute_web_only` / else `execute_tui`（696-705） |
| `execute_web_only` | 759 | 改名为 `execute_web` 的目标（W2） |
...
| `docs/automation/local-tui-control.md` | 当前 ❌ 已删除（放弃 TUI）；`check_docs_consistency.sh` 断言已 repoint 到 `code-control.md`/`web-only.md`（W13） |
