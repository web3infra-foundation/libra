...
> Companion docs: Web/runtime 现状见 [`docs/development/commands/_general.md`](commands/_general.md)；控制面契约见 [`docs/commands/code-control.md`](../commands/code-control.md)；长期 agent backlog 见 [`docs/development/commands/agent.md`](commands/agent.md)；MCP stdio 独立命令拆分见 [`mcp.md`](mcp.md)；外部 Agent 会话捕获（`libra agent` / `refs/libra/agent-traces`，与本计划**正交**的独立子系统）见 [`docs/development/commands/_general.md`](commands/_general.md)。
...
- **外部 Agent 捕获是并行轨道。** `libra agent` 捕获 Claude Code/Gemini/Cursor/Codex/OpenCode/Copilot/FactoryAI 等外部编码 Agent 的 transcript、hook 事件和 checkpoint；它可以复用 `SessionStore`/Web 展示能力，但不得与内部 AgentRuntime 的 turn state/checkpoint 类型混同。
- **`claudecode` provider 不得复活。** 当前 `src/cli.rs` 已对 `--provider claudecode` 给出移除错误；本文只允许 `claude-code` 作为 observed external agent slug，不允许重新引入 `src/internal/ai/claudecode/` 或 provider 分支。
- **兼容 entireio/cli 是“能力接近一致”，不是照搬存储路径或命令名。** Libra 保留 `refs/libra/agent-traces`、SQLite 表和 `libra agent` 命名空间；对齐重点是 capability contract、lifecycle event、checkpoint payload、review/investigate workflow、skill 事件和可验证测试。
...
| `/Volumes/Data/entireio/cli/cmd/entire/cli/agent/chunking.go` | 50 MiB line-safe chunk/reassemble，JSONL fallback，Gemini JSON 检测 | 扩展 `TranscriptChunker`：line-safe JSONL chunk、chunk manifest、可选压缩；`checkpoint show` 默认 lazy metadata，不一次性读完整 transcript |
| `/Volumes/Data/entireio/cli/docs/architecture/sessions-and-checkpoints.md` | committed checkpoint = sharded root metadata + numbered session dirs；稳定 checkpoint id/trailer；temporary/shadow lifecycle；并发 session handling | Libra 继续写 `refs/libra/agent-traces`，但 export/show payload 兼容 root `metadata.json` + per-session `metadata.json/full.jsonl/prompt.txt/context.md/content_hash.txt` |
...
> 本块是 AG-16~AG-24 的**冻结引用**：凡卡正文/Phase 出现「命名可调整」「至少包含」「等价 contract」，都以本块的精确标识符为准。这些是 entire 当前源码的事实（非旧 d0a714 推断），Libra 侧采用 Rust 命名但 wire/JSON 形态必须一一对应。核对时仍须 `rg` 复跑（entire 仓库会演进），但下列标识符在 2026-06-16 均已逐个读源确认。
>
...
```
任何刷新后发现 E 契约或归档形态变化，必须先在本节“已验证 wire 契约”打补丁，再反向同步 `commands/agent.md` 的差距表，再开工 AG 卡。
...
- Gemini 整体-JSON 检测：`DetectAgentTypeFromContent`（trim 后首字符 `{`、解析 `{messages:[...]}`、≥1 message → Gemini，否则 `""`）。
...
**E7 — Skill event**（`cmd/entire/cli/agent/skill_events*.go`）：`SkillEvent{ID, EventType, Skill{Name}, Source{Agent,Signal,Confidence}, TurnID, Timestamp, TranscriptAnchor{Unit,Start,End,EntryIDs,ToolUseID}, Native, Collapse{Target,Label,DefaultCollapsed}}`。EventType `prompt_invocation`/`tool_invocation`；signal `input_slash_command`/`prompt_slash_command`/`skill_tool_use`。curated registry：`claude-code`→`/review`,`/security-review`,`/simplify`；`codex`→`/review`；`gemini`→{}。仅 `ClaudeCodeAgent` 实现 `ExtractSkillEvents`；prompt slash-command 事件由正则 + filesystem-path 排除 fallback 生成。
...
**E9 — Agent roster**（`registry.go` + 各 provider 包）：entire 当前 **1 个 stable**（`claude-code`）+ **7 个 preview**（`codex`/`copilot-cli`/`cursor`/`factoryai-droid`/`gemini`/`opencode`/`pi`）+ **1 个 test-only canary**（`vogon`，`IsTestOnly()==true`，排除出 `enable`）+ external 动态路径。注意：entire 的 `gemini` `IsPreview()==true`，而 Libra 把 `gemini` 当 stable hook-installable——这是有意差异，AG-16 capability matrix 以 Libra 的 `STABLE_AGENT_SLUGS=["claude-code","gemini"]` 为准，但须在文档标注与 entire 的 stability 口径不同。Libra 无 `pi`/`vogon`，导入这些 kind 走 quarantine/unsupported（见 AG-16）。
...
| checkpoint/export | `refs/libra/agent-traces` + SQLite 表是 Libra-native 存储 | 缺 entire-style root/session export contract、multi-session stable index、content_hash/prompt/context/token_usage、large transcript lazy show | AG-20 |
...
| docs/tests | `docs/development/commands/agent.md` 描述外部 capture；本文描述内部 runtime | 两份文档缺 AG-16+ 双向追踪；`docs/commands/agent.md`、`COMPATIBILITY.md`、`tests/INDEX.md` 需同步 | AG-24 |
...
### 与主计划 (docs/development/commands/agent.md) 的关系

本计划是 [`docs/development/commands/agent.md`](commands/agent.md) Part B「`libra code` 实现规格」中 **Implementation Phase 3: Code UI Source Of Truth Unification** 的具体落地路线图，同时为 Phase 4/5 及 Part C TUI automation harness 演进创造条件。
...
| installable hook providers | `STABLE_AGENT_SLUGS = ["claude-code", "gemini"]` + `hooks/providers` registry | 只有 Claude Code/Gemini 可安装 hook；其它 known agents 不得误报 |
...
| Capture persistence | `hooks/runtime.rs` + `history.rs` 写 `agent_session` / `agent_checkpoint` / `refs/libra/agent-traces` | 存在；扩展 `LifecycleEvent` dispatcher、owner filtering、large transcript export 待 AG-19/AG-20 |
| Push surface | `src/command/agent/push.rs` 只推 `refs/libra/agent-traces` | 存在；entire-style export 仍保留该 Libra-native ref |
...
rg -n "SessionStart|TurnStart|TurnEnd|SubagentStart|SubagentEnd|ModelUpdate|ToolUse|refs/libra/agent-traces" src/internal/ai/hooks src/internal/ai/history.rs src/command/agent
...
- `agent_for` / registry 输出 7 个当前 known agent 的 capability matrix：`registered`、`stable`、`preview`、`hook_installable`、`transcript_readable`、`launchable_review`、`launchable_investigate`、`external_binary`。`claude-code`/`gemini` 是 hook-installable；Cursor/Codex/OpenCode/Copilot/FactoryAI 只能按实际 adapter 能力标注，不能宣称可安装 hooks。
...
- `libra agent add cursor` 在 cursor 未实现 hook install 时返回 actionable unsupported，而不是静默 success；`libra agent add claude-code` 仍走现有 provider hook install。
...
- export payload 兼容 entire checkpoint 结构，但保留 Libra 存储：root `metadata.json` + numbered session dirs 的 `metadata.json`、`full.jsonl` 或 chunk manifest、`prompt.txt`、`context.md`、`content_hash.txt`、optional `full.jsonl.zst`；实际 refs 仍是 `refs/libra/agent-traces`。
...
- 更新 [`docs/development/commands/agent.md`](commands/agent.md)：成为 `libra agent` 外部捕获的权威 CLI/API 文档，包含 capability matrix、list/add/remove alias、RPC protocol、lifecycle event、checkpoint/export shape、review/investigate 用户流。
...
- `docs/development/agent.md` 与 `docs/development/commands/agent.md` 的边界说明一致：前者是内部 AgentRuntime + Gate 8 追踪计划（含本节完整 E1-E9 + AG-16~AG-24 执行卡 + Rust 契约草图），后者是 `libra agent` public CLI/source-of-truth + 10 节差距表 + “还未实现的功能”表。**任何一处修改 AG 卡、E 契约、已知差距或有意差异，必须同一 PR 内双向同步**（见 AG-24 强制同步规则）。
...
> **如何读一张卡（每张卡的完整执行规格分布在四处，按此拼装）：** ①本节卡正文 = 目标 + 验收要点；②「任务拆分矩阵」= 该卡的 Dependencies / Scope / Evidence / Must-not；③同名「实施阶段 Phase N」= 详细任务步骤与逐条验收（卡号↔Phase 对应：AG-00→Phase 0，AG-01→Phase 1，AG-02→Phase 2，AG-03→Phase 3，AG-04→Phase 4，AG-05→Phase 5，AG-06→Phase 6，AG-07→Phase 7，AG-08→Phase 8，AG-09→Phase 9，AG-10→Phase 10，AG-11→Phase 11，AG-12→Phase 12，AG-13/14/15→Phase 13，AG-16/17/18→Phase 14，AG-19/20→Phase 15，AG-21/22/23→Phase 16，AG-24→Phase 17）；④「验收命令」节 = 必跑命令。开工前先按「Agent 执行协议 → Task card fields」把这四处合并成六字段，再按「Agent 开发前置检查清单」过一遍。锚点一律以「当前基线 → 已核对锚点表」为准 + 现场 `rg` 复核。
...
```
检查本文档内部链接是否有效（`mcp.md`、`docs/development/commands/agent.md` 等）。
...
- 生成当前 known agents 的 capability matrix，并显式区分 `registered`、`stable/preview`、`hook_installable`、`transcript_readable`、`launchable_review`、`launchable_investigate`、`external_binary`。matrix 以 `STABLE_AGENT_SLUGS`（`src/command/agent/mod.rs:216`）为 hook-installable 权威；须在输出/文档标注「Libra 把 `gemini` 当 stable，而 entire 的 `gemini.IsPreview()==true`」这一有意差异。
...
- `claude-code`/`gemini` 之外的 agent 不会被误报为 hook-installable。
...
* Public docs: `docs/commands/agent.md`、[`docs/development/commands/agent.md`](commands/agent.md)
...
- JSON list 输出包含 `slug`、`agent_kind`、`stability`、`hook_installable`、`hooks_installed`、`transcript_readable`、`external_binary`。
- `add cursor` 不安装任何 hook 且错误解释“registered/readable 但 not hook-installable”。
...
* Protocol docs: [`docs/development/commands/agent.md`](commands/agent.md)
...
* 存储：`refs/libra/agent-traces` + 三表 `agent_session`/`agent_checkpoint`/`agent_usage_stats`（**raw SQL，无 Sea-ORM entity**；migration `2026050303_agent_capture.sql`）
...
- `docs/development/commands/agent.md`（公共 CLI/行为 source-of-truth + 差距表 + “还未实现的功能”表）必须与本文件双向同步：任一处更新 AG-16~AG-24 描述、E 契约、已知差距或有意差异，必须在同一 commit 刷新另一处，并更新 `docs/commands/agent.md` 用户文档 + `COMPATIBILITY.md` + `tests/INDEX.md`。
...
##### 2. 实施要求
- `docs/development/commands/agent.md` 成为 `libra agent` public behavior source-of-truth；本文只保留 Gate 8 追踪和内部 AgentRuntime 边界。
