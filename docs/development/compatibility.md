...
| 最后核对代码现状日期 | 2026-06-10 |
| Grit 参考版本 | **可选补充**（见 PRE-1 修订；主要参考为公开的 upstream Git 文档与 git.git `t/` 测试套件；Grit 如可用则作为额外测试思路来源，但不作为前置阻塞项） |
...
> 本节基于对 GitButler Grit（`/run/media/eli/data/gitbutler/grit`，commit `@166c45534` 2026-06-09）的架构、测试策略和兼容性模型的实地分析，明确 Libra 应借鉴什么、应避免什么。

### Grit 的核心经验

Grit 是从头重实现 Git 的 Rust 项目，其兼容性策略可概括为**"测试即真相源"**：

1. **无人工维护的兼容性矩阵**。Grit 没有 `COMPATIBILITY.md` 式的 feature-by-feature 人工分类表；兼容性的唯一真相源是 ~1,600 个 per-test TOML（`data/tests/**/*.toml`）和自动生成的 HTML dashboard。每个测试的通过/失败状态直接驱动兼容性声明。
2. **行为等价优先于语义优雅**。Grit 的设计哲学是 *"Where Git's behavior is surprising, Grit reproduces the surprise."* 这意味着不依赖人工判断某个 Git 行为是否"合理"，而是以测试通过作为兼容的唯一度量。
3. **性能回归防护制度化**。`bench/OPTIMIZATION.md` 要求任何优化都必须通过 **no-regression gate**：修改后的 `data/tests/` 通过数不得低于基线。性能改进与正确性测试挂钩，而非独立的 benchmark。
4. **范围文档极简但明确**。`docs/v1-scope.md` 用 in-scope / out-of-scope 两列清晰划定边界，没有模糊的 `evaluate` 或 `partial` 中间态。
5. **最小外部依赖与显式上下文**。`unsafe_code = "forbid"`；时间、随机性、环境通过显式 context 注入，不从隐藏全局状态读取。
...
| **兼容性声明方式** | 测试通过即兼容，无人工矩阵 | 四 tier 人工矩阵（supported/partial/unsupported/intentionally-different） | Libra 需要显式管理用户预期，因为大量 intentional difference（如 `.libra_attributes` 替代 `.gitattributes`）无法通过"测试是否通过"来表达 |
| **测试策略** | 直接跑上游 Git `t/` 测试套件 | 自建 integration-runner + 选择性映射 upstream 用例 | Libra 不能跑完整 Git `t/`（大量测试假设 `.git/` 布局和 Git 存储语义），只能提取代表性用例 |
| **hook/filter 策略** | 复刻 Git hooks / clean / smudge | 明确拒绝 stock Git hooks；LFS 使用内置 pointer 管理 | Libra 的安全模型要求 fail-closed，避免任意用户脚本执行 |
...
3. **建立性能 no-regression gate 的等价机制**。
   - Grit 用测试通过数作为性能优化的约束；Libra 应在 runner 中增加**性能断言切片**（如大仓库 `status` 超时上限、`log` 流式输出内存上限）。
   - 任何影响大历史、大文件、batch、regex、network 的改动，必须在 PR 中附带性能边界证据（不是独立 benchmark，而是集成场景中的耗时/内存断言）。

4. **拒绝模糊的中间态**。
   - Grit 没有 `evaluate` 这种悬而未决的分类；Libra 的 `evaluate` 条目必须有明确的决策期限和责任人，到期未决默认 `reject`。
   - `partial` tier 必须具体到"哪些 flag 已支持、哪些未支持"，不能作为笼统的免责条款。

5. **不 chasing Git 的每一个 surprised behavior**。
   - Grit 必须 reproduce Git 的 surprise，因为目标是 drop-in 替代。
   - Libra 的目标是"足够兼容以支持常见脚本和工具链"，因此对 Git 的 edge-case surprise 有权声明 intentional difference，只要该差异在 `COMPATIBILITY.md`、命令文档和 runner 断言中三方一致。
...
| 性能与效率 | 分阶段、小切片可控，但**缺乏性能基准与回归防护机制**。Grit 的 no-regression gate（测试通过数不可降）证明性能优化必须与正确性测试挂钩。 | 每个参数至少 1 条端到端测试 + 1 条代表性 upstream/Grit 用例；**大历史/大文件/batch/regex/network 类条目必须在矩阵中标注 `performance_note`**（输入上限、超时、流式策略）；runner 增加性能断言切片；不得在单 PR 迁移整个 Git `t/` 文件。 |
...
| 可扩展性与可维护 | 机器矩阵 + owner scenario + check-plan 是正确维护模型，但**人工矩阵权重过高**，存在矩阵与代码实现漂移的风险（Grit 用 per-test TOML 作为唯一真相源避免了此问题）。 | `compatibility-matrix.yaml` schema 必须稳定；每行绑定 owner 场景、declined_ref、测试来源和状态；**新增 `test_evidence` 和 `last_verified` 字段**，确保矩阵不是"僵尸状态"；当矩阵与自动化测试冲突时，以测试为准修正矩阵。 |
...
| `performance_note` | 性能边界说明：输入大小上限、超时阈值、流式/分页策略、内存上限；对大历史/大文件/batch/regex/network/archive/filter 类条目必填 |
...
- 新增或扩展 compat guard。推荐新建 `tests/compat/parameter_matrix_alignment.rs`；如维护者选择复用 `tests/compat/matrix_alignment.rs`，必须在测试名中清楚区分参数矩阵检查。guard 至少校验：
  ...
  - (g) `risk=high` 或涉及大历史/大文件/batch/regex/network/archive/filter 的行 `performance_note` 非空；
...
7. **收尾**：矩阵行置 `done`，回填 `tests`、`git_tests` 与 `grit_tests` 迁移状态。
...
1. **矩阵完整性**：`compatibility-matrix.yaml` 覆盖本文全部参数级种子项且每行有 `action`/`phase`/`status`/`risk`/`owner_scenario`（用户可见行为）/来源字段；任何 `evaluate` 行不得带入其所属实现阶段；参数矩阵 guard 在 CI 常绿。
...
11. **性能边界明确**：大历史、大文件、batch、regex、network 和 archive/filter 类条目有输入大小、超时、流式输出或分页策略；测试至少覆盖一个边界条件。
...
14. **性能边界可验证**：所有 `risk=high` 或大历史/大文件/batch/regex/network/archive/filter 类条目都有非空的 `performance_note`，且 runner 或命令测试中至少覆盖一个边界条件（如输入上限、超时、流式输出）。

（执行本计划时必须同时遵守上文全部 0-11 条规则以及新增的 13-14 项验收标准。）
