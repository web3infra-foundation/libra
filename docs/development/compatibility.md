...
## 计划执行实施规则（13条强制要求，编号 0-12）

以下是执行本计划时的**强制操作规程**（0-11 为既有用户要求，12 为本文档为消除执行歧义新增的交叉核对要求）。所有参与者（包括 AI agents）必须严格遵守：

0. 出现任何其他的内部服务错误，也不暂停 goal 的任务，可以设置暂停 10 分钟，重试到任务重新开始；同一操作连续 3 次失败时，必须记录失败命令、错误摘要、已尝试的恢复动作和下一次可重试条件，避免无界忙等，并继续推进其他不依赖该操作的核对；
...
3. 每一个分析出来的**代码改动需求**，改动完成后是对当前版本 patch 加 1，发布一个新的版本，例子：当前的版本如果是 version = "0.17.500"，则 patch 加 1 后 version = "0.17.501"，worker 和 web 子目录下的 package.json 中的版本同步保持。不修改 Cargo.lock，而是执行 `cargo build --release` 命令，让工具链进行修改；完成修改版本后调用系统 `libra` 命令执行显式 `libra add <files>`、检查 `libra diff --staged`，再 `libra commit -s -m ...` / `libra push origin main` 推送到 GitHub，构建出来的 release 版本的 `libra` 命令拷贝到 `$HOME/.libra/bin/libra` 这个路径。历史要求中出现的 `commit -a` 不授权纳入无关 tracked 变更；若工作区含与本条目无关的修改，必须暂停提交并说明，不得清理、覆盖或顺手提交；
...
12. 每次编辑本文档或推进本计划前，必须重新执行交叉核对：读取或搜索 `src/cli.rs`、`COMPATIBILITY.md`、相关 `docs/commands/<cmd>.md`、`docs/development/integration-scenarios.yaml`、`tools/integration-runner/src/registry.rs`、相关 `tools/integration-runner/src/scenarios/*.rs`、`docs/improvement/compatibility/declined.md`、相关 SQL 迁移和实现证据。最低验证门是 `cargo test --test compat_matrix_alignment` + `cargo run --manifest-path tools/integration-runner/Cargo.toml -- check-plan`；若本次只改文档且环境不允许运行，必须在最终说明中列出未运行原因和已完成的静态核对证据。

这些规则优先于其他流程，是本计划在 Libra 格式仓库中执行的最高约束。

**注意（规则 12 交叉核对结果）**：本文档曾为精简 stub（执行规则 + 占位）；当前已恢复为执行规则 + 详细计划。更细的 C1-C9 路线、矩阵、Grit 参考、集成测试方案、declined 证据等活文档仍分散到：
...
- Grit 复核采用 `/run/media/eli/data/gitbutler/grit` 当前 HEAD `166c45534319cb280e7a026336a8a06c53cea30f`（commit date `2026-06-09T15:45:09+02:00`）：`grit/src/commands/*.rs` 当前 169 个，`data/tests/**/*.toml` 1,605 个，汇总 `tests_total=46,364`、`passed_last=42,295`、`fully_passing=1,303`、`expect_failure=98`、`in_scope=yes=1,362`、`in_scope=skip=243`，与本文后续 Grit 参考一致。
- 2026-06-12 补充复核：上述 Grit 数字由当前 `data/tests/*/*.toml` 重新计算，不来自 dashboard HTML；其中 43 个 TOML 含 `expect_failure > 0`，19 个同时标记 `fully_passing=true`，且当前数据中有 20 个 `fully_passing=true` 文件的 `passed_last != tests_total`，因此 TOML 快照只能作为测试状态线索，不能单独作为 Libra 完成证据；`docs/development/integration-scenarios.yaml` 与 `integration-scenarios/*.md` 当前均为 39 个场景（含 1 个 `live.github-*`）；`tests/compat/` 当前 23 个 Rust guard 文件，根 `Cargo.toml` 注册 `compat_matrix_alignment`。
- 人工长文计数已出现可观测漂移：`integration-scenarios.yaml`、runner registry 和场景 MD 当前均为 39 个场景，但 `docs/development/integration-test-plan.md` 仍有若干历史 "38/38" 叙述。本文后续所有场景数量、完成率、`last_verified`、`done` 状态都必须来自 `check-plan`/guard/状态视图的生成结果；人工叙述中的计数只能作为背景，不得作为完成证据。
- 本轮最低验证门已实际执行并通过：`cargo test --test compat_matrix_alignment`（5 tests passed）与 `cargo run --manifest-path tools/integration-runner/Cargo.toml -- check-plan`（yaml/doc/implemented 场景均为 39，`documented_but_not_implemented=0`）。
- 已知不准确点已在本节更新（原 "parameter_matrix_alignment" guard 不存在；主 guard 是 `compat_matrix_alignment`；Grit 仅为可选补充，不是 Libra 的规范来源）。
- 未来对本文档的任何编辑都必须重新执行规则 12 交叉核对（读 `src/cli.rs`、`COMPATIBILITY.md`、`tools/integration-runner/src/registry.rs`、`scenarios/*.rs`、相关迁移等），并运行 `cargo test --test compat_matrix_alignment` + `cargo run --manifest-path tools/integration-runner/Cargo.toml -- check-plan`。
...
## 本轮实地核对新增发现（2026-06-12）

本轮对 `/run/media/eli/data/gitbutler/grit` 与当前 Libra 仓库的实地核对，进一步把本计划收敛为"先产品化事实来源，再批量实现 flag"。新增发现如下：

| 发现 | 对方案维度的影响 | 本文档修正 |
|------|------------------|------------|
| Grit 的状态树是每测试文件一份 TOML，且 `fully_passing` 由 `failing == 0` 推导；当前仍存在 `expect_failure` 与历史计数不一致。 | 完整性、功能正确性、可靠性：不能把历史 dashboard/TOML 等同于真实兼容完成态。 | `expected_failure_handling`、`verification_command`、`status_source=generated` 变为完成态必备字段；Grit TOML 只作为测试选择线索。 |
| Grit 公开了 hooks、submodule、sparse checkout、server protocol、GPG/SSH signing 等 Libra 非目标或高风险表面。 | 合理性、安全性、接口兼容性：Grit in-scope 不能自动转成 Libra backlog。 | 固定来源裁决顺序；高风险表面默认 `reject` / `intentional-diff`，除非独立 RFC 通过。 |
| `grit-protocol` 通过 `Command::new` 包装 `upload-pack`/`receive-pack`，并在部分非零退出但有 stdout 时继续返回。 | 数据流/控制流、安全性、容错性：该模式会绕过 Libra 的 SQLite 事务、稳定错误码和 JSON envelope 边界。 | 新增 protocol subprocess 安全门；任何类似桥接必须证明 env 清理、超时/kill、stderr 脱敏、退出码映射和协议帧边界。 |
| Grit 性能文档显示 per-file attributes/config 重复解析可造成 50x-1000x 级退化；当前代码仍有 576 处 `ConfigSet::load` 文本匹配。 | 性能与效率、可扩展性：attributes/config/pathspec 不能先实现再补缓存。 | `performance_note` 对 pathspec/config/attributes/filter/regex/batch/network 类条目必填，且需声明缓存失效策略。 |
| Libra 当前没有 `docs/development/compatibility-matrix.yaml`，但 runner/yaml/MD/registry 已能由 `check-plan` 生成 39 个场景的事实计数；其他长文仍有旧 "38/38" 计数。 | 可维护性、可靠性：人工长文计数会漂移，不能作为状态事实。 | PRE-5 增加硬编码计数漂移检测要求；状态事实只能来自 guard/check-plan/状态视图。 |

## 对标分析：Grit 经验与 Libra 选型差异（2026-06-10）

> 本节基于对 GitButler Grit（`/run/media/eli/data/gitbutler/grit`，commit `166c45534319cb280e7a026336a8a06c53cea30f`，commit date `2026-06-09T15:45:09+02:00`，2026-06-12 重新核对）的架构、测试策略和兼容性模型的实地分析，明确 Libra 应借鉴什么、应避免什么。

### Grit 的核心经验

Grit 是从头重实现 Git 的 Rust 项目（workspace 版本 `0.3.99`，当前复核为 169 个 `grit/src/commands/*.rs` command module），其兼容性策略可概括为**"以上游测试套件结果为主真相源、手工文档划定范围边界"的混合模型**：

1. **手工范围边界 + 自动测试状态的混合模型**（2026-06-11 复核修正，原稿表述需更正）。Grit 确实没有 `COMPATIBILITY.md` 式 feature-by-feature 的四 tier 人工分类表，但它**并非"纯测试真相源、无任何人工文档"**：范围边界由手工文档 `docs/v1-scope.md`（in-scope / out-of-scope 两列表）与 `docs/manpage-parity.md`（11 个 plumbing 命令的行为核对清单，当前多为未填模板）划定；`data/tests/**/*.toml` 共 1,605 个、每测试文件一份，是**自动生成的测试状态缓存**（字段 `tests_total` / `passed_last` / `failing` / `fully_passing` / `status`），其中仅 `in_scope` 是手工覆盖字段。当前快照中 1,362 个文件 `in_scope=yes`、243 个 `in_scope=skip`、46,364 个 `tests_total`、42,295 个 `passed_last`、1,303 个文件全绿。换言之 Grit 的行为真相是"运行上游 `t/` 套件的结果"，TOML 只是机器维护的状态镜像，HTML dashboard 由 Python 脚本据其生成（opt-in）。
   - 重要细节：`fully_passing=true` 不是简单等同于 `passed_last == tests_total`。Grit 的状态更新脚本按 `failing == 0` 计算 `fully_passing`，同时仍记录 `test_expect_failure`，当前快照还存在少量 pass-count 与 total-count 不一致的历史状态文件。Libra 的矩阵不得用"预期失败"、历史 dashboard 或 TOML 快照伪装完成态；若一个 Git 用例被判定为 Libra non-goal，必须转化为明确拒绝/差异测试，而不是记为通过。
   - Grit `docs/v1-scope.md` 也明确提醒 `data/tests/` 数字可能滞后于当前 binary。Libra 因此不能把 dashboard 或 TOML 快照当作永久证据；`last_verified` 必须来自同一轮可重跑命令，而不是从历史状态文件手工抄数。
...
5. **最小外部依赖与显式上下文**。workspace 默认 `unsafe_code = "forbid"`、clippy `unwrap_used` / `expect_used` = `deny`；但 `grit-lib/Cargo.toml` 当前将 `unsafe_code = "allow"` 作为 crate 级例外，实测 unsafe 主要集中在 `grit-lib/src/git_date` 的 libc/time 兼容 FFI（对标 Git `date.c`），另有少量测试工具路径。Libra 不应复制这种 crate-wide 例外：若未来确需 unsafe，必须是模块级、注释化、测试覆盖的最小边界。注意 Grit 另 vendor 了整棵上游 Git C 源码（629 个 `.c`，仅作参考与少量 crypto FFI 绑定）——Libra 不应复用该 C 树（与 PRE-1 一致）。
...
3. **建立性能 no-regression gate 的等价机制**。
   - **Grit 的实证教训（2026-06-12 复核）**：Grit 在 1 万文件级仓库上比 C Git 慢约 50×–1030×（最差 `grep` 1031×、`stash` 1030×、`merge` 945×），根因是在循环内**逐文件重复加载 `.gitattributes` 与 config 级联**；当前代码文本匹配到 `ConfigSet::load` 576 处，`load_gitattributes_for_diff` 仍是集中风险点。其修复计划（`bench/OPTIMIZATION.md` P1–P5）以缓存为主，且每个缓存必须在相关 mutation（config 写入、`.gitattributes` 变更）上失效。Libra 在 Phase 5 实现 attributes/config 读取时应**从一开始就避免 per-file 重复解析**，并以 config/attributes 测试家族作为缓存失效的守护。
...
7. **文档声明必须与代码事实交叉验证**。
   - Grit 的 `grit-lib/README.md` 仍写有 "No unsafe code"，但当前代码中 `grit-lib/src/git_date/**` 存在 libc/time 兼容 FFI，`grit-protocol` 公开 server-side smart protocol 包装也会 `Command::new` 启动 `grit upload-pack` / `receive-pack`。这些不是对 Grit 的否定，而是提醒 Libra：兼容性计划不能只引用对方 README、scope 文档或 dashboard，必须读取实现路径并记录证据锚点。
   - Libra 的矩阵字段 `source_of_truth` 因此不得只写文档来源；凡是安全、数据流、控制流相关条目，必须同时有 Libra 代码路径或测试路径证据，Grit 只能作为补充观察。

### Grit 再分析补充：对 Libra 方案的多维评估（2026-06-11）

本节基于对 `/run/media/eli/data/gitbutler/grit` 的实际读取补充评估，重点参考：`AGENTS.md`、`TESTING.md`、`docs/v1-scope.md`、`docs/manpage-parity.md`、`bench/OPTIMIZATION.md`、`scripts/test_status.py`、`scripts/apply-test-run-results.py`、`grit-lib/README.md`、`grit-lib/Cargo.toml`、`grit-protocol/src/{upload_pack,receive_pack}.rs`、`grit-http-server/src/main.rs`、`data/tests/**/*.toml`。结论是：Libra 当前方案方向正确，但必须把"评估意见"固化为机器矩阵字段和 CI 守护，否则容易变成一份无法驱动实现的长文档。
...
6. **Grit v1 in-scope 不等于 Libra backlog**：Grit 把 hooks、submodule、sparse checkout、GPG/SSH signing、server protocol 等列入 v1 范围，但这些在 Libra 中多为 non-goal 或 intentional difference。引用 Grit 测试时必须先经过 Libra 产品边界分类，禁止因为 Grit 通过某测试而自动把对应功能升级为 Libra 必做项。
7. **非零退出与部分输出不得自动视为成功**：Grit 的 protocol wrapper 对部分 server-side 非零退出在存在 stdout 时会返回成功。Libra 的 CLI/runner 不能采用这种兼容捷径；任何非零退出降级为 warning 的行为必须在 `compat_contract`、exit code、JSON envelope、stderr 和测试中显式建模。
8. **expected-failure 不得伪装为完成态**：从 Git/Grit 迁移测试时，`test_expect_failure`、known failure、manual skip、dashboard stale 都不能作为 Libra `status=done` 的证据。要么转成 Libra 明确拒绝/差异测试，要么保持 `planned`/`blocked` 并写明重启条件。
9. **host 工具依赖必须显式化**：任何依赖系统 `git`、`ssh`、`gh`、`gpg`、editor、pager、browser launcher 或外部 helper 的测试/实现，都必须在矩阵和场景中标注 `host_dependencies`、隔离 PATH、凭据来源和 skip/失败语义，避免把 host 行为误判为 Libra 能力。
10. **来源与许可证边界必须可审计**：upstream Git/Grit 只能作为行为参考和测试来源。不得复制 Grit vendored Git C 树、libgit-sys 绑定或 protocol bridge；若移植 upstream/Grit 测试片段，必须记录来源路径、commit/tag、license/provenance 处理方式，并优先改写为 Libra 自有黑盒场景。
...
结合当前 Libra 与 Grit 证据，方案可以继续推进，但只有 Phase 0 可立即执行；Phase 1+ 的任何代码实现都必须先等 `compatibility-matrix.yaml`、参数矩阵 guard、declined 对账和 P0 拒绝/差异测试落地。

| 审计项 | 当前判断 | 本文档补强 |
|--------|----------|------------|
| 合理性 / 可行性 | 方向成立：Libra 选择性兼容 Git，不能复制 Grit 的 drop-in 目标；但现有方案仍依赖人工矩阵，不能直接批量实现。 | 明确 Grit 为补充来源；Phase 0 先做机器矩阵、守护和状态视图，不把 PRE-1～PRE-5 误作 Phase 0 入口。 |
| 完整性 / 功能正确性 | 顶层命令、集成场景和 declined 登记基本齐备；参数级完整性仍未证明。 | `compatibility-matrix.yaml` 必须从 CLI grammar、命令文档、Git docs/tests、runner owner 场景联合生成，且禁止 `unclassified` 进入实现阶段。 |
| 安全性 / 可靠性 | 最大风险不是普通 flag 缺失，而是 hooks/filter/server protocol/subprocess/secret/network 这类信任边界。 | 高风险安全门新增 protocol subprocess、env 清理、超时、stderr 脱敏、非零退出建模和负向测试要求。 |
| 数据流 / 控制流 | SQLite refs/HEAD/reflog/sequencer 与 `.libra_attributes` 是 Libra 的正确边界。 | 每个矩阵行必须记录 `data_flow`、`control_flow`、事务/锁/失败出口；禁止为通过 Git 测试引入 `.git/refs` 或 `.git/sequencer` 事实源。 |
| 性能 / 效率 | Grit 的 attributes/config per-file 重复解析暴露出设计期性能门缺口。 | pathspec/config/attributes/filter/regex/batch/network 类条目必须先写 `performance_note` 与缓存失效策略，再实现。 |
| 互操作 / 合规 | Grit 的测试与 vendored Git 树只能作为行为参考；真实 remote、host git/ssh/gh 依赖和 license/provenance 必须可追踪。 | 矩阵新增 host 依赖、互操作面和来源合规字段；复制 upstream/Grit 测试或片段时必须保留来源说明，不得复用 vendored Git C 或 Grit protocol bridge。 |

因此，本计划的首要改进不是立即实现更多 Git flag，而是先把"参数级事实来源、状态机、守护测试、风险门"产品化。只有当单个条目满足本文的入口条件和验证门时，才允许进入代码实现。

### 本次 Grit 再分析后的方案修正（2026-06-12）

本次重新读取 `/run/media/eli/data/gitbutler/grit` 后，本文档对 Libra 方案的结论进一步收敛为：方向可取，但必须把 Grit 的“机器状态驱动”经验落到 Libra 自己的 runner 和矩阵工具链，而不是继续扩大人工说明文字。

| 评估维度 | 当前结论 | 新增或强化的文档要求 |
|----------|----------|----------------------|
| 合理性 | Libra 以选择性兼容 Git 为目标是合理的；Grit 的 drop-in 目标只适合作为对照，不适合作为 Libra backlog。 | 保持 Grit 为 optional supplement；任何 Grit in-scope 功能进入 Libra 前必须先经过 non-goal / intentional-difference 分类。 |
| 可行性 | 全量移植 Grit 的 upstream `t/` harness 不可行；用 owner scenario + 参数矩阵抽样是可行路径。 | Phase 0 只交付矩阵、guard、状态视图和 P0 拒绝/差异测试，不允许批量实现 Git flag。 |
| 完整性 | `COMPATIBILITY.md` 的命令级 tier 已有价值，但无法证明 flag 级覆盖完整。 | `compatibility-matrix.yaml` 必须从 `src/cli.rs`、命令文档、Git docs/tests、runner registry 和 declined 登记簿联合生成；未知项只能是 `unclassified`，不能被省略。 |
| 安全性 | 最大风险集中在 hooks/filter/protocol subprocess/server-side/secret/remote 写入，而非普通只读 flag。 | 这些表面默认 `risk=high`；公开 server-side Git 命令和外部 helper 桥接默认拒绝。 |
| 功能正确性与接口兼容性 | 用户可见契约不能只用 `supported`/`partial` 概括；同一 flag 的 human、JSON、exit code 和文档必须一致。 | `compat_contract` 成为必填字段；`accepted-no-op`、`explicitly-rejected` 和 `intentional-difference` 必须有测试断言。 |
| 数据流与控制流正确性 | Libra 的 SQLite refs/HEAD/reflog/sequencer 是架构边界；为通过 Git/Grit 测试引入 `.git` 事实源会破坏设计。 | 每行矩阵记录 `data_flow` / `control_flow`；写操作必须说明事务、锁、失败出口和健康检查。 |
| 性能与效率 | Grit 的 `bench/OPTIMIZATION.md` 证明 attributes/config/pathspec 类实现若缺少缓存，会出现 50x-1000x 级退化。 | `performance_note` 对大历史、大文件、regex、pathspec、config、attributes、ignore、filter、network 类条目必填；Phase 0 要产出最小性能边界清单。 |
| 可靠性与容错性 | Grit 以每测试文件独占 TOML + atomic write 降低状态漂移；Libra 当前缺少等价的生成状态视图。 | 新增 PRE-5：兼容状态视图必须由矩阵 + runner/check-plan 生成，不得手工维护 `last_verified` 完成态。 |
| 兼容性与互操作性 | Grit 的 host Git、HTTPD、GPG skip 和 server protocol 路径提醒 Libra 必须区分“host 工具成功”和“Libra 能力成功”。 | `host_dependencies` / `interop_surface` 必须进矩阵；真实 remote 属于 Wave 3 或 blocked。 |
| 可扩展性与可维护性 | 长文档会过期；Grit 的 dashboard 价值在于把缺口和 pass count 机器化。 | `check-plan` 或新工具必须输出缺口摘要：unclassified、done-without-evidence、stale-last-verified、high-risk-without-controls。 |
| 合规性与标准符合性 | Git/Grit 只能作为行为与测试来源；复制实现代码、vendored C、FFI 或 protocol bridge 不可接受。 | `compliance_note` 必填来源 commit/tag；禁止把 Grit vendored Git C 树、libgit-sys 或 protocol subprocess bridge 作为实现来源。 |

本节形成一个额外执行判断：若未来执行者只能更新本文档叙述，而不能让 `compatibility-matrix.yaml`、guard、runner 或状态视图同步表达同一事实，则该变更不应视为兼容性计划推进完成。
...
## 前置条件（Phase 0 出口，阻塞 Phase 1+ 批量实现）

PRE-1～PRE-5 是 Phase 0 必须完成的交付物，而不是进入 Phase 0 的入口条件。Phase 0 的任务正是事实核对、矩阵建模、拒绝项对账、守护测试建设和状态视图生成；PRE 未完成前，只阻塞 Phase 1 及之后的批量兼容实现，不阻塞 Phase 0 本身。
...
- **Grit 角色**（可选补充，非阻塞）：Grit（GitButler 项目的 Grit）如可用，可作为额外测试用例思路和边界分析的补充来源。推荐位置：与 gitmono 平行的 `gitbutler/grit`（当前发现路径示例 `/run/media/eli/data/gitbutler/grit` @`166c45534319cb280e7a026336a8a06c53cea30f`，2026-06-12 复核），或通过 `LIBRA_GRIT_PATH` 环境变量指定。可读取其 `grit/src/commands/`、`grit-lib/src/`、`grit-protocol/src/`、`grit-http-server/`、`tests/`、`data/tests/`、`docs/` 与 `bench/` 来提取命令/flag、测试状态、安全和性能边界；**不要**尝试复用其 vendored `git/` C 树、libgit-sys 绑定、server subprocess bridge 或 unsafe FFI。所有使用必须在兼容矩阵 `grit_tests` 字段中标注 commit + 来源。不存在时不影响计划推进（主参考为公开 upstream Git）。
- **动作**：
...
- yaml 顶层必须包含 `schema_version`、`matrix_created`、`generated_from` 和 `manual_fields`。`generated_from` 至少记录 `src/cli.rs`、`COMPATIBILITY.md`、`docs/commands/`、`integration-scenarios.yaml`、`tools/integration-runner/src/registry.rs`、upstream Git 文档/测试来源和可选 Grit commit；`manual_fields` 明确哪些字段允许人工编辑，避免把测试结果和 `last_verified` 当作普通叙述性文本维护。
- 每行 schema 建议包含以下字段：
...
| `interop_surface` | `local-only` / `host-git-fixture` / `remote-client` / `live-remote` / `server-side` / `libra-native`；触达 remote、host 工具或 server-side 表面时必填 |
| `host_dependencies` | 运行或验证本行需要的外部工具/服务（如 `git`、`ssh`、`gh`、`gpg`、editor、pager、browser、credential helper）；无依赖填 `none`，不得省略 |
...
| `verification_command` | 最近一次验证本行 `test_evidence` 的可复现命令；`status=done` 时必填（例如具体 `cargo test --test ...` 或 `integration-runner run --only ...`） |
| `status_source` | `generated` / `manual-bootstrap` / `imported`；`done` 行最终必须是 `generated`，Phase 0 初建时才允许 `manual-bootstrap` |
| `expected_failure_handling` | 引用的 Git/Grit 用例若含 `test_expect_failure`、skip、known failure 或 dashboard stale，必须说明是 `converted-to-negative-test` / `converted-to-intentional-diff` / `blocked` / `not-applicable` |
| `risk_controls` | high/medium 风险条目的控制措施摘要：输入校验、事务/回滚、超时、env 清理、脱敏、资源上限、负向测试等 |
...
| `compliance_note` | upstream Git/Grit 来源、commit/tag、license/provenance 处理方式；仅作行为参考时写 `behavior-reference-only`，复制或改写测试片段时必须写来源 |
| `decision_deadline` | `evaluate` 行专用：决策截止日期（ISO 8601）；到期未收敛则按默认规则处理 |
| `decision_owner` | `evaluate` 行专用：负责收敛决策的维护者或团队标识 |
| `last_verified` | 本条目的矩阵状态最后一次与代码/测试核对通过的日期（ISO 8601）；`status=done` 时必填。自动刷新落地前可由维护者人工回填，但同一 PR 必须保留 `test_evidence` 指向的命令/场景结果；不得只靠叙述性文档背书 |
| `notes` | 备注 |
...
- 新增或扩展 compat guard。推荐新建 `tests/compat/parameter_matrix_alignment.rs`；如维护者选择复用 `tests/compat/matrix_alignment.rs`，必须在测试名中清楚区分参数矩阵检查。guard 至少校验：
  ...
  - (i) `last_verified` 必须是合法 ISO 8601 日期、不得早于 `matrix_created`、不得晚于当前日期；当验证器能读取运行记录时，`status=done` 的 `last_verified` 不得早于对应 `test_evidence` 的最近通过时间。不得要求普通本地 CI 查询 PR 合并日期；没有外部 PR 元数据时以测试运行证据为准。
  - (j) `status=done` 的行必须有 `verification_command`，且命令字符串必须对应仓库已知测试入口（`cargo test`、`integration-runner check-plan/run`、Web/worker 检查等）；
  - (k) `status=done` 的行 `status_source` 必须为 `generated`；`manual-bootstrap` 只允许在 Phase 0 初建且 `status!=done` 时出现；
  - (l) `risk=high` 或触达 refs/index/object/network/filter/hook/secret/subprocess 的行 `risk_controls` 非空，且至少包含一个负向测试或 fail-closed 证据；
  - (m) `owner_scenario` 必须能在 `integration-scenarios.yaml`、Command → Scenario Map 和 runner registry 三处解析到同一场景。
  - (n) `interop_surface` / `host_dependencies` 必须与 owner 场景一致；`live-remote` 行必须绑定 Wave 3 或显式 `blocked`；`server-side` 行默认只能是 `reject` / `intentional-diff`，除非有 RFC 引用；
  - (o) 引用 Git/Grit expected-failure、skip 或 stale dashboard 的行，`expected_failure_handling` 必须非空，且 `status=done` 不得依赖这些状态本身；
  - (p) 复制、改写或直接引用 upstream Git/Grit 测试片段的行必须有 `compliance_note`；任何 `compliance_note` 指向 Grit vendored Git C 树、libgit-sys 绑定或 protocol bridge 作为实现来源时，guard 必须失败。
...
- 建立只读生成的兼容状态视图，来源只能是 `docs/development/compatibility-matrix.yaml`、integration-runner registry / 最近一次运行结果、`check-plan` 输出、compat guard 结果和可选 Grit/Git 测试来源索引。人工不得直接维护 `done`、`last_verified`、`test_evidence` 的完成态事实。
- 状态视图可以落地为 `check-plan` 汇总输出、单独生成的 Markdown/JSON，或兼容 guard 的结构化报告；不强制固定文件名，但必须可在 CI 或本地命令中重建。
- 最低汇总项：`unclassified` 条目数、`status=done` 但缺少 `test_evidence` / `verification_command` / `last_verified` 的条目、`last_verified` 早于矩阵或测试证据的条目、`risk=high` 但缺少 `risk_controls` 的条目、需要 `performance_note` 但为空的条目、`host_dependencies != none` 但 owner 场景未声明隔离/skip 的条目、引用 Git/Grit expected-failure/stale status 但未填 `expected_failure_handling` 的条目、人工长文中与生成源冲突的硬编码计数（例如场景总数、已实现数、测试通过数）。
- 生成状态必须保留输入来源摘要：yaml 场景数、MD 场景文件数、runner registry 数、矩阵行数、`done` 行数、blocked 行数、以及可选 Grit/Git 测试来源 commit/tag。若人工文档中的计数与这些生成值冲突，生成视图必须报告 drift；实现阶段不得以"文档说明已更新"绕过该 drift。
- 状态视图只表达当前事实，不替代 `COMPATIBILITY.md` 或命令文档的用户承诺；当状态视图与人工文档冲突时，必须先修正文档或矩阵，再继续实现。
- **验收**：本地可运行一个明确命令生成或打印状态视图；Phase 0 出口时该视图显示 0 个阻塞类缺口，或每个缺口都有 `blocked` 原因和重启条件；该命令接入 `check-plan` 或 compat guard，避免手工巡检。
...
- 不把 host 工具或第三方实现当作 Libra 能力。测试可用系统 `git`/`ssh`/`gh` 构造 fixture 或验证真实 remote，但必须在矩阵和场景中标注依赖、隔离环境、skip 条件和凭据来源。
- 不复制外部实现。upstream Git/Grit 的代码、vendored C 树、protocol bridge、unsafe FFI 或 libgit-sys 绑定只能作为行为研究输入；移植测试片段必须记录来源和 license/provenance，并优先改写为 Libra 自有黑盒场景。
...
| Matrix/status evidence | 兼容状态事实由测试和 runner 生成；人工文档只能描述计划、范围和理由。 | guard 检查 `status_source`、`verification_command`、`test_evidence`；状态视图由工具生成且不手改。 |
| Host interop | fixture 生成、真实 remote、host git/ssh/gh/gpg/editor/pager 依赖必须与 Libra 行为分层，不得把 host 成功误记为 Libra 支持。 | `host_dependencies` / `interop_surface` 字段；隔离 PATH；Wave 3 或 env-skip 记录。 |
| Source compliance | Git/Grit 来源只作为行为参考；复制测试片段必须记录 provenance；不得引入 vendored Git C、Grit protocol bridge 或 libgit-sys 作为实现来源。 | `compliance_note`；review 检查新增文件 license/header；guard 拒绝禁止来源。 |
...
Grit 实现中的 `grit-protocol/src/upload_pack.rs` 与 `receive_pack.rs` 是本安全门的反例参考：它们为 HTTP smart transport 包装 server-side 子进程，并在部分非零退出但有 stdout 时继续返回数据。Libra 若未来确需类似内部协议桥，必须先证明超时、kill、env allowlist、stderr 脱敏、stdout/stderr 帧边界、exit-code 映射和 SQLite 事务边界，而不能直接复制该模式。
...
- **可选**：若 Grit 可用，从其 `AGENTS.md` / `TESTING.md` / `docs/v1-scope.md` / `grit/src/commands/**/*.rs` / `grit-lib/src/` / `grit-protocol/src/` 提取补充的命令/flag 发现、边界测试思路、安全约束和性能风险。
...
4. **测试**：`tests/command/` 下相关测试。按 action 分类（implement/enhance、reject 五件套、intentional-diff 等）；若引用 Git/Grit 用例，必须把 `test_expect_failure`、skip、known failure 或 dashboard stale 转成 `expected_failure_handling`，不能作为完成证据。
...
7. **收尾**：矩阵行置 `done`，回填 `tests`、`git_tests`、`grit_tests`、`test_evidence`、`last_verified`、`verification_command`、`status_source`、`compat_contract`、`source_of_truth`、`interop_surface`、`host_dependencies`、`expected_failure_handling`、`risk_controls` 与 `compliance_note`；若测试结果与矩阵声明冲突，先修正矩阵再合并实现。
...
| expected-failure / stale status | Git/Grit 用例含 `test_expect_failure`、manual skip、known failure，或 Grit TOML/dashboard 可能滞后。 | 不能直接作为完成证据；必须在 `expected_failure_handling` 中转成拒绝/差异测试、重新运行验证，或保持 blocked。 |
...
- 使用 Grit `data/tests/` 时必须记录该 TOML 的 commit、最近验证命令和是否含 `expect_failure`；如果只读取历史状态而未重跑对应文件，只能作为测试选择线索，不能作为 `last_verified`。
...
**入口**：当前 Libra 仓库、公开 Git 文档/测试来源和可选 Grit 路径可读取；不要求 PRE-1～PRE-5 已完成。PRE-1～PRE-5 是本阶段出口条件。
...
- 产出 PRE-5 定义的生成式状态视图或 `check-plan` 汇总，暴露矩阵漂移、缺少证据、缺少风险控制和 stale verification 问题。
...
**出口**：矩阵无 `unclassified` 条目；参数矩阵 guard 进 CI；PRE-5 状态视图可重建且无未解释的阻塞类缺口；上述 P0 拒绝/差异测试全部落地并绿；declined.md 对账完成。
...
13. **测试证据完整性**：矩阵中 `status=done` 的每一行都有非空的 `test_evidence`、`verification_command`、`last_verified`，指向真实存在的测试或场景；guard 能校验 `test_evidence` 的可达性，且 `status_source=generated`。
14. **性能边界可验证**：所有 `risk=high` 或大历史/大文件/batch/regex/network/archive/filter/pathspec/config/attributes/ignore 类条目都有非空的 `performance_note`，且 runner 或命令测试中至少覆盖一个边界条件（如输入上限、超时、流式输出、缓存失效），避免与 Grit 观察到的 per-file config/attributes 重复解析退化同类复发。
...
17. **人工状态不得合并为完成态**：任何只靠人工阅读或文档描述得到的行最多为 `planned`/`in-progress`，不得标 `done`；必须有可重跑验证命令和生成状态后，才能完成阶段出口。
18. **expected-failure 处理可审计**：引用 Git/Grit `test_expect_failure`、manual skip、known failure 或 stale dashboard 的矩阵行必须有 `expected_failure_handling`；`status=done` 行不得以这些状态本身作为通过证据。
19. **host 依赖与互操作面明确**：所有 `host_dependencies != none` 或 `interop_surface != local-only` 的行必须在 owner 场景中说明隔离 PATH、凭据/环境变量来源、skip 条件和 Wave 归属；真实 remote 只能落入 Wave 3 或 blocked。
20. **来源与合规可追踪**：每个 `git_tests` / `grit_tests` 引用都必须能追到路径和 commit/tag；复制或改写测试片段时 `compliance_note` 非空；不得引入 Grit vendored Git C、libgit-sys 绑定或 protocol bridge 作为实现代码来源。
21. **生成计数优先**：场景数量、矩阵行数、完成数、测试通过数和 `last_verified` 只能从 guard、runner、状态视图或测试运行结果生成；人工长文里的硬编码计数若与生成值冲突，必须被状态视图或 guard 标为 drift，并先修正文档或移除硬编码计数后再进入下一阶段。

（执行本计划时必须同时遵守上文全部 0-12 条规则以及新增的 13-21 项验收标准。）
