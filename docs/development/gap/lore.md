...
| diff/merge 深度 | `A..B` 已支持 | 缺 `diff A B`、`A...B`、空白选项、`--diff3` | 可落地 |
...
| 编号 | 项目 | 落地性分析 | 依赖 |
|---|---|---|---|
| 1.1 | dirty-set cache、`libra dirty`、`status --cached`、`status --check-dirty` | 通过 SQLite `working_dirty` 表可落地；默认 `status` 仍全量安全 reconcile，`--cached` 才走快路径，`--check-dirty` 只复核已缓存 dirty 集合 | migration |
...
| 1.3 | diff3 conflict markers ✅、`merge --dry-run`、`--restart` | diff3 已落地：Git 兼容配置 `merge.conflictStyle`（merge/diff3，非法值硬错），merge+cherry-pick 共享行级渲染器输出 `||||||| base` 祖先块（rebase 独立整文件实现暂不支持）。dry-run/restart 待做：merge state 已有 base/target/orig_head，可内存演算；文件写入路径需隔离——注意 `merge_tree_items` 内 `try_merge_blob_contents` 会 save_object 自动合并 blob（dry-run 需处理）；真实 merge 冲突退出码为 128 | 1.2 |
| 1.4 | positional diff、whitespace flags、`--diff3` | `A..B` 已存在，继续补 `A B`、`A...B` 和空白选项 | rev-parse |
| 1.5 | branch/repo metadata KV | protect/archive/lineage 的基石；SQLite 表最自然 | migration |
...
| 1.8 | `merge --autostash` | 复用 stash；冲突后保留 stash | stash |
| 1.9 | `log --trailer`（含 `--only-trailers` 展示） | `commit --trailer` 已实现（commit.rs:143，含 `append_trailers` 与 `-s` Signed-off-by），本项只补 log 侧 trailer 解析与格式化展示，provenance 用 Git 生态自然表达 | log parser |
| 1.10 | typed metadata 命令族 | repo 用 `config_kv`，branch 用新表，revision 用 trailers/notes，file 用 side-tree | 1.5、1.9 |
| 1.11 | headless `libra service` + notification v1 | 从 `libra code` 抽 loopback/UDS 事件总线；强制 UDS-only（文件权限 0600）或 loopback-only 绑定，不开放任何对外 TCP 端口；承载 dirty-set/automation 触发时须有最小本机访问控制；不要做 hosted server | 1.1、1.6 |
