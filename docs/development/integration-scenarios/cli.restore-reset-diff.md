...
- 真实 merge conflict 后，`libra --json restore tracked.txt` 退出 128 且 stderr 含 `is unmerged`；`restore --ignore-unmerged` 返回结构化 JSON 且不改写冲突文件；`restore --ours` 写入 ours 内容，`restore --theirs` 写入 theirs 内容。
