...
| `show-ref --head` / `--heads` / `--hash` / `--dereference` / `--exists` / `--verify` / pattern | `cli.object-readback` | HEAD/分支引用可列出，annotated tag peel、完整 refname 存在性、精确验证、hash-only 输出、pattern 过滤和缺失 ref 退出码可观察 |
...
| `show-ref --head` / `--heads` / `--hash` / `--dereference` / `--verify` / `--exists` | `cli.object-readback` | HEAD/分支引用、hash-only 输出、annotated tag peel、精确 ref 验证和存在性检查可观察 |
