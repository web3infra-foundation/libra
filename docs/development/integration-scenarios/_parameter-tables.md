...
| `rev-list HEAD` / multi revision / `A..B` / `^A` / `A...B` / `-n` / `--max-count` / `--skip` / `--count` / `--since` / `--after` / `--until` / `--before` / `--first-parent` / `--no-min-parents` / `--no-max-parents` | `cli.object-readback` | 可达提交、范围/排除、限制、跳过、计数、时间过滤、first-parent 和 parent bound reset 输出符合 fixture |
...
| `rev-list HEAD` / multi revision / `A..B` / `^A` / `A...B` / `--count` / `-n` / `--skip` / `--since` / `--after` / `--until` / `--before` / `--first-parent` | `cli.object-readback` | 可达提交输出、范围/排除、计数/限制、时间过滤、first-parent 和 JSON envelope 可观察 |
