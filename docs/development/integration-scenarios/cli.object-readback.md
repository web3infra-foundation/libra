...
- `rev-list --count` 输出过滤后的提交数量；`rev-list -n` 限制输出行数；`rev-list --skip --max-count` 可跳过当前 HEAD 后定位父提交；`--since` / `--after` 与 `--until` / `--before` 时间过滤可观察；multi revision、`A..B`、`^A`、`A...B`、`--first-parent` 和 parent bound reset aliases 均有正向断言。
...
- `ls-files`、高级 `for-each-ref --contains/--merged`、高级 `rev-parse` 和 `rev-list` author/path/cherry-pick traversal filters 不属于当前场景正向覆盖。
