...
- `rev-list --count` 输出过滤后的提交数量；`rev-list -n` 限制输出行数；`rev-list --skip --max-count` 可跳过当前 HEAD 后定位父提交；`--since` / `--after` 与 `--until` / `--before` 时间过滤可观察；multi revision、`A..B`、`^A`、`A...B`、`--first-parent`、`--author`、`--committer`、`--grep`、`-- <path>` path limitation、`--left-right`、`--right-only`、`--cherry-pick`、`--cherry-mark` 和 parent bound reset aliases 均有正向断言；重复 `--grep` 按 OR 匹配，默认大小写敏感，path limitation 会在 JSON 中回显 `pathspecs[]`，`--cherry-pick` 会在 JSON 中回显 `cherry_pick` 并限制 `commits[]`，`--count --left-right --cherry-mark` 输出 Git 兼容三字段计数。
...
- `ls-files`、高级 `for-each-ref --contains/--merged`、高级 `rev-parse`、`rev-list --cherry` 简写和对象/边界遍历输出不属于当前场景正向覆盖。
