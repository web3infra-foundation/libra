...
目的：覆盖 `branch`、`switch`、`checkout` 的分支创建、切换、`switch -C`、`switch --orphan`、`--track` 远端分支、`branch -r`/`-a` 远端列出、detached HEAD（`switch --detach` 与 `checkout --detach`）、兼容 alias、分支重命名/删除和路径恢复行为，并保留 `switch -f` 与 guess 参数未公开的负向检查。
...
- `--track` 远端分支后 `branch --show-current` 必须输出 guessed；`--guess` / `--no-guess` 保持未公开负向行为。
