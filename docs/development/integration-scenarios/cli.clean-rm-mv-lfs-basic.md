...
- 远端面（`lfs locks/lock/unlock/push/fetch/prune/checkout`、`rm --cached/-r/-f/--dry-run/--ignore-unmatch/--pathspec-*`、`mv -f/-k`、`clean -i/-e/-ff/-x` 行为面）委托 `tests/command/remove_test.rs`、`tests/command/mv_test.rs`、`tests/command/clean_test.rs`、`tests/command/lfs_test.rs` 命令测试（见参数覆盖表）。
