...
- Runner scenario code is intentionally one file per scenario under `tools/integration-runner/src/scenarios/`; do not add central per-command match arms or bundle unrelated scenarios into one file. `registry.rs::scenario_registry()` is only the index, and `docs/development/integration-scenarios.yaml` remains the list of truth.
...
- **Assertion-category gate**: `check-plan` enforces that each scenario's declared *source-verifiable* `key_assertion_categories` (`json_envelope`/`fsck`/`gitfix_isolation`/`negative_exit`/`lbr_error`/`conflict_markers`/`gh_lifecycle`/`cleanup_guard`/`file_exists`) leave a matching assertion signal in `scenarios/<id>.rs` (`assert_json_ok`/`--json`, `fsck`, `ctx.gitfix`, a `, false)` negative call, `assert_lbr_or_text`, `<<<<<<<`, `ctx.gh`, `GhRepoCleanupGuard`, `ensure_file`/`.join(`). Declaring a category without exercising it fails `check-plan`. Runner-enforced/semantic categories (`global_db_isolation`/`vault_isolation`/`no_secret_leak`/`intentional_difference`) are advisory.
