...
These commands are slated for future implementation according to the project roadmap. Several commands once listed here are now implemented — `gc`, `prune`, `maintenance`, `remote show <name>`, `notes` (add/append/copy/edit/show/list/remove), and `archive` — and have moved out of this list. The full per-command compatibility status (`supported` / `partial` / `unsupported` / `intentionally-different`) is tracked in [`COMPATIBILITY.md`](COMPATIBILITY.md).
