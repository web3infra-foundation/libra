...
async fn test_intent_flow() {
    ...
    let intent_history = HistoryManager::new_with_ref(
        storage.clone(), 
        libra_dir.clone(), 
        "refs/libra/intent"
    );
    ...
    let main_history = HistoryManager::new(
        storage.clone(), 
        libra_dir.clone()
    );
    ...
    let root_hash = storage.put_tracked(&root_intent, &intent_history).await.unwrap();
    ...
    let child_hash = storage.put_tracked(&child_intent, &intent_history).await.unwrap();
    ...
    let head_hash = intent_history.resolve_history_head().await.unwrap().unwrap();
    ...
    assert_ne!(head_hash, main_head, "Intent branch and Main history branch must be distinct");
}
