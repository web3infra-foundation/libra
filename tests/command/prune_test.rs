...
fn test_prune_packed_v2_prunes_reachable_duplicate() {
	...
	write_fake_idx_v2(repo.path(), &[ObjectHash::from_bytes(&hex::decode(&head).unwrap()).unwrap()]);
...
fn test_prune_packed_v1_prunes_reachable_duplicate() {
	...
	write_fake_idx_v1(repo.path(), &[ObjectHash::from_bytes(&hex::decode(&head).unwrap()).unwrap()]);
