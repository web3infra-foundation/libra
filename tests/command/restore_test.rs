...
#[test]
#[serial]
fn test_restore_source_refuses_locked_agent_traces_branch() {
    // docs/improvement/entire.md Risk #5 ("分支保护遗漏") requires explicit
    // regression coverage that `restore --source <ref>` rejects every
    // member of the locked-ref set — `intent` is covered above, the
    // revision-suffix variant covers `agent-traces~1`, but the plain
    // `agent-traces` form was implicit and only protected indirectly via
    // the suffix-stripping branch of `is_locked_revision`. Pin the
    // direct form so a refactor that loses the `agent-traces` literal
    // from `is_locked_branch` fails this test rather than silently
    // allowing reads from the Libra-managed agent traces history.
    let repo = create_committed_repo_via_cli();
    std::fs::write(repo.path().join("tracked.txt"), "modified\n")
        .expect("failed to modify tracked file");

    let output = run_libra_command(
        &["restore", "--source", "agent-traces", "tracked.txt"],
        repo.path(),
    );
    assert_eq!(
        output.status.code(),
    ...
    let (human, report) = parse_cli_error_stderr(&output.stderr);
    assert!(
        human.contains("refusing to restore from locked branch 'agent-traces'"),
        "unexpected stderr: {human}"
    );
    assert_eq!(report.error_code, "LBR-CLI-003");
    assert_eq!(report.exit_code, 128);

    // The locked-source guard must not modify the worktree before
    // returning the error.
    let content = std::fs::read_to_string(repo.path().join("tracked.txt"))
        .expect("failed to read tracked file");
    assert_eq!(content, "modified\n");
