...
/// Scenario: `branch -m <locked> <new>` and `branch -m <existing>
/// <locked>` must both reject the same three locked names.
/// `rename_branch_impl` (`branch.rs:760`/`:763`) guards both the
/// destination and the source independently — pin both axes so a
/// refactor that loses either guard fails this test rather than
/// letting users rename a normal branch onto `main` / `intent` /
/// `agent-traces` or rename one of those refs to a non-locked name.
///
/// The test creates a `feature` branch to act as the non-locked
/// counterpart in each rename pair; the locked-name guard fires
/// before any storage interaction so the `feature` branch's
/// commit lineage does not affect the outcome.
#[test]
fn test_branch_rename_rejects_locked_branch_names() {
    for locked in ["main", "intent", "agent-traces"] {
        let repo = create_committed_repo_via_cli();
        // Create a non-locked branch we can attempt to rename
        // to/from. `branch feature` is exercised by
        // `test_branch_create_outputs_confirmation` so any failure
        // here is purely about the locked-name guard.
        ...
        // Rename destination is locked.
        let to_locked = run_libra_command(&["branch", "-m", "feature", locked], repo.path());
        assert!(
            !to_locked.status.success(),
            "rename feature -> '{locked}' must fail; stderr: {}",
            String::from_utf8_lossy(&to_locked.stderr),
        );
        let stderr = String::from_utf8_lossy(&to_locked.stderr);
        assert!(
            stderr.contains(&format!("the '{locked}' branch is locked")),
            "unexpected stderr for rename to '{locked}': {stderr}",
        );
        assert!(
            stderr.contains("Error-Code: LBR-CONFLICT-002"),
            "expected LBR-CONFLICT-002 for rename to '{locked}', got: {stderr}",
        );

        // Rename source is locked.
        let from_locked =
            run_libra_command(&["branch", "-m", locked, "feature-renamed"], repo.path());
        assert!(
            !from_locked.status.success(),
            "rename '{locked}' -> feature-renamed must fail; stderr: {}",
            String::from_utf8_lossy(&from_locked.stderr),
        );
        let stderr = String::from_utf8_lossy(&from_locked.stderr);
        assert!(
            stderr.contains(&format!("the '{locked}' branch is locked")),
            "unexpected stderr for rename from '{locked}': {stderr}",
        );
        assert!(
            stderr.contains("Error-Code: LBR-CONFLICT-002"),
            "expected LBR-CONFLICT-002 for rename from '{locked}', got: {stderr}",
        );
    }
}
