...
#[test]
fn test_rev_parse_path_and_state_flags() {
    let repo = create_committed_repo_via_cli();
    let nested = repo.path().join("src/command");
    std::fs::create_dir_all(&nested).expect("failed to create nested dir");

    let prefix = run_libra_command(&["rev-parse", "--show-prefix"], &nested);
    assert_cli_success(&prefix, "rev-parse --show-prefix");
    assert_eq!(
        String::from_utf8_lossy(&prefix.stdout).trim(),
        "src/command/"
    );

    let cdup = run_libra_command(&["rev-parse", "--show-cdup"], &nested);
    assert_cli_success(&cdup, "rev-parse --show-cdup");
    assert_eq!(String::from_utf8_lossy(&cdup.stdout).trim(), "../../");

    let git_dir = run_libra_command(&["rev-parse", "--git-dir"], repo.path());
    assert_cli_success(&git_dir, "rev-parse --git-dir");
    assert!(
        String::from_utf8_lossy(&git_dir.stdout).contains(libra::utils::util::ROOT_DIR),
        "--git-dir should point at Libra storage"
    );

    let inside_worktree = run_libra_command(&["rev-parse", "--is-inside-work-tree"], repo.path());
    assert_cli_success(&inside_worktree, "rev-parse --is-inside-work-tree");
    assert_eq!(
        String::from_utf8_lossy(&inside_worktree.stdout).trim(),
        "true"
    );

    let storage = repo.path().join(libra::utils::util::ROOT_DIR);
    let inside_git_dir = run_libra_command(&["rev-parse", "--is-inside-git-dir"], &storage);
    assert_cli_success(&inside_git_dir, "rev-parse --is-inside-git-dir");
    assert_eq!(
        String::from_utf8_lossy(&inside_git_dir.stdout).trim(),
        "true"
    );

    let storage_worktree = run_libra_command(&["rev-parse", "--is-inside-work-tree"], &storage);
    assert_cli_success(
        &storage_worktree,
        "rev-parse --is-inside-work-tree in storage",
    );
    assert_eq!(
        String::from_utf8_lossy(&storage_worktree.stdout).trim(),
        "false"
    );

    let bare = run_libra_command(&["rev-parse", "--is-bare-repository"], repo.path());
    assert_cli_success(&bare, "rev-parse --is-bare-repository");
...
fn commit_file_revparse(repo: &std::path::Path, file: &str, content: &str, msg: &str) {
    std::fs::write(repo.join(file), content).expect("write rev-parse fixture file");
...
#[test]
fn test_rev_parse_range_two_dot_outputs_b_then_caret_a() {
    let repo = create_committed_repo_via_cli();
    commit_file_revparse(repo.path(), "second.txt", "second\n", "second");
    let base = run_libra_command(&["rev-parse", "HEAD~1"], repo.path());
    assert_cli_success(&base, "rev-parse HEAD~1");
    ...
    let range = run_libra_command(&["rev-parse", "HEAD~1..HEAD"], repo.path());
    assert_cli_success(&range, "rev-parse HEAD~1..HEAD");
    let lines = String::from_utf8_lossy(&range.stdout)
        .lines()
        .map(str::to_string)
        .collect::<Vec<_>>();
    assert_eq!(lines, vec![head_hash, format!("^{base_hash}")]);
}

#[test]
fn test_rev_parse_range_json_exposes_ordered_values() {
    let repo = create_committed_repo_via_cli();
    commit_file_revparse(repo.path(), "second.txt", "second\n", "second");
    let base = run_libra_command(&["rev-parse", "HEAD~1"], repo.path());
    assert_cli_success(&base, "rev-parse HEAD~1");
    ...
    let output = run_libra_command(&["--json", "rev-parse", "HEAD~1..HEAD"], repo.path());
    assert_cli_success(&output, "json rev-parse HEAD~1..HEAD");
    let json = parse_json_stdout(&output);
    assert_eq!(json["data"]["mode"], "range");
    assert_eq!(json["data"]["value"], format!("{head_hash}\n^{base_hash}"));
    assert_eq!(
        json["data"]["values"],
        serde_json::json!([head_hash, format!("^{base_hash}")])
    );
}

#[test]
fn test_rev_parse_sq_quotes_resolved_revisions() {
    let repo = create_committed_repo_via_cli();
    commit_file_revparse(repo.path(), "second.txt", "second\n", "second");
    let head = run_libra_command(&["rev-parse", "HEAD"], repo.path());
    assert_cli_success(&head, "rev-parse HEAD");
    ...
    let output = run_libra_command(&["rev-parse", "--sq", "HEAD", "HEAD~1"], repo.path());
    assert_cli_success(&output, "rev-parse --sq");
    assert_eq!(
        String::from_utf8_lossy(&output.stdout).trim_end(),
        format!("'{head_hash}' '{parent_hash}'")
    );
}

#[test]
fn test_rev_parse_sq_quote_literal_args_outside_repo() {
    let temp = tempdir().expect("tempdir");
    let output = run_libra_command(
        &["rev-parse", "--sq-quote", "-x", "a b", "c'd"],
        temp.path(),
    );
    assert_cli_success(&output, "rev-parse --sq-quote outside repo");
    assert_eq!(
        String::from_utf8_lossy(&output.stdout).trim_end(),
        " '-x' 'a b' 'c'\\''d'"
    );
...
#[test]
fn test_rev_parse_sq_quote_trailing_dashdash_literal() {
    let temp = tempdir().expect("tempdir");
    let output = run_libra_command(&["rev-parse", "--sq-quote", "a", "--", "b"], temp.path());
    assert_cli_success(&output, "rev-parse --sq-quote with literal --");
    assert_eq!(
        String::from_utf8_lossy(&output.stdout).trim_end(),
        " 'a' '--' 'b'"
    );
...
#[test]
fn test_rev_parse_symbolic_full_name_branch() {
    let repo = create_committed_repo_via_cli();
    let output = run_libra_command(&["rev-parse", "--symbolic-full-name", "main"], repo.path());
    assert_cli_success(&output, "rev-parse --symbolic-full-name main");
    assert_eq!(
        String::from_utf8_lossy(&output.stdout).trim(),
        "refs/heads/main"
    );
...
#[test]
fn test_rev_parse_symbolic_full_name_tag() {
    let repo = create_committed_repo_via_cli();
    assert_cli_success(
        &run_libra_command(&["tag", "v-test"], repo.path()),
        "create lightweight tag",
    );
    let output = run_libra_command(
        &["rev-parse", "--symbolic-full-name", "v-test"],
        repo.path(),
    );
    assert_cli_success(&output, "rev-parse --symbolic-full-name tag");
    assert_eq!(
        String::from_utf8_lossy(&output.stdout).trim(),
        "refs/tags/v-test"
    );
...
#[test]
fn test_rev_parse_sq_rejects_json_output() {
    let repo = create_committed_repo_via_cli();
    let output = run_libra_command(&["--json", "rev-parse", "--sq", "HEAD"], repo.path());
    assert_eq!(output.status.code(), Some(129));

    let machine = run_libra_command(
        &["--machine", "rev-parse", "--sq-quote", "HEAD"],
        repo.path(),
    );
    assert_eq!(machine.status.code(), Some(129));
}
