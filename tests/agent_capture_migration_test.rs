...
async fn agent_capture_rollback_drops_tables_and_indexes_only() {
    ...
    // agent_usage_stats agent_name column, source_call_log, the
    // source_call_log agent_run_id column, and cherry_pick_state). Rollback
    ...
    // agent_capture itself. New built-in migrations must extend this list.
    ...
    assert_eq!(
        rolled_back,
        vec![
            2026060401, 2026060201, 2026052301, 2026050801, 2026050601, 2026050501, 2026050303
        ]
    );
