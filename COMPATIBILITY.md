...
| rev-list | partial | single revision reachability listing plus `--count`, `-n`/`--max-count`, `--skip`, `--parents`, and `--timestamp` supported; ranges, exclusions, and parent filters are not exposed |
