...
| rev-list | partial | multi-revision reachability, `^` exclusions, `A..B`/`A...B` ranges, `--count`, `-n`/`--max-count`, `--skip`, parent-count filters and reset aliases, `--parents`, and `--timestamp` supported; advanced traversal filters are not exposed |
