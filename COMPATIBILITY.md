...
| fsck | supported | `--full` (default) verifies packed objects too: every `objects/pack/*.idx` (index v1 + v2) is parsed with `verify-pack`'s validated reader and merged with loose objects; `--no-full` restricts the check to loose objects. |
