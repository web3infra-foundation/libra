...
| format-patch | partial | `-o`/`--output-directory`, `--stdout`, `-n`/`--numbered`, `--start-number`, `--subject-prefix`, `--cover-letter`, `--thread`/`--no-thread`, `--in-reply-to`, `-v`/`--reroll-count`, `-s`/`--signoff`, `--full-index`, `--no-stat`, `--keep-subject`, and `A..B`/single-commit revision range supported; merge commits are skipped; `--attach`, `--inline`, `--suffix`, `--signature`, `--from`, `--to`, `--cc`, `--base`, `--interdiff`, `--range-diff`, `--notes`, `--encode-email-headers`, `--force`, and `--zero-commit` are not exposed |
