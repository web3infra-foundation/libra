...
| fetch | partial | `--all`, refspec, `--depth`/`--unshallow`, `--shallow-since`/`--shallow-exclude`, `--prune`/`fetch.prune`, `--porcelain`, `--dry-run`, FETCH_HEAD write + `--append` (long-only; `-a` stays `--all`, intentionally-different from Git's `-a`=`--append`), `-v`/`--verbose` (announces the remote being contacted on stderr; stdout contract unchanged), and `--json`/`--machine` (Libra extension) supported. `--recurse-submodules` is declined (intentionally-different, exit 129). `--deepen` currently behaves as absolute depth (true relative `deepen-relative` deferred); `--update-shallow`, `--tags`/`--no-tags`, `--atomic`, `--refmap`, `--force` are planned. |
