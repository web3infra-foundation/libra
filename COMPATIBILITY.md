...
| fetch | partial | `--all`, refspec, `--depth`/`--unshallow`, `--shallow-since`/`--shallow-exclude`, `--prune`/`fetch.prune`, `--porcelain`, `--dry-run`, FETCH_HEAD write + `--append` (long-only; `-a` stays `--all`, intentionally-different from Git's `-a`=`--append`), `-v`/`--verbose` (announces the remote being contacted on stderr; stdout contract unchanged), `-t`/`--tags`/`--no-tags` + `remote.<name>.tagOpt` (import every advertised tag into the global `refs/tags/*`, pulling each tag's object; existing local tags are preserved without `--force`; `--no-tags` is long-only, intentionally-different from Git's `-n`), and `--json`/`--machine` (Libra extension) supported. `--recurse-submodules` is declined (intentionally-different, exit 129). `--deepen` currently behaves as absolute depth (true relative `deepen-relative` deferred); the default tag auto-follow (Git's second-round fetch of tags pointing at fetched commits) imports no tags pending implementation; `--update-shallow`, `--atomic`, `--refmap`, `--force` are planned. |
